                               ...
                               unsigned int data, unsigned int data_byte_num)
{
    ...
    while (1) {
        ret = xmedia_i2c_master_send(&client, (const char *)tmp_buf, idx);
        if (ret == idx) {
            break;
        }
#ifdef __LITEOS__
...
#endif
        {
            ...
            osal_printk("[%s %d] xmedia_i2c_master_send error, ret=%d. \n", __func__, __LINE__, ret);
            return ret;
        }
    }
...
static void __exit dev_exit(void)
{
    ...
    printk("unload sensor_i2c.ko OK!\n");
}
...
int __init sensor_i2c_driver_init(void)
{
    return dev_init();
}
