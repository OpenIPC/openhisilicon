...
{
    return HI_MPI_CIPHER_EncryptVir(hCipher, pu8SrcData, pu8DestData, u32ByteLength);
}

HI_S32 HI_UNF_CIPHER_DecryptVir(HI_HANDLE hCipher, HI_U8 *pu8SrcData, HI_U8 *pu8DestData, HI_U32 u32ByteLength)
{
    return HI_MPI_CIPHER_DecryptVir(hCipher, pu8SrcData, pu8DestData, u32ByteLength);
}
...
HI_S32 HI_UNF_CIPHER_HashVir(HI_UNF_CIPHER_HASH_ATTS_S *pstHashAttr,
                             HI_U8 *pu8InputData, HI_U32 u32ByteLength,
                             HI_U8 *pu8OutputHash)
{
    return HI_MPI_CIPHER_HashVir(pstHashAttr, pu8InputData, u32ByteLength, pu8OutputHash);
}
#endif
#endif
