...
HI_S32 HAL_Cipher_SetInBufNum(HI_U32 chnId, HI_U32 num)
{
    ...
    if ((CI_PKGx1_CHAN == chnId) || (CI_PKGxN_CHAN_MAX < chnId))
    {
...
HI_S32 HAL_Cipher_GetInBufNum(HI_U32 chnId, HI_U32 *pNum)
{
    ...
    if ((CI_PKGx1_CHAN == chnId) || (CI_PKGxN_CHAN_MAX < chnId) || (HI_NULL == pNum))
    {
        return HI_ERR_CIPHER_INVALID_PARA;
    }
...
HI_BOOL HAL_Cipher_IsIdle(HI_U32 chn)
{
    ...
    if (CI_PKGxN_CHAN_MAX < chn)
    {
        HI_ERR_CIPHER("Invalid chn:%d.\n",chn);
        return HI_ERR_CIPHER_INVALID_PARA;
    }
...
HI_S32 HAL_Cipher_SetDataSinglePkg(HI_UNF_CIPHER_INFO_S * info)
{
    ...
    if (HI_NULL == info)
    {
        HI_ERR_CIPHER("Invalid point, info is null.\n");
...
HI_S32 HAL_Cipher_ReadDataSinglePkg(HI_U32 *pData)
{
    ...
    if (HI_NULL == pData)
    {
        HI_ERR_CIPHER("Invalid point, pData is null.\n");
        return HI_ERR_CIPHER_INVALID_POINT;
    }
...
HI_S32 HAL_Cipher_GetOutIV(HI_U32 chnId, HI_UNF_CIPHER_CTRL_S* pCtrl)
{
    ...
    if (CI_PKGxN_CHAN_MAX < chnId || HI_NULL == pCtrl)
    {
        HI_ERR_CIPHER("Invalid parameter!\n");
        return HI_ERR_CIPHER_INVALID_PARA;
    }
...
HI_S32 HAL_Cipher_SetInIV(HI_U32 chnId, HI_UNF_CIPHER_CTRL_S* pCtrl)
{
    ...
    if (CI_PKGx1_CHAN != chnId || HI_NULL == pCtrl)
    {
        HI_ERR_CIPHER("Invalid parameter!chnId:%d\n",chnId);
        return HI_ERR_CIPHER_INVALID_PARA;
    }
...
HI_S32 HAL_Cipher_Config(HI_U32 chnId, HI_BOOL bDecrypt, HI_BOOL bIVChange, HI_UNF_CIPHER_CTRL_S* pCtrl)
{
    ...
    HI_BOOL bKeyByCA = HI_TRUE;

    if (HI_NULL == pCtrl)
    {
        HI_ERR_CIPHER("Invalid point, pCtrl is null.\n");
        return HI_ERR_CIPHER_INVALID_POINT;
    }

    bKeyByCA = pCtrl->bKeyByCA;
...
//    regValue |= ((bIVChange & 0x1) << 8);
    if (bIVChange && CI_PKGx1_CHAN == chnId) ///?
    {
        ret = HAL_Cipher_SetInIV(chnId, pCtrl);
        if (HI_SUCCESS != ret)
        {
            HI_ERR_CIPHER("HAL_Cipher_SetInIV failed.\n");
            return HI_ERR_CIPHER_INVALID_POINT;
        }
...
HI_VOID HAL_Cipher_Init(void)
{
    ...
    pvirt = (void __iomem *)ioremap_nocache(REG_SYS_CIPHER_CLK_ADDR_PHY, 0x4);
