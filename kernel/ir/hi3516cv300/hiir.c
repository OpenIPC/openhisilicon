...
#define IR_ENABLE   ((unsigned long)pIrRegBase + 0x00)
#define IR_CONFIG   ((unsigned long)pIrRegBase + 0x04)
#define CNT_LEADS   ((unsigned long)pIrRegBase + 0x08)
#define CNT_LEADE   ((unsigned long)pIrRegBase + 0x0c)
#define CNT_SLEADE  ((unsigned long)pIrRegBase + 0x10)
#define CNT0_B      ((unsigned long)pIrRegBase + 0x14)
#define CNT1_B      ((unsigned long)pIrRegBase + 0x18)
#define IR_BUSY     ((unsigned long)pIrRegBase + 0x1c)
#define IR_DATAH    ((unsigned long)pIrRegBase + 0x20)
#define IR_DATAL    ((unsigned long)pIrRegBase + 0x24)
#define IR_INTM     ((unsigned long)pIrRegBase + 0x28)
#define IR_INTS     ((unsigned long)pIrRegBase + 0x2c)
#define IR_INTC     ((unsigned long)pIrRegBase + 0x30)
#define IR_START    ((unsigned long)pIrRegBase + 0x34)
...
retry :
    ...
    if((hiir_dev.head)==(hiir_dev.tail))
    {
        ...
        res = osal_wait_event_interruptible(&(hiir_dev.irkey_wait),hiir_WaitConditionCallBack, NULL);
        if (0 != res)
        {
            return res;
        }
