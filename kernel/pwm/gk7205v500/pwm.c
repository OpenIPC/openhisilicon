...
#endif
{
    ...
    switch (cmd) {
        case PWM_CMD_WRITE: {
            ...
            if (read_user_linear_space_valid((unsigned char *)(XMEDIA_UINTPTR_T)arg, sizeof(PWM_DATA_S)) == 0) {
                osal_printk("pwm address is invalid!\n");
                return -EFAULT;
            }

            ret = copy_from_user(&stPwmData, (PWM_DATA_S __user *)(XMEDIA_UINTPTR_T)arg, sizeof(PWM_DATA_S));
            if (ret) {
                return -EFAULT;
            }
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(5,0,0)
    if (reg_pwmI_base_va) {
        osal_iounmap((XMEDIA_VOID *)reg_pwmI_base_va);
        reg_pwmI_base_va = XMEDIA_NULL;
    }
#else
    reg_pwmI_base_va = NULL;
#endif
...
int __init pwm_driver_init(void)
{
    return osal_platform_driver_register(&pwm_driver);
}
#endif
