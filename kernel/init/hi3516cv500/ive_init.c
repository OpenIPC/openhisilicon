...
static int hi35xx_ive_probe(struct platform_device *pf_dev)
{
    ...
    osal_platform_get_module_param(pf_dev, "save_power", uchar, &g_ive_power_save_en);
    osal_platform_get_module_param(pf_dev, "max_node_num", ushort, &g_ive_node_num);
...
MODULE_DEVICE_TABLE(of, g_hi35xx_ive_match);

static struct platform_driver g_hi35xx_ive_driver = {
    .probe = hi35xx_ive_probe,
    .remove = hi35xx_ive_remove,
    .driver = {
        .name = "hi35xx_ive",
        .of_match_table = g_hi35xx_ive_match,
    },
};
