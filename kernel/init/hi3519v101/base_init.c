/*
 * Init wrapper for hi3519v101_base blob module (hi3519v101 / V3A family).
 */
...
DECLARE_BLOB_FUNC(VB_ModInit);			EXPORT_SYMBOL(VB_ModInit);
DECLARE_BLOB_FUNC(VB_ModExit);			EXPORT_SYMBOL(VB_ModExit);
DECLARE_BLOB_FUNC(VB_GetPoolId);		EXPORT_SYMBOL(VB_GetPoolId);
DECLARE_BLOB_FUNC(VB_GetSupplementSize);	EXPORT_SYMBOL(VB_GetSupplementSize);
DECLARE_BLOB_FUNC(VB_CheckSupplement);		EXPORT_SYMBOL(VB_CheckSupplement);
DECLARE_BLOB_FUNC(VB_FlushSupplement);		EXPORT_SYMBOL(VB_FlushSupplement);
DECLARE_BLOB_FUNC(VB_IsLowDelaySupported);	EXPORT_SYMBOL(VB_IsLowDelaySupported);
DECLARE_BLOB_FUNC(VB_CheckModParam);		EXPORT_SYMBOL(VB_CheckModParam);
DECLARE_BLOB_FUNC(VbGetModParam);		EXPORT_SYMBOL(VbGetModParam);
DECLARE_BLOB_FUNC(VbSetModParam);		EXPORT_SYMBOL(VbSetModParam);
DECLARE_BLOB_FUNC(VbSetSupplementConf);		EXPORT_SYMBOL(VbSetSupplementConf);
DECLARE_BLOB_FUNC(VB_ValueSupplementCachedAddr);  EXPORT_SYMBOL(VB_ValueSupplementCachedAddr);
DECLARE_BLOB_FUNC(VB_ValueSupplementNoCacheAddr);  EXPORT_SYMBOL(VB_ValueSupplementNoCacheAddr);
DECLARE_BLOB_FUNC(WaitConditionCallBack);	EXPORT_SYMBOL(WaitConditionCallBack);
...
static int __init base_mod_init(void)
{
	...
	ret = _3519v101_hi3519v101_base_init();
	if (ret)
		return ret;
