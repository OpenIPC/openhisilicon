...
#define IO_PIRISI_ADDRESS(dev, x)  ((XMEDIA_UINTPTR_T)reg_pirisI_base_va[dev] + ((x)-(PIRISI_ADRESS_BASE)))
...
XMEDIA_BOOL g_bPirisIoRemap = XMEDIA_FALSE;
...
#else
    osal_set_timer(&pstPiris->timer, 1);
#endif
...
#endif
{
...
#else
    long int dev = (long int)(XMEDIA_UINTPTR_T)(file->private_data);
#endif
    PIRIS_DEV *pstPiris;
    ...
    switch (cmd) {
        case PIRIS_SET_FD:
#ifdef __LITEOS__
            if (read_user_linear_space_valid((XMEDIA_U8 *)arg, sizeof(int)) == XMEDIA_FALSE) {
                up(&pstPiris->sem);
                return -EFAULT;
            }
...
#else
            if (read_user_linear_space_valid((XMEDIA_U8 *)(XMEDIA_UINTPTR_T)arg, sizeof(long int)) == XMEDIA_FALSE) {
                up(&pstPiris->sem);
                return -EFAULT;
            }
            ret = copy_from_user(&dev, (long int __user *)(XMEDIA_UINTPTR_T)arg, sizeof(long int));
            if (ret) {
                up(&pstPiris->sem);
                return -EFAULT;
            }
            file->private_data = (void *)(XMEDIA_UINTPTR_T)(dev);
#endif
            break;

        case PIRIS_SET_ACT_ARGS:

            if (read_user_linear_space_valid((XMEDIA_U8 *)(XMEDIA_UINTPTR_T)arg, sizeof(int)) == XMEDIA_FALSE) {
                up(&pstPiris->sem);
                return -EFAULT;
            }

            ret = copy_from_user(&PirisPos, (int __user *)(XMEDIA_UINTPTR_T)arg, sizeof(int));
            if (ret) {
                up(&pstPiris->sem);
                return -EFAULT;
            }
        ...
        case PIRIS_SET_ORGIN:
            if (read_user_linear_space_valid((XMEDIA_U8 *)(XMEDIA_UINTPTR_T)arg, sizeof(PIRIS_DATA_S)) == XMEDIA_FALSE) {
                up(&pstPiris->sem);
                return -EFAULT;
            }

            ret = copy_from_user(&piris_data, (PIRIS_DATA_S __user *)(XMEDIA_UINTPTR_T)arg, sizeof(PIRIS_DATA_S));
            if (ret) {
                up(&pstPiris->sem);
                return -EFAULT;
            }
        ...
        case PIRIS_GET_STATUS:
            ...
            if (write_user_linear_space_valid((XMEDIA_U8 *)(XMEDIA_UINTPTR_T)arg, sizeof(PIRIS_STATUS_E)) == XMEDIA_FALSE) {
                up(&pstPiris->sem);
                return -EFAULT;
            }

            ret = copy_to_user((PIRIS_STATUS_E __user *)(XMEDIA_UINTPTR_T)arg, &PirisStatus, sizeof(PIRIS_STATUS_E));
            if (ret) {
                up(&pstPiris->sem);
                return -EFAULT;
            }
...
#endif
{
    ...
    unsigned char bits;
    unsigned long u32Flags;
    unsigned long ulArgUse;
    PIRIS_DEV *pstPiris = NULL;

    ulArgUse=osal_timer_get_private_data((void *)arg);
    pstPiris = (PIRIS_DEV *)(XMEDIA_UINTPTR_T)ulArgUse;
    ...
    for (i = 0; i < PIRIS_MAX_DEV_NUM; i++) {
        ...
        if (!pdev) {
            ...
            for (j = 0; j < PIRIS_MAX_DEV_NUM; j++) {
                if (g_bPirisIoRemap == XMEDIA_TRUE) {
...
#endif       
        p_piris_dev[i]->timer.data = (unsigned long)(XMEDIA_UINTPTR_T)p_piris_dev[i];
        //p_piris_dev[i]->timer.expires = jiffies + HZ; /* one second */
        osal_timer_init(&p_piris_dev[i]->timer);
...
MODULE_LICENSE("GPL");
#else
int __init piris_driver_init(void)
{
    return piris_init();
}
#endif
...
static int piris_probe(struct platform_device *pdev)
{
    ...
    for (i = 0; i < PIRIS_MAX_DEV_NUM - 1; i++) {
    ...
    reg_pirisI_base_va[PIRIS_MAX_DEV_NUM -1] = reg_pirisI_base_va[0];
...
int __init piris_driver_init(void)
{
    return osal_platform_driver_register(&piris_driver);
}
#endif
