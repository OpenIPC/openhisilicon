...
#define FBIOPUT_SHOW_GFBG           _IOW(IOC_TYPE_GFBG, 101, XMEDIA_BOOL)
/** To obtain the display state of an overlay layer */
#define FBIOGET_SHOW_GFBG           _IOR(IOC_TYPE_GFBG, 102, XMEDIA_BOOL)
...
#define FBIOPUT_COMPRESSION_GFBG    _IOW(IOC_TYPE_GFBG, 133, XMEDIA_BOOL)
/**To obtain the compression function status of an overlay layer*/
#define FBIOGET_COMPRESSION_GFBG    _IOR(IOC_TYPE_GFBG, 134, XMEDIA_BOOL)
...
#ifndef __LITEOS__
static inline XMEDIA_U8  gfbg_rgb(const struct fb_bitfield* pBit, XMEDIA_S32 color)
{
    return ((XMEDIA_U8)((((XMEDIA_U32)color)>>pBit->offset) << (8-pBit->length)) +
             ((XMEDIA_U8)(((XMEDIA_U32)(color)>>pBit->offset) << (8-pBit->length)) >> pBit->length));
}

static inline XMEDIA_S32 gfbg_color2key(const struct fb_var_screeninfo* pVar, XMEDIA_S32 color)
{
...
typedef struct
{
    XMEDIA_BOOL bKeyEnable;         /* colorkey enable flag */
    XMEDIA_U32 u32Key;              /* colorkey value, maybe contains alpha */
}GFBG_COLORKEY_S;
...
typedef struct
{
    XMEDIA_S32 s32XPos;         /**<  horizontal position */
    XMEDIA_S32 s32YPos;         /**<  vertical position */
}GFBG_POINT_S;

typedef struct GFBG_DEFLICKER_S
{
    XMEDIA_U32  u32HDfLevel;    /**<  horizontal deflicker level */
    XMEDIA_U32  u32VDfLevel;    /**<  vertical deflicker level */
    XMEDIA_U8   ATTRIBUTE *pu8HDfCoef;    /**<  horizontal deflicker coefficient */
    XMEDIA_U8   ATTRIBUTE *pu8VDfCoef;    /**<  vertical deflicker coefficient */
}GFBG_DEFLICKER_S;

/** Alpha info */
typedef struct
{
    XMEDIA_BOOL bAlphaEnable;   /**<  alpha enable flag */
    XMEDIA_BOOL bAlphaChannel;  /**<  alpha channel enable flag */
    XMEDIA_U8 u8Alpha0;         /**<  alpha0 value, used in ARGB1555 */
    XMEDIA_U8 u8Alpha1;         /**<  alpha1 value, used in ARGB1555 */
    XMEDIA_U8 u8GlobalAlpha;    /**<  global alpha value */
...
{
    XMEDIA_BOOL bKeyRgb;
    XMEDIA_BOOL bKeyAlpha;      /**<  whether support colorkey alpha */
    XMEDIA_BOOL bGlobalAlpha;   /**<  whether support global alpha */
    XMEDIA_BOOL bCmap;          /**<  whether support color map */
    XMEDIA_BOOL bHasCmapReg;    /**<  whether has color map register*/
    XMEDIA_BOOL bColFmt[GFBG_FMT_BUTT]; /**<  support which color format */
    XMEDIA_BOOL bVoScale;       /**< support vo scale*/
    XMEDIA_BOOL bLayerSupported;    /**< whether support a certain layer, for example:x5 HD support GFBG_SD_0 not support GFBG_SD_1*/
    XMEDIA_U32  u32MaxWidth;    /**<  the max pixels per line */
    XMEDIA_U32  u32MaxHeight;   /**<  the max lines */
    XMEDIA_U32  u32MinWidth;    /**<  the min pixels per line */
    XMEDIA_U32  u32MinHeight;   /**<  the min lines */
    XMEDIA_U32  u32VDefLevel;   /**<  vertical deflicker level, 0 means vertical deflicker is unsupported */
    XMEDIA_U32  u32HDefLevel;   /**<  horizontal deflicker level, 0 means horizontal deflicker is unsupported  */
    XMEDIA_BOOL  bDcmp;
    XMEDIA_BOOL  bPreMul;
    XMEDIA_BOOL  bGHDR;         /* NEW Feature. Is GHDR supported. */
}GFBG_CAPABILITY_S;
...
typedef struct
{
    XMEDIA_U64  u64PhyAddr;     /**<  start physical address */
    XMEDIA_U64  u64GBPhyAddr;   // TODO new
    XMEDIA_U32  u32Width;       /**<  width pixels */
    XMEDIA_U32  u32Height;      /**<  height pixels */
    XMEDIA_U32  u32Pitch;       /**<  line pixels */
...
typedef struct
{
    XMEDIA_U32 u32StartSection;
    XMEDIA_U32 u32ZoneNums;
} GFBG_DDRZONE_S;
...
#define FBIOPUT_CURSOR_STATE          _IOW(IOC_TYPE_GFBG, 106, XMEDIA_BOOL )
#define FBIOGET_CURSOR_STATE          _IOW(IOC_TYPE_GFBG, 107, XMEDIA_BOOL )
...
#define FBIOPUT_CURSOR_ATTCHCURSOR    _IOW(IOC_TYPE_GFBG, 114, XMEDIA_U32 )
#define FBIOPUT_CURSOR_DETACHCURSOR   _IOW(IOC_TYPE_GFBG, 115, XMEDIA_U32 )
...
typedef struct
{
    ...
    XMEDIA_S32 s32XPos;           /**<  the x pos of origion point in screen */
    XMEDIA_S32 s32YPos;           /**<  the y pos of origion point in screen */
    XMEDIA_U32 u32CanvasWidth;    /**<  the width of canvas buffer */
    XMEDIA_U32 u32CanvasHeight;   /**<  the height of canvas buffer */
    XMEDIA_U32 u32DisplayWidth;   /**<  the width of display buf in fb.for 0 buf ,there is no display buf in fb, so it's effectless*/
    XMEDIA_U32 u32DisplayHeight;  /**<  the height of display buf in fb. */
    XMEDIA_U32 u32ScreenWidth;    /**<  the width of screen */
    XMEDIA_U32 u32ScreenHeight;   /**<  the height of screen */
    XMEDIA_BOOL bPreMul;          /**<  The data drawed in buf is premul data or not*/
    XMEDIA_U32 u32Mask;           /**<  param modify mask bit*/
}GFBG_LAYER_INFO_S;
...
extern XMEDIA_S32 gfbg_init(void* pArgs);
#endif
#ifdef __cplusplus
#if __cplusplus
}
