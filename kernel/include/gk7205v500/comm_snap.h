...
typedef struct SNAP_PRO_MANUAL_PARAM_S {
    XMEDIA_U32 au32ManExpTime[PRO_MAX_FRAME_NUM];
    XMEDIA_U32 au32ManSysgain[PRO_MAX_FRAME_NUM];
} SNAP_PRO_MANUAL_PARAM_S;

typedef struct SNAP_PRO_AUTO_PARAM_S {
    XMEDIA_U16 au16ProExpStep[PRO_MAX_FRAME_NUM];
...
typedef struct SNAP_NORMAL_ATTR_S {
    ...
    XMEDIA_U32  u32RepeatSendTimes;

    XMEDIA_BOOL bZSL;
    XMEDIA_U32  u32FrameDepth;
    XMEDIA_U32  u32RollbackMs;
