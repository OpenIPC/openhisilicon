...
static inline td_bool read_user_linear_space_valid(td_u8 *addr_start, td_u32 len)
{
    ...
    if (osal_access_ok(OSAL_VERIFY_READ, addr_start, len) == 0) {
        return TD_FALSE;
    }

    addr_end = addr_start + len - 1;
    if (osal_copy_from_user(&check, addr_end, 1) != 0) {
        return TD_FALSE;
    }
...
static inline td_bool write_user_linear_space_valid(td_u8 *addr_start, td_u32 len)
{
    ...
    if (osal_access_ok(OSAL_VERIFY_WRITE, addr_start, len) == 0) {
        return TD_FALSE;
    }

    addr_end = addr_start + len - 1;
    if (osal_copy_to_user(addr_end, &check, 1) != 0) {
        return TD_FALSE;
    }
