...
#define ckfn_vb_entry() check_func_entry(OT_ID_VB)
#define vb_entry() (func_entry(vb_export_func, OT_ID_VB))

#undef VB_DEBUG_SUPPORT
#ifdef VB_DEBUG_SUPPORT
vb_blk_handle vb_get_blk_by_pool_id_debug(td_u32 pool_id, td_u32 uid, const td_char *func, td_u32 line);
vb_blk_handle vb_get_blk_by_size_debug(td_u64 blk_size, ot_vb_uid uid, const td_char *mmz_name,
    const td_char *func, td_u32 line);
vb_blk_handle vb_get_blk_by_size_and_module_debug(td_u64 blk_size, ot_vb_uid uid, const td_char *mmz_name,
    const td_char *func, td_u32 line);
vb_blk_handle vb_get_blk_by_size_and_pool_id_debug(td_u32 pool_id, td_u64 blk_size, td_u32 uid,
    const td_char *func, td_u32 line);
vb_blk_handle vb_get_blk_by_pool_id_with_pid_debug(td_u32 pool_id, td_u32 uid, td_s32 pid,
    const td_char *func, td_u32 line);
vb_blk_handle vb_get_blk_by_size_and_pool_id_with_pid_debug(td_u32 pool_id, td_u64 blk_size, td_u32 uid,
    td_s32 pid, const td_char *func, td_u32 line);
td_s32 vb_user_add_debug(td_u32 pool_id, td_phys_addr_t phy_addr, td_u32 uid, const td_char *func, td_u32 line);
td_s32 vb_user_sub_debug(td_u32 pool_id, td_phys_addr_t phy_addr, td_u32 uid, const td_char *func, td_u32 line);

#define vb_get_blk_by_pool_id_(pool_id, uid) \
    vb_get_blk_by_pool_id_debug(pool_id, uid, __FUNCTION__, __LINE__)
#define vb_get_blk_by_size_(blk_size, uid, mmz_name) \
    vb_get_blk_by_size_debug(blk_size, uid, mmz_name, __FUNCTION__, __LINE__)
#define vb_get_blk_by_size_and_module_(pool_id, uid, mmz_name) \
    vb_get_blk_by_size_and_module_debug(blk_size, uid, mmz_name, __FUNCTION__, __LINE__)
#define vb_get_blk_by_size_and_pool_id_(pool_id, blk_size, uid) \
    vb_get_blk_by_size_and_pool_id_debug(pool_id, blk_size, uid, __FUNCTION__, __LINE__)
#define vb_get_blk_by_pool_id_with_pid_(pool_id, uid, pid) \
    vb_get_blk_by_pool_id_with_pid_debug(pool_id, uid, pid, __FUNCTION__, __LINE__)
#define vb_get_blk_by_size_and_pool_id_with_pid_(pool_id, blk_size, uid, pid) \
    vb_get_blk_by_size_and_pool_id_with_pid_debug(pool_id, blk_size, uid, pid, __FUNCTION__, __LINE__)
#define vb_user_add_(pool_id, phy_addr, uid) vb_user_add_debug(pool_id, phy_addr, uid, __FUNCTION__, __LINE__)
#define vb_user_sub_(pool_id, phy_addr, uid) vb_user_sub_debug(pool_id, phy_addr, uid, __FUNCTION__, __LINE__)
#else
#define vb_get_blk_by_pool_id_(pool_id, uid) \
    vb_entry()->pfn_vb_get_blk_by_pool_id(pool_id, uid)
#define vb_get_blk_by_size_(blk_size, uid, mmz_name) \
    vb_entry()->pfn_vb_get_blk_by_size(blk_size, uid, mmz_name)
#define vb_get_blk_by_size_and_module_(pool_id, uid, mmz_name) \
    vb_entry()->pfn_vb_get_blk_by_size_and_module(blk_size, uid, mmz_name)
#define vb_get_blk_by_size_and_pool_id_(pool_id, blk_size, uid) \
    vb_entry()->pfn_vb_get_blk_by_size_and_pool_id(pool_id, blk_size, uid)
#define vb_get_blk_by_pool_id_with_pid_(pool_id, uid, pid) \
    vb_entry()->pfn_vb_get_blk_by_pool_id_with_pid(pool_id, uid, pid)
#define vb_get_blk_by_size_and_pool_id_with_pid_(pool_id, blk_size, uid, pid) \
    vb_entry()->pfn_vb_get_blk_by_size_and_pool_id_with_pid(pool_id, blk_size, uid, pid)
#define vb_user_add_(pool_id, phy_addr, uid) vb_entry()->pfn_vb_user_add(pool_id, phy_addr, uid)
#define vb_user_sub_(pool_id, phy_addr, uid) vb_entry()->pfn_vb_user_sub(pool_id, phy_addr, uid)
#endif

#define ckfn_vb_create_pool() \
    (vb_entry()->pfn_vb_create_pool != NULL)
#define call_vb_create_pool(pool_id, mmz_name, info) \
    vb_entry()->pfn_vb_create_pool(pool_id, mmz_name, info)

#define ckfn_vb_create_supplement_pool() \
    (vb_entry()->pfn_vb_create_supplement_pool != NULL)
#define call_vb_create_supplement_pool(pool_id, mmz_name, info) \
    vb_entry()->pfn_vb_create_supplement_pool(pool_id, mmz_name, info)

#define ckfn_vb_destroy_pool() \
    (vb_entry()->pfn_vb_destroy_pool != NULL)
#define call_vb_destroy_pool(pool_id) \
    vb_entry()->pfn_vb_destroy_pool(pool_id)

#define ckfn_vb_get_blk_by_pool_id() \
    (vb_entry()->pfn_vb_get_blk_by_pool_id != NULL)
#define call_vb_get_blk_by_pool_id(pool_id, uid) \
    vb_get_blk_by_pool_id_(pool_id, uid)

#define ckfn_vb_get_pool_type_by_pool_id() \
        (vb_entry()->pfn_vb_get_pool_type_by_pool_id != NULL)
#define call_vb_get_pool_type_by_pool_id(pool_id) \
        vb_entry()->pfn_vb_get_pool_type_by_pool_id(pool_id)

#define ckfn_vb_get_blk_by_size() \
    (vb_entry()->pfn_vb_get_blk_by_size != NULL)
#define call_vb_get_blk_by_size(blk_size, uid, mmz_name) \
    vb_get_blk_by_size_(blk_size, uid, mmz_name)

#define ckfn_vb_get_blk_by_size_and_module() \
    (vb_entry()->pfn_vb_get_blk_by_size_and_module != NULL)
#define call_vb_get_blk_by_size_and_module(blk_size, uid, mmz_name) \
    vb_get_blk_by_size_and_module_(blk_size, uid, mmz_name)

#define ckfn_vb_get_blk_by_size_and_pool_id() \
    (vb_entry()->pfn_vb_get_blk_by_size_and_pool_id != NULL)
#define call_vb_get_blk_by_size_and_pool_id(pool_id, blk_size, uid) \
    vb_get_blk_by_size_and_pool_id_(pool_id, blk_size, uid)

#define ckfn_vb_get_blk_by_pool_id_with_pid() \
    (vb_entry()->pfn_vb_get_blk_by_pool_id_with_pid != NULL)
#define call_vb_get_blk_by_pool_id_with_pid(pool_id, uid, pid) \
    vb_get_blk_by_pool_id_with_pid_(pool_id, uid, pid)

#define ckfn_vb_get_blk_by_size_and_pool_id_with_pid() \
    (vb_entry()->pfn_vb_get_blk_by_size_and_pool_id_with_pid != NULL)
#define call_vb_get_blk_by_size_and_pool_id_with_pid(pool_id, blk_size, uid, pid) \
    vb_get_blk_by_size_and_pool_id_with_pid_(pool_id, blk_size, uid, pid)

#define ckfn_vb_get_pool_id_by_size_and_module() \
    (vb_entry()->pfn_vb_get_pool_id_by_size_and_module != NULL)
#define call_vb_get_pool_id_by_size_and_module(blk_size, pool_id, mmz_name, uid) \
    vb_entry()->pfn_vb_get_pool_id_by_size_and_module(blk_size, pool_id, mmz_name, uid)
#define ckfn_vb_get_pool_id() \
    (vb_entry()->pfn_vb_get_pool_id != NULL)
#define call_vb_get_pool_id(blk_size, pool_id, mmz_name, uid) \
    vb_entry()->pfn_vb_get_pool_id(blk_size, pool_id, mmz_name, uid)

#define ckfn_vb_get_pool_info() \
    (vb_entry()->pfn_vb_get_pool_info != NULL)
#define call_vb_get_pool_info(info) \
    vb_entry()->pfn_vb_get_pool_info(info)

#define ckfn_vb_phy_to_handle() \
    (vb_entry()->pfn_vb_phy_to_handle != NULL)
#define call_vb_phy_to_handle(phy_addr) \
    vb_entry()->pfn_vb_phy_to_handle(phy_addr)

#define ckfn_vb_handle_to_pool_id() \
    (vb_entry()->pfn_vb_handle_to_pool_id != NULL)
#define call_vb_handle_to_pool_id(handle) \
    vb_entry()->pfn_vb_handle_to_pool_id(handle)

#define ckfn_vb_handle_to_blk_id() \
    (vb_entry()->pfn_vb_handle_to_blk_id != NULL)
#define call_vb_handle_to_blk_id(handle) \
    vb_entry()->pfn_vb_handle_to_blk_id(handle)

#define ckfn_vb_handle_to_kern() \
    (vb_entry()->pfn_vb_handle_to_kern != NULL)
#define call_vb_handle_to_kern(handle) \
    vb_entry()->pfn_vb_handle_to_kern(handle)

#define ckfn_vb_handle_to_phys() \
    (vb_entry()->pfn_vb_handle_to_phys != NULL)
#define call_vb_handle_to_phys(handle) \
    vb_entry()->pfn_vb_handle_to_phys(handle)

#define ckfn_vb_handle_to_blk_size() \
    (vb_entry()->pfn_vb_handle_to_blk_size != NULL)
#define call_vb_handle_to_blk_size(handle) \
    vb_entry()->pfn_vb_handle_to_blk_size(handle)

#define ckfn_vb_user_add() \
    (vb_entry()->pfn_vb_user_add != NULL)
#define call_vb_user_add(pool_id, phy_addr, uid) \
    vb_user_add_(pool_id, phy_addr, uid)

#define ckfn_vb_user_sub() \
    (vb_entry()->pfn_vb_user_sub != NULL)
#define call_vb_user_sub(pool_id, phy_addr, uid) \
    vb_user_sub_(pool_id, phy_addr, uid)

#define ckfn_vb_inquire_user_cnt() \
    (vb_entry()->pfn_vb_inquire_user_cnt != NULL)
#define call_vb_inquire_user_cnt(handle) \
    vb_entry()->pfn_vb_inquire_user_cnt(handle)

#define ckfn_vb_inquire_one_user_cnt() \
    (vb_entry()->pfn_vb_inquire_one_user_cnt != NULL)
#define call_vb_inquire_one_user_cnt(handle, uid) \
    vb_entry()->pfn_vb_inquire_one_user_cnt(handle, uid)

#define ckfn_vb_inquire_total_user_cnt() \
    (vb_entry()->pfn_vb_inquire_total_user_cnt != NULL)
#define call_vb_inquire_total_user_cnt(handle) \
    vb_entry()->pfn_vb_inquire_total_user_cnt(handle)

#define ckfn_vb_is_blk_valid() \
    (vb_entry()->pfn_vb_is_blk_valid != NULL)
#define call_vb_is_blk_valid(pool_id, phy_addr) \
    vb_entry()->pfn_vb_is_blk_valid(pool_id, phy_addr)

#define ckfn_vb_is_pid_valid() \
    (vb_entry()->pfn_vb_is_pid_valid != NULL)
#define call_vb_is_pid_valid(pool_id, phy_addr) \
    vb_entry()->pfn_vb_is_pid_valid(pool_id, phy_addr)

#define ckfn_vb_is_pool_id_valid() \
    (vb_entry()->pfn_vb_is_pool_id_valid != NULL)
#define call_vb_is_pool_id_valid(pool_id) \
    vb_entry()->pfn_vb_is_pool_id_valid(pool_id)

#define ckfn_vb_inquire_pool_user_cnt() \
    (vb_entry()->pfn_vb_inquire_pool_user_cnt != NULL)
#define call_vb_inquire_pool_user_cnt(pool_id, uid, cnt) \
    vb_entry()->pfn_vb_inquire_pool_user_cnt(pool_id, uid, cnt)

#define ckfn_vb_handle_to_supplement() \
    (vb_entry()->pfn_vb_handle_to_supplement != NULL)
#define call_vb_handle_to_supplement(handle) \
    vb_entry()->pfn_vb_handle_to_supplement(handle)

#define ckfn_vb_is_supplement_support() \
    (vb_entry()->pfn_vb_is_supplement_support != NULL)
#define call_vb_is_supplement_support(mask) \
    vb_entry()->pfn_vb_is_supplement_support(mask)

#define ckfn_vb_copy_supplement() \
    (vb_entry()->pfn_vb_copy_supplement != NULL)
#define call_vb_copy_supplement(dst_v_frame, src_v_frame) \
    vb_entry()->pfn_vb_copy_supplement(dst_v_frame, src_v_frame)
...
#define ckfn_vb_get_blk_by_size_ex() \
    (vb_entry()->pfn_vb_get_blk_by_size_ex != NULL)
#define call_vb_get_blk_by_size_ex(blk_size, uid, mmz_name, chn) \
    vb_entry()->pfn_vb_get_blk_by_size_ex(blk_size, uid, mmz_name, chn)

#define ckfn_vb_get_blk_by_size_and_pool_id_ex() \
    (vb_entry()->pfn_vb_get_blk_by_size_and_pool_id_ex != NULL)
#define call_vb_get_blk_by_size_and_pool_id_ex(pool_id, blk_size, uid, chn) \
    vb_entry()->pfn_vb_get_blk_by_size_and_pool_id_ex(pool_id, blk_size, uid, chn)

#define ckfn_vb_get_blk_by_size_and_module_ex() \
    (vb_entry()->pfn_vb_get_blk_by_size_and_module_ex != NULL)
#define call_vb_get_blk_by_size_and_module_ex(blk_size, uid, mmz_name, chn) \
    vb_entry()->pfn_vb_get_blk_by_size_and_module_ex(blk_size, uid, mmz_name, chn)
