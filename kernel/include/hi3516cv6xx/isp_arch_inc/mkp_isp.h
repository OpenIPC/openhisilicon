...
#define ISP_FE_WEIGHT_NUM  ((OT_ISP_AE_ZONE_ROW * OT_ISP_AE_ZONE_COLUMN + 3) / 4) /* Round up to the nearest integer */
#endif
...
typedef enum {
    ...
    IOC_NR_ISP_GET_VD_RUNEND_TIMEOUT,
    IOC_NR_ISP_GET_VD_STANDBY_END_TIMEOUT,
    ...
    IOC_NR_ISP_GET_DIST_FPS_STATUS,
#endif
    IOC_NR_ISP_BUTT,
} ioc_nr_isp;
...
typedef struct {
    ...
    td_u16   seg_max_val[ISP_WDR_SEG_NUM];
    td_u16   seg_idx_base[ISP_WDR_SEG_NUM];
...
#define ISP_AGAMMA_NODE_NUM          257
typedef struct {
    td_bool resh;
    td_u8   bitw;
...
typedef struct {
    td_u8   resh;
    td_u32  adgamma_lut[ISP_AGAMMA_NODE_NUM];
...
typedef struct {
    ...
    isp_fe_dg_reg_cfg            fe_dg2_reg_cfg;
...
#define ISP_GET_VD_RUNEND_TIMEOUT       _IOWR(IOC_TYPE_ISP, IOC_NR_ISP_GET_VD_RUNEND_TIMEOUT, isp_vd_timeout)
#define ISP_GET_VD_STANDBY_END_TIMEOUT  _IOWR(IOC_TYPE_ISP, IOC_NR_ISP_GET_VD_STANDBY_END_TIMEOUT, isp_vd_timeout)
...
#define ISP_STANDBY_CFG_INIT            _IOW(IOC_TYPE_ISP, IOC_NR_ISP_STANDBY_CFG_INIT, ot_isp_sns_regs_info)
...
#define ISP_GET_DIST_FPS_STATUS          _IOR(IOC_TYPE_ISP, IOC_NR_ISP_GET_DIST_FPS_STATUS, td_bool)
#endif
...
#define isp_check_noprint_return(pipe, ret, ...) \
    do { \
        td_s32 _ret = (ret); \
        if (_ret != TD_SUCCESS) { \
            return _ret; \
