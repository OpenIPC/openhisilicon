...
typedef struct {
    td_bool all_online;         /* vi-vpss all online or not */
    /*
     ...
     */
    td_u32 frame_rate;
    td_u32 full_lines_std;      /* total height of sensor, include of VBlank */
    ot_size large_stream_size;  /* VENC large stream Size */
    ot_size small_stream_size;  /* VENC small stream Size */
} ot_vpss_venc_wrap_param;

typedef struct {
    td_bool vi_vpss_online;    /* vi-vpss mode is online or not */
    td_bool vpss_venc_wrap;    /* vpss-venc mode is wrap or not */
    ot_size vpss_chn0_size;    /* vpss chn0 size */
    ot_size large_stream_size; /* VENC large stream Size */
    ot_size small_stream_size; /* VENC small stream Size */
} ot_vi_slave_wrap_param;

typedef struct {
    ot_size pipe_size;                  /* vi set pipe attr size */
    td_u32 full_lines_std;              /* total height of sensor, include of VBlank */
    td_u32 frame_rate_100x;             /* 100 times of sensor frame rate */
    ot_vi_slave_wrap_param slave_param; /* vi offline vpss online mode effect param */
} ot_vi_pipe_wrap_param;

typedef struct {
    td_u32 pipe_num;
    td_bool is_slave;
    ot_vi_pipe_wrap_param wrap_param[OT_VI_MAX_PHYS_PIPE_NUM];
} ot_vi_wrap_in_param;

typedef struct {
    td_u32 buf_line[OT_VI_MAX_PHYS_PIPE_NUM];
} ot_vi_wrap_out_param;
