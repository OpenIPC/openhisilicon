...
#define SVP_ACL_EVENT_TIME_LINE 0x00000008u
...
typedef enum svp_acl_rt_event_status {
    SVP_ACL_EVENT_STATUS_COMPLETE  = 0,
    SVP_ACL_EVENT_STATUS_NOT_READY = 1,
    SVP_ACL_EVENT_STATUS_RESERVED  = 2,
} svp_acl_rt_event_status;

typedef enum svp_acl_rt_event_wait_status {
    SVP_ACL_EVENT_WAIT_STATUS_COMPLETE  = 0,
    SVP_ACL_EVENT_WAIT_STATUS_NOT_READY = 1,
    SVP_ACL_EVENT_WAIT_STATUS_RESERVED  = 0xFFFF,
 ...
 * @retval OtherValues Failure
 */
SVP_ACL_FUNC_VISIBILITY svp_acl_error svp_acl_rt_create_event(svp_acl_rt_event *event);
 ...
 * @retval OtherValues Failure
 */
SVP_ACL_FUNC_VISIBILITY svp_acl_error svp_acl_rt_create_event_with_flag(svp_acl_rt_event *event,
    uint32_t flag);
 ...
 *
 * @par Function
 *  Only events created through the svp_acl_rt_create_event interface can be
 *  destroyed, synchronous interfaces. When destroying an event,
 *  the user must ensure that the tasks involved in the svp_acl_rt_synchronize_event
 *  interface or the svp_acl_rt_stream_wait_event interface are completed before
 *  they are destroyed.
 *
 ...
 * @retval OtherValues Failure
 *
 * @see svp_acl_rt_create_event | svp_acl_rt_synchronize_event | svp_acl_rt_stream_wait_event
 */
SVP_ACL_FUNC_VISIBILITY svp_acl_error svp_acl_rt_destroy_event(svp_acl_rt_event event);
 ...
 * @retval OtherValues Failure
 */
SVP_ACL_FUNC_VISIBILITY svp_acl_error svp_acl_rt_record_event(svp_acl_rt_event event, svp_acl_rt_stream stream);
 ...
 * @retval OtherValues Failure
 */
SVP_ACL_FUNC_VISIBILITY svp_acl_error svp_acl_rt_stream_wait_event(svp_acl_rt_stream stream,
 ...
 * @retval OtherValues Failure
 */
SVP_ACL_FUNC_VISIBILITY svp_acl_error svp_acl_rt_reset_event(svp_acl_rt_event event, svp_acl_rt_stream stream);
 ...
 * @retval OtherValues Failure
 */
SVP_ACL_FUNC_VISIBILITY svp_acl_error svp_acl_rt_query_event(svp_acl_rt_event event,
    svp_acl_rt_event_status *status);
...
* @retval OtherValues Failure
*/
SVP_ACL_FUNC_VISIBILITY svp_acl_error svp_acl_rt_query_event_wait_status(svp_acl_rt_event event,
    svp_acl_rt_event_wait_status *status);
 ...
 * @retval OtherValues Failure
 */
SVP_ACL_FUNC_VISIBILITY svp_acl_error svp_acl_rt_synchronize_event(svp_acl_rt_event event);
 ...
 * @retval OtherValues Failure
 *
 * @see svp_acl_rt_create_event | svp_acl_rt_record_event | svp_acl_rt_synchronize_stream
 */
SVP_ACL_FUNC_VISIBILITY svp_acl_error svp_acl_rt_event_elapsed_time(float *ms,
    svp_acl_rt_event start_event, svp_acl_rt_event end_event);
...
/**
 * @ingroup svp_acl
 * @brief alloc security memory on device
 *
 * @par Function
 *  alloc for size linear security memory on device
 *  and return a pointer to allocated memory by *dev_ptr
 *
 * @par Restriction
 * @li The memory requested by the svp_acl_rt_malloc_sec interface needs to be released
 * through the svp_acl_rt_free_sec interface.
 * @param dev_ptr [IN|OUT]  pointer to pointer to allocated memory on device
 * @param size [IN]   alloc memory size
 * @param class_id [IN]   secure id of smmz buf
 * @retval SVP_ACL_SUCCESS The function is successfully executed.
 * @retval OtherValues Failure
 *
 * @see svp_acl_rt_free_sec
 */
SVP_ACL_FUNC_VISIBILITY svp_acl_error svp_acl_rt_malloc_sec(void **dev_ptr, size_t size, size_t class_id);
...
/**
 * @ingroup svp_acl
 * @brief free smmz memory
 *
 * @par Function
 *  can only free memory allocated through the svp_acl_rt_malloc_sec interface in sec policy
 * @param  dev_ptr [IN]  Pointer to memory to be freed
 * @retval SVP_ACL_SUCCESS The function is successfully executed.
 * @retval OtherValues Failure
 *
 * @see svp_acl_rt_malloc_sec
 */
SVP_ACL_FUNC_VISIBILITY svp_acl_error svp_acl_rt_free_sec(const void *dev_ptr);
