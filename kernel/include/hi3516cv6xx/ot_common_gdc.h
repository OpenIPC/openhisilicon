...
#define OT_FISHEYE_MAX_RGN_NUM          9
#define OT_FISHEYE_LMF_COEF_NUM         128

typedef td_u32 ot_gdc_handle;

typedef struct {
    ot_lut_cell_size    cell_size; /* gdc function cell size */
...
typedef struct {
    td_u32 width; /* query picture width; Range: [OT_FISHEYE_MIN_OUT_IMAGE_WIDTH, OT_GDC_MAX_IMAGE_WIDTH] */
    td_u32 height; /* query picture height; Range: [OT_FISHEYE_MIN_OUT_IMAGE_HEIGHT, OT_GDC_MAX_IMAGE_HEIGHT] */
    ot_dynamic_range dynamic_range; /* query picture dynamic range */
} ot_gdc_query_pic_info;
...
typedef enum {
    ...
    OT_FISHEYE_VIEW_MODE_SKIP = 4, /* skip trans and write out a solid color */
...
typedef struct {
    ...
    td_u32 hor_zoom; /* RW; Range: [1, 5265], horizontal zoom of normal mode correction region,
                        Range: [1, 4095], horizontal zoom of 360/180 panorama mode correction region */
    td_u32 ver_zoom; /* RW; Range: [1, 8191], vertical zoom of 180 panorama mode correction region,
                        Range: [1, 4095], vertical zoom of normal/360 panorama mode correction region */
    ot_rect out_rect; /* RW; out image rectangle attribute */
} ot_fisheye_rgn_attr;
...
typedef struct {
    ...
    td_u32 rgn_num; /* RW; range: [1, 9]; gdc correction region number */
...
typedef struct {
    ...
    ot_size dst_size; /* RW; range: width:[OT_FISHEYE_MIN_OUT_IMAGE_WIDTH,
                         OT_GDC_MAX_IMAGE_WIDTH],height:[OT_FISHEYE_MIN_OUT_IMAGE_HEIGHT, OT_GDC_MAX_IMAGE_HEIGHT]; */
