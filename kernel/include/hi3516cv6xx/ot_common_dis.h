...
/* Different reference frame mode of DIS */
typedef enum {
    OT_DIS_REF_MODE_REFRESH = 0,
    OT_DIS_REF_MODE_FIXED,
    OT_DIS_REF_MODE_BUTT
} ot_dis_ref_mode;
...
typedef struct {
    ...
    ot_dis_ref_mode ref_mode;
} ot_dis_param;

/* Rotation compensation of DIS */
typedef struct {
    td_u32 max_roll_delta;            /* RW; Range:[0,1800]; Maximum range of rotationable angle for current frame. */
    td_s32 final_roll;                /* RW; Range:[-1800,1800]; Final rotation degree for roll angle. */
} ot_dis_rotation_compensation;
...
typedef struct {
    ...
    td_bool rotation_compensation_en; /* RW; Rotation compensation enable. */
    ot_dis_rotation_compensation compensation; /* RW; Attribute of rotation compensation. */
} ot_dis_attr;
...
typedef struct {
    ...
    ot_src_point_info point_info;
} ot_dis_alg_attr;

typedef struct {
    td_u32 match_frame;         /* RW; Range:[0, OT_VI_MAX_WDR_FRAME_NUM - 1]; only support wdr and gyro mode;
