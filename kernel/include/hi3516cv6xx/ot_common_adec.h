...
typedef enum {
    OT_ADEC_MODE_PACK = 0, /* require input is valid dec pack(a complete frame encode result), e.g. the stream get from
                              AENC is a valid dec pack, the stream know actually pack len from file is also a dec pack.
                              this mode is high-performative. */
    OT_ADEC_MODE_STREAM,   /* input is stream, low-performative, if you couldn't find out whether a stream is valid dec
                              pack, you could use this mode. */
    OT_ADEC_MODE_BUTT
} ot_adec_mode;
...
typedef struct {
    td_u8 **in_buf;       /* pointer of input data, may be changed after decode */
    td_s32 *in_left_byte; /* left length of input data, may be changed after decode */

    td_u16 *out_buf;       /* pointer of output buf */
...
typedef struct {
    ...
    td_s32 (*func_open_decoder)(td_void *decoder_attr, td_void **decoder); /* struct decoder is packed by user,
                                                                              ...
                                                                              for this struct */
    td_s32 (*func_dec_frame)(td_void *decoder, ot_adec_buf_info *buf_info, td_u32 *out_chns);
