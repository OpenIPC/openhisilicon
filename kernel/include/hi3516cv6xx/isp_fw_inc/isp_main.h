...
typedef struct {
    ...
    td_bool isp_dgain_en;
    td_u32  fe_isp_dgain;
    td_bool sync_fe_isp_dgain_en_buf[ISP_SYNC_ISO_BUF_MAX];
#endif
    td_u32  again;
    td_u32  dgain;
...
typedef union {
    ...
    struct {
        ...
        td_u64  bit1_agamma      : 1;   /* [40] */
        td_u64  bit1_adgamma     : 1;   /* [41] */
...
#define isp_check_mem_init_notrace_goto(pipe, ret, failed)                 \
    do {                                               \
...
#define isp_check_isp_user_init_goto(pipe, ret, exit)                                             \
    do {                                                                    \
        isp_usr_ctx *isp_usr_ctx = isp_get_usr_ctx(pipe);                       \
        if (isp_usr_ctx->para_rec.init != TD_TRUE) {                            \
...
#define isp_check_isp_user_init_notrace_goto(pipe, ret, exit)                                             \
    do {                                                                    \
        isp_usr_ctx *isp_usr_ctx = isp_get_usr_ctx(pipe);                       \
        if (isp_usr_ctx->para_rec.init != TD_TRUE) {                            \
            (ret) = OT_ERR_ISP_NOT_INIT;                                      \
...
td_s32 isp_check_sns_register(ot_vi_pipe vi_pipe, const isp_usr_ctx *isp_ctx);
