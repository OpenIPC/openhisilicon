...
XMEDIA_S32 sync_call_back(XMEDIA_U64 u64Data);
XMEDIA_S32 sync_af_calc(XMEDIA_U64 u64Data);
    ...
    switch (cmd) {
        ...
        case SAMPLE_IST_DEL_NODE:
            if (CKFN_ISP_UnRegisterSyncTask()) {
                if (CALL_ISP_UnRegisterSyncTask(ViPipe, &syncNode[node_index]) == XMEDIA_FAILURE) {
...
int __init sample_ist_driver_init(void)
{
    return sample_ist_init();
}
