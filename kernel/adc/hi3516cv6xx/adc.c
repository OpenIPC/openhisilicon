...
static osal_mutex g_lsadc_open_mutex;
...
static int reset_adc_control(void)
{
...
static int ot_lsadc_open(void *private_data)
{
    ...
    osal_mutex_lock(&g_lsadc_open_mutex);
    is_opened = 1;
    osal_mutex_unlock(&g_lsadc_open_mutex);

    if (reset_adc_control() != 0) {
        lsadc_print("reset adc control error.\n");
        return -1;
    }
...
static int lsadc_get_chn_value(unsigned int chn)
{
    ...
    return (int)lsadc_readl(OT_LSADC_CHNDATA + (chn << 2)); /* 2: each chn has 4 bytes int data, so shift left 2 bits */
}
...
static int adc_do_ioctl(unsigned int cmd, void *arg, void *private_data)
{
    ...
    switch (cmd) {
        ...
        case LSADC_IOC_GET_CHNVAL:
            param = *(int *)arg;
            ret = lsadc_get_chn_value((unsigned int)param);
            break;
...
int lsadc_init(void)
{
    ...
    if (osal_mutex_init(&g_lsadc_open_mutex) < 0) {
        lsadc_print("g_lsadc_open_mutex init failed!\n");
        goto init_mutex_fail;
    }
...
init_spin_lock_fail:
    osal_mutex_destroy(&g_lsadc_open_mutex);
init_mutex_fail:
