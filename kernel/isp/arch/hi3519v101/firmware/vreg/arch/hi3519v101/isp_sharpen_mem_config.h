...
static __inline void hi_sharpen_mem_array_data_write(ISP_DEV IspDev, HI_U32 index, HI_U32 data) {
    HI_U32 addr = HI_SHARPEN_MEM_BASE_ADDR + (index << 2);
    IOWR_32DIRECT(ISP_REG_BASE(IspDev) + VI_ISP_OFFSET + addr, data & 0xFFFFFFFF);
}
static __inline HI_U16 hi_sharpen_mem_array_data_read(ISP_DEV IspDev, HI_U32 index) {
    HI_U32 addr = HI_SHARPEN_MEM_BASE_ADDR + (index << 2);
    return (HI_U16) IORD_32DIRECT(ISP_REG_BASE(IspDev) + VI_ISP_OFFSET + addr);
}
