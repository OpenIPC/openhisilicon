...
HI_S32     g_as32IspFd[ISP_MAX_DEV_NUM] = {-1, -1};
...
HI_S32 HI_MPI_ISP_Init(ISP_DEV IspDev)
{
    ...
    /*  init Dng info bufs. */
    s32Ret = ISP_DngInfoInit(IspDev, &pstIspCtx->stDngInfoCtrl);
    if (HI_SUCCESS != s32Ret)
    {
        goto fail4;
    }
    ...
    ISP_DngExtRegsInitialize(IspDev, (ISP_DNG_COLORPARAM_S*)&pstSnsDft->stDngColorParam);
    hi_ext_system_dng_static_info_valid_write(IspDev, pstSnsDft->stSensorMode.bValidDngRawFormat);
    ...
    return HI_SUCCESS;
fail5:  
    ISP_DngInfoExit(IspDev, &pstIspCtx->stDngInfoCtrl);
fail4:  
