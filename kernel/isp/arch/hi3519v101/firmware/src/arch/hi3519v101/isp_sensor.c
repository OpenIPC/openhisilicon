...
HI_S32 ISP_SensorRegCallBack(ISP_DEV IspDev, SENSOR_ID SensorId, ISP_SENSOR_REGISTER_S *pstRegister)
{
    ...
    {
       pstSensor->stRegister.stSnsExp.pfn_cmos_sensor_global_init(IspDev);
    }
...
HI_S32 ISP_SensorUpdateAll(ISP_DEV IspDev)
{
    ...
    {
        pstSensor->stRegister.stSnsExp.pfn_cmos_get_isp_default(IspDev, &pstSensor->stSnsDft);
    }
    else
    {
    ...
    {
        pstSensor->stRegister.stSnsExp.pfn_cmos_get_isp_black_level(IspDev, &pstSensor->stSnsBlackLevel);
    }
...
HI_S32 ISP_SensorInit(ISP_DEV IspDev)
{
    ...
    {
        pstSensor->stRegister.stSnsExp.pfn_cmos_sensor_init(IspDev);
    }
    else
    {
        return HI_FAILURE;
...
HI_S32 ISP_SensorSwitch(ISP_DEV IspDev)
{
    ISP_SENSOR_S *pstSensor = HI_NULL;
    SENSOR_GET_CTX(IspDev, pstSensor);
    HI_S8 s8SspDev;
...
HI_S32 ISP_SensorExit(ISP_DEV IspDev)
{
    ...
    {
        pstSensor->stRegister.stSnsExp.pfn_cmos_sensor_exit(IspDev);
    }
    else
    {
        return HI_FAILURE;
...
HI_S32 ISP_SensorSetWDRMode(ISP_DEV IspDev, HI_U8 u8Mode)
{
    ...
    {
        pstSensor->stRegister.stSnsExp.pfn_cmos_set_wdr_mode(IspDev, u8Mode);
    }
...
HI_S32 ISP_SensorSetImageMode(ISP_DEV IspDev, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSnsImageMode)
{
    ...
    {
        if (HI_SUCCESS != pstSensor->stRegister.stSnsExp.pfn_cmos_set_image_mode(IspDev, pstSnsImageMode))
        {
            return HI_FAILURE;
        }
    }
...
HI_S32 ISP_SensorSetPixelDetect(ISP_DEV IspDev, HI_BOOL bEnable)
{
    ...
    {
        pstSensor->stRegister.stSnsExp.pfn_cmos_set_pixel_detect(IspDev, bEnable);
    }
    else
    {
        return HI_FAILURE;
...
HI_S32 ISP_SensorUpdateSnsReg(ISP_DEV IspDev)
{
    ...
    {
        pstSensor->stRegister.stSnsExp.pfn_cmos_get_sns_reg_info(IspDev, &pstSensor->stSnsRegInfo);
    }
    else
    {
        return HI_FAILURE;
