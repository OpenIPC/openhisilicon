...
HI_S32 ISP_UpdateInfoInit(ISP_DEV IspDev,ISP_UPDATE_INFO_CTRL_S* pstUpdateInfoCtrl)
{
    ...
    hi_ext_system_update_info_phyaddr_write(IspDev, pstUpdateInfoCtrl->u32UpdateInfoPhyaddr);

    pstUpdateInfoCtrl->pstIspUpdateInfo = HI_MPI_SYS_Mmap(pstUpdateInfoCtrl->u32UpdateInfoPhyaddr,
    ...
    if (HI_NULL == pstUpdateInfoCtrl->pstIspUpdateInfo)
    {
        ...
        if (HI_SUCCESS != s32Ret)
        {
            printf("exit update info bufs failed %x!\n", s32Ret);
            return s32Ret;
        }
