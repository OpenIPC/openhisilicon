...
/* ISP ModParam info */
XMEDIA_U32  g_PwmNumber[ISP_MAX_PIPE_NUM]  = { [0 ...(ISP_MAX_PIPE_NUM - 1)] = 3 };
XMEDIA_U32  g_UpdatePos[ISP_MAX_PIPE_NUM]  = { 0 }; /* 0: frame start; 1: frame end */

/* The time(unit:ms) of interrupt timeout */
XMEDIA_U32  g_IntTimeout[ISP_MAX_PIPE_NUM] = { [0 ...(ISP_MAX_PIPE_NUM - 1)] = 200 };

/* update isp statistic information per stat-intval frame, purpose to reduce CPU load */
XMEDIA_U32  g_StatIntvl[ISP_MAX_PIPE_NUM]  = { [0 ...(ISP_MAX_PIPE_NUM - 1)] = 1 };

/* 0: close proc; n: write proc info's interval int num */
XMEDIA_U32  g_ProcParam[ISP_MAX_PIPE_NUM]  = { [0 ...(ISP_MAX_PIPE_NUM - 1)] = 30 };
XMEDIA_U32  g_PortIntDelay[ISP_MAX_PIPE_NUM] = { 0 }; /* Port intertupt delay value */
XMEDIA_BOOL g_IntBothalf = XMEDIA_FALSE;  /* 1 to enable interrupt processing at bottom half */
XMEDIA_BOOL g_QuickStart = XMEDIA_FALSE;  /* 1 to enable interrupt processing at bottom half */
XMEDIA_BOOL g_bLongFrmIntEn = XMEDIA_FALSE; /* 1 : enable long frame  pipe interrupt*/
XMEDIA_BOOL g_UseBothalf = XMEDIA_FALSE;  /* 1 to use interrupt processing at bottom half */
XMEDIA_U32  g_IspExitTimeout = 2000;  /* The time(unit:ms) of exit be buffer timeout */
XMEDIA_U16  g_au16ResMaxWidth[ISP_MAX_PIPE_NUM]  = { ISP_RES_WIDTH_MAX };
XMEDIA_U16  g_au16ResMaxHeight[ISP_MAX_PIPE_NUM] = { ISP_RES_HEIGHT_MAX };

/* ldci temporal filter enable */
XMEDIA_BOOL  g_LdciTprFltEn[ISP_MAX_PIPE_NUM]  = { [0 ...(ISP_MAX_PIPE_NUM - 1)] = XMEDIA_FALSE };

// #define TEST_TIME

#ifdef TEST_TIME
XMEDIA_U32  g_test_phyaddr;
XMEDIA_U16 *g_test_pviraddr;
...
XMEDIA_VOID ISP_DRV_SlavePipeIntEnable(VI_PIPE ViPipe, XMEDIA_BOOL bEn)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    XMEDIA_U8 k;
    XMEDIA_U8 u8ChnNumMax;
    VI_PIPE ViPipeBind;
    pstDrvCtx = ISP_DRV_GET_CTX(ViPipe);
    if(g_bLongFrmIntEn == XMEDIA_FALSE)
    {
        return ;
    }
    if(IS_FS_WDR_MODE(pstDrvCtx->stSyncCfg.u8WDRMode) == XMEDIA_FALSE)
    {
        return;
    }
 ...
 ****************************************************************************/
XMEDIA_S32 ISP_DRV_SetIntEnable(VI_PIPE ViPipe, XMEDIA_BOOL bEn)
{
    XMEDIA_S32 ViDev;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_WaitConditionCallback(const XMEDIA_VOID *pParam)
{
    XMEDIA_BOOL bCondition;

    bCondition = *(XMEDIA_BOOL *)pParam;

    return (bCondition == XMEDIA_TRUE);
}

XMEDIA_S32 ISP_DRV_WaitExitCallback(const XMEDIA_VOID *pParam)
{
    XMEDIA_S32 bCondition;

    bCondition = *(XMEDIA_S32 *)pParam;

    return (bCondition == 0);
}

XMEDIA_S32 ISP_GetFrameEdge(VI_PIPE ViPipe, XMEDIA_U32 *pu32Status)
{
...
static int ISP_GetVdStartTimeOut(VI_PIPE ViPipe, XMEDIA_U32 u32MilliSec, XMEDIA_U32 *pu32status)
{
    unsigned long u32Flags;
    XMEDIA_S32 s32Ret;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ...
    osal_spin_lock_irqsave(&g_stIspLock[ViPipe], &u32Flags);
    pstDrvCtx->bVdStart = XMEDIA_FALSE;
    osal_spin_unlock_irqrestore(&g_stIspLock[ViPipe], &u32Flags);
...
static int ISP_GetVdEndTimeOut(VI_PIPE ViPipe, XMEDIA_U32 u32MilliSec, XMEDIA_U32 *pu32status)
{
    unsigned long u32Flags;
    XMEDIA_S32 s32Ret;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
...
static int ISP_GetVdBeEndTimeOut(VI_PIPE ViPipe, XMEDIA_U32 u32MilliSec, XMEDIA_U32 *pu32status)
{
    unsigned long u32Flags;
    XMEDIA_S32 s32Ret;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ...
    osal_spin_lock_irqsave(&g_stIspLock[ViPipe], &u32Flags);
    pstDrvCtx->bVdBeEnd = XMEDIA_FALSE;
    osal_spin_unlock_irqrestore(&g_stIspLock[ViPipe], &u32Flags);
...
XMEDIA_S32 ISP_DRV_WorkModeInit(VI_PIPE ViPipe, ISP_BLOCK_ATTR_S  *pstBlkAttr)
{
    ...
    if ((stPipeSplitAttr.enPixFmt == PIXEL_FORMAT_YUV_SEMIPLANAR_420) ||
        ...
        pstDrvCtx->bYuvMode = XMEDIA_TRUE;
        pstDrvCtx->stWorkMode.enIspDataInputMode = ISP_MODE_BT1120_YUV;
    } else {
        pstDrvCtx->bYuvMode = XMEDIA_FALSE;
...
XMEDIA_S32 ISP_DRV_WorkModeExit(VI_PIPE ViPipe)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ISP_CHECK_PIPE(ViPipe);
...
XMEDIA_U8 ISP_DRV_GetBlockNum(VI_PIPE ViPipe)
{
    return g_astIspDrvCtx[ViPipe].stWorkMode.u8BlockNum;
}

XMEDIA_S8 ISP_DRV_GetBlockId(VI_PIPE ViPipe, ISP_RUNNING_MODE_E enRunningMode)
{
    XMEDIA_S8 s8BlockId = 0;
...
XMEDIA_S32 ISP_DRV_GetBeRegsAttr(VI_PIPE ViPipe, S_ISPBE_REGS_TYPE *apstBeReg[], ISP_BE_REGS_ATTR_S *pstBlkAttr)
{
    XMEDIA_U8  k = 0;
    XMEDIA_S8  s8BlockId, u8BlkDev, u8BlkNum;
    ISP_RUNNING_MODE_E enIspRunningMode;
    ISP_BE_WO_REG_CFG_S *pstIspBeRegCfg = XMEDIA_NULL;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ...
    switch (enIspRunningMode) {
        ...
        case ISP_MODE_RUNNING_OFFLINE:
            ...
            if (pstDrvCtx->pstUseNode == XMEDIA_NULL) {
...
XMEDIA_S32 ISP_DRV_ChnSelectWrite(VI_PIPE ViPipe, XMEDIA_U32 u32ChannelSel)
{
    XMEDIA_U8   k;
    XMEDIA_U8   u8BlkDev, u8BlkNum;
    XMEDIA_U32  au32ChnSwitch[5] = { 0 };
    XMEDIA_U32  s32Ret;
    ISP_BE_REGS_ATTR_S stIspBeRegsAttr = { 0 };
    S_ISPBE_REGS_TYPE *apstBeReg[ISP_STRIPING_MAX_NUM] = { XMEDIA_NULL };
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    pstDrvCtx = ISP_DRV_GET_CTX(ViPipe);
    ...
    if (pstDrvCtx->bYuvMode == XMEDIA_TRUE) {
        au32ChnSwitch[4] = 1;
    } else {
        au32ChnSwitch[4] = 0;
    }
...
/* ISP FE read sta */
XMEDIA_S32 ISP_DRV_FE_StatisticsRead(VI_PIPE ViPipe, ISP_STAT_INFO_S *pstStatInfo)
{
    XMEDIA_U8          u8ChnNumMax;
    XMEDIA_U32         k;
    VI_PIPE        ViPipeBind;
    ...
    ISP_DRV_CTX_S     *pstDrvCtx = XMEDIA_NULL;
    ISP_STAT_S        *pstStat   = XMEDIA_NULL;
    S_ISPFE_REGS_TYPE *pstFeReg  = XMEDIA_NULL;
    ...
    pstDrvCtx = ISP_DRV_GET_CTX(ViPipe);
    if (pstDrvCtx->stWDRAttr.bMastPipe == XMEDIA_FALSE) {
        return XMEDIA_SUCCESS;
    }

    pstStat = (ISP_STAT_S *)pstStatInfo->pVirtAddr;
    if (pstStat == XMEDIA_NULL) {
        return XMEDIA_FAILURE;
    }
...
XMEDIA_S32 ISP_DRV_BeStatisticsRead(VI_PIPE ViPipe, ISP_STAT_INFO_S *pstStatInfo)
{
    XMEDIA_S32 s32Ret;
    XMEDIA_U8  u8BlkDev, u8BlkNum;
    ISP_STAT_S *pstStat = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_BeOfflineStitchStatisticsRead(VI_PIPE ViPipe, ISP_STAT_INFO_S *pstStatInfo)
{
    XMEDIA_S32 k = 0;
    XMEDIA_S32 s32Ret;
    XMEDIA_U8 u8BlkDev;
    ...
    if ((pstDrvCtx->stStitchAttr.bMainPipe != XMEDIA_TRUE) && (pstDrvCtx->stStitchAttr.bStitchEnable != XMEDIA_TRUE)) {
        return  XMEDIA_SUCCESS;
    }
    ...
    for (k = 0; k < pstDrvCtx->stStitchAttr.u8StitchPipeNum; k++) {
        ...
        if (stBeStitch.pstSyncBeStt[k] == XMEDIA_NULL) {
    ...
    if (CKFN_VI_GetStitchSyncBeSttInfo()) {
        ...
        if (s32Ret != XMEDIA_SUCCESS) {
            pstStat->bBEUpdate = XMEDIA_FALSE;
    ...
    if (pstStat->bBEUpdate == XMEDIA_FALSE) {
...
XMEDIA_S32 ISP_DRV_BeOfflineStatisticsRead(VI_PIPE ViPipe, ISP_STAT_INFO_S *pstStatInfo)
{
    XMEDIA_S32 k;
    XMEDIA_S32 s32Ret;
    XMEDIA_U8  u8BlkNum, u8BlkDev;
    XMEDIA_S8  s8BlockId = 0;
    ISP_STAT_KEY_U unStatkey;
    ISP_STAT_S *pstStat = XMEDIA_NULL;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;

    S_ISP_STT_REGS_TYPE *apstBeStt[ISP_STRIPING_MAX_NUM] = { [0 ...(ISP_STRIPING_MAX_NUM - 1)] = XMEDIA_NULL };
    ISP_RUNNING_MODE_E enIspRunningMode;
    ...
    if (g_LdciTprFltEn[ViPipe] == XMEDIA_TRUE) {
...
XMEDIA_S32 ISP_DRV_StatisticsRead(VI_PIPE ViPipe, ISP_STAT_INFO_S *pstStatInfo)
{
    XMEDIA_S32 s32Ret;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ...
    if (pstDrvCtx->stStitchAttr.bStitchEnable == XMEDIA_TRUE) {
        if (pstDrvCtx->stStitchAttr.bMainPipe == XMEDIA_TRUE) {
...
XMEDIA_S32 ISP_DRV_StatBufInit(VI_PIPE ViPipe, XMEDIA_U64 *pu64PhyAddr)
{
    XMEDIA_S32 s32Ret, i;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    XMEDIA_U64 u64PhyAddr, u64Size;
    XMEDIA_U8  *pu8VirAddr;
    XMEDIA_CHAR acName[MAX_MMZ_NAMELEN] = {0};
    unsigned long u32Flags;
    ...
    s32Ret = CMPI_MmzMallocCached(XMEDIA_NULL, acName, &u64PhyAddr, (XMEDIA_VOID **)&pu8VirAddr, u64Size);
    ...
    pstDrvCtx->stStatisticsBuf.pVirAddr = (XMEDIA_VOID *)pu8VirAddr;
...
XMEDIA_S32 ISP_DRV_StatBufExit(VI_PIPE ViPipe)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ...
    pu8VirAddr = (XMEDIA_U8 *)pstDrvCtx->stStatisticsBuf.pVirAddr;

    pstDrvCtx->stStatisticsBuf.pVirAddr = XMEDIA_NULL;
    pstDrvCtx->stStatisticsBuf.astNode[0].stStatInfo.pVirtAddr = XMEDIA_NULL;
    pstDrvCtx->stStatisticsBuf.astNode[1].stStatInfo.pVirtAddr = XMEDIA_NULL;
    ...
    pstDrvCtx->stStatisticsBuf.bInit    = XMEDIA_FALSE;
...
XMEDIA_S32 ISP_DRV_StatBufUserGet(VI_PIPE ViPipe, ISP_STAT_INFO_S **ppstStatInfo)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    struct osal_list_head *plist;
    ISP_STAT_NODE_S *pstNode = XMEDIA_NULL;
    unsigned long u32Flags;
...
XMEDIA_S32 ISP_DRV_StatBufUserPut(VI_PIPE ViPipe, ISP_STAT_INFO_S *pstStatInfo)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    struct osal_list_head *plist = XMEDIA_NULL;
    ISP_STAT_NODE_S *pstNode = XMEDIA_NULL;
    XMEDIA_BOOL bValid = XMEDIA_FALSE;
    unsigned long u32Flags;
...
XMEDIA_S32 ISP_DRV_SetLdciSttAddr(VI_PIPE ViPipe, XMEDIA_U64 u64ReadSttAddr, XMEDIA_U64 u64WriteSttAddr)
{
    XMEDIA_U8  k, u8BlkDev, u8BlkNum;
    XMEDIA_S32 s32Ret;
    XMEDIA_U32 u32Size = sizeof(ISP_LDCI_STAT_S);
    XMEDIA_U64 u64WriteAddr;
    ISP_BE_REGS_ATTR_S  stIspBeRegsAttr = { 0 };
    S_VIPROC_REGS_TYPE *apstViprocReg[ISP_STRIPING_MAX_NUM] = { XMEDIA_NULL };
    ...
    for (k = 0; k < u8BlkNum; k++) {
        ...
        if (g_LdciTprFltEn[ViPipe] == XMEDIA_FALSE) {
...
XMEDIA_S32 ISP_DRV_UpdateLdciNormalOnlineAddr(VI_PIPE ViPipe)
{
    XMEDIA_U8 u8ReadBufIdx, u8WriteBufIdx;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_CopyLdciStat(VI_PIPE ViPipe, ISP_STAT_S *pstStat)
{
    ISP_DRV_CTX_S   *pstDrvCtx   = XMEDIA_NULL;
    ISP_LDCI_STAT_S *pstLdciStat = XMEDIA_NULL;
    XMEDIA_U8 u8CurReadBufIdx;
    ISP_CHECK_PIPE(ViPipe);
    ...
    if (pstLdciStat != XMEDIA_NULL) {
        osal_memcpy(&pstStat->stLdciStat, pstLdciStat, sizeof(ISP_LDCI_STAT_S));
    }
    return XMEDIA_SUCCESS;
}
XMEDIA_S32 ISP_DRV_UpdateLdciTprOnlineAddr(VI_PIPE ViPipe)
{
    XMEDIA_U8 u8CurReadBufIdx;
    ISP_DRV_CTX_S   *pstDrvCtx   = XMEDIA_NULL;
    ISP_CHECK_PIPE(ViPipe);
...
XMEDIA_S32 ISP_DRV_UpdateLdciNormalOfflineAttr(VI_PIPE ViPipe, ISP_BE_SYNC_PARA_S *pstBeSyncPara)
{
    XMEDIA_U8  i, u8ReadBufIdx, u8WriteBufIdx, u8BlkNum;
    XMEDIA_U32 u32Size;
    XMEDIA_U64 u64WriteSttAddr, u64ReadSttAddr;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_UpdateLdciTprOfflineAttr(VI_PIPE ViPipe, ISP_BE_SYNC_PARA_S *pstBeSyncPara)
{
    XMEDIA_U8 i;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_LdciOnlineAttrUpdate(VI_PIPE ViPipe, ISP_STAT_S *pstStat)
{
    ISP_CHECK_PIPE(ViPipe);
    if (g_LdciTprFltEn[ViPipe] == XMEDIA_TRUE) {
...
XMEDIA_S32 ISP_DRV_StatBufBusyPut(VI_PIPE ViPipe)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    struct osal_list_head *plist;
    ISP_STAT_NODE_S *pstNode = XMEDIA_NULL;
...
static XMEDIA_VOID ISP_DRV_LdciBufAttrReset(ISP_DRV_CTX_S *pstDrvCtx)
{
    XMEDIA_U8 i;

    pstDrvCtx->stLdciReadBufAttr.bInit  = XMEDIA_FALSE;
    pstDrvCtx->stLdciWriteBufAttr.bInit = XMEDIA_FALSE;
    ...
    for (i = 0; i < MAX_ISP_BE_BUF_NUM; i++) {
        ...
        pstDrvCtx->stLdciWriteBufAttr.astLdciBuf[i].pVirAddr   = XMEDIA_NULL;
        pstDrvCtx->stLdciWriteBufAttr.astLdciBuf[i].u64Size    = 0;
    }

    for (i = 0; i < MAX_ISP_BE_BUF_NUM; i++) {
        pstDrvCtx->stLdciReadBufAttr.astLdciBuf[i].u64PhyAddr = 0;
        pstDrvCtx->stLdciReadBufAttr.astLdciBuf[i].pVirAddr   = XMEDIA_NULL;
        pstDrvCtx->stLdciReadBufAttr.astLdciBuf[i].u64Size    = 0;
    }
}

XMEDIA_S32 ISP_DRV_LdciTprBufInit(VI_PIPE ViPipe)
{
    XMEDIA_U8 i, j, u8WriteBufNum, u8ReadBufNum, u8BufNumAll;
    XMEDIA_U8 *pu8VirAddr;
    XMEDIA_S32 s32Ret;
    XMEDIA_U64 u64PhyAddr, u64Size;
    unsigned long u32Flags;
    ...
    s32Ret = CMPI_MmzMallocNocache(XMEDIA_NULL, acName, &u64PhyAddr, (XMEDIA_VOID **)&pu8VirAddr, u64Size * u8BufNumAll);
    ...
    pstDrvCtx->stLdciWriteBufAttr.bInit    = XMEDIA_TRUE;
    pstDrvCtx->stLdciWriteBufAttr.u8BufNum = u8WriteBufNum;
    pstDrvCtx->stLdciWriteBufAttr.u8BufIdx = 0;

    pstDrvCtx->stLdciReadBufAttr.bInit    = XMEDIA_TRUE;
    ...
    for (i = 0; i < u8WriteBufNum; i++) {
        ...
        pstDrvCtx->stLdciWriteBufAttr.astLdciBuf[i].pVirAddr   = (XMEDIA_VOID *)(pu8VirAddr + i * u64Size);
    ...
    for (i = 0; i < u8ReadBufNum; i++) {
        ...
        pstDrvCtx->stLdciReadBufAttr.astLdciBuf[i].pVirAddr   = (XMEDIA_VOID *)(pu8VirAddr + j * u64Size);
...
fail0:
    ...
    if (u64PhyAddr != 0) {
        CMPI_MmzFree(u64PhyAddr, (XMEDIA_VOID *)pu8VirAddr);
    }
    return XMEDIA_FAILURE;
}

XMEDIA_S32 ISP_DRV_LdciTprBufExit(VI_PIPE ViPipe)
{
    XMEDIA_U64 u64PhyAddr;
...
XMEDIA_S32 ISP_DRV_LdciNormalBufInit(VI_PIPE ViPipe)
{
    XMEDIA_U8 i, u8BufNum;
    XMEDIA_S32 s32Ret;
    XMEDIA_U64 u64PhyAddr, u64Size;
    unsigned long u32Flags;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ...
    u64PhyAddr = CMPI_MmzMalloc(XMEDIA_NULL, acName, u64Size * u8BufNum);
    if (u64PhyAddr == 0) {
...
fail0:
    ...
    if (u64PhyAddr != 0) {
        CMPI_MmzFree(u64PhyAddr, XMEDIA_NULL);
    }

    return XMEDIA_SUCCESS;
}

XMEDIA_S32 ISP_DRV_LdciNormalBufExit(VI_PIPE ViPipe)
{
    XMEDIA_U64 u64PhyAddr;
    unsigned long u32Flags;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_LdciBufInit(VI_PIPE ViPipe)
{
    ISP_CHECK_PIPE(ViPipe);
...
XMEDIA_S32 ISP_DRV_LdciBufExit(VI_PIPE ViPipe)
{
    ISP_CHECK_PIPE(ViPipe);
    ISP_CHECK_EXIT_STATE(ViPipe);
...
XMEDIA_S32 ISP_DRV_LdciReadSttBufGet(VI_PIPE ViPipe, ISP_LDCI_READ_STT_BUF_S *pstLdciReadSttBuf)
{
    unsigned long u32Flags;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_ClutBufInit(VI_PIPE ViPipe)
{
    XMEDIA_S32 s32Ret;
    XMEDIA_U64 u64PhyAddr, u64Size;
    XMEDIA_U8 *pu8VirAddr;
    ...
    u64Size =  ISP_CLUT_LUT_LENGTH * sizeof(XMEDIA_U32);
    ...
    pstDrvCtx->stClutBufAttr.bInit                = XMEDIA_TRUE;
    pstDrvCtx->stClutBufAttr.stClutBuf.u64Size    = u64Size;
    pstDrvCtx->stClutBufAttr.stClutBuf.u64PhyAddr = u64PhyAddr;
    pstDrvCtx->stClutBufAttr.stClutBuf.pVirAddr   = (XMEDIA_VOID *)pu8VirAddr;

    return XMEDIA_SUCCESS;
}

XMEDIA_S32 ISP_DRV_ClutBufExit(VI_PIPE ViPipe)
{
    XMEDIA_U64   u64PhyAddr;
    XMEDIA_VOID *pVirAddr;
    ISP_DRV_CTX_S      *pstDrvCtx = XMEDIA_NULL;
    ...
    pstDrvCtx->stClutBufAttr.bInit                = XMEDIA_FALSE;
    pstDrvCtx->stClutBufAttr.stClutBuf.u64Size    = 0;
    pstDrvCtx->stClutBufAttr.stClutBuf.u64PhyAddr = 0;
    pstDrvCtx->stClutBufAttr.stClutBuf.pVirAddr   = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_SpecAwbBufInit(VI_PIPE ViPipe)
{
    ...
    pstDrvCtx->stSpecAwbBufAttr.bInit                   = XMEDIA_TRUE;
    pstDrvCtx->stSpecAwbBufAttr.stSpecAwbBuf.u64Size    = u64Size;
    pstDrvCtx->stSpecAwbBufAttr.stSpecAwbBuf.u64PhyAddr = u64PhyAddr;
    pstDrvCtx->stSpecAwbBufAttr.stSpecAwbBuf.pVirAddr   = (XMEDIA_VOID *)pu8VirAddr;
#endif
    return XMEDIA_SUCCESS;
}

XMEDIA_S32 ISP_DRV_SpecAwbBufExit(VI_PIPE ViPipe)
{
#ifdef CONFIG_ISP_SPECAWB_SUPPORT
    XMEDIA_U64   u64PhyAddr;
    XMEDIA_VOID *pVirAddr;
    ...
    pstDrvCtx->stSpecAwbBufAttr.bInit                   = XMEDIA_FALSE;
    pstDrvCtx->stSpecAwbBufAttr.stSpecAwbBuf.u64Size    = 0;
    pstDrvCtx->stSpecAwbBufAttr.stSpecAwbBuf.u64PhyAddr = 0;
    pstDrvCtx->stSpecAwbBufAttr.stSpecAwbBuf.pVirAddr   = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_BeBufInit(VI_PIPE ViPipe, XMEDIA_U64 *pu64PhyAddr)
{
    XMEDIA_S32 s32Ret;
    XMEDIA_U32 i;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ISP_BE_BUF_NODE_S *pstNode = XMEDIA_NULL;
    XMEDIA_U64 u64PhyAddr, u64Size;
    XMEDIA_U8  *pu8VirAddr;
    XMEDIA_CHAR acName[MAX_MMZ_NAMELEN] = {0};
    unsigned long u32Flags;
    ...
    s32Ret = CMPI_MmzMallocCached(XMEDIA_NULL, acName, &u64PhyAddr, (XMEDIA_VOID **)&pu8VirAddr, u64Size * MAX_ISP_BE_BUF_NUM);
    ...
    pstDrvCtx->stBeBufInfo.bInit                   = XMEDIA_TRUE;
    pstDrvCtx->stBeBufInfo.stBeBufHaddr.u64PhyAddr = u64PhyAddr;
    pstDrvCtx->stBeBufInfo.stBeBufHaddr.pVirAddr   = (XMEDIA_VOID *)pu8VirAddr;
    ...
    for (i = 0; i < MAX_ISP_BE_BUF_NUM; i++) {
        ...
        pstNode->stBeCfgBuf.pVirAddr   = (XMEDIA_VOID *)(pu8VirAddr + i * u64Size);
    ...
    pstDrvCtx->pstUseNode        = XMEDIA_NULL;
...
fail0:
    pstDrvCtx->stBeBufInfo.bInit                   = XMEDIA_FALSE;
    pstDrvCtx->stBeBufInfo.stBeBufHaddr.u64PhyAddr = 0;
    pstDrvCtx->stBeBufInfo.stBeBufHaddr.pVirAddr   = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_BeBufExit(VI_PIPE ViPipe)
{
    XMEDIA_S32 s32Ret;
    XMEDIA_U64 u64PhyAddr;
    XMEDIA_VOID *pVirAddr;
...
XMEDIA_S32 ISP_DRV_GetReadyBeBuf(VI_PIPE ViPipe, ISP_BE_WO_CFG_BUF_S *pstBeCfgBuf)
{
...
XMEDIA_VOID ISP_DRV_PutBusyToFree(VI_PIPE ViPipe, ISP_BE_WO_CFG_BUF_S *pstBeCfgBuf)
{
    ISP_BE_BUF_NODE_S *pstNode;
    struct osal_list_head *pListTmp = XMEDIA_NULL;
    struct osal_list_head *pListNode = XMEDIA_NULL;
    ISP_DRV_CTX_S *pstDrvCtx  = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_PutFreeBeBuf(VI_PIPE ViPipe, ISP_BE_WO_CFG_BUF_S *pstBeCfgBuf)
{
    osal_spinlock_t *pIspSpinLock = XMEDIA_NULL;
    ISP_DRV_CTX_S *pstDrvCtx  = XMEDIA_NULL;
    unsigned long u32Flags;
...
XMEDIA_S32 ISP_DRV_HoldBusyBeBuf(VI_PIPE ViPipe, ISP_BE_WO_CFG_BUF_S *pstBeCfgBuf)
{
    osal_spinlock_t *pIspSpinLock = XMEDIA_NULL;
    ISP_BE_BUF_NODE_S *pstNode;
    struct osal_list_head *pListTmp = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_GetBeSyncPara(VI_PIPE ViPipe, ISP_BE_SYNC_PARA_S *pstBeSyncPara)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    XMEDIA_S32 s32Ret;
    unsigned long u32Flags;
...
XMEDIA_S32 ISP_DRV_BeBufSwitchState(VI_PIPE ViPipe)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    unsigned long u32Flags;
    osal_spinlock_t *pIspSpinLock = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_SwitchMode(VI_PIPE ViPipe, ISP_DRV_CTX_S *pstDrvCtx)
{
    XMEDIA_U8   i, j, k;
    XMEDIA_U8   u8BlkDev, u8BlkNum;
    XMEDIA_S32  ViDev;
    XMEDIA_U32  s32Ret;
    XMEDIA_U32  au32ChnSwitch[5] = { 0 };
    ISP_SYNC_CFG_S *pstSyncCfg = XMEDIA_NULL;
    ISP_BE_REGS_ATTR_S stIspBeRegsAttr = { 0 };
    S_ISPBE_REGS_TYPE *apstBeReg[ISP_STRIPING_MAX_NUM] = { XMEDIA_NULL };
...
XMEDIA_S32 ISP_DRV_GetSyncControlnfo(VI_PIPE ViPipe, ISP_SYNC_CFG_S *pstSyncCfg)
{
    XMEDIA_S32 ViDev;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ISP_SYNC_CFG_BUF_NODE_S *pstCurNode = XMEDIA_NULL;
...
static XMEDIA_S32 ISP_DRV_CalcExpRatio(ISP_SYNC_CFG_S *pstSyncCfg, ISP_SYNC_CFG_BUF_NODE_S *pstCurNode,
                                   ISP_SYNC_CFG_BUF_NODE_S *pstPreNode, XMEDIA_U64 *au64CurSnsGain,
                                   XMEDIA_U64 *au64PreSnsGain)
{
    XMEDIA_U32 i, j;
    XMEDIA_U64 u64Tmp     = 0;
    XMEDIA_U64 u64Ratio   = 0;
    XMEDIA_U64 au64Exp[4] = { 0 };
    ...
    if ((IS_LINE_WDR_MODE(pstSyncCfg->u8WDRMode)) &&
        ...
        for (j = 0; j < 3; j++) {
            ...
            pstSyncCfg->u32ExpRatio[j][0] = (XMEDIA_U32)u64Ratio;
...
static XMEDIA_S32 ISP_DRV_CalcDrcComp(ISP_SYNC_CFG_S *pstSyncCfg, ISP_SYNC_CFG_BUF_NODE_S *pstCurNode,
                                  ISP_SYNC_CFG_BUF_NODE_S *pstPreNode)
{
    XMEDIA_U32 i;
    XMEDIA_U64 u64Tmp, u64CurExp, u64PreExp;
    ...
    pstSyncCfg->u32DRCComp[0] = (XMEDIA_U32)u64CurExp;

    return XMEDIA_SUCCESS;
}

XMEDIA_S32 ISP_DRV_CalcSyncCfg(ISP_SYNC_CFG_S *pstSyncCfg)
{
    XMEDIA_U32 i;
    ISP_SYNC_CFG_BUF_NODE_S *pstCurNode = XMEDIA_NULL;
    ISP_SYNC_CFG_BUF_NODE_S *pstPreNode = XMEDIA_NULL;
    XMEDIA_U64 au64CurSnsGain[2] = { 0 };
    XMEDIA_U64 au64PreSnsGain[2] = { 0 };
    XMEDIA_BOOL bErr = XMEDIA_FALSE;
    ...
    if (pstCurNode->bValid == XMEDIA_FALSE) {
        return XMEDIA_SUCCESS;
    }
    ...
    if (bErr == XMEDIA_TRUE) {
        if (pstCurNode->stSnsRegsInfo.enSnsType == ISP_SNS_I2C_TYPE) {
            for (i = 0; i < pstCurNode->stSnsRegsInfo.u32RegNum; i++) {
                pstCurNode->stSnsRegsInfo.astI2cData[i].bUpdate = XMEDIA_TRUE;
            }
        } else if (pstCurNode->stSnsRegsInfo.enSnsType == ISP_SNS_SSP_TYPE) {
            for (i = 0; i < pstCurNode->stSnsRegsInfo.u32RegNum; i++) {
                pstCurNode->stSnsRegsInfo.astSspData[i].bUpdate = XMEDIA_TRUE;
            }
        } else {
            /* do nothing */
        }

        pstCurNode->stSnsRegsInfo.stSlvSync.bUpdate = XMEDIA_TRUE;
    }

    if (pstSyncCfg->apstNode[1] == XMEDIA_NULL) {
        pstPreNode = pstSyncCfg->apstNode[0];
    } else {
        if (pstSyncCfg->apstNode[1]->bValid == XMEDIA_FALSE) {
            pstPreNode = pstSyncCfg->apstNode[0];
        } else {
    ...
    if (IS_2to1_WDR_MODE(pstSyncCfg->u8WDRMode)) {
        if (pstSyncCfg->u8VCCfgNum == 0) {
            ...
            au64PreSnsGain[0] = osal_div_u64(au64PreSnsGain[0], DIV_0_TO_1((XMEDIA_U64)pstPreNode->stAERegCfg.u32IspDgain * pstPreNode->stAERegCfg.au32WDRGain[0] >> 8));
            ...
            au64CurSnsGain[0] = osal_div_u64(au64CurSnsGain[0], DIV_0_TO_1((XMEDIA_U64)pstCurNode->stAERegCfg.u32IspDgain * pstCurNode->stAERegCfg.au32WDRGain[0] >> 8));
            ...
            au64PreSnsGain[1] = osal_div_u64(au64PreSnsGain[1], DIV_0_TO_1((XMEDIA_U64)pstPreNode->stAERegCfg.u32IspDgain * pstPreNode->stAERegCfg.au32WDRGain[1] >> 8));
            ...
            au64CurSnsGain[1] = osal_div_u64(au64CurSnsGain[1], DIV_0_TO_1((XMEDIA_U64)pstCurNode->stAERegCfg.u32IspDgain * pstCurNode->stAERegCfg.au32WDRGain[1] >> 8));
...
XMEDIA_U8 isp_drv_get_fe_sync_index(ISP_DRV_CTX_S *drv_ctx)
{
    XMEDIA_U8 index;
...
XMEDIA_U8 ISP_DRV_GetBeSyncIndex(ISP_DRV_CTX_S *pstDrvCtx)
{
    XMEDIA_U8 index;
...
XMEDIA_S32 ISP_DRV_RegConfigIsp(VI_PIPE ViPipe, ISP_DRV_CTX_S *pstDrvCtx)
{

    XMEDIA_S32  i;
    XMEDIA_U8   u8CfgNodeIdx, u8CfgNodeVC;
    XMEDIA_U32  u32IspDgain;
    XMEDIA_S32  s32Ret;
    XMEDIA_U32  u32Ratio[3] = { 0x40 };
    XMEDIA_U8   u8BlkDev, u8BlkNum;
    XMEDIA_U8   u8SnapBlkDev = 0;
    XMEDIA_U8   u8SnapBlkNum = 1;
    XMEDIA_U32  au32WDRGain[4]   = { 0x100, 0x100, 0x100, 0x100 };
    XMEDIA_U32  au32WDRWBGain[4] = { 0x100, 0x100, 0x100, 0x100 };
    XMEDIA_U32  au32WBViGain[4]  = { 0x100, 0x100, 0x100, 0x100 };
    XMEDIA_U16  au16CCM[9] = {
        0x01A7,  0x809D,  0x800A,
        0x804F,  0x018E,  0x803F,
        0x000B,  0x8098,  0x018D
    };
    ISP_AE_REG_CFG_2_S *pstAERegCfg = XMEDIA_NULL;
    ISP_AWB_REG_CFG_2_S *pstAWBRegCfg = XMEDIA_NULL;
    ISP_SYNC_CFG_BUF_NODE_S *pstCfgNode = XMEDIA_NULL;
    ISP_BE_REGS_ATTR_S stIspBeRegsAttr = { 0 };
    ISP_BE_REGS_ATTR_S stIspSnapBeRegsAttr = { 0 };
    S_ISPBE_REGS_TYPE *apstBeReg[ISP_STRIPING_MAX_NUM] = { XMEDIA_NULL };
    S_ISPBE_REGS_TYPE *apstSnapBeReg[ISP_STRIPING_MAX_NUM] = { XMEDIA_NULL };
    VI_PIPE MainPipeS;
    ISP_DRV_CTX_S *pstDrvCtxS = XMEDIA_NULL;
    VI_PIPE PicturePipe;
    ISP_DRV_CTX_S *pstDrvCtxPic = XMEDIA_NULL;
    ISP_RUNNING_MODE_E enPictureRunningMode = ISP_MODE_RUNNING_OFFLINE;
    XMEDIA_BOOL bOnlineHavePictruePipe = XMEDIA_FALSE;
    XMEDIA_U8   u8SyncIndex, u8ExpRatioIndex;
    ISP_CHECK_PIPE(ViPipe);
    ISP_CHECK_POINTER(pstDrvCtx);
    ...
    if (pstDrvCtx->stSnapAttr.s32PicturePipeId >= 0) {
        ...
        if ((IS_ONLINE_MODE(enPictureRunningMode)) || \
            ...
            if (PicturePipe != pstDrvCtx->stSnapAttr.s32PreviewPipeId) {
                bOnlineHavePictruePipe = XMEDIA_TRUE;
            }
        }
    }
    u8BlkDev = stIspBeRegsAttr.u8BlockDev;
    ...
    if (bOnlineHavePictruePipe == XMEDIA_TRUE) {
    ...
    if (pstDrvCtx->stSnapInfoLoad.enSnapState == SNAP_STATE_CFG) {
        ...
        if (pstDrvCtx->stSnapAttr.bLoadCCM == XMEDIA_TRUE) {
        ...
        if (pstDrvCtx->stStitchAttr.bStitchEnable == XMEDIA_TRUE) {
            if (pstDrvCtx->stStitchAttr.bMainPipe == XMEDIA_FALSE) {
                MainPipeS  = pstDrvCtx->stStitchAttr.as8StitchBindId[0];
        ...
        if (pstAERegCfg == XMEDIA_NULL) {
            return XMEDIA_SUCCESS;
        }

        if (pstAWBRegCfg == XMEDIA_NULL) {
            return XMEDIA_SUCCESS;
        }
    ...
    /* config Piris */
    if (pstAERegCfg != XMEDIA_NULL) {
        if (pstAERegCfg->bPirisValid == XMEDIA_TRUE) {
            if (pstDrvCtx->stPirisCb.pfn_piris_gpio_update != XMEDIA_NULL) {
...
ISP_SYNC_CFG_BUF_NODE_S *ISP_DRV_GetSnsRegConfigNode(ISP_DRV_CTX_S *pstDrvCtx, XMEDIA_U8 u8DelayFrmNum)
{
    XMEDIA_U8 u8WDRMode, u8CfgNodeIdx, u8CfgNodeVC;
    ISP_SYNC_CFG_BUF_NODE_S *pstCfgNode = XMEDIA_NULL;
    ISP_SYNC_CFG_BUF_NODE_S *pstPreCfgNode = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_StitchRegsCfgSensor(VI_PIPE ViPipe, ISP_DRV_CTX_S *pstDrvCtx, XMEDIA_U32 u32UpdatePos)
{
    XMEDIA_S32 i;
    XMEDIA_U32 u32SlaveDev;
    VI_PIPE MainPipeS = 0;
    ISP_SYNC_CFG_BUF_NODE_S *pstCurNode = XMEDIA_NULL;
    ISP_SYNC_CFG_BUF_NODE_S *pstCfgNode = XMEDIA_NULL;

    ISP_I2C_DATA_S *pstI2cData = XMEDIA_NULL;
    ISP_SSP_DATA_S *pstSspData = XMEDIA_NULL;
    XMEDIA_S8 s8I2cDev, s8SspDev, s8SspCs;
    ...
    if (pstCurNode->stSnsRegsInfo.enSnsType == ISP_SNS_I2C_TYPE) {
        ...
        for (i = 0; i < pstCurNode->stSnsRegsInfo.u32RegNum; i++) {
            ...
            if (pstCfgNode->bValid == XMEDIA_TRUE) {
                pstI2cData = &pstCfgNode->stSnsRegsInfo.astI2cData[i];
                if ((pstI2cData->bUpdate == XMEDIA_TRUE) && (u32UpdatePos == pstI2cData->u8IntPos)) {
                    if (pstDrvCtx->stBusCb.pfnISPWriteI2CData != XMEDIA_NULL) {
        ...
        for (i = 0; i < pstCurNode->stSnsRegsInfo.u32RegNum; i++) {
            ...
            if (pstCfgNode->bValid == XMEDIA_TRUE) {
                ...
                if ((pstSspData->bUpdate == XMEDIA_TRUE) && (u32UpdatePos == pstSspData->u8IntPos)) {
                    if (pstDrvCtx->stBusCb.pfnISPWriteSSPData != XMEDIA_NULL) {
    ...
    if ((pstCfgNode->bValid == XMEDIA_TRUE) && (pstCfgNode->stSnsRegsInfo.stSlvSync.bUpdate == XMEDIA_TRUE)) {
...
XMEDIA_S32 ISP_DRV_NormalRegsCfgSensor(VI_PIPE ViPipe, ISP_DRV_CTX_S *pstDrvCtx)
{
    XMEDIA_S32 i = 0;
    XMEDIA_U32 u32SlaveDev = 0;
    ISP_SYNC_CFG_BUF_NODE_S *pstCurNode = XMEDIA_NULL;
    ISP_SYNC_CFG_BUF_NODE_S *pstCfgNode = XMEDIA_NULL;
    ISP_I2C_DATA_S *pstI2cData = XMEDIA_NULL;
    ISP_SSP_DATA_S *pstSspData = XMEDIA_NULL;
    ...
    if (pstCurNode->stSnsRegsInfo.enSnsType == ISP_SNS_I2C_TYPE) {
        for (i = 0; i < pstCurNode->stSnsRegsInfo.u32RegNum; i++) {
            ...
            if (pstCfgNode->bValid == XMEDIA_TRUE) {
                ...
                if ((pstI2cData->bUpdate == XMEDIA_TRUE) && (pstDrvCtx->u32IntPos == pstI2cData->u8IntPos)) {
                    if (pstDrvCtx->stBusCb.pfnISPWriteI2CData != XMEDIA_NULL) {
                        pstDrvCtx->stBusCb.pfnISPWriteI2CData(s8I2cDev, pstI2cData->u8DevAddr,
                                                              pstI2cData->u32RegAddr, pstI2cData->u32AddrByteNum,
                                                              pstI2cData->u32Data, pstI2cData->u32DataByteNum);
        ...
        for (i = 0; i < pstCurNode->stSnsRegsInfo.u32RegNum; i++) {
            ...
            if (pstCfgNode->bValid == XMEDIA_TRUE) {
                ...
                if ((pstSspData->bUpdate == XMEDIA_TRUE) && (pstDrvCtx->u32IntPos == pstSspData->u8IntPos)) {
                    if (pstDrvCtx->stBusCb.pfnISPWriteSSPData != XMEDIA_NULL) {
                        pstDrvCtx->stBusCb.pfnISPWriteSSPData(s8SspDev, s8SspCs, pstSspData->u32DevAddr,
                                                              pstSspData->u32DevAddrByteNum, pstSspData->u32RegAddr,
                                                              pstSspData->u32RegAddrByteNum, pstSspData->u32Data,
...
XMEDIA_S32 ISP_DRV_RegConfigSensor(VI_PIPE ViPipe, ISP_DRV_CTX_S *pstDrvCtx)
{
    XMEDIA_S32 s32Ret;
    volatile  XMEDIA_U32 u32UpdatePos;
    ISP_CHECK_PIPE(ViPipe);
    ISP_CHECK_POINTER(pstDrvCtx);
...
XMEDIA_S32 ISP_RegisterBusCallBack(VI_PIPE ViPipe,
...
XMEDIA_S32 ISP_RegisterPirisCallBack(VI_PIPE ViPipe, ISP_PIRIS_CALLBACK_S *pstPirisCb)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    unsigned long u32Flags;
...
XMEDIA_S32 ISP_RegisterViBusCallBack(XMEDIA_S32 ViPipe, ISP_VIBUS_CALLBACK_S *pstViBusCb)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    unsigned long u32Flags;
...
XMEDIA_S32 ISP_SaveSnapConfig(VI_PIPE ViPipe, ISP_CONFIG_INFO_S *pstSnapInfo)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    unsigned long u32Flags;
    XMEDIA_U8 u8CfgDlyMax;
    ISP_CHECK_PIPE(ViPipe);
...
/* vi send Proenable */
XMEDIA_S32 ISP_SetProEnable(VI_PIPE ViPipe)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    unsigned long u32Flags;
    ISP_CHECK_PIPE(ViPipe);
    ...
    osal_spin_lock_irqsave(&g_stIspLock[ViPipe], &u32Flags);
    pstDrvCtx->bProEnable = XMEDIA_TRUE;
    pstDrvCtx->bProStart  = XMEDIA_FALSE;
    osal_spin_unlock_irqrestore(&g_stIspLock[ViPipe], &u32Flags);
    return XMEDIA_SUCCESS;
}

/* vi get pubAttr */
XMEDIA_S32 ISP_GetPubAttr(VI_PIPE ViPipe, ISP_PUB_ATTR_S *pstPubAttr)
{
    unsigned long u32Flags;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
...
/* vi get ProCtrl */
XMEDIA_BOOL ISP_GetProCtrl(VI_PIPE ViPipe, ISP_PRO_CTRL_S *pstProCtrl)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    unsigned long u32Flags;
    XMEDIA_U8 u8CfgDlyMax;
    XMEDIA_BOOL bProStart = XMEDIA_FALSE;
    ...
    if (pstDrvCtx->u32ProTrigFlag == 2) {
        ...
        bProStart = XMEDIA_FALSE;
    }

    return bProStart;
}

XMEDIA_S32 ISP_SetSnapAttr(VI_PIPE ViPipe, ISP_SNAP_ATTR_S *pstSnapAttr)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    unsigned long u32Flags;
    XMEDIA_U8 i;
...
static XMEDIA_S32 ISP_CheckProNrParam(ISP_NR_AUTO_ATTR_S *pstNrAttr)
{
    XMEDIA_U8 i;
...
XMEDIA_S32 ISP_SetProNrParam(VI_PIPE ViPipe, const ISP_PRO_BNR_PARAM_S *pstProNrParam)
{
    XMEDIA_S32 s32Ret;
    XMEDIA_U32 u32ParamIdex = 0;
    XMEDIA_U32 u32ParamNum;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ISP_PRO_NR_PARAM_S *pstProNrParamCtx = XMEDIA_NULL;
    ISP_NR_AUTO_ATTR_S *pstProNrAttr = XMEDIA_NULL;
    ISP_CHECK_PIPE(ViPipe);
    ISP_CHECK_POINTER(pstProNrParam);
    pstDrvCtx = ISP_DRV_GET_CTX(ViPipe);

    if (pstProNrParam->pastNrAttr == XMEDIA_NULL) {
        ISP_TRACE(MODULE_DBG_ERR, "NrParam buf address can't null!\n");
        s32Ret =  ERR_CODE_ISP_NULL_PTR;
        goto failed0;
    }

    s32Ret = read_user_linear_space_valid((XMEDIA_U8 *)(pstProNrParam->pastNrAttr), sizeof(ISP_NR_AUTO_ATTR_S));
    if (s32Ret == XMEDIA_FALSE) {
    ...
    if (pstDrvCtx->stProInfo.bInit == XMEDIA_FALSE) {
        ISP_TRACE(MODULE_DBG_WARN, "ProInfo buf doesn't initialized!\n");
        s32Ret = ERR_CODE_ISP_NOT_INIT;
        goto failed1;
    }

    if (pstDrvCtx->stProInfo.stProNrParamInfo.pVirAddr == XMEDIA_NULL) {
...
XMEDIA_S32 ISP_GetProNrParam(VI_PIPE ViPipe, ISP_PRO_BNR_PARAM_S *pstProNrParam)
{
    XMEDIA_S32 s32Ret;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ISP_PRO_NR_PARAM_S *pstProNrParamCtx = XMEDIA_NULL;
    ...
    s32Ret = write_user_linear_space_valid((XMEDIA_U8 *)(pstProNrParam->pastNrAttr), sizeof(ISP_NR_AUTO_ATTR_S));
    if (s32Ret == XMEDIA_FALSE) {
...
static XMEDIA_S32 ISP_CheckProShpParam(ISP_SHARPEN_AUTO_ATTR_S *pstShpAttr)
{
    XMEDIA_U8 i, j;
...
XMEDIA_S32 ISP_SetProShpParam(VI_PIPE ViPipe, const ISP_PRO_SHARPEN_PARAM_S *pstProShpParam)
{
    XMEDIA_S32 s32Ret;
    XMEDIA_U32 u32ParamIdex = 0;
    XMEDIA_U32 u32ParamNum;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ISP_PRO_SHP_PARAM_S *pstProShpParamCtx = XMEDIA_NULL;
    ISP_SHARPEN_AUTO_ATTR_S *pastProShpAttr = XMEDIA_NULL;

    ISP_CHECK_PIPE(ViPipe);
    ISP_CHECK_POINTER(pstProShpParam);
    pstDrvCtx = ISP_DRV_GET_CTX(ViPipe);

    if (pstProShpParam->pastShpAttr == XMEDIA_NULL) {
        ISP_TRACE(MODULE_DBG_ERR, "ShpParam buf address can't null!\n");
        s32Ret = ERR_CODE_ISP_NULL_PTR;
        goto failed0;
    }

    s32Ret = read_user_linear_space_valid((XMEDIA_U8 *)(pstProShpParam->pastShpAttr), sizeof(ISP_SHARPEN_AUTO_ATTR_S));
    if (s32Ret == XMEDIA_FALSE) {
        ISP_TRACE(MODULE_DBG_ERR, "Pipe:%d pastShpAttr address is invalid!\n", ViPipe);
        s32Ret = ERR_CODE_ISP_INVALID_ADDR;
        goto failed0;
    ...
    if (pstDrvCtx->stProInfo.stProShpParamInfo.pVirAddr == XMEDIA_NULL) {
        ISP_TRACE(MODULE_DBG_WARN, "ShpParam buf don't init ok!\n");
        s32Ret = ERR_CODE_ISP_NOT_INIT;
        goto failed1;
    }
...
XMEDIA_S32 ISP_GetProShpParam(VI_PIPE ViPipe, ISP_PRO_SHARPEN_PARAM_S *pstProShpParam)
{
    XMEDIA_S32 s32Ret;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ISP_PRO_SHP_PARAM_S *pstProShpParamCtx = XMEDIA_NULL;
    ...
    s32Ret = write_user_linear_space_valid((XMEDIA_U8 *)(pstProShpParam->pastShpAttr), sizeof(ISP_SHARPEN_AUTO_ATTR_S));
    if (s32Ret == XMEDIA_FALSE) {
        ISP_TRACE(MODULE_DBG_ERR, "Pipe:%d pastShpAttr address is invalid!\n", ViPipe);
        return ERR_CODE_ISP_INVALID_ADDR;
    }
...
XMEDIA_S32 ISP_DRV_BeEndIntProc(VI_PIPE ViPipe)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    unsigned long u32Flags;
    ...
    if (pstDrvCtx->bRunOnceOk == XMEDIA_TRUE) {
        ISP_DRV_StatBufBusyPut(ViPipe);
        pstDrvCtx->bRunOnceOk = XMEDIA_FALSE;
    }

    if (pstDrvCtx->bYUVRunOnceOk == XMEDIA_TRUE) {
        ISP_DRV_StatBufBusyPut(ViPipe);
        pstDrvCtx->bYUVRunOnceOk = XMEDIA_FALSE;
    }

    osal_spin_lock_irqsave(&g_stIspLock[ViPipe], &u32Flags);
    pstDrvCtx->bVdBeEnd = XMEDIA_TRUE;
...
XMEDIA_S32 ISP_DRV_ProcInit(VI_PIPE ViPipe, ISP_PROC_MEM_S *pstIspProcMem)
{
    XMEDIA_S32 s32Ret;
    XMEDIA_U64 u64PhyAddr;
    XMEDIA_U8  *pu8VirAddr;
    ...
    s32Ret = CMPI_MmzMallocNocache(XMEDIA_NULL, acName, &u64PhyAddr, (XMEDIA_VOID **)&pu8VirAddr, ISP_PROC_SIZE);
    ...
    ((XMEDIA_CHAR *)pu8VirAddr)[0] = '\0';
    ((XMEDIA_CHAR *)pu8VirAddr)[ISP_PROC_SIZE - 1] = '\0';
    ...
    pstDrvCtx->stPorcMem.bInit          = XMEDIA_TRUE;
    pstDrvCtx->stPorcMem.u64ProcPhyAddr = u64PhyAddr;
    pstDrvCtx->stPorcMem.u32ProcSize    = ISP_PROC_SIZE;
    pstDrvCtx->stPorcMem.pProcVirtAddr  = (XMEDIA_VOID *)pu8VirAddr;
...
XMEDIA_S32 ISP_DRV_ProcExit(VI_PIPE ViPipe)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    XMEDIA_U64 u64PhyAddr;
    XMEDIA_U8  *pu8VirAddr;
    ...
    pu8VirAddr = (XMEDIA_U8 *)pstDrvCtx->stPorcMem.pProcVirtAddr;

    if (osal_down_interruptible(&pstDrvCtx->stProcSem)) {
        return -ERESTARTSYS;
    }

    pstDrvCtx->stPorcMem.bInit          = XMEDIA_FALSE;
    pstDrvCtx->stPorcMem.u64ProcPhyAddr = 0;
    pstDrvCtx->stPorcMem.u32ProcSize    = 0;
    pstDrvCtx->stPorcMem.pProcVirtAddr  = XMEDIA_NULL;
    osal_up(&pstDrvCtx->stProcSem);
...
XMEDIA_S32 ISP_DRV_ProcPrintf(VI_PIPE ViPipe, osal_proc_entry_t *s)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    XMEDIA_U32 u32ProcBufLen;
    const XMEDIA_CHAR *pszStr;
    XMEDIA_CHAR *pszBuf;
    ...
    if (pstDrvCtx->stPorcMem.pProcVirtAddr != XMEDIA_NULL) {
        ...
        pszStr = (XMEDIA_CHAR *)pstDrvCtx->stPorcMem.pProcVirtAddr;
        u32ProcBufLen = osal_strlen((XMEDIA_CHAR *)pstDrvCtx->stPorcMem.pProcVirtAddr);
...
XMEDIA_S32 ISP_UpdateInfoSync(VI_PIPE ViPipe, ISP_DCF_UPDATE_INFO_S *pstIspUpdateInfo)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    unsigned long u32Flags;
    XMEDIA_S32 i;
    ISP_DCF_UPDATE_INFO_S *pUpdateInfoVirAddr = XMEDIA_NULL;
    ISP_CHECK_PIPE(ViPipe);
    pstDrvCtx = ISP_DRV_GET_CTX(ViPipe);

    osal_spin_lock_irqsave(&g_stIspLock[ViPipe], &u32Flags);

    if (pstDrvCtx->stTransInfo.stUpdateInfo.pVirAddr == XMEDIA_NULL) {
...
XMEDIA_S32 ISP_frameInfoSync(VI_PIPE ViPipe, ISP_FRAME_INFO_S *pstIspframeInfo)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    unsigned long u32Flags;
    XMEDIA_S32 i;
    ISP_FRAME_INFO_S *pframeInfoVirAddr = XMEDIA_NULL;
    ISP_CHECK_PIPE(ViPipe);
    pstDrvCtx = ISP_DRV_GET_CTX(ViPipe);

    osal_spin_lock_irqsave(&g_stIspLock[ViPipe], &u32Flags);

    if (pstDrvCtx->stTransInfo.stFrameInfo.pVirAddr == XMEDIA_NULL) {
        ISP_TRACE(MODULE_DBG_WARN, "UpdateInfo buf don't init ok!\n");
        osal_spin_unlock_irqrestore(&g_stIspLock[ViPipe], &u32Flags);
        return ERR_CODE_ISP_NOT_INIT;
    }
...
XMEDIA_S32 ISP_CalSyncInfoIndex(VI_PIPE ViPipe, XMEDIA_S32 *ps32Index)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    XMEDIA_U8 u8CfgDlyMax;
    ISP_CHECK_PIPE(ViPipe);
...
XMEDIA_S32 ISP_GetDCFInfo(VI_PIPE ViPipe, ISP_DCF_INFO_S *pstIspDCF)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ISP_DCF_UPDATE_INFO_S *pstIspUpdateInfo = XMEDIA_NULL;
    ISP_DCF_CONST_INFO_S *pstIspDCFConstInfo = XMEDIA_NULL;
    unsigned long u32Flags;
    XMEDIA_S32 s32Index = 0;
    ISP_DCF_UPDATE_INFO_S *pUpdateInfoVirAddr = XMEDIA_NULL;
    ISP_CHECK_PIPE(ViPipe);
...
XMEDIA_S32 ISP_SetDCFInfo(VI_PIPE ViPipe, ISP_DCF_INFO_S *pstIspDCF)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ISP_DCF_UPDATE_INFO_S *pstIspUpdateInfo = XMEDIA_NULL;
    ISP_DCF_CONST_INFO_S *pstIspDCFConstInfo = XMEDIA_NULL;
...
XMEDIA_S32 ISP_GetIspUpdateInfo(VI_PIPE ViPipe, ISP_DCF_UPDATE_INFO_S *pstIspUpdateInfo)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    unsigned long u32Flags;
    ISP_DCF_UPDATE_INFO_S *pUpdateInfoVirAddr = XMEDIA_NULL;
...
XMEDIA_S32 ISP_SetIspUpdateInfo(VI_PIPE ViPipe, ISP_DCF_UPDATE_INFO_S *pstIspUpdateInfo)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    unsigned long u32Flags;
    XMEDIA_S32 s32Index = 0;
...
XMEDIA_S32 ISP_DRV_ProInfoBufInit(VI_PIPE ViPipe, ISP_PRO_INFO_BUF_S *pstProInfo)
{
    XMEDIA_S32 s32Ret;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    XMEDIA_U64 u64PhyAddr;
    XMEDIA_U64 u64Size = 0;
    XMEDIA_U8  *pu8VirAddr;
    XMEDIA_U32 u32SizeProNrParam, u32SizeProShpParam;
    XMEDIA_CHAR acName[MAX_MMZ_NAMELEN] = { 0 };
    ...
    pstDrvCtx->stProInfo.bInit                        = XMEDIA_TRUE;

    pstDrvCtx->stProInfo.stProNrParamInfo.u64PhyAddr  = u64PhyAddr;
    pstDrvCtx->stProInfo.stProNrParamInfo.pVirAddr    = (XMEDIA_VOID *)pu8VirAddr;
    ...
    pstDrvCtx->stProInfo.stProShpParamInfo.pVirAddr   = \
            (XMEDIA_VOID *)((XMEDIA_U8 *)pstDrvCtx->stProInfo.stProNrParamInfo.pVirAddr + u32SizeProNrParam);
...
XMEDIA_S32 ISP_DRV_ProInfoBufExit(VI_PIPE ViPipe)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    XMEDIA_U64 u64PhyAddr;
    XMEDIA_U8  *pu8VirAddr;
    ...
    pu8VirAddr = (XMEDIA_U8 *)pstDrvCtx->stProInfo.stProNrParamInfo.pVirAddr;

    if (osal_down_interruptible(&pstDrvCtx->stProInfoSem)) {
        return -ERESTARTSYS;
    }

    pstDrvCtx->stProInfo.bInit                        = XMEDIA_FALSE;

    pstDrvCtx->stProInfo.stProNrParamInfo.u64PhyAddr  = 0;
    pstDrvCtx->stProInfo.stProNrParamInfo.pVirAddr    = XMEDIA_NULL;

    pstDrvCtx->stProInfo.stProShpParamInfo.u64PhyAddr = 0;
    pstDrvCtx->stProInfo.stProShpParamInfo.pVirAddr   = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_TransInfoBufInit(VI_PIPE ViPipe, ISP_TRANS_INFO_BUF_S *pstTransInfo)
{
    XMEDIA_S32 s32Ret;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    XMEDIA_U64 u64PhyAddr;
    XMEDIA_U64 u64Size = 0;
    XMEDIA_U8  *pu8VirAddr;
    XMEDIA_U32 u32SizeDngInfo;
    XMEDIA_U32 u32SizeUpdateInfo, u32SizeFrameInfo;
    XMEDIA_U32 u32SizeAttachInfo, u32SizeColorGammut;
    XMEDIA_CHAR acName[MAX_MMZ_NAMELEN] = { 0 };
    unsigned long u32Flags;
    ...
    pstDrvCtx->stTransInfo.bInit                        = XMEDIA_TRUE;

    pstDrvCtx->stTransInfo.stDngInfo.u64PhyAddr         = u64PhyAddr;
    pstDrvCtx->stTransInfo.stDngInfo.pVirAddr           = (XMEDIA_VOID *)pu8VirAddr;
    ...
    pstDrvCtx->stTransInfo.stAttaInfo.pVirAddr          = \
            (XMEDIA_VOID *)((XMEDIA_U8 *)pstDrvCtx->stTransInfo.stDngInfo.pVirAddr + u32SizeDngInfo);
    ...
    pstDrvCtx->stTransInfo.stColorGammutInfo.pVirAddr   = \
            (XMEDIA_VOID *)((XMEDIA_U8 *)pstDrvCtx->stTransInfo.stAttaInfo.pVirAddr + u32SizeAttachInfo);
    ...
    pstDrvCtx->stTransInfo.stFrameInfo.pVirAddr         = \
            (XMEDIA_VOID *)((XMEDIA_U8 *)pstDrvCtx->stTransInfo.stColorGammutInfo.pVirAddr + u32SizeColorGammut);
    ...
    pstDrvCtx->stTransInfo.stUpdateInfo.pVirAddr        = \
            (XMEDIA_VOID *)((XMEDIA_U8 *)pstDrvCtx->stTransInfo.stFrameInfo.pVirAddr + u32SizeFrameInfo);
...
XMEDIA_S32 ISP_DRV_TransInfoBufExit(VI_PIPE ViPipe)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    XMEDIA_U64 u64PhyAddr;
    XMEDIA_U8  *pu8VirAddr;
    ...
    pu8VirAddr = (XMEDIA_U8 *)pstDrvCtx->stTransInfo.stDngInfo.pVirAddr;

    pstDrvCtx->stTransInfo.bInit                        = XMEDIA_FALSE;

    pstDrvCtx->stTransInfo.stDngInfo.u64PhyAddr         = 0;
    pstDrvCtx->stTransInfo.stDngInfo.pVirAddr           = XMEDIA_NULL;

    pstDrvCtx->stTransInfo.stAttaInfo.u64PhyAddr        = 0;
    pstDrvCtx->stTransInfo.stAttaInfo.pVirAddr          = XMEDIA_NULL;

    pstDrvCtx->stTransInfo.stColorGammutInfo.u64PhyAddr = 0;
    pstDrvCtx->stTransInfo.stColorGammutInfo.pVirAddr   = XMEDIA_NULL;

    pstDrvCtx->stTransInfo.stFrameInfo.u64PhyAddr       = 0;
    pstDrvCtx->stTransInfo.stFrameInfo.pVirAddr         = XMEDIA_NULL;

    pstDrvCtx->stTransInfo.stUpdateInfo.u64PhyAddr      = 0;
    pstDrvCtx->stTransInfo.stUpdateInfo.pVirAddr        = XMEDIA_NULL;
...
XMEDIA_S32 ISP_SetFrameInfo(VI_PIPE ViPipe, ISP_FRAME_INFO_S *pstIspFrame)
{
    unsigned long u32Flags;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    XMEDIA_S32 s32Index = 0;
...
XMEDIA_S32 ISP_GetFrameInfo(VI_PIPE ViPipe, ISP_FRAME_INFO_S *pstIspFrame)
{
    XMEDIA_U8 u8ViPipeS;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ISP_DRV_CTX_S *pstDrvCtxS = XMEDIA_NULL;
    ...
    if ((pstDrvCtx->bMemInit == XMEDIA_FALSE) && (pstDrvCtx->stWDRAttr.enWDRMode == WDR_MODE_NONE)) {
        for (u8ViPipeS = 0; u8ViPipeS < ISP_MAX_PIPE_NUM; u8ViPipeS++) {
            pstDrvCtxS = ISP_DRV_GET_CTX(u8ViPipeS);

            if ((pstDrvCtxS->bMemInit == XMEDIA_TRUE) && (IS_WDR_MODE(pstDrvCtxS->stWDRAttr.enWDRMode))) {
...
XMEDIA_S32 ISP_GetAttachInfo(VI_PIPE ViPipe, ISP_ATTACH_INFO_S *pstIspAttachInfo)
{
    unsigned long u32Flags;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ISP_CHECK_PIPE(ViPipe);
...
XMEDIA_S32 ISP_GetColorGamutInfo(VI_PIPE ViPipe, ISP_COLORGAMMUT_INFO_S *pstIspColorGamutInfo)
{
    unsigned long u32Flags;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ISP_CHECK_PIPE(ViPipe);
...
XMEDIA_S32 ISP_DRV_GetDngInfo(VI_PIPE ViPipe, DNG_IMAGE_STATIC_INFO_S *pstDngInfo)
{
    unsigned long u32Flags;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ISP_CHECK_PIPE(ViPipe);
...
XMEDIA_S32 ISP_DRV_GetDngImageDynamicInfo(VI_PIPE ViPipe, DNG_IMAGE_DYNAMIC_INFO_S *pstDngImageDynamicInfo)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
...
XMEDIA_S32 ISP_SetModParam(ISP_MOD_PARAM_S *pstModParam)
{
    VI_PIPE ViPipe;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ...
    for (ViPipe = 0; ViPipe < ISP_MAX_PIPE_NUM; ViPipe++) {
        ...
        if (pstDrvCtx->bMemInit == XMEDIA_TRUE) {
...
XMEDIA_S32 ISP_GetModParam(ISP_MOD_PARAM_S *pstModParam)
{
    ISP_CHECK_POINTER(pstModParam);
...
XMEDIA_S32 ISP_SetCtrlParam(VI_PIPE ViPipe, ISP_CTRL_PARAM_S *pstIspCtrlParam)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ...
    if ((g_ProcParam[ViPipe] != 0) && (pstIspCtrlParam->u32ProcParam == 0) && (pstDrvCtx->bMemInit == XMEDIA_TRUE)) {
...
XMEDIA_S32 ISP_GetCtrlParam(VI_PIPE ViPipe, ISP_CTRL_PARAM_S *pstIspCtrlParam)
{
    ISP_CHECK_PIPE(ViPipe);
    ISP_CHECK_POINTER(pstIspCtrlParam);
...
XMEDIA_S32 ISP_DRV_StitchSyncEx(VI_PIPE ViPipe)
{
    XMEDIA_U8 k;
    VI_PIPE ViPipeId;
    ISP_DRV_CTX_S *pstDrvCtx  = XMEDIA_NULL;
    ...
    for (k = 0; k < pstDrvCtx->stStitchAttr.u8StitchPipeNum; k++) {
        ...
        if (pstDrvCtxS->bStitchSync != XMEDIA_TRUE) {
            return XMEDIA_FAILURE;
        }
    }
...
XMEDIA_S32 ISP_DRV_StitchSync(VI_PIPE ViPipe)
{
    XMEDIA_U8 k;
    VI_PIPE ViPipeId;
    ISP_DRV_CTX_S *pstDrvCtx  = XMEDIA_NULL;
    ...
    for (k = 0; k < pstDrvCtx->stStitchAttr.u8StitchPipeNum; k++) {
        ...
        if (pstDrvCtxS->bIspInit != XMEDIA_TRUE) {
            return XMEDIA_FAILURE;
        }
    }
...
XMEDIA_S32 ISP_DRV_GetVersion(ISP_VERSION_S *pVersion)
{
...
XMEDIA_S32 ISP_DRV_SetDngInfo(VI_PIPE ViPipe, DNG_IMAGE_DYNAMIC_INFO_S *pstDngImgDynInfo)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_GetUserStatBuf(VI_PIPE ViPipe, ISP_STAT_INFO_S *pstStat)
{
    ISP_STAT_INFO_S *pstIspStatInfo = XMEDIA_NULL;
    ISP_CHECK_POINTER(pstStat);

    ISP_DRV_StatBufUserGet(ViPipe, &pstIspStatInfo);

    if (pstIspStatInfo == XMEDIA_NULL) {
        return XMEDIA_FAILURE;
    }
...
XMEDIA_S32 ISP_DRV_GetRgbirFormat(VI_PIPE ViPipe, ISP_IRBAYER_FORMAT_E *pRgbirFormat)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ISP_CHECK_PIPE(ViPipe);
...
XMEDIA_S32 ISP_DRV_SetOnLineStaticsSttAddr(VI_PIPE ViPipe,XMEDIA_U32 u32ViProcIrqStatus)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    XMEDIA_U32 u32CurReadFlag;
    ISP_CHECK_PIPE(ViPipe);
    pstDrvCtx = ISP_DRV_GET_CTX(ViPipe);
    if(!pstDrvCtx->bIspInit)
    {
...
XMEDIA_S32 ISP_DRV_GetStatInfoActive(VI_PIPE ViPipe, ISP_STAT_INFO_S *pstIspStatInfo)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_SetRegKernelCfgs(VI_PIPE ViPipe, ISP_KERNEL_REG_CFG_S *pstRegKernelCfg)
{
    XMEDIA_U32 u32Flag;
    unsigned long u32Flags;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_SetConfigInfo(VI_PIPE ViPipe, ISP_CONFIG_INFO_S *pstIspConfigInfo)
{
    XMEDIA_U32 i;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_GetSnapInfo(VI_PIPE ViPipe, ISP_SNAP_INFO_S *pstIspSnapInfo)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
...
XMEDIA_VOID ISP_DRV_BeBufQueuePutBusy(VI_PIPE ViPipe)
{
    XMEDIA_U64  u64PhyAddr;
    XMEDIA_U64  u64Size;
    XMEDIA_VOID *pVirAddr;
...
XMEDIA_S32 ISP_DRV_RunOnceProcess(VI_PIPE ViPipe)
{
    unsigned long u32Flags;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ISP_SYNC_CFG_S *pstSyncCfg = XMEDIA_NULL;
    ...
    if (pstDrvCtx->bMemInit == XMEDIA_FALSE) {
    ...
    osal_spin_lock_irqsave(&g_stIspLock[ViPipe], &u32Flags);
    if (pstDrvCtx->bRunOnceFlag == XMEDIA_TRUE) {
        ISP_DRV_BeBufQueuePutBusy(ViPipe);
    }
    osal_spin_unlock_irqrestore(&g_stIspLock[ViPipe], &u32Flags);

    pstDrvCtx->bRunOnceOk = XMEDIA_TRUE;

    return XMEDIA_SUCCESS;
}

XMEDIA_S32 ISP_DRV_YUVRunOnceProcess(VI_PIPE ViPipe)
{
    unsigned long u32Flags;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ...
    osal_spin_lock_irqsave(&g_stIspLock[ViPipe], &u32Flags);
    if (pstDrvCtx->bYUVRunOnceFlag == XMEDIA_TRUE) {
        ISP_DRV_BeBufQueuePutBusy(ViPipe);
    }
    osal_spin_unlock_irqrestore(&g_stIspLock[ViPipe], &u32Flags);

    pstDrvCtx->bYUVRunOnceOk = XMEDIA_TRUE;

    return XMEDIA_SUCCESS;
}

XMEDIA_S32 ISP_DRV_OptRunOnceInfo(VI_PIPE ViPipe, XMEDIA_BOOL *pbRunOnceFlag)
{
    XMEDIA_BOOL bRunOnceFlag;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    unsigned long u32Flags;
...
XMEDIA_S32 ISP_DRV_YUVRunOnceInfo(VI_PIPE ViPipe, XMEDIA_BOOL *pbRunOnceFlag)
{
    XMEDIA_BOOL bYUVRunOnceFlag;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    unsigned long u32Flags;
    ...
    if (bYUVRunOnceFlag == XMEDIA_TRUE) {
        if (pstDrvCtx->pstUseNode) {
            osal_spin_unlock_irqrestore(&g_stIspLock[ViPipe], &u32Flags);
            ISP_TRACE(MODULE_DBG_ERR, "Pipe[%d] isp is running!\r\n", ViPipe);
            return XMEDIA_FAILURE;
...
XMEDIA_VOID ISP_DRV_StitchBeBufCtl(VI_PIPE ViPipe)
{
    XMEDIA_S32  i;
    XMEDIA_S32  s32Ret;
    VI_PIPE ViPipeS;
    ...
    for (i = 0; i < pstDrvCtx->stStitchAttr.u8StitchPipeNum; i++) {
        ...
        if (pstDrvCtxS->stBeBufInfo.bInit != XMEDIA_TRUE) {
            ISP_TRACE(MODULE_DBG_ERR, "Pipe[%d] BeBuf (bInit != TRUE) !\n", ViPipe);
            osal_spin_unlock_irqrestore(&g_stIspSyncLock[MainPipeS], &u32Flags);
            return;
        }

        if (pstDrvCtxS->bRunOnceFlag != XMEDIA_TRUE) {
...
XMEDIA_S32 ISP_DRV_WriteAllLdciSttAddr(VI_PIPE ViPipe)
{
    XMEDIA_U8  i, k, u8WriteBufIdx, u8BlkNum, u8FreeNum, u8WriteBufNum;
    XMEDIA_U32 u32Size;
    XMEDIA_U64 u64WriteSttHeadAddr, u64WriteSttAddr;
    ISP_BE_WO_REG_CFG_S *pstBeRegCfg = XMEDIA_NULL;
    ISP_BE_BUF_NODE_S   *pstNode     = XMEDIA_NULL;
    ISP_DRV_CTX_S       *pstDrvCtx   = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_WriteBeFreeBuf(VI_PIPE ViPipe)
{
    XMEDIA_S32 i;
    XMEDIA_S32 s32FreeNum, s32Ret;
    ISP_RUNNING_MODE_E enIspRunningMode;
    ISP_BE_BUF_NODE_S *pstNode = XMEDIA_NULL;
    ISP_BE_WO_REG_CFG_S *pstBeRegCfgSrc = XMEDIA_NULL;
    ISP_BE_WO_REG_CFG_S *pstBeRegCfgDst = XMEDIA_NULL;
    ISP_DRV_CTX_S *pstDrvCtx  = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_BeBufSwitchFinishState(VI_PIPE ViPipe)
{
    XMEDIA_S32  i;
    VI_PIPE ViPipeS;
    ISP_DRV_CTX_S *pstDrvCtxS = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_StitchWriteBeBufAll(VI_PIPE ViPipe)
{
    XMEDIA_S32 i;
    XMEDIA_S32 s32Ret;
    VI_PIPE ViPipeS;
    ...
    for (i = 0; i < pstDrvCtx->stStitchAttr.u8StitchPipeNum; i++) {
        ...
        if (pstDrvCtxS->pstUseNode == XMEDIA_NULL) {
            osal_spin_unlock_irqrestore(&g_stIspSyncLock[MainPipeS], &u32Flags);
        ...
        pstDrvCtxS->pstUseNode = XMEDIA_NULL;
        pstDrvCtxS->enIspRunningState = ISP_BE_BUF_STATE_INIT;
    }
...
XMEDIA_S32 ISP_DRV_GetBeFreeBuf(VI_PIPE ViPipe, ISP_BE_WO_CFG_BUF_S *pstBeWoCfgBuf)
{
    osal_spinlock_t *pIspSpinLock = XMEDIA_NULL;
    unsigned long u32Flags;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ISP_BE_WO_CFG_BUF_S *pstCurNodeBuf = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_GetBeBufFirst(VI_PIPE ViPipe, XMEDIA_U64 *pu64PhyAddr)
{
    unsigned long u32Flags;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ISP_BE_BUF_NODE_S *pstNode = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_GetBeLastBuf(VI_PIPE ViPipe, XMEDIA_U64 *pu64PhyAddr)
{
    XMEDIA_U8  i;
    ISP_DRV_CTX_S     *pstDrvCtx = XMEDIA_NULL;
    ISP_BE_BUF_NODE_S *pstNode   = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_BeBufRunState(VI_PIPE ViPipe)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    unsigned long u32Flags;
    osal_spinlock_t *pIspSpinLock = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_BeBufCtl(VI_PIPE ViPipe)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    unsigned long u32Flags;
    ...
    if (pstDrvCtx->stStitchAttr.bStitchEnable != XMEDIA_TRUE) {
        if (pstDrvCtx->stBeBufInfo.bInit != XMEDIA_TRUE) {
            ISP_TRACE(MODULE_DBG_ERR, "Pipe[%d] BeBuf (bInit != TRUE) !\n", ViPipe);
            return XMEDIA_FAILURE;
        }
...
XMEDIA_S32 ISP_DRV_AllBeBufInit(VI_PIPE ViPipe)
{
    XMEDIA_S32  s32Ret = XMEDIA_SUCCESS;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    unsigned long u32Flags;
...
XMEDIA_S32 ISP_DRV_SyncCfgSet(VI_PIPE ViPipe, ISP_SYNC_CFG_BUF_NODE_S *pstIspSyncCfgBuf)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ISP_SYNC_CFG_BUF_S  *pstSyncCfgBuf = XMEDIA_NULL;
    ISP_SYNC_CFG_BUF_NODE_S  *pstCurNode = XMEDIA_NULL;
    unsigned long u32Flags;
...
XMEDIA_S32 ISP_DRV_SetWdrCfg(VI_PIPE ViPipe, ISP_WDR_CFG_S *pstIspWdrCfg)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    unsigned long u32Flags;
...
XMEDIA_S32 ISP_DRV_GetStitchAttr(VI_PIPE ViPipe, VI_STITCH_ATTR_S *pstStitchAttr)
{
    XMEDIA_U8   i;
    XMEDIA_S32  s32Ret;
    ISP_DRV_CTX_S    *pstDrvCtx = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_GetPipeSize(VI_PIPE ViPipe, SIZE_S *pstPipeSize)
{
    XMEDIA_S32  s32Ret;
    SIZE_S  stPipeSize = { 0 };
...
XMEDIA_S32 ISP_DRV_GetHdrAttr(VI_PIPE ViPipe, VI_PIPE_HDR_ATTR_S *pstHDRAttr)
{
    XMEDIA_S32  s32Ret;
...
XMEDIA_S32 ISP_DRV_GetWdrAttr(VI_PIPE ViPipe, VI_PIPE_WDR_ATTR_S *pstWDRAttr)
{
    XMEDIA_U8   i;
    XMEDIA_U32  u32Num;
    XMEDIA_S32  s32Ret;
...
XMEDIA_S32 ISP_DRV_ChnSelectCfg(VI_PIPE ViPipe, XMEDIA_U32 u32ChnSel)
{
    XMEDIA_U32 i;
    XMEDIA_S32 s32Ret = XMEDIA_SUCCESS;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_SetIspRunState(VI_PIPE ViPipe, XMEDIA_U64 *pu64HandSignal)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ...
    if (*pu64HandSignal == ISP_INIT_HAND_SIGNAL) {
        pstDrvCtx->bIspRunFlag = XMEDIA_TRUE;
    } else if (*pu64HandSignal == ISP_EXIT_HAND_SIGNAL) {
        pstDrvCtx->bIspRunFlag = XMEDIA_FALSE;
...
XMEDIA_S32 ISP_DRV_ResetCtx(VI_PIPE ViPipe)
{
    XMEDIA_U8 i;
    VI_PIPE ViPipeS = 0;
    unsigned long u32Flags;
    ...
    pstDrvCtx->bMemInit     = XMEDIA_FALSE;
    pstDrvCtx->bIspInit     = XMEDIA_FALSE;
    pstDrvCtx->bRunOnceOk   = XMEDIA_FALSE;
    pstDrvCtx->bRunOnceFlag = XMEDIA_FALSE;
    pstDrvCtx->bYUVRunOnceOk   = XMEDIA_FALSE;
    pstDrvCtx->bYUVRunOnceFlag = XMEDIA_FALSE;
    ...
    if (pstDrvCtx->stStitchAttr.bStitchEnable == XMEDIA_TRUE) {
        if (pstDrvCtx->stStitchAttr.bMainPipe == XMEDIA_TRUE) {
            for (i = 0; i < pstDrvCtx->stStitchAttr.u8StitchPipeNum; i++) {
                ...
                pstDrvCtxS = ISP_DRV_GET_CTX(ViPipeS);
                pstDrvCtxS->bStitchSync  = XMEDIA_FALSE;
            }
        }
    }

    pstDrvCtx->stStitchAttr.bStitchEnable = XMEDIA_FALSE;
    ...
    pstDrvCtx->stSnapAttr.bLoadCCM            = XMEDIA_TRUE;
    ...
    g_IntBothalf = XMEDIA_FALSE;
    g_UseBothalf = XMEDIA_FALSE;
    g_QuickStart = XMEDIA_FALSE;
    g_bLongFrmIntEn = XMEDIA_FALSE;
...
XMEDIA_S32 ISP_SetPubAttrInfo(VI_PIPE ViPipe, ISP_PUB_ATTR_S *pstPubAttr)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    unsigned long u32Flags;
...
static long ISP_ioctl(unsigned int cmd, unsigned long arg, void *private_data)
{
    ...
    switch (cmd) {
        case ISP_DEV_SET_FD: {
            ...
            *((XMEDIA_U32 *)(private_data)) = *(XMEDIA_U32 *)(XMEDIA_UINTPTR_T)arg;

            return XMEDIA_SUCCESS;
        }

        case ISP_GET_VERSION: {
            ISP_VERSION_S *pVersion = XMEDIA_NULL;
            pVersion = (ISP_VERSION_S *)(XMEDIA_UINTPTR_T)arg;

            return ISP_DRV_GetVersion(pVersion);
        }

        case ISP_GET_FRAME_EDGE: {
            XMEDIA_U32 *pu32Status = XMEDIA_NULL;
            pu32Status = (XMEDIA_U32 *)(XMEDIA_UINTPTR_T)arg;
        ...
        case ISP_GET_VD_TIMEOUT: {
            ...
            ISP_VD_TIMEOUT_S *pstVdTimeOut = XMEDIA_NULL;

            ISP_CHECK_POINTER(arg);

            pstVdTimeOut = (ISP_VD_TIMEOUT_S *)(XMEDIA_UINTPTR_T)arg;
        ...
        case ISP_UPDATE_INFO_SET: {
            ...
            pstDrvCtx = ISP_DRV_GET_CTX(ViPipe);
            pstIspUpdateInfo = (ISP_DCF_UPDATE_INFO_S *)(XMEDIA_UINTPTR_T)arg;
        ...
        case ISP_FRAME_INFO_SET: {
            ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
            ISP_FRAME_INFO_S *pstIspFrameInfo = XMEDIA_NULL;
            ISP_CHECK_PIPE(ViPipe);
            ISP_CHECK_POINTER(arg);

            pstDrvCtx = ISP_DRV_GET_CTX(ViPipe);
            pstIspFrameInfo = (ISP_FRAME_INFO_S *)(XMEDIA_UINTPTR_T)arg;
        ...
        case ISP_DNG_INFO_SET: {
            DNG_IMAGE_DYNAMIC_INFO_S *pstDngImgDynInfo = XMEDIA_NULL;
            pstDngImgDynInfo = (DNG_IMAGE_DYNAMIC_INFO_S *)(XMEDIA_UINTPTR_T)arg;

            return ISP_DRV_SetDngInfo(ViPipe, pstDngImgDynInfo);
        }

        case ISP_DNG_INFO_GET: {
            DNG_IMAGE_STATIC_INFO_S *pstDngStaticInfo = XMEDIA_NULL;
            pstDngStaticInfo = (DNG_IMAGE_STATIC_INFO_S *)(XMEDIA_UINTPTR_T)arg;

            return ISP_DRV_GetDngInfo(ViPipe, pstDngStaticInfo);
        }

        case ISP_SET_INT_ENABLE: {
            XMEDIA_BOOL bEn = XMEDIA_FALSE;
            ISP_CHECK_POINTER(arg);
            bEn = *(XMEDIA_BOOL *)(XMEDIA_UINTPTR_T)arg;

            return ISP_DRV_SetIntEnable(ViPipe, bEn);
        }

        case ISP_TRANS_BUF_INIT: {
            ISP_TRANS_INFO_BUF_S *pstTransInfo = XMEDIA_NULL;
            pstTransInfo = (ISP_TRANS_INFO_BUF_S *)(XMEDIA_UINTPTR_T)arg;
        ...
        case ISP_PRO_BUF_INIT: {
            ISP_PRO_INFO_BUF_S *pstProInfo = XMEDIA_NULL;
            pstProInfo = (ISP_PRO_INFO_BUF_S *)(XMEDIA_UINTPTR_T)arg;
        ...
        case ISP_STAT_BUF_INIT: {
            XMEDIA_U64 *pu64StatPhyAddr = XMEDIA_NULL;
            pu64StatPhyAddr = (XMEDIA_U64 *)(XMEDIA_UINTPTR_T)arg;
        ...
        case ISP_STAT_BUF_GET: {
            ISP_STAT_INFO_S *pstStat = XMEDIA_NULL;
            pstStat = (ISP_STAT_INFO_S *)(XMEDIA_UINTPTR_T)arg;

            return ISP_DRV_GetUserStatBuf(ViPipe, pstStat);
        }

        case ISP_STAT_BUF_PUT: {
            ISP_STAT_INFO_S *pstStatInfo = XMEDIA_NULL;
            pstStatInfo = (ISP_STAT_INFO_S *)(XMEDIA_UINTPTR_T)arg;
        ...
        case ISP_STAT_ACT_GET: {
            ISP_STAT_INFO_S *pstIspStatInfo = XMEDIA_NULL;
            pstIspStatInfo = (ISP_STAT_INFO_S *)(XMEDIA_UINTPTR_T)arg;

            return ISP_DRV_GetStatInfoActive(ViPipe, pstIspStatInfo);
        }

        case ISP_REG_CFG_SET: {
            ISP_KERNEL_REG_CFG_S *pstRegKernelCfg = XMEDIA_NULL;
            pstRegKernelCfg = (ISP_KERNEL_REG_CFG_S *)(XMEDIA_UINTPTR_T)arg;

            return ISP_DRV_SetRegKernelCfgs(ViPipe, pstRegKernelCfg);
        }

        case ISP_BE_CFG_BUF_INIT: {
            XMEDIA_U64 *pu64BePhyAddr = XMEDIA_NULL;
            pu64BePhyAddr = (XMEDIA_U64 *)(XMEDIA_UINTPTR_T)arg;

            return ISP_DRV_BeBufInit(ViPipe, pu64BePhyAddr);
        }

        case ISP_GET_BE_BUF_FIRST: {
            XMEDIA_U64 *pu64PhyAddr = XMEDIA_NULL;
            pu64PhyAddr = (XMEDIA_U64 *)(XMEDIA_UINTPTR_T)arg;

            return ISP_DRV_GetBeBufFirst(ViPipe, pu64PhyAddr);
        }

        case ISP_BE_FREE_BUF_GET: {
            ISP_BE_WO_CFG_BUF_S *pstBeWoCfgBuf = XMEDIA_NULL;
            pstBeWoCfgBuf = (ISP_BE_WO_CFG_BUF_S *)(XMEDIA_UINTPTR_T)arg;
        ...
        case ISP_SYNC_CFG_SET: {
            ISP_SYNC_CFG_BUF_NODE_S *pstIspSyncCfgBuf = XMEDIA_NULL;
            pstIspSyncCfgBuf = (ISP_SYNC_CFG_BUF_NODE_S *)(XMEDIA_UINTPTR_T)arg;

            return ISP_DRV_SyncCfgSet(ViPipe, pstIspSyncCfgBuf);
        }

        case ISP_WDR_CFG_SET: {
            ISP_WDR_CFG_S *pstIspWdrCfg = XMEDIA_NULL;
            pstIspWdrCfg = (ISP_WDR_CFG_S *)(XMEDIA_UINTPTR_T)arg;
        ...
        case ISP_BE_SYNC_PARAM_INIT: {
            ISP_DRV_CTX_S      *pstDrvCtx      = XMEDIA_NULL;
            ISP_BE_SYNC_PARA_S *pstBeSyncParam = XMEDIA_NULL;
            ISP_CHECK_PIPE(ViPipe);
            ISP_CHECK_POINTER(arg);

            pstDrvCtx = ISP_DRV_GET_CTX(ViPipe);
            pstBeSyncParam = (ISP_BE_SYNC_PARA_S *)(XMEDIA_UINTPTR_T)arg;
        ...
        case ISP_CHN_SELECT_CFG: {
            XMEDIA_U32 u32ChannelSel = 0;
            ISP_CHECK_POINTER(arg);
            u32ChannelSel = *(XMEDIA_U32 *)(XMEDIA_UINTPTR_T)arg;

            return ISP_DRV_ChnSelectCfg(ViPipe, u32ChannelSel);
        }

        case ISP_PROC_INIT: {
            ISP_PROC_MEM_S *pstIspProcMem = XMEDIA_NULL;
            pstIspProcMem = (ISP_PROC_MEM_S *)(XMEDIA_UINTPTR_T)arg;
        ...
        case ISP_PROC_PARAM_GET: {
            ...
            *(XMEDIA_U32 *)(XMEDIA_UINTPTR_T)arg = g_ProcParam[ViPipe];
        ...
        case ISP_MEM_INFO_SET: {
            ...
            ISP_CHECK_BOOL(*(XMEDIA_BOOL *)(XMEDIA_UINTPTR_T)arg);
            g_astIspDrvCtx[ViPipe].bMemInit = *(XMEDIA_BOOL *)(XMEDIA_UINTPTR_T)arg;
        ...
        case ISP_MEM_INFO_GET: {
            ...
            *(XMEDIA_BOOL *)(XMEDIA_UINTPTR_T)arg = g_astIspDrvCtx[ViPipe].bMemInit;

            return XMEDIA_SUCCESS;
        }

        case ISP_P2EN_INFO_GET: {
            XMEDIA_BOOL *pbP2En = XMEDIA_NULL;

            ISP_CHECK_PIPE(ViPipe);
            ISP_CHECK_POINTER(arg);

            pbP2En = (XMEDIA_BOOL *)(XMEDIA_UINTPTR_T)arg;
        ...
        case ISP_SYNC_INIT_SET: {
            ...
            pstDrvCtx->bStitchSync = XMEDIA_FALSE;
            if (pstDrvCtx->stStitchAttr.bStitchEnable == XMEDIA_TRUE) {
                pstDrvCtx->bStitchSync = *(XMEDIA_BOOL *)(XMEDIA_UINTPTR_T)arg;
            }
        ...
        case ISP_INIT_INFO_SET: {
            ...
            ISP_CHECK_BOOL(*(XMEDIA_BOOL *)(XMEDIA_UINTPTR_T)arg);
            g_astIspDrvCtx[ViPipe].bIspInit = *(XMEDIA_BOOL *)(XMEDIA_UINTPTR_T)arg;
        ...
        case ISP_RUN_STATE_SET: {
            XMEDIA_U64 *pu64HandSignal = XMEDIA_NULL;
            pu64HandSignal = (XMEDIA_U64 *)(XMEDIA_UINTPTR_T)arg;

            return ISP_DRV_SetIspRunState(ViPipe, pu64HandSignal);
        }

        case ISP_CONFIG_INFO_SET: {
            ISP_CONFIG_INFO_S *pstIspConfigInfo = XMEDIA_NULL;
            pstIspConfigInfo = (ISP_CONFIG_INFO_S *)(XMEDIA_UINTPTR_T)arg;

            return ISP_DRV_SetConfigInfo(ViPipe, pstIspConfigInfo);
        }

        case ISP_SNAP_INFO_GET: {
            ISP_SNAP_INFO_S *pstIspSnapInfo = XMEDIA_NULL;
            pstIspSnapInfo = (ISP_SNAP_INFO_S *)(XMEDIA_UINTPTR_T)arg;
        ...
        case ISP_PRO_TRIGGER_GET: {
            ...
            pstDrvCtx = ISP_DRV_GET_CTX(ViPipe);
            *(XMEDIA_BOOL *)(XMEDIA_UINTPTR_T)arg = pstDrvCtx->bProEnable;

            if (pstDrvCtx->bProEnable == XMEDIA_TRUE) {
                pstDrvCtx->bProEnable = XMEDIA_FALSE;
        ...
        case ISP_SNAP_ATTR_GET: {
            ISP_SNAP_ATTR_S *pstIspSnapAttr = XMEDIA_NULL;
            ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
            ISP_CHECK_PIPE(ViPipe);
            ISP_CHECK_POINTER(arg);

            pstIspSnapAttr = (ISP_SNAP_ATTR_S *)(XMEDIA_UINTPTR_T)arg;
            pstDrvCtx = ISP_DRV_GET_CTX(ViPipe);
        ...
        case ISP_UPDATE_POS_GET: {
            ...
            *(XMEDIA_U32 *)(XMEDIA_UINTPTR_T)arg = g_UpdatePos[ViPipe];
        ...
        case ISP_FRAME_CNT_GET: {
            ...
            pstDrvCtx = ISP_DRV_GET_CTX(ViPipe);
            *(XMEDIA_U32 *)(XMEDIA_UINTPTR_T)arg = pstDrvCtx->u32FrameCnt;

            return XMEDIA_SUCCESS;
        }
        case ISP_PUB_ATTR_INFO: {
            ISP_PUB_ATTR_S *pstIspPubAttr = XMEDIA_NULL;
            pstIspPubAttr = (ISP_PUB_ATTR_S *)(XMEDIA_UINTPTR_T)arg;
        ...
        case ISP_PWM_NUM_GET: {
            ...
            *(XMEDIA_U32 *)(XMEDIA_UINTPTR_T)arg = g_PwmNumber[ViPipe];

            return XMEDIA_SUCCESS;
        }

        case ISP_SET_MOD_PARAM: {
            ISP_MOD_PARAM_S *pstModeParam = XMEDIA_NULL;
            pstModeParam = (ISP_MOD_PARAM_S *)(XMEDIA_UINTPTR_T)arg;
        ...
        case ISP_SET_CTRL_PARAM: {
            ISP_CTRL_PARAM_S *pstCtrlParam = XMEDIA_NULL;
            pstCtrlParam = (ISP_CTRL_PARAM_S *)(XMEDIA_UINTPTR_T)arg;
        ...
        case ISP_WORK_MODE_INIT: {
            ISP_BLOCK_ATTR_S *pstIspBlockAttr = XMEDIA_NULL;
            pstIspBlockAttr = (ISP_BLOCK_ATTR_S *)(XMEDIA_UINTPTR_T)arg;
        ...
        case ISP_WORK_MODE_GET: {
            ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
            ISP_WORKING_MODE_S *pstIspWorkMode = XMEDIA_NULL;
            ISP_CHECK_PIPE(ViPipe);
            ISP_CHECK_POINTER(arg);

            pstIspWorkMode = (ISP_WORKING_MODE_S *)(XMEDIA_UINTPTR_T)arg;
            pstDrvCtx = ISP_DRV_GET_CTX(ViPipe);
        ...
        case ISP_PRE_BLK_NUM_UPDATE: {
            ...
            pstDrvCtx = ISP_DRV_GET_CTX(ViPipe);
            pstDrvCtx->stWorkMode.u8PreBlockNum = *(XMEDIA_U8 *)(XMEDIA_UINTPTR_T)arg;

            return XMEDIA_SUCCESS;
        }

        case ISP_GET_PIPE_SIZE: {
            SIZE_S *pstPipeSize = XMEDIA_NULL;
            pstPipeSize = (SIZE_S *)(XMEDIA_UINTPTR_T)arg;

            return ISP_DRV_GetPipeSize(ViPipe, pstPipeSize);
        }

        case ISP_GET_HDR_ATTR: {
            VI_PIPE_HDR_ATTR_S *pstHDRAttr = XMEDIA_NULL;
            pstHDRAttr = (VI_PIPE_HDR_ATTR_S *)(XMEDIA_UINTPTR_T)arg;

            return ISP_DRV_GetHdrAttr(ViPipe, pstHDRAttr);
        }

        case ISP_GET_WDR_ATTR: {
            VI_PIPE_WDR_ATTR_S *pstWDRAttr = XMEDIA_NULL;
            pstWDRAttr = (VI_PIPE_WDR_ATTR_S *)(XMEDIA_UINTPTR_T)arg;

            return ISP_DRV_GetWdrAttr(ViPipe, pstWDRAttr);
        }

        case ISP_GET_STITCH_ATTR: {
            VI_STITCH_ATTR_S *pstStitchAttr = XMEDIA_NULL;
            pstStitchAttr = (VI_STITCH_ATTR_S *)(XMEDIA_UINTPTR_T)arg;
        ...
        case ISP_CLUT_BUF_GET: {
            ISP_CHECK_PIPE(ViPipe);
            ISP_CHECK_POINTER(arg);
            if (g_astIspDrvCtx[ViPipe].stClutBufAttr.bInit == XMEDIA_FALSE) {
                return ERR_CODE_ISP_NOMEM;
            }
            *(XMEDIA_U64 *)(XMEDIA_UINTPTR_T)arg = g_astIspDrvCtx[ViPipe].stClutBufAttr.stClutBuf.u64PhyAddr;
        ...
        case ISP_LDCI_READ_STT_BUF_GET: {
            return ISP_DRV_LdciReadSttBufGet(ViPipe, (ISP_LDCI_READ_STT_BUF_S *)(XMEDIA_UINTPTR_T)arg);
        }

        case ISP_OPT_RUNONCE_INFO: {
            return ISP_DRV_OptRunOnceInfo(ViPipe, (XMEDIA_BOOL *)(XMEDIA_UINTPTR_T)arg);
        }

        case ISP_YUV_RUNONCE_INFO: {
            return ISP_DRV_YUVRunOnceInfo(ViPipe, (XMEDIA_BOOL *)(XMEDIA_UINTPTR_T)arg);
        }
        ...
        case ISP_SET_PROCALCDONE: {
            ISP_CHECK_PIPE(ViPipe);
            g_astIspDrvCtx[ViPipe].bProStart = XMEDIA_TRUE;
        ...
        case ISP_STT_ADDR_INIT: {
            ...
            if (pstDrvCtx->stStitchAttr.bStitchEnable == XMEDIA_FALSE) {
        ...
        case ISP_BE_LUT_STT_BUF_GET: {
            ...
            if (g_astIspDrvCtx[ViPipe].stBeLut2SttAttr.bInit == XMEDIA_FALSE) {
                return ERR_CODE_ISP_NOMEM;
            }
            u64PhyAddr = g_astIspDrvCtx[ViPipe].stBeLut2SttAttr.astBeLutSttBuf[0].astLutSttBuf[0].u64PhyAddr;
            *(XMEDIA_U64 *)(XMEDIA_UINTPTR_T)arg = u64PhyAddr;
        ...
        case ISP_SPECAWB_BUF_GET: {
            ...
            ISP_CHECK_PIPE(ViPipe);
            ISP_CHECK_POINTER(arg);
            if (g_astIspDrvCtx[ViPipe].stSpecAwbBufAttr.bInit == XMEDIA_FALSE) {
                return ERR_CODE_ISP_NOMEM;
            }
            *(XMEDIA_U64 *)(XMEDIA_UINTPTR_T)arg = g_astIspDrvCtx[ViPipe].stSpecAwbBufAttr.stSpecAwbBuf.u64PhyAddr;
        ...
        case ISP_SET_RGBIR_FORMAT: {
            ISP_CHECK_PIPE(ViPipe);
            g_astIspDrvCtx[ViPipe].enIspRgbirFormat = *(XMEDIA_U32 *)(XMEDIA_UINTPTR_T)arg;
            return XMEDIA_SUCCESS;
        }
        default: {
...
static long ISP_compat_ioctl(unsigned int cmd, unsigned long arg, void *private_data)
{
    switch (cmd) {
        ...
        case ISP_BE_CFG_BUF_INIT: {
            ISP_BE_WO_CFG_BUF_S *pstBeWoCfgBuf = (ISP_BE_WO_CFG_BUF_S *)(XMEDIA_UINTPTR_T)arg;
            COMPAT_POINTER(pstBeWoCfgBuf->pVirAddr, XMEDIA_VOID *);
            break;
        }
...
static XMEDIA_S32 ISP_Freeze(osal_dev_t *pdev)
{
    return XMEDIA_SUCCESS;
}

static XMEDIA_S32 ISP_Restore(osal_dev_t *pdev)
{
    VI_PIPE ViPipe;
...
static osal_dev_t *s_pstISPDevice = XMEDIA_NULL;
...
static inline int ISP_ISR(int irq, void *id)
{
    ...
    XMEDIA_S32  ViDev;
    VI_PIPE ViPipe;
    XMEDIA_U32 u32PortIntStatus = 0;
    XMEDIA_U32 u32PortIntFStart = 0;
    XMEDIA_U32 u32PortIntErr = 0;
    XMEDIA_U32 u32IspRawIntStatus = 0;
    XMEDIA_U32 u32IspIntStatus = 0;
    XMEDIA_U32 u32WCHIntStatus = 0;
    XMEDIA_U32 u32WCHIntFStart = 0;
    XMEDIA_U64 u64IspIrqBeginTime;
    XMEDIA_BOOL bViCapInt;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
...
XMEDIA_VOID ISP_DRV_WakeUpThread(VI_PIPE ViPipe)
{
    XMEDIA_BOOL bWakeUpTread = XMEDIA_TRUE;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ...
    if (IS_OFFLINE_MODE(pstDrvCtx->stWorkMode.enIspRunningMode) ||
        ...
        if (pstDrvCtx->stStitchAttr.bStitchEnable == XMEDIA_TRUE) {
            if (pstDrvCtx->enIspRunningState != ISP_BE_BUF_STATE_INIT) {
                bWakeUpTread = XMEDIA_FALSE;
            }
        }
    }

    if (bWakeUpTread == XMEDIA_TRUE) {
        pstDrvCtx->bEdge = XMEDIA_TRUE;
        pstDrvCtx->bVdStart = XMEDIA_TRUE;
...
XMEDIA_S32 ISP_DRV_GetUseNode(VI_PIPE ViPipe, XMEDIA_U32 u32IspIntStatus, XMEDIA_U32 u32WCHIntStatus)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ISP_SYNC_CFG_S *pstSyncCfg = XMEDIA_NULL;
    unsigned long u32Flags;
    ...
    if (IS_OFFLINE_MODE(pstDrvCtx->stWorkMode.enIspRunningMode) ||
        ...
        if ((pstDrvCtx->bYuvMode == XMEDIA_TRUE) && u32WCHIntStatus&& (pstDrvCtx->bIspRunFlag == XMEDIA_TRUE)) {
            if (pstDrvCtx->pstUseNode) {
                osal_spin_lock_irqsave(&g_stIspLock[ViPipe], &u32Flags);
...
XMEDIA_VOID ISP_LongFrmCfgSensor(VI_PIPE ViPipe)
{
    ISP_DRV_CTX_S *pstDrvCtx;
    XMEDIA_U32 u32BindPipeIntStatus;
    XMEDIA_U32 u32PipeIntPosSave;
    XMEDIA_U32 u32PipeIntStatusSave;
    XMEDIA_U32 k;
    VI_PIPE ViPipeBind;
    XMEDIA_U32 u32CfgSnsIntPos;
    if(g_bLongFrmIntEn == XMEDIA_FALSE)
    {
        return ;
    }
    pstDrvCtx  = ISP_DRV_GET_CTX(ViPipe);
    if(IS_FS_WDR_MODE(pstDrvCtx->stWDRAttr.enWDRMode) == XMEDIA_FALSE)
    {
        return;
    }
...
XMEDIA_S32 ISP_DRV_StitchGetUseNode(VI_PIPE ViPipe, XMEDIA_U32 u32IspIntStatus)
{
    VI_PIPE MainPipeS;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ISP_SYNC_CFG_S *pstSyncCfg = XMEDIA_NULL;
...
XMEDIA_S32 ISP_DRV_GetBeBufUseNode(VI_PIPE ViPipe, XMEDIA_U32 u32IspIntStatus)
{
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    XMEDIA_U32 u32WCHIntFStart;
    ISP_CHECK_PIPE(ViPipe);
...
int  ISP_IrqRoute(VI_PIPE ViPipe)
{
    ...
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    ISP_SYNC_CFG_S *pstSyncCfg = XMEDIA_NULL;
    XMEDIA_U64 u64SensorCfgTime1 = 0;
    XMEDIA_U64 u64SensorCfgTime2 = 0;
    XMEDIA_U32 u32PortIntFStart, u32IspIntStatus, u32WCHIntFStart, u32SensorCfgInt;
    XMEDIA_U64 u64PtTime1 = 0;
    XMEDIA_U64 u64PtTime2 = 0;
    XMEDIA_U64 u64IspTime1 = 0;
    XMEDIA_U64 u64IspTime2 = 0;
    ...
    /*  * * * * * * * isp_int_process * * * * * * *  */
    if (u32WCHIntFStart && (pstDrvCtx->bYuvMode == XMEDIA_TRUE)&& (pstDrvCtx->bIspRunFlag == XMEDIA_TRUE)) {  /* WCH int */
...
XMEDIA_VOID ISP_GetSlavePipeIntStatus(VI_PIPE ViPipe,XMEDIA_BOOL *bSlavePipeIntActive)
{
    XMEDIA_U8 u8ChnNumMax;
    XMEDIA_U8 k;
    XMEDIA_U8 u8IntStatusIndex;
    ISP_DRV_CTX_S *pstDrvCtx = XMEDIA_NULL;
    XMEDIA_U32  u32IspIntStatus = 0;
    XMEDIA_U32 isp_raw_int = 0;
    VI_PIPE ViPipeBind;
    if(g_bLongFrmIntEn == XMEDIA_FALSE)
    {
        *bSlavePipeIntActive = XMEDIA_FALSE;
        return ;
    }
    ...
    if((pstDrvCtx->stWDRAttr.bMastPipe == XMEDIA_FALSE) ||
       (IS_FS_WDR_MODE(pstDrvCtx->stWDRAttr.enWDRMode) == XMEDIA_FALSE))
    {
    ...
    else
    {
        ...
        for(k=0;k<u8ChnNumMax;k++)
        {
            ...
            else
            {
                ...
                {
                    *bSlavePipeIntActive = XMEDIA_TRUE;
                }
            }
        }
    }
...
int  ISP_DoIntBottomHalf(int irq, void *id)
{
    ...
    XMEDIA_U32  u32IspIntStatus = 0;
    XMEDIA_U32  u32WCHIntFStart = 0;
    XMEDIA_BOOL bSlavePipeIntActive;
    ...
    for (i = 0; i < ISP_MAX_PHY_PIPE_NUM; i++) {
        ...
        if ((!u32PortIntStatus) && (!u32IspIntStatus) && (!u32WCHIntFStart) &&(bSlavePipeIntActive == XMEDIA_FALSE)) {
            continue;
        }
...
{
    CMPI_MmzMallocNocache(XMEDIA_NULL, "ISPStatTest", &g_test_phyaddr, (XMEDIA_VOID **)&g_test_pviraddr, 256 * 2);
}
...
XMEDIA_S32 ISP_KernInit(void *p)
{
    XMEDIA_U32 ViPipe;
...
XMEDIA_VOID ISP_KernExit(void)
{
    XMEDIA_U32 ViPipe;

    for (ViPipe = 0; ViPipe < ISP_MAX_PIPE_NUM; ViPipe++) {
        if ((reg_vicap_base_va != XMEDIA_NULL) && (reg_ispfe_base_va[ViPipe] != XMEDIA_NULL)) {
            ISP_DRV_SetIntEnable(ViPipe, XMEDIA_FALSE);
        }
        osal_memset(&g_astIspDrvCtx[ViPipe].stDrvDbgInfo, 0, sizeof(ISP_DRV_DBG_INFO_S));
    }
...
static XMEDIA_U32 ISP_GetVerMagic(XMEDIA_VOID)
{
...
int ISP_ModInit(void)
{
    ...
    if (osal_registerdevice(s_pstISPDevice) < 0) {
        XMEDIA_PRINT("Kernel: Could not register isp devices\n");
        return XMEDIA_FAILURE;
    }

#ifndef DISABLE_DEBUG_INFO
    proc = osal_create_proc_entry(PROC_ENTRY_ISP, XMEDIA_NULL);

    if (proc == XMEDIA_NULL) {
        XMEDIA_PRINT("Kernel: Register isp proc failed!\n");
        goto OUT2;
    }
    ...
    ISP_Test_Init();
#endif
    XMEDIA_PRINT("ISP Mod init!\n");
    return XMEDIA_SUCCESS;

OUT1:
#ifndef DISABLE_DEBUG_INFO
    osal_remove_proc_entry(PROC_ENTRY_ISP, XMEDIA_NULL);
#endif
OUT2:
    osal_deregisterdevice(s_pstISPDevice);
    osal_destroydev(s_pstISPDevice);

    XMEDIA_PRINT("ISP Mod init failed!\n");
    return XMEDIA_FAILURE;
}
