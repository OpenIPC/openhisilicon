...
typedef struct ISP_SYNC_CFG_BUF_S {
    XMEDIA_U8                   u8BufWRFlag;    /* FW write node */
    XMEDIA_U8                   u8BufRDFlag;    /* ISR read, then write to ISP reg or sensor */
    ISP_SYNC_CFG_BUF_NODE_S stSyncCfgBufNode[ISP_SYNC_BUF_NODE_NUM];
} ISP_SYNC_CFG_BUF_S;

typedef struct ISP_SYNC_CFG_S {
    XMEDIA_U8  u8WDRMode;
    XMEDIA_U8  u8PreWDRMode;
    XMEDIA_U8  u8VCNum;          /* if 3to1, u8VCNum = 0,1,2,0,1,...... */
    XMEDIA_U8  u8VCNumMax;       /* if 3to1, u8VCNumMax = 2 */
    XMEDIA_U8  u8VCCfgNum;
    XMEDIA_U8  u8Cfg2VldDlyMAX;

    XMEDIA_U64 u64PreSnsGain;
    XMEDIA_U64 u64CurSnsGain;
    XMEDIA_U64 u64PreSnsGainSF;
    XMEDIA_U64 u64CurSnsGainSF;
    XMEDIA_U32 u32DRCComp[CFG2VLD_DLY_LIMIT];  /* drc exposure compensation */
    XMEDIA_U32 u32ExpRatio[3][CFG2VLD_DLY_LIMIT];
    XMEDIA_U8  u8LFMode[CFG2VLD_DLY_LIMIT];
    XMEDIA_U32 u32WDRGain[4][CFG2VLD_DLY_LIMIT];
    XMEDIA_U8  u8AlgProc[CFG2VLD_DLY_LIMIT];
...
typedef struct ISP_SYNC_STITCH_CTRL_S {
    XMEDIA_S8  s8StitchFlag;
    XMEDIA_U8  u8ReadAll;
    XMEDIA_U8  u8FirstReadAll;
    XMEDIA_U8  u8FirstOrder;
    XMEDIA_U8  u8AllInit;
    XMEDIA_U32 u32ViCallbackCount;
    XMEDIA_U64 u64PTS;
} ISP_SYNC_STITCH_CTRL_S;

typedef struct ISP_DRV_DBG_INFO_S {
    XMEDIA_U64 u64IspLastIntTime;           /* Time of last interrupt, for debug */
    XMEDIA_U64 u64IspLastRateTime;          /* Time of last interrupt rate, for debug */
    XMEDIA_U32 u32IspIntCnt;                /* Count of interrupt, for debug */
    XMEDIA_U32 u32IspIntTime;               /* Process time of interrupt, for debug */
    XMEDIA_U32 u32IspIntTimeMax;            /* Maximal process time of interrupt, for debug */
    XMEDIA_U32 u32IspIntGapTime;            /* Gap of two interrupts, for debug */
    XMEDIA_U32 u32IspIntGapTimeMax;         /* Maximal gap of two interrupts, for debug */
    XMEDIA_U32 u32IspRateIntCnt;            /* Count of interrupt rate, for debug */
    XMEDIA_U32 u32IspRate;                  /* Interrupt Rate, interrupt count per sencond, for debug */

    XMEDIA_U64 u64PtLastIntTime;            /* Time of last interrupt, for debug */
    XMEDIA_U64 u64PtLastRateTime;           /* Time of last interrupt rate, for debug */
    XMEDIA_U32 u32PtIntCnt;                 /* Count of interrupt, for debug */
    XMEDIA_U32 u32PtIntTime;                /* Process time of interrupt, for debug */
    XMEDIA_U32 u32PtIntTimeMax;             /* Maximal process time of interrupt, for debug */
    XMEDIA_U32 u32PtIntGapTime;             /* Gap of two interrupts, for debug */
    XMEDIA_U32 u32PtIntGapTimeMax;          /* Maximal gap of two interrupts, for debug */
    XMEDIA_U32 u32PtRateIntCnt;             /* Count of interrupt rate, for debug */
    XMEDIA_U32 u32PtRate;                   /* Interrupt Rate, interrupt count per sencond, for debug */

    XMEDIA_U32 u32SensorCfgTime;            /* Time of sensor config, for debug */
    XMEDIA_U32 u32SensorCfgTimeMax;         /* Maximal time of sensor config, for debug */

    XMEDIA_U32 u32IspResetCnt;              /* Count of ISP reset when vi width or height changed */
    XMEDIA_U32 u32IspBeStaLost;             /* Count of ISP BE statistics lost number When the ISP processing is not timely, for debug */
} ISP_DRV_DBG_INFO_S;

typedef struct ISP_INTERRUPT_SCH_S {
    XMEDIA_U32 u32PortIntStatus;
    XMEDIA_U32 u32IspIntStatus;
    XMEDIA_U32 u32PortIntErr;
    XMEDIA_U32 u32WCHIntStatus;
} ISP_INTERRUPT_SCH_S;
...
typedef struct ISP_DRV_CTX_S {
    XMEDIA_BOOL                  bEdge;
    XMEDIA_BOOL                  bVdStart;
    XMEDIA_BOOL                  bVdEnd;
    XMEDIA_BOOL                  bYuvMode;
    XMEDIA_BOOL                  bVdBeEnd;
    XMEDIA_BOOL                  bMemInit;
    XMEDIA_BOOL                  bIspInit;
    XMEDIA_BOOL                  bStitchSync;
    XMEDIA_BOOL                  bDngInfoInit;
    XMEDIA_BOOL                  bProEnable;
    XMEDIA_BOOL                  bProStart;
    XMEDIA_BOOL                  bRunOnceOk;
    XMEDIA_BOOL                  bRunOnceFlag;
    XMEDIA_BOOL                  bYUVRunOnceOk;
    XMEDIA_BOOL                  bYUVRunOnceFlag;
    XMEDIA_BOOL                  bIspRunFlag;
    XMEDIA_CHAR                  acName[8];
    XMEDIA_U8                    u8Vcnum;
    XMEDIA_U32                   u32IntPos;
    XMEDIA_U32                   u32FrameCnt;
    XMEDIA_U32                   u32Status;
    XMEDIA_U32                   u32bit16IsrAccess;
    XMEDIA_U32                   u32ProTrigFlag;
    XMEDIA_U32                   u32RegCfgInfoFlag;
    XMEDIA_U8                    *pu8SaveSttStat;
...
static XMEDIA_S32 __inline ISPSyncBufInit(ISP_SYNC_CFG_BUF_S *pstSyncCfgBuf)
{
    osal_memset(pstSyncCfgBuf, 0, sizeof(ISP_SYNC_CFG_BUF_S));
    return 0;
}

static XMEDIA_S32 __inline ISPSyncBufIsFull(ISP_SYNC_CFG_BUF_S *pstSyncCfgBuf)
{
    return ((pstSyncCfgBuf->u8BufWRFlag + 1) % ISP_SYNC_BUF_NODE_NUM) == pstSyncCfgBuf->u8BufRDFlag;
}

static XMEDIA_S32 __inline ISPSyncBufIsEmpty(ISP_SYNC_CFG_BUF_S *pstSyncCfgBuf)
{
    return pstSyncCfgBuf->u8BufWRFlag == pstSyncCfgBuf->u8BufRDFlag;
}

static XMEDIA_S32 __inline ISPSyncBufIsErr(ISP_SYNC_CFG_BUF_S *pstSyncCfgBuf)
{
...
static XMEDIA_S32 __inline ISPSyncBufWrite(ISP_SYNC_CFG_BUF_S *pstSyncCfgBuf,
...
static XMEDIA_S32 __inline ISPSyncBufRead(ISP_SYNC_CFG_BUF_S *pstSyncCfgBuf, ISP_SYNC_CFG_BUF_NODE_S **ppstSyncCfgBufNode)
{
    if (ISPSyncBufIsEmpty(pstSyncCfgBuf)) {
        return -1;
    }
...
static XMEDIA_S32 __inline ISPSyncBufRead2(ISP_SYNC_CFG_BUF_S *pstSyncCfgBuf, ISP_SYNC_CFG_BUF_NODE_S **ppstSyncCfgBufNode)
{
    XMEDIA_U8 u8Tmp = 0;
...
XMEDIA_S32 IspSyncTaskProcess(VI_PIPE ViPipe);
