...
#define MIPI0_IRQ           60
#define MIPI1_IRQ           61
unsigned int mipi0_irq = MIPI0_IRQ;
unsigned int mipi1_irq = MIPI1_IRQ;
...
static inline const char* mipi_print_mipi_wdr_mode(mipi_wdr_mode_e wdr_mode)
{
    switch(wdr_mode)
    {
...
static inline const char* mipi_print_lvds_wdr_mode(wdr_mode_e wdr_mode)
{
    switch(wdr_mode)
    {
...
static inline const char* mipi_print_linkid(COMBO_DEV devno, char *pbuf)
{
    short idx;
    char *ptr = pbuf;
    link_info_s *plink_info = &g_lane_info[devno];

    for (idx = 0; idx < plink_info->link_num; idx++)
    {
        osal_sprintf(ptr, "%2d,", plink_info->link_arr[idx].link_idx);
        ptr += 3;
    }
    ...
    {
        osal_sprintf(ptr, "%s", "N/A");
    }

    return pbuf;
}

static inline const char* mipi_print_lvds_sync_mode(lvds_sync_mode_e sync_mode)
{
    if (LVDS_SYNC_MODE_SOF == sync_mode)
    {
        return "SOF";
    }
    else if (LVDS_SYNC_MODE_SAV == sync_mode)
    {
        return "SAV";
    }
...
static inline const char* mipi_print_lvds_endian(lvds_bit_endian endian)
{
    if (LVDS_ENDIAN_LITTLE == endian)
    {
        return "Little";
    }
    else if (LVDS_ENDIAN_BIG == endian)
    {
        return "Big";
    }
...
static inline const char* mipi_print_valid_lane(unsigned int valid_mask, char *pbuf)
{
    int lane;
    char *ptr = pbuf;

    for (lane = 0; lane < LANE_NUM_PER_LINK; lane++)
    {
        if (valid_mask & (1 << lane))
        {
            osal_sprintf(ptr, "%2d,", lane);
            ptr += 3;
        }
    }
...
void  hi_mipi_init(void)
{
	...
	mipi_drv_apb_reset(0);

	mipi_drv_apb_unreset(0);
	for (i = 0; i < COMBO_MAX_DEV_NUM; i++)
    {
    ...
    if (COMBO_MAX_DEV_NUM >= 2)
    {
        mipi_drv_set_chn_int_mask(1, MIPI_CHN1_INT_MASK);
    }
    ...
    if (COMBO_MAX_DEV_NUM >= 2)
    {
        mipi_drv_set_lvds_ctrl_int_mask(1, LVDS_CTRL1_INT_MASK);
        mipi_drv_set_mipi_ctrl_int_mask(1, MIPI_CTRL_INT_MASK);
        //mipi_drv_set_mipi_pkt_int1_mask(1, MIPI_PKT_INT_MASK);
        mipi_drv_set_mipi_frame_int_mask(1, MIPI_FRAME_INT_MASK);
        //mipi_drv_set_align_int_mask(1, ALIGN1_INT_MASK);
    }
#endif
}
...
static int get_lane_info(COMBO_DEV devno, input_mode_t input_mode, short* p_lane_id, link_info_s** p_dev_link_info)
{
	...
	{
		start_link_idx = 2;
		lane_id_size = LANE_NUM_PER_LINK;
	}
    /* i is link index */
    for (i = 0; i < lane_id_size / LANE_NUM_PER_LINK; i++)
    {
        ...
        for (j = 0; j < LANE_NUM_PER_LINK; j++)
        {
            ...
            if (-1 != p_lane_id[i * LANE_NUM_PER_LINK + j])
            {
                ...
                {
					p_link_info->link_arr[temp_link_idx].link_idx =  start_link_idx + i;
                }
            }
        }
    ...
    // check sensor1's lane id 
    // if input mode is LVDS or MIPI, sensor1 can only use PHY2, so maximum 4 lanes is supported
    // and the lane_id can only be 0, 1, 2, 3
    if ( 1 == devno)
    {
        if (p_link_info->total_lane_num > 4 || p_link_info->link_num != 1)
        {
            HI_ERR("do not support lane_id for sensor 1, total_lane_num = %d, link_num = %d\n", p_link_info->total_lane_num
				,p_link_info->link_num);
            *p_dev_link_info = NULL;
            return HI_FAILURE;
        }

        /* lane id must be 0,1,2,3, and convert {0,1,2,3} to {8,9,10,11} */
        for (i = 0; i < p_link_info->link_arr[0].lane_num; i++)
        {
            if (p_link_info->link_arr[0].lane_idx[i] >= 4)
            {
                HI_ERR("lane_id can not lt 4\n");
                *p_dev_link_info = NULL;
                return HI_FAILURE;
            }
			#if 0
            if (-1 != p_link_info->link_arr[0].lane_idx[i])
            {
                p_link_info->link_arr[0].lane_idx[i] += 8;
            }
			#endif
        }

        /* sensor 1 can only use phy2 */
        p_link_info->link_arr[0].link_idx = 2;
    }
 ...
 */
static int is_mipi_rx_cfged(void)
{
    COMBO_DEV devno;
...
static void mipi_clear_config(COMBO_DEV devno, work_mode_t work_mode)
{
    ...
    for (i = 0; i < p_link_info->link_num; i++)
    {
        ...
        mipi_drv_set_phy_lp_ck_en(link_id, work_mode, temp_lane_id);
        mipi_drv_set_phy_te_hs_en(link_id, temp_lane_id);
        mipi_drv_set_phy_mode(link_id, work_mode, PHY_CLK_SHARE_NONE, temp_lane_id);
        mipi_drv_set_link_lane_id(devno, link_id, temp_lane_id);
#ifndef HI_FPGA
        mipi_drv_set_phy_ckgt_en(devno, link_id, HI_FALSE);
#endif
    }
...
int mipi_set_mipi_dev_attr(COMBO_DEV devno, phy_clk_share_e phy_clk_share,
            mipi_dev_attr_t *p_attr, img_rect_t *p_img_rect)
{
    ...
    mipi_drv_set_mem_cken(devno, HI_TRUE);
    
    mipi_drv_set_mipi_image_rect(devno, p_img_rect);
    ...
    *******************************************************/
    for (i = 0; i < p_link_info->link_num; i++)
    {
        ...
        mipi_drv_set_phy_mode(link_id, WORK_MODE_MIPI, phy_clk_share, p_link_info->link_arr[i].lane_idx);

#ifndef HI_FPGA
        mipi_drv_set_phy_ckgt_en(devno, link_id, HI_TRUE);
#endif        
    }
...
int mipi_set_lvds_dev_attr(COMBO_DEV devno, input_mode_t input_mode,
        phy_clk_share_e phy_clk_share, lvds_dev_attr_t* p_attr, img_rect_t *p_rect)
{
    int ret = HI_SUCCESS;
    link_info_s* p_link_info = NULL;
    ...
    ***************************************************************/
    ...
    mipi_drv_set_lvds_image_rect(devno, p_rect, p_link_info->total_lane_num);
    ...
    {
        mipi_drv_set_lvds_presync_clr_en(devno, HI_TRUE);
    }
    ...
    for (i = 0; i < p_link_info->link_num; i++)
    {
        ...
        mipi_drv_set_phy_mode(link_id, work_mode, phy_clk_share, p_link_info->link_arr[i].lane_idx);
#ifndef HI_FPGA
        mipi_drv_set_phy_ckgt_en(devno, link_id, HI_TRUE);
#endif
    }
...
static int mipi_set_cmos_attr(COMBO_DEV devno, input_mode_t input_mode)
{
	...
	if(devno == 0)
	{
    mipi_drv_set_phy_mode(0, work_mode, PHY_CLK_SHARE_NONE, temp_lane_id);
    mipi_drv_set_phy_mode(1, work_mode, PHY_CLK_SHARE_NONE, temp_lane_id);
	}
	else if(devno == 1)
	{
		mipi_drv_set_phy_mode(2, work_mode, PHY_CLK_SHARE_NONE, temp_lane_id);
		phy_chn1_ctrl(devno);		
	}
	else
	{
...
static inline int check_comm_attr(combo_dev_attr_t* p_attr)
{
    ...
    if (p_attr->phy_clk_share < PHY_CLK_SHARE_NONE || p_attr->phy_clk_share >= PHY_CLK_SHARE_BUTT)
    {
        HI_ERR("invalid phy clock share mode (%d).\n", p_attr->phy_clk_share);
        return HI_FAILURE;
    }

    if (p_attr->img_rect.x % 2 != 0 || p_attr->img_rect.y % 2 != 0)
    {
    ...
    if (p_attr->img_rect.width < LVDS_MIN_WIDTH || p_attr->img_rect.height < LVDS_MIN_HEIGHT)
    {
        HI_ERR("invalid img_size(%d, %d), can't be smaller than (%d, %d)\n",
               p_attr->img_rect.width, p_attr->img_rect.height, LVDS_MIN_WIDTH, LVDS_MIN_HEIGHT);
        return HI_FAILURE;
    }

	if(1 == p_attr->devno)
	{
		if((HI_TRUE == is_dev_cfged(0))&&
			((2 == g_lane_info[0].link_arr[0].link_idx)
			||(2 == g_lane_info[0].link_arr[1].link_idx)
			||(2 == g_lane_info[0].link_arr[2].link_idx)))
		{
			HI_ERR("mipi_0 used link2 ,mipi_1 do not use link2,please check mipi0's combo dev attr.\n");
	        return HI_FAILURE;
		}
	}
    return HI_SUCCESS;
...
int mipi_set_combo_dev_attr(combo_dev_attr_t* p_attr)
{
    ...
    switch (p_attr->input_mode)
    {
        ...
        {
            ret = mipi_set_lvds_dev_attr(p_attr->devno, INPUT_MODE_LVDS, 
                        p_attr->phy_clk_share, &p_attr->lvds_attr, &p_attr->img_rect);
            break;
        }
        case INPUT_MODE_MIPI:
        {
            ret = mipi_set_mipi_dev_attr(p_attr->devno, p_attr->phy_clk_share, 
                        &p_attr->mipi_attr, &p_attr->img_rect);
            break;
        }
        case INPUT_MODE_CMOS:
        case INPUT_MODE_BT1120:
        {
            ret = mipi_set_cmos_attr(p_attr->devno, INPUT_MODE_CMOS);
            break;
        }       
        case INPUT_MODE_SUBLVDS:
        {
            ret = mipi_set_lvds_dev_attr(p_attr->devno, INPUT_MODE_SUBLVDS, 
                        p_attr->phy_clk_share, &p_attr->lvds_attr, &p_attr->img_rect);
            break;
        }
        case INPUT_MODE_HISPI:
        {
            ret = mipi_set_lvds_dev_attr(p_attr->devno, INPUT_MODE_HISPI, 
                        p_attr->phy_clk_share, &p_attr->lvds_attr, &p_attr->img_rect);
            break;
        }
        default:
    ...
    // mipi_drv_set_phy_cfg_en generate a pulse to enable the phy configures
    if (HI_SUCCESS == ret)
    {
...
static long hi_mipi_ioctl(unsigned int cmd, unsigned long arg, void* private_data)
{
    ...
    unsigned int mipi_clk_edge, msb;
    ...
    switch (cmd)
    {
        case HI_MIPI_SET_DEV_ATTR:
        {
            ...
            if (HI_SUCCESS == ret)
            {
                osal_memcpy(&g_dev_ctx[pstcombo_dev_attr->devno].dev_attr, pstcombo_dev_attr, sizeof(combo_dev_attr_t));
            }
            break;
        }
...
static inline void mipi_int_statics(COMBO_DEV devno)
{
    ...
    if (pkt_int1 || pkt_int2 || frame_int||mipi_ctrl_int)
    {
        mipi_drv_clear_mipi_csi_int(devno, 0xffffffff);
		mipi_drv_clear_mipi_ctrl_int(devno, 0xffffffff);
    }
...
static inline void lvds_int_statics(COMBO_DEV devno)
{
    ...
    if (lvds_ctrl_int)
    {
        mipi_drv_clear_lvds_ctrl_int(devno, 0xffffffff);
    }
    ...
    if (lvds_ctrl_int & mask)
    {
        ...
        if (0 == devno)
        {
            ...
            {
                lvds_err_info[devno].link_write_err_cnt[1]++;
            }
            if (lvds_ctrl_int & LINK2_WRITE_ERR)
            {
                lvds_err_info[devno].link_write_err_cnt[2]++;
            }

            if (lvds_ctrl_int & LINK1_READ_ERR)
            {
                lvds_err_info[devno].link_read_err_cnt[1]++;
            }
            if (lvds_ctrl_int & LINK2_READ_ERR)
            {
                lvds_err_info[devno].link_read_err_cnt[2]++;
            }

            if (lvds_ctrl_int & LINK1_VSYNC_ERR)
            {
                lvds_err_info[devno].link_vsync_err_cnt[1]++;
            }
            if (lvds_ctrl_int & LINK2_VSYNC_ERR)
            {
                lvds_err_info[devno].link_vsync_err_cnt[2]++;
            }	

            if (lvds_ctrl_int & LINK1_HSYNC_ERR)
            {
                lvds_err_info[devno].link_hsync_err_cnt[1]++;
            }
            if (lvds_ctrl_int & LINK2_HSYNC_ERR)
            {
                lvds_err_info[devno].link_hsync_err_cnt[2]++;
            }
        }
    }
}
...
static void align_int_statis(COMBO_DEV devno)
{
    ...
    if (align_int)
    {
       mipi_drv_clear_align_int(devno, 0xffffffff);
    }
...
void mipi_clear_mipi_chn_int(COMBO_DEV devno)
{
	mipi_drv_clear_mipi_chn_int(devno);	
}
...
void mipi_cil_int_statis(COMBO_LINK link_id)
{
    ...
    if (link_int_status)
    {
        mipi_drv_clear_cil_int(link_id, 0xffffffff);
...
static int mipi_interrupt_route(int irq, void* dev_id)
{
    ...
    for (i = 0; i < COMBO_MAX_DEV_NUM; i++)
    {
        ...
        lvds_int_statics(i);
        align_int_statis(i);
		mipi_clear_mipi_chn_int(i);
    }
      
    if(((0==s32MipiDev)||(1==s32MipiDev))&&
        (INPUT_MODE_MIPI == g_dev_ctx[s32MipiDev].dev_attr.input_mode))
    {
...
static void proc_show_combo_device(osal_proc_entry_t *s)
{
    COMBO_DEV devno;
    work_mode_t work_mode;
    raw_data_type_e raw_data_type = RAW_DATA_BUTT;
    char buf[32] = {0};
    
    osal_seq_printf(s, "\n-----Combo DEV ATTR-----------------------------------------------------------------------------------------------------\n");

    osal_seq_printf(s, "%8s" "%10s" "%10s" "%10s" "%12s" "%8s" "%8s" "%8s" "%8s" "%10s" "%12s" "%16s" "\n",
    ...
    for (devno = 0; devno < COMBO_MAX_DEV_NUM; devno++)
    {
        combo_dev_attr_t *pdev_attr = &g_dev_ctx[devno].dev_attr;
		
		work_mode = mipi_drv_get_sys_work_mode(devno);
		
		if (!is_dev_cfged(devno) &&(WORK_MODE_CMOS != work_mode))
        {
            continue;
        }
    

        osal_seq_printf(s, "%8d" "%10s", devno, mipi_get_work_mode_name(work_mode));
        if (WORK_MODE_CMOS == work_mode)
        {
            osal_seq_printf(s, "%10s" "%10s" "%12s" "%8s" "%8s" "%8s" "%8s" "%10s" "%12s" "%16s" "\n",
                    "N/A", "N/A", "N/A", "N/A", "N/A", "N/A", "N/A", "N/A", "N/A", "N/A");
        }
        else if (WORK_MODE_LVDS == work_mode) 
        {
            raw_data_type = mipi_drv_get_lvds_raw_data_type(devno);
            osal_seq_printf(s, "%10s" "%10s" "%12s" "%8d" "%8d" "%8d" "%8d" "%10s" "%12s" "%16s" "\n",
                    mipi_get_raw_data_type_name(raw_data_type),
                    mipi_print_lvds_wdr_mode(pdev_attr->lvds_attr.wdr_mode),
                    mipi_print_linkid(devno, buf),
                    pdev_attr->img_rect.x,
                    pdev_attr->img_rect.y,
                    pdev_attr->img_rect.width,
                    pdev_attr->img_rect.height,
                    mipi_print_lvds_sync_mode(pdev_attr->lvds_attr.sync_mode),
                    mipi_print_lvds_endian(pdev_attr->lvds_attr.data_endian),
                    mipi_print_lvds_endian(pdev_attr->lvds_attr.sync_code_endian));
        }
        else
        {
            ...
            osal_seq_printf(s, "%10s" "%10s" "%12s" "%8d" "%8d" "%8d" "%8d" "%10s" "%12s" "%16s" "\n",
                    mipi_get_raw_data_type_name(raw_data_type),
                    mipi_print_mipi_wdr_mode(pdev_attr->mipi_attr.wdr_mode),
                    mipi_print_linkid(devno, buf),
                    pdev_attr->img_rect.x,
                    pdev_attr->img_rect.y,
                    pdev_attr->img_rect.width,
...
static void proc_show_link_info(osal_proc_entry_t *s)
{
    COMBO_DEV devno;
    COMBO_LINK link_id;
    work_mode_t work_mode;
    link_info_s *plink_info;
    char buf[32] = {0};
    HI_S32 idx;
    
    osal_seq_printf(s, "\n-----LINK INFO------------------------------------------------------\n");

    osal_seq_printf(s, "%8s" "%10s" "%14s" "%12s" "%12s" "%12s" "\n",
    ...
    for (devno = 0; devno < COMBO_MAX_DEV_NUM; devno++)
    {
        ...
        for (idx = 0; idx < g_lane_info[devno].link_num; idx++)
        {
            unsigned int valid_mask;
            unsigned int phy_data, aligned_data;
            
            plink_info = &g_lane_info[devno];
            link_id = plink_info->link_arr[idx].link_idx;
            
            valid_mask = mipi_drv_get_phy_valid_lane(link_id, work_mode);
            phy_data = mipi_drv_get_phy_data(link_id);
            aligned_data = mipi_drv_get_phy_aligned_data(link_id);
            
            osal_seq_printf(s, "%8d" "%10d" "   %2d,%2d,%2d,%2d" "%#12x" "%#12x" "%12s" "\n",
                    link_id,
                    plink_info->link_arr[idx].lane_num,
                    plink_info->link_arr[idx].lane_idx[0],
                    plink_info->link_arr[idx].lane_idx[1],
                    plink_info->link_arr[idx].lane_idx[2],
                    plink_info->link_arr[idx].lane_idx[3],
                    phy_data, aligned_data,
                    mipi_print_valid_lane(valid_mask, buf));
        }
    }
}


static void proc_show_mipi_detect_info(osal_proc_entry_t *s, COMBO_DEV devno_array[], int mipi_cnt)
{
...
static inline int mipi_get_lvds_wdr_frame_num(wdr_mode_e wdr_mode)
{
    int num = 0;
    if (HI_WDR_MODE_2F == wdr_mode || HI_WDR_MODE_DOL_2F == wdr_mode)
    {
        num = 2;
    }
    else if (HI_WDR_MODE_3F == wdr_mode || HI_WDR_MODE_DOL_3F == wdr_mode)
    {
        num = 3;
    }
    else if (HI_WDR_MODE_4F == wdr_mode || HI_WDR_MODE_DOL_4F == wdr_mode)
    {
        num = 4;
    }
    return num;
...
static void proc_show_lvds_detect_info(osal_proc_entry_t *s, COMBO_DEV devno_array[], int lvds_cnt)
{
    img_size_t image_size;
    unsigned int ef, frame_num;
    COMBO_DEV devno;
    int devno_idx;
    wdr_mode_e wdr_mode;
    ...
    osal_seq_printf(s, "\n-----lvds detect info----------------------------------------------------\n");
    osal_seq_printf(s, "%8s" "%10s" "%12s" "\n", "Devno", "Lane", "LaneWidth");
    for (devno_idx = 0; devno_idx < lvds_cnt; devno_idx++)
    {
        int lane_idx, lane_cnt;
        devno = devno_array[devno_idx];
        lane_cnt = g_lane_info[devno].total_lane_num;

        for (lane_idx = 0; lane_idx < lane_cnt; lane_idx++)
        {
            mipi_drv_get_lvds_lane_imgsize_statis(devno, lane_idx, &image_size);
            osal_seq_printf(s, "%8d" "%10d" "%12d""\n", devno, lane_idx, image_size.width);
        }
    }
    ...
    {
       devno = devno_array[devno_idx];
       wdr_mode = g_dev_ctx[devno].dev_attr.lvds_attr.wdr_mode;

       if (HI_WDR_MODE_NONE == wdr_mode)
       {
           osal_seq_printf(s, "%8s" "%12s" "%12s" "\n", "Devno", "width", "height");
           mipi_drv_get_lvds_imgsize_statis(devno, 0, &image_size);
           osal_seq_printf(s, "%8d" "%12d" "%12d" "\n", devno, image_size.width, image_size.height);
       }
       else
       {
           osal_seq_printf(s, "%8s" "%10s" "%12s" "%12s" "\n", "Devno", "WDR_Frame", "width", "height");

           frame_num = mipi_get_lvds_wdr_frame_num(wdr_mode);
           for (ef = 0; ef < frame_num; ef++)
           {
               mipi_drv_get_lvds_imgsize_statis(devno, ef, &image_size);
               osal_seq_printf(s, "%8d" "%10s" "%12d" "%12d" "\n", devno, ef_name[ef],
...
static void proc_show_cil_int_err(osal_proc_entry_t *s)
{
    ...
    {
        osal_seq_printf(s, "%5d%6d%12d%12d%12d%12d%10d%10d%10d%10d%10d\n",
                link_id,
                phy_int_state[link_id].clk_fsm_timeout_cnt,
                phy_int_state[link_id].d0_fsm_timeout_cnt,
                phy_int_state[link_id].d1_fsm_timeout_cnt,
...
static void proc_show_lvds_int_err(osal_proc_entry_t *s, COMBO_DEV devno_array[], int lvds_cnt)
{
    ...
    osal_seq_printf(s, "%6s%8s%10s%10s%10s%10s%10s%10s%13s%13s%13s%13s%13s%13s\n", 
            "Devno", "StatErr", 
            "Link0WErr", "Link1WErr", "Link2WErr",
            "Link0RErr", "Link1RErr", "Link2RErr",
            "Link0HSynErr", "Link1HSynErr", "Link2HSynErr",
            "Link0VSynErr", "Link1VSynErr", "Link2VSynErr");
    for (devno_idx = 0; devno_idx < lvds_cnt; devno_idx++)
    {
        devno = devno_array[devno_idx];
        osal_seq_printf(s, "%6d%7d%10d%10d%10d%10d%10d%10d%13d%13d%13d%13d%13d%13d\n",
                devno,
                lvds_err_info[devno].lvds_stat_err_cnt,
                lvds_err_info[devno].link_write_err_cnt[0],
                lvds_err_info[devno].link_write_err_cnt[1],
    ...
    osal_seq_printf(s, "%6s%10s%10s%10s%10s%10s%10s%10s%10s%10s%10s%10s%10s\n", 
            "Devno", 
...
static void proc_show_align_int_err(osal_proc_entry_t *s)
{
    ...
    for (devno = 0; devno < COMBO_MAX_DEV_NUM; devno++) 
    {
        ...
        if (0 == devno)
        {
            ...
            osal_seq_printf(s, "%6s%13s%10s%10s%10s%10s%10s%10s%10s%10s%10s%10s%10s%10s\n", 
                    "Devno", "FIFO_FullErr", 
...
int mipi_init(void)
{
    ...
    ret = osal_request_irq(mipi0_irq, mipi_interrupt_route, NULL, "mipi0_int", mipi_interrupt_route);
    if (0 != ret)
    {
    ...
    if (COMBO_MAX_DEV_NUM >= 2)
    {
        ret = osal_request_irq(mipi1_irq, mipi_interrupt_route, 0, "mipi1_int", mipi_interrupt_route);
        if (0 != ret)
        {
            HI_ERR("mipi: failed to register IRQ(%d)\n", MIPI1_IRQ);
	...
	osal_memset(&g_lane_info, 0, sizeof(g_lane_info));
...
MIPI_INIT_FAIL3:
    ...
    if (COMBO_MAX_DEV_NUM >= 2)
    {
        osal_free_irq(MIPI1_IRQ, NULL);
    }

MIPI_INIT_FAIL2:
    osal_free_irq(MIPI0_IRQ, NULL);
...
void mipi_exit(void)
{
    ...
    osal_free_irq(MIPI0_IRQ, mipi_interrupt_route);
    if (COMBO_MAX_DEV_NUM >= 2)
    {
        osal_free_irq(MIPI1_IRQ, mipi_interrupt_route);
    }
