/*
 * GDC (Geometric Distortion Correction) module init - hi3516cv500 only
 *
...
extern GK_U32 max_gdc_job;
extern GK_U32 max_gdc_task;
extern GK_U32 max_gdc_node;

module_param_array(gdc_en, uint, GK_NULL, S_IRUGO);
...
{
	GK_U32 i;
	GK_CHAR GdcDevName[GDC_DEV_NAME_LENGTH] = "gdc";
	GK_CHAR NnieDevName[GDC_DEV_NAME_LENGTH] = "nnie0";
	struct resource *mem;
	...
	for (i = 0; i < GDC_MAX_DEV_NUM; i++) {
		...
		g_gdc_irq[i] = osal_platform_get_irq_byname(pdev,
							     GdcDevName);
		if (g_gdc_irq[i] <= 0)
...
static COMPAT_REMOVE_RETURN hi35xx_gdc_remove(struct platform_device *pdev)
{
	GK_U32 i;
