...
{
	if (0 == online_flag ||
	    1 == online_flag) /* VI_OFFLINE_VPSS_OFFLINE/VI_OFFLINE_VPSS_ONLINE */
	{
...
{
	SYS_WRITEL(reg_iocfg_ahb_base + 0x000c,
		   0x0603); /* 0x0:GPIO0_3, 0x3:I2C1_SCL */
	SYS_WRITEL(reg_iocfg_ahb_base + 0x0010,
		   0x1003); /* 0x0:GPIO0_4, 0x3:I2C1_SDA */
}

void i2c1_for_aic31_pin_mux_gk7205v300(void)
{
	SYS_WRITEL(reg_iocfg_ahb_base + 0x000c,
		   0x1033); /* 0x0:GPIO0_3, 0x3:I2C1_SCL */
	SYS_WRITEL(reg_iocfg_ahb_base + 0x0010,
		   0x1033); /* 0x0:GPIO0_4, 0x3:I2C1_SDA */
}
...
{
	reg_write32(0x1 << 12, 0x1 << 12,
		    reg_crg_base + 0x01bc); /* SPI0 clock for LCD*/
...
void vo_16bit_lcd_mux(void)
{
	...
	reg_write32(0x1 << 7, 0x1 << 7,
		    reg_crg_base + 0x01bc); /* PWM clock for LCD*/
...
{
	SYS_WRITEL(reg_iocfg_core_base + 0x0000,
		   0x1000); /* 0x0:GPIO1_0(now), 0x1:LSADC_CH0 */
...
{
	SYS_WRITEL(reg_iocfg_ahb_base + 0x00A0,
		   0x1000); /* 0x0:GPIO9_1(now), 0x1:PWM3 */
...
{
	SYS_WRITEL(
		reg_iocfg_core_base + 0x0014,
		0x1E02); /* 0x0:JTAG_TCK, 0x1:SPI1_SCLK, 0x2:GPIO1_5(now), 0x4:UART2_TXD, 0x5:I2S_BCLK */
...
{
	SYS_WRITEL(reg_iocfg_core_base + 0x0000,
		   0x1001); /* 0x0:GPIO1_0, 0x1:LSADC_CH0(now) */
	SYS_WRITEL(reg_iocfg_core_base + 0x0004,
		   0x1001); /* 0x0:GPIO1_1, 0x1:LSADC_CH1(now) */
}

void ir_mux(void)
{
	SYS_WRITEL(reg_iocfg_ahb_base + 0x0008,
		   0x1D02); /* 0x0:GPIO0_0, 0x1:UPDATE_MODE, 0x2:IR_IN(now) */
}
