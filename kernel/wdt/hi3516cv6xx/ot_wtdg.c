...
static int ot_dog_set_heartbeat(int t)
{
    ...
    if (lt <= 0) {
        ...
        osal_printk("set heartbeat range error, %d exceeds maximum value.\n", t);
        lt = cnt;
        ret = -1;
    }
...
static int get_margin(void)
{
    ...
    if (ot_dog_set_heartbeat(g_default_margin)) {
        ...
        osal_printk("default_margin value must be 0 < default_margin < %u, force to use %d\n",
            ((~0x0U) / g_rate) + 1, OT_DOG_TIMER_MARGIN);
    }
