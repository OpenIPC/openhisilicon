 ...
 * Description: mmz user dev source file.
...
static int mmz_userdev_ioctl_of_d(struct file *file, unsigned int cmd, unsigned long arg, struct mmz_userdev_info *pmu)
{
    int ret = 0;
    hil_mmb_t *mmb = NULL;
    ...
    if ((uintptr_t)area.dirty_virt_start != (uintptr_t)pmi->mapped + offset) {
        printk(KERN_WARNING "dirty_virt_start addr was not consistent with dirty_phys_start addr!\n");
        ret = -EFAULT;
        return ret;
    }
...
static long mmz_userdev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    if (_IOC_TYPE(cmd) == 'm') {
        ...
        ret = mmz_userdev_ioctl_of_d(file, cmd, arg, pmu);
...
int mmz_userdev_mmap(struct file *file, struct vm_area_struct *vma)
{
    ...
    if (file->f_flags & O_SYNC) {
#ifdef CONFIG_64BIT
        vma->vm_page_prot = __pgprot(pgprot_val(vma->vm_page_prot) | PTE_WRITE | PTE_DIRTY);
#endif
        ...
        vma->vm_page_prot = __pgprot(pgprot_val(vma->vm_page_prot) | PTE_WRITE | PTE_DIRTY);
#else
        vma->vm_page_prot = __pgprot(pgprot_val(vma->vm_page_prot) | L_PTE_PRESENT | L_PTE_YOUNG |
