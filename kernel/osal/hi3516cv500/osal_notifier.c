 ...
 * Description: osal notifier source file.
...
static struct osal_notifier_block *osal_find_ob(struct notifier_block *nb)
{
    ...
    if (osal_list_empty(&g_nb_list)) {
        osal_printk("find nb failed! g_nb_list is empty!\n");
        return NULL;
    }
    osal_list_for_each(this, &g_nb_list) {
...
static int osal_del_nb(struct osal_notifier_block *nb)
{
    ...
    osal_list_for_each_safe(this, next, &g_nb_list) {
        struct nb_node *ns = osal_list_entry(this, struct nb_node, node);
        if (ns->osal_nb == nb) {
            osal_list_del(this);
            kfree(ns);
...
int osal_register_reboot_notifier(struct osal_notifier_block *ob)
{
    ...
    node->osal_nb = ob;
    node->nb = nb;
    osal_list_add(&(node->node), &g_nb_list);
    return 0;
}
