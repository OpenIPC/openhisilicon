...
{
    seq_printf(seq, "%8s%12s", "mirror", "name");
#ifdef CONFIG_PM
    seq_printf(seq, "%21s%21s%21s%21s\n",
        "suspend_start(us)", "suspend_cost(us)", "resume_start(us)", "resume_cost(us)");
#else
    seq_printf(seq, "\n");
#endif
...
static int media_seq_show(struct seq_file *seq, void *v)
{
    ...
    seq_printf(seq, "%8i%12s", p->minor, (char *)p->name ? (char *)p->name : "");
#ifdef CONFIG_PM
    seq_printf(seq, "%21llu%21llu%21llu%21llu\n",
        div_u64(p->pm_time[0], 24), div_u64(p->pm_time[1] - p->pm_time[0], 24), /* 24: time conversion */
        div_u64(p->pm_time[2], 24), div_u64(p->pm_time[3] - p->pm_time[2], 24)); /* 2, 3, 24: resume start/end */
#else
    seq_printf(seq, "\n");
#endif
    return 0;
...
#ifdef CONFIG_PM
    dev_set_drvdata(&(*bdev)->dev, media);
#endif
