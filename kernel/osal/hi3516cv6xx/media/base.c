...
static inline void media_pm_get_cur_cnt(pm_device *media, int index)
{
#ifdef CONFIG_PM
    media->pm_time[index] = __arch_counter_get_cntpct();
#endif
}
...
static int media_pm_suspend(struct device *dev)
{
    ...
    drv = dev->driver;
    media_pm_get_cur_cnt(dev_get_drvdata(dev), 0);
    if (drv->pm) {
        if (drv->pm->suspend) {
    ...
    media_pm_get_cur_cnt(dev_get_drvdata(dev), 1);
    return ret;
}
...
static int media_pm_resume(struct device *dev)
{
    ...
    drv = dev->driver;
    media_pm_get_cur_cnt(dev_get_drvdata(dev), 2); /* 2: resume start */
    if (drv->pm != NULL) {
    ...
    media_pm_get_cur_cnt(dev_get_drvdata(dev), 3); /* 3: resume end */
    return ret;
}
