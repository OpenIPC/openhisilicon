    ...
    ot_mmz_t *mmz, unsigned long size, unsigned long align)
{
    ...
    unsigned long grain_align;
    ot_mmb_t *p = NULL;

    grain_align = mmz_grain_align(align);
    if (grain_align == 0) {
        grain_align = MMZ_GRAIN;
    }
    start = mmz_align2(mmz->phys_start, grain_align);
    ...
    osal_list_for_each_entry(p, &mmz->mmb_list, list) {
        ...
        start = mmz_align2((p->phys_addr + p->length), grain_align);
