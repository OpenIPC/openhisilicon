...
void osal_pm_media_suspend(void)
{
    osal_dev_info *dev_node = NULL;
    ...
    osal_list_for_each_entry(dev_node, &dev_list, node) {
        if ((dev_node->dev != NULL) && (dev_node->dev->pmops != NULL) &&
            (dev_node->dev->pmops->pm_suspend != NULL)) {
            dev_node->dev->pmops->pm_suspend(dev_node->dev->pmops->private_data);
        }
    }
    read_unlock(&g_dev_rwlock);
}
EXPORT_SYMBOL(osal_pm_media_suspend);

void osal_pm_media_resume(void)
{
    osal_dev_info *dev_node = NULL;
    ...
    osal_list_for_each_entry_reverse(dev_node, &dev_list, node) {
        if ((dev_node->dev != NULL) && (dev_node->dev->pmops != NULL) &&
            (dev_node->dev->pmops->pm_resume != NULL)) {
            dev_node->dev->pmops->pm_resume(dev_node->dev->pmops->private_data);
        }
    }
    read_unlock(&g_dev_rwlock);
}
EXPORT_SYMBOL(osal_pm_media_resume);
