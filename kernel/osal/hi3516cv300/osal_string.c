...
int osal_vsnprintf(char *str, int size, const char *fmt, osal_va_list args){
    ...
    int iTemp;
	long long int lliTemp;
    unsigned int uintTemp;
	long long unsigned int lluintTemp;
    char *strTemp;
    char tmp[20];
		...
		else                
		{
			switch(fmt[i+1])
			{
				...
				case 'D':
                    ...
                    strTemp = osal_itoa(iTemp, tmp);
                    if(index+osal_strlen(strTemp)+1 >= size)
                    {
                        goto OUT;
                    }
				...
				case 'U':
					...
					strTemp = osal_uitoa(uintTemp, tmp);
                    if(index+osal_strlen(strTemp)+1 >= size)
                    {
                        goto OUT;
                    }
				...
				case 'l':   
				case 'L':
					if ((fmt[i+2] == 'l' || fmt[i+2] == 'L') && (fmt[i+3] == 'u' || fmt[i+3] == 'U'))
					{
                        if ((unsigned int)args%8)
                        {
                            args += 4;
                        }
						
						lluintTemp = osal_va_arg(args,unsigned long long);
						strTemp = osal_lluitoa(lluintTemp, tmp);
						if(index+osal_strlen(strTemp)+1 >= size)
                        {
                            goto OUT;
                        }
					...
					else if ((fmt[i+2] == 'l' || fmt[i+2] == 'L') && (fmt[i+3] == 'd' || fmt[i+3] == 'D'))
					{
						if ((unsigned int)args%8)
                        {
							args += 4;
						...
						lliTemp = osal_va_arg(args,long long int);
						strTemp = osal_llitoa(lliTemp, tmp);
                        if(index+osal_strlen(strTemp)+1 >= size)
                        {
                            goto OUT;
                        }
					...
					else if ((fmt[i+2] == 'u' || fmt[i+2] == 'U'))
					{
						uintTemp = osal_va_arg(args,long unsigned int);
						strTemp = osal_uitoa(uintTemp, tmp);
                        if(index+osal_strlen(strTemp)+1 >= size)
                        {
                            goto OUT;
					...
					else if ((fmt[i+2] == 'd' || fmt[i+2] == 'D'))
					{
						iTemp = osal_va_arg(args,long int);
						strTemp = osal_itoa(iTemp, tmp);
                        if(index+osal_strlen(strTemp)+1 >= size)
                        {
                            goto OUT;
				...
				case 'X':
					uintTemp = osal_va_arg(args,unsigned int);
					strTemp = osal_uitoxa(uintTemp, tmp);
                    if(index+osal_strlen(strTemp)+1 >= size)
                    {
                        goto OUT;
                    }
				...
				case 'S':                    
                    ...
                    {
                        iTemp = osal_va_arg(args, int);
                        if(index+osal_strlen(NULL_STRING)+1 >= size)
                        {
                            goto OUT;
                        }
				...
				case 'C':                    
					...
					str[index++] = (char)uintTemp;
					i++;
					break;	
				case '#':   
					if ((fmt[i+2] == 'x' || fmt[i+2] == 'X'))
                    {
                        if(index+2 >= size)
                        {
                            goto OUT;
