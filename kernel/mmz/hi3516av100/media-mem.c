...
void *hil_mmf_map2kern_nocache(phys_addr_t phys, int len)
{
    void *virt = __mmf_map(phys, len, 0);
    if (virt)
        return virt;
...
void *hil_mmf_map2kern_cache(phys_addr_t phys, int len)
{
    void *virt = __mmf_map(phys, len, 1);
    if (virt)
        return virt;
