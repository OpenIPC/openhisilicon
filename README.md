...
**OSAL (OS Abstraction Layer)** — A wrapper around kernel APIs (mutexes, timers, proc filesystem, memory mapping, interrupts) that isolates the vendor binary blobs from direct kernel calls. Because the blobs call OSAL functions instead of kernel functions directly, the OSAL source can be recompiled for a new kernel version while the blobs remain unchanged. This is the key mechanism that allows vendor modules from kernel 4.9 to run on kernel 7.0.
...
make BOARD=hi3516cv500_lite br-hisilicon-opensdk   # V3.5 (not yet available)
