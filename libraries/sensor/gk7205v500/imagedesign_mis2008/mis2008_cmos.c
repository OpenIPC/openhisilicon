...
ISP_SNS_STATE_S *g_pastMis2008[ISP_MAX_PIPE_NUM] = { XMEDIA_NULL };
...
#define MIS2008_SENSOR_RESET_CTX(dev) (g_pastMis2008[dev] = XMEDIA_NULL)

ISP_SNS_COMMBUS_U g_aunMis2008BusInfo[ISP_MAX_PIPE_NUM] = { [0] = { .s8I2cDev = 0 },
                                [1 ... ISP_MAX_PIPE_NUM - 1] = { .s8I2cDev = -1 } };

static XMEDIA_U32 gu32MaxTimeGetCnt[ISP_MAX_PIPE_NUM] = { 0 };
static XMEDIA_U32 g_au32InitExposure[ISP_MAX_PIPE_NUM] = { 0 };
static XMEDIA_U32 g_au32LinesPer500ms[ISP_MAX_PIPE_NUM] = { 0 };
static XMEDIA_U16 g_au16InitWBGain[ISP_MAX_PIPE_NUM][3] = { { 0 } };
static XMEDIA_U16 g_au16SampleRgain[ISP_MAX_PIPE_NUM] = { 0 };
static XMEDIA_U16 g_au16SampleBgain[ISP_MAX_PIPE_NUM] = { 0 };
static XMEDIA_U32 au32WDRIntTime[4] = { 0 };
...
XMEDIA_U16 Dgain_mapping_realgain_to_sensorgain(XMEDIA_FLOAT gain_real);

XMEDIA_U16 Dgain_mapping_realgain_to_sensorgain(XMEDIA_FLOAT gain_real)
{
    XMEDIA_U16 u16sensorgain;
    XMEDIA_U32 u32Dgain = gain_real * 128;
    XMEDIA_U8 dCoarseGain = 0;
    XMEDIA_U8 dFineGain = 0;
    XMEDIA_U8 u8Reg0x3e06 = 0;
    ...
    u16sensorgain = ((XMEDIA_U16)u8Reg0x3e06) << 8;
    u16sensorgain += dFineGain;

    return u16sensorgain;
}

static XMEDIA_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ISP_SNS_STATE_S *pstSnsState = XMEDIA_NULL;
    ...
    pstAeSnsDft->bAERouteExValid = XMEDIA_FALSE;
...
static XMEDIA_VOID cmos_fps_set(VI_PIPE ViPipe, XMEDIA_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ISP_SNS_STATE_S *pstSnsState = XMEDIA_NULL;
    XMEDIA_FLOAT f32maxFps;
...
static XMEDIA_VOID cmos_slow_framerate_set(VI_PIPE ViPipe, XMEDIA_U32 u32FullLines, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ISP_SNS_STATE_S *pstSnsState = XMEDIA_NULL;
...
static XMEDIA_VOID cmos_inttime_update(VI_PIPE ViPipe, XMEDIA_U32 u32IntTime)
{
    ISP_SNS_STATE_S *pstSnsState = XMEDIA_NULL;
    static XMEDIA_BOOL bFirst = XMEDIA_TRUE;
    static XMEDIA_U32 u32ShortIntTime = 0;
    static XMEDIA_U32 u32LongIntTime = 0;
    MIS2008_SENSOR_GET_CTX(ViPipe, pstSnsState);
    CMOS_CHECK_POINTER_VOID(pstSnsState);
...
static const XMEDIA_U16 u16AgainTab[64] = { 1024,  1088,  1152,  1216,  1280,  1344,  1408,	 1472,	1536,  1600,  1664,
...
struct gain_tbl_info_s {
    ...
    XMEDIA_U16 idxBase;
    XMEDIA_U8 regGain;
    XMEDIA_U8 regGainFineBase;
    XMEDIA_U8 regGainFineStep;
};
...
static XMEDIA_VOID cmos_again_calc_table(VI_PIPE ViPipe, XMEDIA_U32 *pu32AgainLin, XMEDIA_U32 *pu32AgainDb)
{
    XMEDIA_U32 i;
    static XMEDIA_U8 again_table_size = 63;
...
static XMEDIA_VOID cmos_dgain_calc_table(VI_PIPE ViPipe, XMEDIA_U32 *pu32DgainLin, XMEDIA_U32 *pu32DgainDb)
{
    XMEDIA_U32 i;
    static XMEDIA_U8 dgain_table_size = 255;
...
static XMEDIA_VOID cmos_gains_update(VI_PIPE ViPipe, XMEDIA_U32 u32Again, XMEDIA_U32 u32Dgain)
{
    ISP_SNS_STATE_S *pstSnsState = XMEDIA_NULL;
...
static XMEDIA_VOID cmos_get_inttime_max(VI_PIPE ViPipe, XMEDIA_U16 u16ManRatioEnable, XMEDIA_U32 *au32Ratio, XMEDIA_U32 *au32IntTimeMax,
                    XMEDIA_U32 *au32IntTimeMin, XMEDIA_U32 *pu32LFMaxIntTime)
{
    XMEDIA_U32 u32ShortTimeMinLimit = 0;
    XMEDIA_U32 u32ShortTimeMaxlimit = 0;
...
static XMEDIA_S32 cmos_init_ae_exp_function(AE_SENSOR_EXP_FUNC_S *pstExpFuncs)
{
...
static XMEDIA_S32 cmos_get_awb_default(VI_PIPE ViPipe, AWB_SENSOR_DEFAULT_S *pstAwbSnsDft)
{
    ISP_SNS_STATE_S *pstSnsState = XMEDIA_NULL;
...
static XMEDIA_S32 cmos_init_awb_exp_function(AWB_SENSOR_EXP_FUNC_S *pstExpFuncs)
{
    CMOS_CHECK_POINTER(pstExpFuncs);
...
static XMEDIA_S32 cmos_get_isp_default(VI_PIPE ViPipe, ISP_CMOS_DEFAULT_S *pstDef)
{
    ISP_SNS_STATE_S *pstSnsState = XMEDIA_NULL;
    ...
    pstDef->stSensorMode.bValidDngRawFormat = XMEDIA_TRUE;

    return XMEDIA_SUCCESS;
}

static XMEDIA_S32 cmos_get_isp_black_level(VI_PIPE ViPipe, ISP_CMOS_BLACK_LEVEL_S *pstBlackLevel)
{
    ISP_SNS_STATE_S *pstSnsState = XMEDIA_NULL;
    ...
    pstBlackLevel->bUpdate = XMEDIA_FALSE;
...
static XMEDIA_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, XMEDIA_BOOL bEnable)
{
    XMEDIA_U32 u32FullLines_5Fps, u32MaxIntTime_5Fps;
    ISP_SNS_STATE_S *pstSnsState = XMEDIA_NULL;
    ...
    if (bEnable) { /* setup for ISP pixel calibration mode */
        ...
        pstSnsState->bSyncInit = XMEDIA_FALSE;
    }

    return;
}

static XMEDIA_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    XMEDIA_S32 i;
    ISP_SNS_STATE_S *pstSnsState = XMEDIA_NULL;
    ...
    if ((pstSnsState->bSyncInit == XMEDIA_FALSE) || (pstSnsRegsInfo->bConfig == XMEDIA_FALSE)) {
        ...
        for (i = 0; i < pstSnsState->astRegsInfo[0].u32RegNum; i++) {
            pstSnsState->astRegsInfo[0].astI2cData[i].bUpdate = XMEDIA_TRUE;
    ...
    pstSnsRegsInfo->bConfig = XMEDIA_FALSE;
...
static XMEDIA_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
    XMEDIA_U8 u8SensorImageMode = 0;
    ISP_SNS_STATE_S *pstSnsState = XMEDIA_NULL;
    ...
    if ((pstSnsState->bInit == XMEDIA_TRUE) && (u8SensorImageMode == pstSnsState->u8ImgMode)) {
...
static XMEDIA_VOID sensor_mirror_flip(VI_PIPE ViPipe, ISP_SNS_MIRRORFLIP_TYPE_E eSnsMirrorFlip)
{
    ISP_SNS_STATE_S *pstSnsState = XMEDIA_NULL;
    MIS2008_SENSOR_GET_CTX(ViPipe, pstSnsState);
    CMOS_CHECK_POINTER_VOID(pstSnsState);
    XMEDIA_U8 FRAME_H_ST, FRAME_H_END;
    XMEDIA_U8 FRAME_W_ST, FRAME_W_END;
    XMEDIA_U8 value = 0;
...
static XMEDIA_VOID sensor_global_init(VI_PIPE ViPipe)
{
    ISP_SNS_STATE_S *pstSnsState = XMEDIA_NULL;
    ...
    pstSnsState->bInit = XMEDIA_FALSE;
    pstSnsState->bSyncInit = XMEDIA_FALSE;
...
static XMEDIA_S32 cmos_init_sensor_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
...
static XMEDIA_S32 sensor_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
...
static XMEDIA_S32 sensor_ctx_init(VI_PIPE ViPipe)
{
    ISP_SNS_STATE_S *pastSnsStateCtx = XMEDIA_NULL;

    MIS2008_SENSOR_GET_CTX(ViPipe, pastSnsStateCtx);

    if (pastSnsStateCtx == XMEDIA_NULL) {
...
static XMEDIA_VOID sensor_ctx_exit(VI_PIPE ViPipe)
{
    ISP_SNS_STATE_S *pastSnsStateCtx = XMEDIA_NULL;
...
static XMEDIA_S32 sensor_register_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    XMEDIA_S32 s32Ret;
    ...
    s32Ret |= XMEDIA_API_ISP_SensorRegCallBack(ViPipe, &stSnsAttrInfo, &stIspRegister);
    if (s32Ret != XMEDIA_SUCCESS) {
    ...
    s32Ret |= XMEDIA_API_AE_SensorRegCallBack(ViPipe, pstAeLib, &stSnsAttrInfo, &stAeRegister);
    if (s32Ret != XMEDIA_SUCCESS) {
    ...
    s32Ret |= XMEDIA_API_AWB_SensorRegCallBack(ViPipe, pstAwbLib, &stSnsAttrInfo, &stAwbRegister);
    if (s32Ret != XMEDIA_SUCCESS) {
...
static XMEDIA_S32 sensor_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    XMEDIA_S32 s32Ret;
    ...
    s32Ret = XMEDIA_API_ISP_SensorUnRegCallBack(ViPipe, MIS2008_ID);
    if (s32Ret != XMEDIA_SUCCESS) {
        ISP_TRACE(MODULE_DBG_ERR, "sensor unregister callback function failed!\n");
        return s32Ret;
    }

    s32Ret = XMEDIA_API_AE_SensorUnRegCallBack(ViPipe, pstAeLib, MIS2008_ID);
    if (s32Ret != XMEDIA_SUCCESS) {
        ISP_TRACE(MODULE_DBG_ERR, "sensor unregister callback function to ae lib failed!\n");
        return s32Ret;
    }

    s32Ret = XMEDIA_API_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, MIS2008_ID);
    if (s32Ret != XMEDIA_SUCCESS) {
...
static XMEDIA_S32 sensor_set_init(VI_PIPE ViPipe, ISP_INIT_ATTR_S *pstInitAttr)
{
