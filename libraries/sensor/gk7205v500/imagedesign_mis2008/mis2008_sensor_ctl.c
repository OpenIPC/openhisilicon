...
int mis2008_read_register(VI_PIPE ViPipe, XMEDIA_U32 addr)
{
	XMEDIA_S32 s32RegVal = 0;
	...
	XMEDIA_S32 s32Ret = 0;
	XMEDIA_U32 u32RegWidth = mis2008_addr_byte;
	XMEDIA_U32 u32DataWidth = mis2008_data_byte;
	XMEDIA_U8 aRecvbuf[4];

#ifdef LOSCFG_HOST_TYPE_VENDOR
	XMEDIA_U32 u32SnsI2cAddr = (mis2008_i2c_addr >> 1);
...
int mis2008_write_register(VI_PIPE ViPipe, XMEDIA_U32 addr, XMEDIA_U32 data)
{
	if (g_fd[ViPipe] < 0) {
		return XMEDIA_SUCCESS;
	}
...
void mis2008_init(VI_PIPE ViPipe)
{
	...
	g_pastMis2008[ViPipe]->bInit = XMEDIA_TRUE;
	return;
}
