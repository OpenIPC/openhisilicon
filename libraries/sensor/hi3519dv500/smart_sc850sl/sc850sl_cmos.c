 ...
 * global variables                                                            *
 ****************************************************************************/
#define sc850sl_sensor_set_ctx(dev, ctx)   ((g_sc850sl_sns_state[dev]) = (ctx))
#define sc850sl_sensor_reset_ctx(dev)      (g_sc850sl_sns_state[dev] = TD_NULL)
...
static td_u32 g_max_short_exp[OT_ISP_MAX_PIPE_NUM] = {[0 ...(OT_ISP_MAX_PIPE_NUM - 1)] = SC850SL_SEXP_MAX_DEFAULT};

ot_isp_sns_commbus g_sc850sl_bus_info[OT_ISP_MAX_PIPE_NUM] = {
    [0] = { .i2c_dev = 0},
    [1 ... OT_ISP_MAX_PIPE_NUM - 1] = { .i2c_dev = -1}
};

static td_float g_dcg_ratio = 3.125;

ot_isp_sns_state *g_sc850sl_sns_state[OT_ISP_MAX_PIPE_NUM] = {TD_NULL};
...
{
    return &g_sc850sl_bus_info[vi_pipe];
}

ot_isp_sns_state *sc850sl_get_ctx(ot_vi_pipe vi_pipe)
{
    return g_sc850sl_sns_state[vi_pipe];
}


const sc850sl_video_mode_tbl g_sc850sl_mode_tbl[SC850SL_MODE_BUTT] = {
    {SC850SL_VMAX_8M_30FPS_12BIT_LINEAR,   SC850SL_FULL_LINES_MAX,          30, 2.5,
     3840, 2160, 0, OT_WDR_MODE_NONE,       "SC850SL_8M_30FPS_12BIT_LINEAR_MODE"},
    {SC850SL_VMAX_8M_30FPS_10BIT_2TO1_WDR,  SC850SL_FULL_LINES_MAX_2TO1_WDR, 30, 10.0,
     3840, 2160, 0, OT_WDR_MODE_2To1_LINE,  "SC850SL_8M_30FPS_10BIT_2TO1_VC_MODE"},
};


#define sc850sl_err_mode_print(sns_image_mode, sns_state) \
do { \
...
{
    if (sns_state->img_mode == SC850SL_8M_30FPS_12BIT_LINEAR_MODE) {
        ae_sns_dft->int_time_accu.accu_type = OT_ISP_AE_ACCURACY_LINEAR;
        ae_sns_dft->int_time_accu.accuracy = 1; /* accuracy: 1 */
        ae_sns_dft->int_time_accu.offset = 0;
    } else if (sns_state->img_mode == SC850SL_8M_30FPS_10BIT_2TO1_VC_MODE) {
        ae_sns_dft->int_time_accu.accu_type = OT_ISP_AE_ACCURACY_LINEAR;
...
static td_void cmos_get_ae_linear_default(ot_vi_pipe vi_pipe, ot_isp_ae_sensor_default *ae_sns_dft,
    const ot_isp_sns_state *sns_state)
{
    ae_sns_dft->max_again = 50800; /* max 50800: 15.875*3.125*1024 = 50800 */
...
static td_void cmos_get_ae_2to1_line_wdr_default(ot_vi_pipe vi_pipe, ot_isp_ae_sensor_default *ae_sns_dft,
    const ot_isp_sns_state *sns_state)
{
    ae_sns_dft->max_int_time = sns_state->fl_std - g_max_short_exp[vi_pipe] - EXP_OFFSET_WDR_LONG;
    ...
    ae_sns_dft->max_again = 50800; /* max 50800 */
    ae_sns_dft->min_again = 1024;  /* min 1024 */
    ae_sns_dft->max_again_target = ae_sns_dft->max_again;
    ae_sns_dft->min_again_target = ae_sns_dft->min_again;
...
static td_s32 cmos_get_ae_default(ot_vi_pipe vi_pipe, ot_isp_ae_sensor_default *ae_sns_dft)
{
    ...
    sc850sl_sensor_get_ctx(vi_pipe, sns_state);
...
static td_void cmos_config_vmax(ot_isp_sns_state *sns_state, td_u32 vmax)
{
    if (sns_state->wdr_mode == OT_WDR_MODE_NONE) {
        ...
        sns_state->regs_info[0].i2c_data[WDR_LOGIC_L_IDX].data = low_8bits(vmax) - 1;
        sns_state->regs_info[0].i2c_data[WDR_LOGIC_H_IDX].data = high_8bits(vmax);
    }
...
static td_void cmos_fps_set(ot_vi_pipe vi_pipe, td_float fps, ot_isp_ae_sensor_default *ae_sns_dft)
{
    ...
    lines = g_sc850sl_mode_tbl[sns_state->img_mode].ver_lines;
    lines_max = g_sc850sl_mode_tbl[sns_state->img_mode].max_ver_lines;
    max_fps = g_sc850sl_mode_tbl[sns_state->img_mode].max_fps;
    min_fps = g_sc850sl_mode_tbl[sns_state->img_mode].min_fps;
    ...
    if (sns_state->wdr_mode == OT_WDR_MODE_NONE) {
        ...
        g_max_short_exp[vi_pipe] = (td_u32)(SC850SL_SEXP_MAX_DEFAULT * max_fps / div_0_to_1_float(fps));
        ae_sns_dft->max_int_time = sns_state->fl_std - g_max_short_exp[vi_pipe] - EXP_OFFSET_WDR_LONG;
...
static td_void cmos_slow_framerate_set(ot_vi_pipe vi_pipe, td_u32 full_lines, ot_isp_ae_sensor_default *ae_sns_dft)
{
    ...
    vmax = full_lines + EXP_OFFSET_LINEAR;
...
static td_void cmos_inttime_update_2to1_line(ot_vi_pipe vi_pipe, td_u32 int_time)
{
    ...
    if (is_first[vi_pipe]) { /* short exposure */
        ...
        sns_state->regs_info[0].i2c_data[WDR_EXPO_L_IDX].data = lower_4bits(long_int_time[vi_pipe]  & 0xfe);
...
#define SC850SL_AGAIN_NODE_NUM               256
#define SC850SL_AGAIN_ADDR_INDEX_NODE_NUM    256
#define SC850SL_DGAIN_NODE_NUM               256
#define SC850SL_DGAIN_ADDR_INDEX_NODE_NUM    256

static td_u32 again_index[SC850SL_AGAIN_NODE_NUM] = {
...
static td_u32 dgain_index[SC850SL_DGAIN_NODE_NUM] = {
    1024, 1040, 1056, 1072, 1088, 1104, 1120, 1136, 1152, 1168, 1184, 1200, 1216, 1232, 1248, 1264,
    1280, 1296, 1312, 1328, 1344, 1360, 1376, 1392, 1408, 1424, 1440, 1456, 1472, 1488, 1504, 1520,
    1536, 1552, 1568, 1584, 1600, 1616, 1632, 1648, 1664, 1680, 1696, 1712, 1728, 1744, 1760, 1776,
    1792, 1808, 1824, 1840, 1856, 1872, 1888, 1904, 1920, 1936, 1952, 1968, 1984, 2000, 2016, 2032,
...
static td_u32 again_table[SC850SL_AGAIN_ADDR_INDEX_NODE_NUM] = {
...
static td_u32 dgain_table[SC850SL_DGAIN_ADDR_INDEX_NODE_NUM] = {
...
static td_void cmos_again_calc_table(ot_vi_pipe vi_pipe, td_u32 *again_lin, td_u32 *again_db)
{
    ...
    if (*again_lin >= (td_u32)(again_index[SC850SL_AGAIN_ADDR_INDEX_NODE_NUM - 1] * g_dcg_ratio)) {
        *again_lin = (td_u32)(again_index[SC850SL_AGAIN_ADDR_INDEX_NODE_NUM - 1] * g_dcg_ratio);
        if (g_dcg_ratio > 1) {
            *again_db = again_table[SC850SL_AGAIN_NODE_NUM - 1] | 0x2000;
        } else {
            *again_db = again_table[SC850SL_AGAIN_NODE_NUM - 1];
        }
        return;
    }
    if (*again_lin > (1024 * g_dcg_ratio)) {  /* 1024 */
        again_lin_tmp = (td_u32)(*again_lin / g_dcg_ratio);
        for (i = 1; i < SC850SL_AGAIN_NODE_NUM; i++) {
...
static td_void cmos_dgain_calc_table(ot_vi_pipe vi_pipe, td_u32 *dgain_lin, td_u32 *dgain_db)
{
    ...
    if (*dgain_lin >= dgain_index[SC850SL_DGAIN_ADDR_INDEX_NODE_NUM - 1]) {
        *dgain_lin = dgain_index[SC850SL_DGAIN_ADDR_INDEX_NODE_NUM - 1];
        *dgain_db = dgain_table[SC850SL_DGAIN_NODE_NUM - 1];
        return;
    }

    for (i = 1; i < SC850SL_DGAIN_NODE_NUM; i++) {
...
static td_void cmos_gains_update(ot_vi_pipe vi_pipe, td_u32 again, td_u32 dgain)
{
    ...
    // config sensor logic
    if (reg_0x3e08 < 0x07) {
        sns_state->regs_info[0].i2c_data[SNS_LOGIC_REG363C].data = 0x05;
    } else {
        sns_state->regs_info[0].i2c_data[SNS_LOGIC_REG363C].data = 0x07;
    }
    return;
}
    ...
    ot_isp_ae_int_time_range *int_time, td_u32 *lf_max_int_time)
{
    ...
    if (g_fswdr_mode[vi_pipe] == OT_ISP_FSWDR_LONG_FRAME_MODE) {
        short_max0 = sns_state->fl[1] - g_max_short_exp[vi_pipe] - EXP_OFFSET_WDR_LONG;
        short_max = sns_state->fl[0] - g_max_short_exp[vi_pipe] - EXP_OFFSET_WDR_LONG;
        short_max = short_max > (g_max_short_exp[vi_pipe] - EXP_OFFSET_WDR_SHORT) ?
                    (g_max_short_exp[vi_pipe] - EXP_OFFSET_WDR_SHORT) : short_max;
        ...
        short_max0 = ((sns_state->fl[1] - g_max_short_exp[vi_pipe] - EXP_OFFSET_WDR_LONG) * 0x40) /
                     div_0_to_1(ratio[0]);
        short_max = ((sns_state->fl[0] - g_max_short_exp[vi_pipe] - EXP_OFFSET_WDR_LONG) * 0x40) /
                    div_0_to_1(ratio[0]);
        short_max = short_max > (g_max_short_exp[vi_pipe] - EXP_OFFSET_WDR_SHORT) ?
                    (g_max_short_exp[vi_pipe] - EXP_OFFSET_WDR_SHORT) : short_max;
        short_max = (short_max0 < short_max) ? short_max0 : short_max;
    ...
    *lf_max_int_time = sns_state->fl[0] - EXP_OFFSET_WDR_SHORT;
...
static td_s32 cmos_get_isp_default(ot_vi_pipe vi_pipe, ot_isp_cmos_default *isp_def)
{
    ...
    isp_def->sns_mode.sns_id = SC850SL_ID;
...
static td_s32 cmos_get_isp_black_level(ot_vi_pipe vi_pipe, ot_isp_cmos_black_level *black_level)
{
    ...
    if (sns_state->wdr_mode == OT_WDR_MODE_NONE) {
        for (i = 0; i < OT_ISP_BAYER_CHN_NUM; i++) {
            black_level->auto_attr.black_level[0][i] = 0x444;
        }
    } else { /* black level of DOL mode */
        for (i = 0; i < OT_ISP_WDR_MAX_FRAME_NUM; i++) {
            black_level->auto_attr.black_level[i][0] = 0x444;
            black_level->auto_attr.black_level[i][1] = 0x444;
            black_level->auto_attr.black_level[i][2] = 0x444; /* index 2 */
            black_level->auto_attr.black_level[i][3] = 0x444; /* index 3 */
        }
    }
...
static td_void cmos_default_logic_sns_reg_info_init(ot_vi_pipe vi_pipe, ot_isp_sns_state *sns_state)
{
    ot_unused(vi_pipe);

    sns_state->regs_info[0].i2c_data[SNS_LOGIC_REG363C].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[SNS_LOGIC_REG363C].reg_addr = SC850SL_LOGIC_ADDR_2;
    return;
}
...
static td_void cmos_comm_sns_reg_info_init(ot_vi_pipe vi_pipe, ot_isp_sns_state *sns_state)
{
    ...
    sns_state->regs_info[0].com_bus.i2c_dev = g_sc850sl_bus_info[vi_pipe].i2c_dev;
    ...
    for (i = 0; i < sns_state->regs_info[0].reg_num; i++) {
        ...
        sns_state->regs_info[0].i2c_data[i].dev_addr = SC850SL_I2C_ADDR;
        sns_state->regs_info[0].i2c_data[i].addr_byte_num = SC850SL_ADDR_BYTE;
        sns_state->regs_info[0].i2c_data[i].data_byte_num = SC850SL_DATA_BYTE;
    }

    /* Linear Mode Regs */
    sns_state->regs_info[0].i2c_data[EXPO_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[EXPO_L_IDX].reg_addr = SC850SL_EXPO_L_ADDR;
    sns_state->regs_info[0].i2c_data[EXPO_M_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[EXPO_M_IDX].reg_addr = SC850SL_EXPO_M_ADDR;
    sns_state->regs_info[0].i2c_data[EXPO_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[EXPO_H_IDX].reg_addr = SC850SL_EXPO_H_ADDR;

    sns_state->regs_info[0].i2c_data[AGAIN_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[AGAIN_L_IDX].reg_addr = SC850SL_AGAIN_L_ADDR;
    sns_state->regs_info[0].i2c_data[AGAIN_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[AGAIN_H_IDX].reg_addr = SC850SL_AGAIN_H_ADDR;

    sns_state->regs_info[0].i2c_data[DGAIN_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[DGAIN_H_IDX].reg_addr = SC850SL_DGAIN_H_ADDR;
    sns_state->regs_info[0].i2c_data[DGAIN_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[DGAIN_L_IDX].reg_addr = SC850SL_DGAIN_L_ADDR;

    sns_state->regs_info[0].i2c_data[VMAX_L_IDX].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[VMAX_L_IDX].reg_addr = SC850SL_VMAX_L_ADDR;
    sns_state->regs_info[0].i2c_data[VMAX_H_IDX].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[VMAX_H_IDX].reg_addr = SC850SL_VMAX_H_ADDR;

    cmos_default_logic_sns_reg_info_init(vi_pipe, sns_state);
    return;
}
...
static td_void cmos_2to1_line_wdr_sns_reg_info_init(ot_vi_pipe vi_pipe, ot_isp_sns_state *sns_state)
{
    ...
    sns_state->regs_info[0].i2c_data[WDR_SHORT_EXPO_L_IDX].reg_addr = SC850SL_SHORT_EXPO_L_ADDR;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_EXPO_M_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_EXPO_M_IDX].reg_addr = SC850SL_SHORT_EXPO_M_ADDR;

    sns_state->regs_info[0].i2c_data[WDR_SHORT_AGAIN_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_AGAIN_L_IDX].reg_addr = SC850SL_SHORT_AGAIN_L_ADDR;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_AGAIN_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_AGAIN_H_IDX].reg_addr = SC850SL_SHORT_AGAIN_H_ADDR;

    sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_L_IDX].reg_addr = SC850SL_SHORT_DGAIN_L_ADDR;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_H_IDX].reg_addr = SC850SL_SHORT_DGAIN_H_ADDR;

    sns_state->regs_info[0].i2c_data[SHORT_EXP_MAX_L_ADDR_IDX].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[SHORT_EXP_MAX_L_ADDR_IDX].reg_addr = SC850SL_SHORT_EXP_MAX_L_ADDR;
    sns_state->regs_info[0].i2c_data[SHORT_EXP_MAX_H_ADDR_IDX].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[SHORT_EXP_MAX_H_ADDR_IDX].reg_addr = SC850SL_SHORT_EXP_MAX_H_ADDR;

    sns_state->regs_info[0].i2c_data[WDR_LOGIC_L_IDX].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[WDR_LOGIC_L_IDX].reg_addr = SC850SL_WDR_LOGIC_L_ADDR;
    sns_state->regs_info[0].i2c_data[WDR_LOGIC_H_IDX].delay_frame_num = 1;
    sns_state->regs_info[0].i2c_data[WDR_LOGIC_H_IDX].reg_addr = SC850SL_WDR_LOGIC_H_ADDR;
    return;
}
...
static td_void cmos_sns_reg_info_update(ot_vi_pipe vi_pipe, ot_isp_sns_state *sns_state)
{
    ...
    if (sns_state->wdr_mode == OT_WDR_MODE_2To1_LINE) {
        if (sns_state->fl[0] > sns_state->fl[1]) {
            ...
            sns_state->regs_info[0].i2c_data[WDR_LOGIC_L_IDX].delay_frame_num = 0;
            sns_state->regs_info[0].i2c_data[WDR_LOGIC_H_IDX].delay_frame_num = 0;
    ...
    ot_isp_sns_state *sns_state)
{
    ...
    switch (sns_image_mode) {
        case SC850SL_8M_30FPS_12BIT_LINEAR_MODE:
            sns_state->fl_std = SC850SL_VMAX_8M_30FPS_12BIT_LINEAR;
            break;
        case SC850SL_8M_30FPS_10BIT_2TO1_VC_MODE:
            sns_state->fl_std = SC850SL_VMAX_8M_30FPS_10BIT_2TO1_WDR;
            break;
        default:
            sns_state->fl_std = SC850SL_VMAX_8M_30FPS_12BIT_LINEAR;
            break;
...
static td_s32 cmos_set_image_mode(ot_vi_pipe vi_pipe, const ot_isp_cmos_sns_image_mode *sns_image_mode)
{
    ...
    for (i = 0; i < SC850SL_MODE_BUTT; i++) {
        if (sns_image_mode->fps <= g_sc850sl_mode_tbl[i].max_fps &&
            sns_image_mode->width <= g_sc850sl_mode_tbl[i].width &&
            sns_image_mode->height <= g_sc850sl_mode_tbl[i].height &&
            sns_state->wdr_mode == g_sc850sl_mode_tbl[i].wdr_mode) {
            image_mode = (sc850sl_res_mode)i;
            break;
        }
    }

    if (i >= SC850SL_MODE_BUTT) {
        sc850sl_err_mode_print(sns_image_mode, sns_state);
        return TD_FAILURE;
    }
...
static td_void sensor_global_init(ot_vi_pipe vi_pipe)
{
    ...
    sns_state->sync_init = TD_FALSE;
    sns_state->img_mode  = SC850SL_8M_30FPS_12BIT_LINEAR_MODE;
    sns_state->wdr_mode  = OT_WDR_MODE_NONE;
    sns_state->fl_std    = SC850SL_VMAX_8M_30FPS_12BIT_LINEAR;
    sns_state->fl[0]     = SC850SL_VMAX_8M_30FPS_12BIT_LINEAR;
    sns_state->fl[1]     = SC850SL_VMAX_8M_30FPS_12BIT_LINEAR;
...
static td_void cmos_set_pixel_detect(ot_vi_pipe vi_pipe, td_bool enable)
{
    ...
    if (sns_state->wdr_mode == OT_WDR_MODE_2To1_LINE) {
        ...
        if (sns_state->img_mode == SC850SL_8M_30FPS_12BIT_LINEAR_MODE) {
            full_lines_5fps = (SC850SL_VMAX_8M_30FPS_12BIT_LINEAR * 30) / 5; /* 30fps, 5fps */
        } else {
            return;
        }
    }
    ...
    if (enable) { /* setup for ISP pixel calibration mode */
        sc850sl_write_register(vi_pipe, SC850SL_AGAIN_L_ADDR, 0x40);
        sc850sl_write_register(vi_pipe, SC850SL_AGAIN_H_ADDR, 0x03);

        sc850sl_write_register(vi_pipe, SC850SL_DGAIN_H_ADDR, 0x00);
        sc850sl_write_register(vi_pipe, SC850SL_DGAIN_L_ADDR, 0x80);

        sc850sl_write_register(vi_pipe, SC850SL_VMAX_L_ADDR, low_8bits(full_lines_5fps));
        sc850sl_write_register(vi_pipe, SC850SL_VMAX_H_ADDR, high_8bits(full_lines_5fps));

        sc850sl_write_register(vi_pipe, SC850SL_EXPO_L_ADDR, lower_4bits(full_lines_5fps - 10));
        sc850sl_write_register(vi_pipe, SC850SL_EXPO_M_ADDR, higher_8bits(full_lines_5fps - 10));
        sc850sl_write_register(vi_pipe, SC850SL_EXPO_H_ADDR, higher_4bits(full_lines_5fps - 10));
    } else { /* setup for ISP 'normal mode' */
        sns_state->fl_std = (sns_state->fl_std > SC850SL_FULL_LINES_MAX) ? SC850SL_FULL_LINES_MAX : sns_state->fl_std;
        sc850sl_write_register(vi_pipe, SC850SL_VMAX_L_ADDR, low_8bits(sns_state->fl_std));
        sc850sl_write_register(vi_pipe, SC850SL_VMAX_H_ADDR, high_8bits(sns_state->fl_std));
        sns_state->sync_init = TD_FALSE;
    }
...
static td_s32 cmos_init_sensor_exp_function(ot_isp_sns_exp_func *sensor_exp_func)
{
    ...
    sensor_exp_func->pfn_cmos_sns_init         = sc850sl_init;
    sensor_exp_func->pfn_cmos_sns_exit         = sc850sl_exit;
...
static td_s32 sc850sl_set_bus_info(ot_vi_pipe vi_pipe, ot_isp_sns_commbus sns_bus_info)
{
    g_sc850sl_bus_info[vi_pipe].i2c_dev = sns_bus_info.i2c_dev;
...
static td_s32 sensor_ctx_init(ot_vi_pipe vi_pipe)
{
    ...
    sc850sl_sensor_get_ctx(vi_pipe, sns_state_ctx);
    ...
    sc850sl_sensor_set_ctx(vi_pipe, sns_state_ctx);
...
static td_void sensor_ctx_exit(ot_vi_pipe vi_pipe)
{
    ...
    sc850sl_sensor_reset_ctx(vi_pipe);
...
static td_s32 sensor_register_callback(ot_vi_pipe vi_pipe, ot_isp_3a_alg_lib *ae_lib, ot_isp_3a_alg_lib *awb_lib)
{
    ...
    sns_attr_info.sns_id = SC850SL_ID;
...
static td_s32 sensor_unregister_callback(ot_vi_pipe vi_pipe, ot_isp_3a_alg_lib *ae_lib, ot_isp_3a_alg_lib *awb_lib)
{
    ...
    ret = ot_mpi_isp_sensor_unreg_callback(vi_pipe, SC850SL_ID);
    if (ret != TD_SUCCESS) {
        isp_err_trace("sensor unregister callback function failed!\n");
        return ret;
    }

    ret = ot_mpi_ae_sensor_unreg_callback(vi_pipe, ae_lib, SC850SL_ID);
    if (ret != TD_SUCCESS) {
        isp_err_trace("sensor unregister callback function to ae lib failed!\n");
        return ret;
    }

    ret = ot_mpi_awb_sensor_unreg_callback(vi_pipe, awb_lib, SC850SL_ID);
    if (ret != TD_SUCCESS) {
...
ot_isp_sns_obj g_sns_sc850sl_obj = {
    .pfn_register_callback     = sensor_register_callback,
    .pfn_un_register_callback  = sensor_unregister_callback,
    .pfn_standby               = sc850sl_standby,
    .pfn_restart               = sc850sl_restart,
    .pfn_mirror_flip           = TD_NULL,
    .pfn_set_blc_clamp         = TD_NULL,
    .pfn_write_reg             = sc850sl_write_register,
    .pfn_read_reg              = sc850sl_read_register,
    .pfn_set_bus_info          = sc850sl_set_bus_info,
