...
const os04a10_video_mode_tbl g_os04a10_mode_tbl[OS04A10_MODE_BUTT] = {
    ...
    {OS04A10_VMAX_4M_30FPS_12BIT_2TO1_WDR,  OS04A10_FULL_LINES_MAX_2TO1_WDR, 30, 10.0,
     2688, 1520, 0, OT_WDR_MODE_2To1_LINE,  "OS04A10_4M_30FPS_12BIT_2TO1_VC_MODE"},
};
...
#define OS04A10_SHORT_EXPO_H_ADDR    0x3541
#define OS04A10_SHORT_EXPO_L_ADDR    0x3542
#define OS04A10_SHORT_AGAIN_H_ADDR   0x3548
#define OS04A10_SHORT_AGAIN_L_ADDR   0x3549
#define OS04A10_SHORT_DGAIN_H_ADDR   0x354a
#define OS04A10_SHORT_DGAIN_L_ADDR   0x354b
#define OS04A10_SHORT_DGAIN_LL_ADDR  0x354c
...
#define OS04A10_STEP_OFFSET          1520
#define OS04A10_MARGIN                40


#define os04a10_err_mode_print(sns_image_mode, sns_state) \
do { \
    ...
    const ot_isp_sns_state *sns_state)
{
    ...
    ae_sns_dft->max_again = 992; /* max 992:63488 */
    ae_sns_dft->min_again = 16;  /* min 16:1024 */
    ae_sns_dft->max_again_target = ae_sns_dft->max_again;
    ae_sns_dft->min_again_target = ae_sns_dft->min_again;
...
static td_void cmos_inttime_update_linear(ot_vi_pipe vi_pipe, td_u32 int_time)
{
    ...
    if (g_quick_start_en[vi_pipe] == TD_TRUE) {
        os04a10_write_register(vi_pipe, OS04A10_EXPO_L_ADDR, low_8bits(value));
        os04a10_write_register(vi_pipe, OS04A10_EXPO_H_ADDR, high_8bits(value));
    } else {
                                   ...
                                   td_u32 again_reg, td_u32 dgain_reg, td_u32 hcg_1_reg)
{
    ...
    if (sns_state->wdr_mode == OT_WDR_MODE_NONE) {
        if (g_quick_start_en[vi_pipe]) {
            os04a10_write_register(vi_pipe, OS04A10_AGAIN_L_ADDR, ((again_reg & 0xf) << 4)); /* shift 4 */
            os04a10_write_register(vi_pipe, OS04A10_AGAIN_H_ADDR, ((again_reg & 0xf0) >> 4)); /* shift 4 */
            os04a10_write_register(vi_pipe, OS04A10_DGAIN_L_ADDR, ((dgain_reg & 0x03) << 6)); /* shift 6 */
            os04a10_write_register(vi_pipe, OS04A10_DGAIN_M_ADDR, ((dgain_reg & 0x03FC) >> 2)); /* shift 2 */
            os04a10_write_register(vi_pipe, OS04A10_DGAIN_H_ADDR, ((dgain_reg & 0x3C00) >> 10)); /* shift 10 */
            os04a10_write_register(vi_pipe, OS04A10_GROUP_1_ADDR, 0x00);
            os04a10_write_register(vi_pipe, OS04A10_GROUP_2_ADDR, 0x01);
            os04a10_write_register(vi_pipe, OS04A10_HCG_1_ADDR, (hcg_1_reg & 0xff));
            os04a10_write_register(vi_pipe, OS04A10_GROUP_3_ADDR, 0x11);
            os04a10_write_register(vi_pipe, OS04A10_GROUP_4_ADDR, 0x05);
            os04a10_write_register(vi_pipe, OS04A10_GROUP_5_ADDR, 0x01);
            os04a10_write_register(vi_pipe, OS04A10_GROUP_6_ADDR, 0xa0);
        } else {
        ...
        if (first_gain[vi_pipe] == TD_TRUE) { /* long frame */
            sns_state->regs_info[0].i2c_data[WDR_AGAIN_L_IDX].data = ((again_reg & 0xf) << 4); /* shift 4 */
            sns_state->regs_info[0].i2c_data[WDR_AGAIN_H_IDX].data = ((again_reg & 0xf0) >> 4); /* shift 4 */

            sns_state->regs_info[0].i2c_data[WDR_DGAIN_L_IDX].data = ((dgain_reg & 0x03) << 6); /* shift 6 */
            sns_state->regs_info[0].i2c_data[WDR_DGAIN_M_IDX].data = ((dgain_reg & 0x03FC) >> 2); /* shift 2 */
            sns_state->regs_info[0].i2c_data[WDR_DGAIN_H_IDX].data = ((dgain_reg & 0x3C00) >> 10); /* shift 10 */

            first_gain[vi_pipe] = TD_FALSE;
        } else {                              /* short frame */
            sns_state->regs_info[0].i2c_data[WDR_SHORT_AGAIN_L_IDX].data = ((again_reg & 0xf) << 4); /* shift 4 */
            sns_state->regs_info[0].i2c_data[WDR_SHORT_AGAIN_H_IDX].data = ((again_reg & 0xf0) >> 4); /* shift 4 */

            sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_LL_IDX].data = ((dgain_reg & 0x03) << 6); /* shift 6 */
            sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_L_IDX].data = ((dgain_reg & 0x03FC) >> 2); /* shift 2 */
            sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_H_IDX].data = ((dgain_reg & 0x3C00) >> 10); /* shift 10 */

            first_gain[vi_pipe] = TD_TRUE;
        }
        sns_state->regs_info[0].i2c_data[WDR_HCG_1_IDX].data = (hcg_1_reg & 0xff);
    }
...
static td_void cmos_step_calculate(ot_vi_pipe vi_pipe, time_step *step)
{
    td_u32 i;
    ot_isp_sns_state *sns_state = TD_NULL;
    os04a10_sensor_get_ctx(vi_pipe, sns_state);
    ...
    for (i = 0; i < 2; ++i) { /* frame number is 2 */
        step->inc[i] = (sns_state->fl[1] - OS04A10_STEP_OFFSET) >> 1; /* shift 1 */
        step->dec[i] = (sns_state->fl[1] - OS04A10_STEP_OFFSET) >> 1; /* shift 1 */
    }
    return;
}
    ...
    ot_isp_ae_int_time_range *int_time, td_u32 *lf_max_int_time)
{
    ...
    cmos_step_calculate(vi_pipe, &step);
    if (g_fswdr_mode[vi_pipe] == OT_ISP_FSWDR_LONG_FRAME_MODE) {
        short_max0 = sns_state->fl[1] - OS04A10_MARGIN - step.inc[0] - sns_state->wdr_int_time[0];
        short_max = sns_state->fl[0] - OS04A10_MARGIN - step.inc[0];
        ...
        short_max0 = ((sns_state->fl[1] - OS04A10_MARGIN - step.inc[0] - sns_state->wdr_int_time[0]) * 0x40) /
            div_0_to_1(ratio[0]);
        short_max = ((sns_state->fl[0] - OS04A10_MARGIN - step.inc[0]) * 0x40) / (ratio[0] + 0x40);
        short_max = (short_max0 < short_max) ? short_max0 : short_max;
        short_max = (short_max == 0) ? 1 : short_max;
        *lf_max_int_time = sns_state->fl[0] - OS04A10_MARGIN - step.inc[0];
...
static td_void cmos_get_isp_dng_default(const ot_isp_sns_state *sns_state, ot_isp_cmos_default *isp_def)
{
    ...
    switch (sns_state->img_mode) {
        ...
        case OS04A10_4M_30FPS_12BIT_2TO1_VC_MODE:
            isp_def->sns_mode.dng_raw_format.bits_per_sample = 12; /* 12bit */
            isp_def->sns_mode.dng_raw_format.white_level = 4095; /* max 4095 */
            break;
...
static td_s32 cmos_get_isp_default(ot_vi_pipe vi_pipe, ot_isp_cmos_default *isp_def)
{
    ...
    isp_def->sns_mode.sns_id = OS04A10_ID;
...
static td_void cmos_sns_reg_info_hcg_init(ot_vi_pipe vi_pipe, ot_isp_sns_state *sns_state)
{
    ot_unused(vi_pipe);
...
static td_void cmos_comm_sns_reg_info_init(ot_vi_pipe vi_pipe, ot_isp_sns_state *sns_state)
{
    ...
    cmos_sns_reg_info_hcg_init(vi_pipe, sns_state);
    return;
}
...
static td_void cmos_2to1_line_wdr_sns_reg_info_init(ot_vi_pipe vi_pipe, ot_isp_sns_state *sns_state)
{
    ...
    sns_state->regs_info[0].i2c_data[WDR_EXPO_L_IDX].reg_addr = OS04A10_EXPO_L_ADDR;
    sns_state->regs_info[0].i2c_data[WDR_EXPO_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_EXPO_H_IDX].reg_addr = OS04A10_EXPO_H_ADDR;

    sns_state->regs_info[0].i2c_data[WDR_SHORT_EXPO_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_EXPO_L_IDX].reg_addr = OS04A10_SHORT_EXPO_L_ADDR;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_EXPO_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_EXPO_H_IDX].reg_addr = OS04A10_SHORT_EXPO_H_ADDR;

    sns_state->regs_info[0].i2c_data[WDR_SHORT_AGAIN_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_AGAIN_L_IDX].reg_addr = OS04A10_SHORT_AGAIN_L_ADDR;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_AGAIN_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_AGAIN_H_IDX].reg_addr = OS04A10_SHORT_AGAIN_H_ADDR;

    sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_LL_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_LL_IDX].reg_addr = OS04A10_SHORT_DGAIN_LL_ADDR;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_L_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_L_IDX].reg_addr = OS04A10_SHORT_DGAIN_L_ADDR;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_H_IDX].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_H_IDX].reg_addr = OS04A10_SHORT_DGAIN_H_ADDR;
    return;
}

static td_void cmos_2to1_line_wdr_config(ot_vi_pipe vi_pipe, ot_isp_sns_state *sns_state)
{
    ot_unused(vi_pipe);
    if (sns_state->wdr_mode == OT_WDR_MODE_2To1_LINE) {
        sns_state->regs_info[0].i2c_data[WDR_SHORT_EXPO_L_IDX].update = TD_TRUE;
        sns_state->regs_info[0].i2c_data[WDR_SHORT_EXPO_H_IDX].update = TD_TRUE;

        sns_state->regs_info[0].i2c_data[WDR_SHORT_AGAIN_L_IDX].update = TD_TRUE;
        sns_state->regs_info[0].i2c_data[WDR_SHORT_AGAIN_H_IDX].update = TD_TRUE;

        sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_LL_IDX].update = TD_TRUE;
        sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_L_IDX].update = TD_TRUE;
        sns_state->regs_info[0].i2c_data[WDR_SHORT_DGAIN_H_IDX].update = TD_TRUE;
    }
...
static td_void cmos_sns_reg_info_update(ot_vi_pipe vi_pipe, ot_isp_sns_state *sns_state)
{
    ...
    for (i = 0; i < sns_state->regs_info[0].reg_num; i++) {
        if (sns_state->regs_info[0].i2c_data[i].data ==
            ...
            if (i != HCG_1_IDX) {
                ...
                cmos_2to1_line_wdr_config(vi_pipe, sns_state);
                break;
            }
        }
    }
    ...
    if (sns_state->wdr_mode == OT_WDR_MODE_2To1_LINE) {
        if (sns_state->fl[0] > sns_state->fl[1]) {
            ...
            sns_state->regs_info[0].i2c_data[WDR_VMAX_L_IDX].delay_frame_num = 0; /* delay 0 */
            sns_state->regs_info[0].i2c_data[WDR_VMAX_H_IDX].delay_frame_num = 0; /* delay 0 */
        } else {
        }
    }
    return;
    ...
    ot_isp_sns_state *sns_state)
{
    ...
    switch (sns_image_mode) {
        case OS04A10_4M_30FPS_12BIT_LINEAR_MODE:
            ...
            break;
        case OS04A10_4M_30FPS_12BIT_2TO1_VC_MODE:
            sns_state->fl_std = OS04A10_VMAX_4M_30FPS_12BIT_2TO1_WDR;
            break;
        default:
            sns_state->fl_std = OS04A10_VMAX_4M_30FPS_12BIT_LINEAR;
            break;
...
static td_s32 cmos_set_image_mode(ot_vi_pipe vi_pipe, const ot_isp_cmos_sns_image_mode *sns_image_mode)
{
    ...
    for (i = 0; i < OS04A10_MODE_BUTT; i++) {
        if (sns_image_mode->fps <= g_os04a10_mode_tbl[i].max_fps &&
            sns_image_mode->width <= g_os04a10_mode_tbl[i].width &&
            sns_image_mode->height <= g_os04a10_mode_tbl[i].height &&
    ...
    if (i >= OS04A10_MODE_BUTT) {
        os04a10_err_mode_print(sns_image_mode, sns_state);
        return TD_FAILURE;
    }
...
static td_s32 cmos_init_sensor_exp_function(ot_isp_sns_exp_func *sensor_exp_func)
{
    ...
    sensor_exp_func->pfn_cmos_sns_init         = os04a10_init;
    sensor_exp_func->pfn_cmos_sns_exit         = os04a10_exit;
...
static td_s32 sensor_register_callback(ot_vi_pipe vi_pipe, ot_isp_3a_alg_lib *ae_lib, ot_isp_3a_alg_lib *awb_lib)
{
    ...
    sns_attr_info.sns_id = OS04A10_ID;
