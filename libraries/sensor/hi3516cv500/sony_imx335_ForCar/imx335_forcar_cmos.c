 ...
 * Description: Function of imx335_forcar_cmos.c
 * Author: ISP SW
 * Create: 2012/06/28
 */
...
{
    return &g_aunImx335_forcar_BusInfo[vi_pipe];
}

ISP_SNS_STATE_S *imx335_forcar_get_ctx(VI_PIPE vi_pipe)
{
    return g_pastImx335_forcar[vi_pipe];
}
...
IMX335_FORCAR_STATE_S g_astimx335_forcar[ISP_MAX_PIPE_NUM] = {{0}};
...
const IMX335_FORCAR_VIDEO_MODE_TBL_S g_astImx335_forcar_ModeTbl[IMX335_MODE_BUTT] = {
    ...
    ISP_SNS_STATE_S *pstSnsState)
{
    ...
    IMX335_FORCAR_SENSOR_GET_CTX(vi_pipe, pstSnsState);
    CMOS_CHECK_POINTER_VOID(pstSnsState);

    u32Lines = g_astImx335_forcar_ModeTbl[pstSnsState->u8ImgMode].u32VertiLines;
    f32MaxFps = g_astImx335_forcar_ModeTbl[pstSnsState->u8ImgMode].f32MaxFps;
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ...
        g_astimx335_forcar[vi_pipe].u32RHS1_MAX = g_astimx335_forcar[vi_pipe].u32BRL * 2; /* x2 */
        g_astimx335_forcar[vi_pipe].u32deltaRHS1 =
            (u32Lines * 2) - (g_astimx335_forcar[vi_pipe].u32BRL * 2) - 2; /* Lines * 2 - BRL * 2 - 2 */
    } else {
...
static HI_VOID cmos_config_vmax(ISP_SNS_STATE_S *pstSnsState, HI_U32 u32VMAX)
{
    if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
        pstSnsState->astRegsInfo[0].astI2cData[5].u32Data = IMX335_FORCAR_LOW_8BITS(u32VMAX); /* index 5 */
        pstSnsState->astRegsInfo[0].astI2cData[6].u32Data = IMX335_FORCAR_MID_8BITS(u32VMAX); /* index 6 */
        pstSnsState->astRegsInfo[0].astI2cData[7].u32Data = IMX335_FORCAR_HIG_4BITS(u32VMAX); /* index 7 */
    } else {
        pstSnsState->astRegsInfo[0].astI2cData[8].u32Data = IMX335_FORCAR_LOW_8BITS(u32VMAX); /* index 8 */
        pstSnsState->astRegsInfo[0].astI2cData[9].u32Data = IMX335_FORCAR_MID_8BITS(u32VMAX); /* index 9 */
        pstSnsState->astRegsInfo[0].astI2cData[10].u32Data = IMX335_FORCAR_HIG_4BITS(u32VMAX); /* index 10 */
    }
...
static HI_VOID cmos_fps_set(VI_PIPE vi_pipe, HI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    u32Lines = g_astImx335_forcar_ModeTbl[pstSnsState->u8ImgMode].u32VertiLines;
    u32LinesMax = g_astImx335_forcar_ModeTbl[pstSnsState->u8ImgMode].u32MaxVertiLines;
    f32MaxFps = g_astImx335_forcar_ModeTbl[pstSnsState->u8ImgMode].f32MaxFps;
    f32MinFps = g_astImx335_forcar_ModeTbl[pstSnsState->u8ImgMode].f32MinFps;
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ...
        g_astimx335_forcar[vi_pipe].u32RHS1_MAX = g_astimx335_forcar[vi_pipe].u32BRL * 2; /* x2 */
        g_astimx335_forcar[vi_pipe].u32deltaRHS1 =
            (u32Lines * 2) - (g_astimx335_forcar[vi_pipe].u32BRL * 2) - 2; /* VMAX * 2 - BRL * 2 - 2 */
    } else {
...
static HI_VOID cmos_slow_framerate_set(VI_PIPE vi_pipe, HI_U32 u32FullLines, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ...
        g_astimx335_forcar[vi_pipe].u32RHS1_MAX = g_astimx335_forcar[vi_pipe].u32BRL * 2; /* x2 */
        g_astimx335_forcar[vi_pipe].u32deltaRHS1 = u32FullLines -
            (g_astimx335_forcar[vi_pipe].u32BRL * 2) - 2; /* FL - BRL * 2 - 2 */
    } else {
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
        pstSnsState->astRegsInfo[0].astI2cData[5].u32Data = /* index 5 */
            IMX335_FORCAR_LOW_8BITS(pstSnsState->au32FL[0]);   /* VMAX */
        pstSnsState->astRegsInfo[0].astI2cData[6].u32Data = /* index 6 */
            IMX335_FORCAR_MID_8BITS(pstSnsState->au32FL[0]);
        pstSnsState->astRegsInfo[0].astI2cData[7].u32Data = /* index 7 */
            IMX335_FORCAR_HIG_4BITS(pstSnsState->au32FL[0]);
    } else {
        pstSnsState->astRegsInfo[0].astI2cData[8].u32Data = /* index 8 */
            IMX335_FORCAR_LOW_8BITS(pstSnsState->au32FL[0] >> 1);  /* VMAX */
        pstSnsState->astRegsInfo[0].astI2cData[9].u32Data = /* index 9 */
            IMX335_FORCAR_MID_8BITS(pstSnsState->au32FL[0] >> 1);
        pstSnsState->astRegsInfo[0].astI2cData[10].u32Data = /* index 10 */
            IMX335_FORCAR_HIG_4BITS(pstSnsState->au32FL[0] >> 1);
    }
...
static HI_VOID cmos_inttime_update_linear(VI_PIPE vi_pipe, HI_U32 u32IntTime)
{
    ...
    pstSnsState->astRegsInfo[0].astI2cData[2].u32Data = IMX335_FORCAR_HIG_4BITS(u32Value); /* index 2 */
...
static HI_VOID cmos_inttime_update_2to1_line_long(VI_PIPE vi_pipe, HI_U32 u32IntTime)
{
    ...
    HI_U32 u32RHS1, u32SHR0, u32SHR1;
    HI_U32 u32delta = 0;
    HI_U16 u16ShortExpMod4, u16ShortExpMod8;
    ...
    if (au32LastShortIntTime[vi_pipe] > pstSnsState->au32WDRIntTime[0]) {
        u32delta = au32LastShortIntTime[vi_pipe] - pstSnsState->au32WDRIntTime[0];
        u32RHS1Limit = (pstSnsState->au32FL[1] - (g_astimx335_forcar[vi_pipe].u32BRL * 2) - 2); /* FL - BRL * 2 -2 */
        if (u32delta > u32RHS1Limit) {
            pstSnsState->au32WDRIntTime[0] =
                au32LastShortIntTime[vi_pipe] - (u32RHS1Limit - (u32RHS1Limit % 8) + 8); /* RHS % 8 + 8 */
        }
    }

    u16ShortExpMod4 = pstSnsState->au32WDRIntTime[0] % 4; /* au32WDRIntTime % 4 */
    u16ShortExpMod8 = pstSnsState->au32WDRIntTime[0] % 8; /* au32WDRIntTime % 8 */
    ...
    if (u16ShortExpMod8 == 0) {
        ...
        if (u16ShortExpMod4 == u16ShortExpMod8) {
            u32SHR1 = 18; /* min 18 */
            u32RHS1 = 18 + pstSnsState->au32WDRIntTime[0] - u16ShortExpMod8; /* min 18 */
        } else {
            u32SHR1 = 22; /* min 22 */
            u32RHS1 = 22 + pstSnsState->au32WDRIntTime[0] - u16ShortExpMod4; /* min 22 */
        }
    }
    ...
    pstSnsState->astRegsInfo[0].astI2cData[2].u32Data = IMX335_FORCAR_HIG_4BITS(u32SHR0); /* index 2 */

    pstSnsState->astRegsInfo[0].astI2cData[5].u32Data =  IMX335_FORCAR_LOW_8BITS(u32SHR1); /* index 5 */
    pstSnsState->astRegsInfo[0].astI2cData[6].u32Data = IMX335_FORCAR_MID_8BITS(u32SHR1); /* index 6 */
    pstSnsState->astRegsInfo[0].astI2cData[7].u32Data = IMX335_FORCAR_HIG_4BITS(u32SHR1); /* index 7 */

    pstSnsState->astRegsInfo[0].astI2cData[11].u32Data =  IMX335_FORCAR_LOW_8BITS(u32RHS1); /* index 11 */
    pstSnsState->astRegsInfo[0].astI2cData[12].u32Data = IMX335_FORCAR_MID_8BITS(u32RHS1); /* index 12 */
    pstSnsState->astRegsInfo[0].astI2cData[13].u32Data = IMX335_FORCAR_HIG_4BITS(u32RHS1); /* index 13 */

    au32LastShortIntTime[vi_pipe] = pstSnsState->au32WDRIntTime[0];
...
{
    static HI_U8 au8ExpFrameType[ISP_MAX_PIPE_NUM] = { [0 ...(ISP_MAX_PIPE_NUM - 1)] = IMX335_FRAME_TYPE_SHORT };

    ISP_SNS_STATE_S *pstSnsState = HI_NULL;
    IMX335_FORCAR_SENSOR_GET_CTX(vi_pipe, pstSnsState);
    CMOS_CHECK_POINTER_VOID(pstSnsState);

    if (au8ExpFrameType[vi_pipe] == IMX335_FRAME_TYPE_SHORT) { /* short exposure */
        pstSnsState->au32WDRIntTime[0] = u32IntTime;

        au8ExpFrameType[vi_pipe] = IMX335_FRAME_TYPE_LONG;
    } else { /* long exposure */
        cmos_inttime_update_2to1_line_long(vi_pipe, u32IntTime);

        au8ExpFrameType[vi_pipe] = IMX335_FRAME_TYPE_SHORT;
    }
...
static HI_VOID cmos_gains_update(VI_PIPE vi_pipe, HI_U32 u32Again, HI_U32 u32Dgain)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        if (u8ExpFrame == IMX335_FRAME_TYPE_LONG) {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[3].u32Data = IMX335_FORCAR_LOW_8BITS(u32Tmp); /* index 3 */
            pstSnsState->astRegsInfo[0].astI2cData[4].u32Data = IMX335_FORCAR_MID_8BITS(u32Tmp); /* index 4 */
            u8ExpFrame = IMX335_FRAME_TYPE_SHORT;
        } else {
            u32Tmp = u32Again + u32Dgain;

            pstSnsState->astRegsInfo[0].astI2cData[14].u32Data = IMX335_FORCAR_LOW_8BITS(u32Tmp); /* index 14 */
            pstSnsState->astRegsInfo[0].astI2cData[15].u32Data = IMX335_FORCAR_MID_8BITS(u32Tmp); /* index 15 */
...
static HI_U32 g_au32LastIntTimeMaxTmp[ISP_MAX_PIPE_NUM] = { [0 ...(ISP_MAX_PIPE_NUM - 1)] = 25 }; /* in timemax 25 */
    ...
    HI_U32 *au32IntTimeMin, HI_U32 *pu32LFMaxIntTime)
{
    ...
    u32ShortTimeMinLimit = 25; /* u32ShortTimeMinLimit 25 */
    ...
    HI_U32 *au32IntTimeMax, HI_U32 *au32IntTimeMin, HI_U32 *pu32LFMaxIntTime)
{
    ...
    u32ShortTimeMinLimit = 25; /* u32ShortTimeMinLimit 25 */
    u32deltaRHS1 =
        g_astimx335_forcar[vi_pipe].u32deltaRHS1 - (g_astimx335_forcar[vi_pipe].u32deltaRHS1 % 8); /* RHS%8 */
    ...
    u32ShortMax = (u32ShortMax0 < u32ShortMax) ? u32ShortMax0 : u32ShortMax;
    u32ShortMax = (u32ShortMax > (g_astimx335_forcar[vi_pipe].u32RHS1_MAX - 18)) ? /* sub 18 */
        (g_astimx335_forcar[vi_pipe].u32RHS1_MAX - 18) : u32ShortMax; /* sub 18 */
    if (u32ShortMax > g_au32LastIntTimeMaxTmp[vi_pipe]) {
        u32delta = u32ShortMax - g_au32LastIntTimeMaxTmp[vi_pipe];
        if (u32delta > g_astimx335_forcar[vi_pipe].u32deltaRHS1) {
    ...
    HI_U32 *au32IntTimeMin, HI_U32 *pu32LFMaxIntTime)
{
    ...
    *pu32LFMaxIntTime = g_astimx335_forcar[vi_pipe].u32RHS1_MAX - 18; /* sub 18 */
...
#define CALIBRATE_STATIC_WB_B_GAIN        0x1ED

/* Calibration results for Auto WB Planck */
#define CALIBRATE_AWB_P1                  8
#define CALIBRATE_AWB_P2                  244
#define CALIBRATE_AWB_Q1                  (-4)
#define CALIBRATE_AWB_A1                  143002
#define CALIBRATE_AWB_B1                  128
#define CALIBRATE_AWB_C1                  (-91850)
...
static HI_S32 cmos_get_isp_black_level(VI_PIPE vi_pipe, ISP_CMOS_BLACK_LEVEL_S *pstBlackLevel)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
        for (i = 0; i < ISP_BAYER_CHN_NUM; i++) {
            pstBlackLevel->au16BlackLevel[i] = 200; /* 12bit, 200 */
        }
...
static HI_VOID cmos_set_pixel_detect(VI_PIPE vi_pipe, HI_BOOL bEnable)
{
    ...
    if (bEnable) { /* setup for ISP pixel calibration mode */
        IMX335_forcar_write_register(vi_pipe, IMX335_GAIN_LONG_LOW, 0x00); /* gain */
        IMX335_forcar_write_register(vi_pipe, IMX335_GAIN_LONG_HIGH, 0x00);
        /* VMAX */
        IMX335_forcar_write_register(vi_pipe, IMX335_VMAX_ADDR_L, IMX335_FORCAR_LOW_8BITS(u32FullLines_5Fps));
        IMX335_forcar_write_register(vi_pipe, IMX335_VMAX_ADDR_M, IMX335_FORCAR_MID_8BITS(u32FullLines_5Fps));
        IMX335_forcar_write_register(vi_pipe, IMX335_VMAX_ADDR_H, IMX335_FORCAR_HIG_4BITS(u32FullLines_5Fps));

        IMX335_forcar_write_register(vi_pipe, IMX335_SHR0_LOW, IMX335_FORCAR_LOW_8BITS(u32MaxIntTime_5Fps));
        IMX335_forcar_write_register(vi_pipe, IMX335_SHR0_MIDDLE, IMX335_FORCAR_MID_8BITS(u32MaxIntTime_5Fps));
        IMX335_forcar_write_register(vi_pipe, IMX335_SHR0_MIDDLE, IMX335_FORCAR_HIG_4BITS(u32MaxIntTime_5Fps));
    } else { /* setup for ISP 'normal mode' */
        pstSnsState->u32FLStd = (pstSnsState->u32FLStd > IMX335_FULL_LINES_MAX) ? IMX335_FULL_LINES_MAX
                                : pstSnsState->u32FLStd;
        IMX335_forcar_write_register(vi_pipe, IMX335_VMAX_ADDR_L, IMX335_FORCAR_LOW_8BITS(pstSnsState->u32FLStd));
        IMX335_forcar_write_register(vi_pipe, IMX335_VMAX_ADDR_M, IMX335_FORCAR_MID_8BITS(pstSnsState->u32FLStd));
        IMX335_forcar_write_register(vi_pipe, IMX335_VMAX_ADDR_H, IMX335_FORCAR_HIG_4BITS(pstSnsState->u32FLStd));
...
static HI_VOID cmos_comm_sns_reg_info_init(VI_PIPE vi_pipe, ISP_SNS_STATE_S *pstSnsState)
{
    ...
    pstSnsState->astRegsInfo[0].unComBus.s8I2cDev = g_aunImx335_forcar_BusInfo[vi_pipe].s8I2cDev;
    ...
    switch (pstSnsState->enWDRMode) {
        case WDR_MODE_2To1_LINE:
            if (IMX335_RES_IS_5M_10BIT_WDR(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height) ||
                ...
                g_astimx335_forcar[vi_pipe].u32BRL = 1984 * 2; /* BRL 1984 * 2 */
 ...
 * callback structure                                                       *
 ****************************************************************************/
static HI_S32 IMX335_forcar_set_bus_info(VI_PIPE vi_pipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
    g_aunImx335_forcar_BusInfo[vi_pipe].s8I2cDev = unSNSBusInfo.s8I2cDev;
...
static HI_S32 sensor_ctx_init(VI_PIPE vi_pipe)
{
    ...
    IMX335_FORCAR_SENSOR_GET_CTX(vi_pipe, pastSnsStateCtx);
    ...
    IMX335_FORCAR_SENSOR_SET_CTX(vi_pipe, pastSnsStateCtx);
...
static HI_VOID sensor_ctx_exit(VI_PIPE vi_pipe)
{
    ...
    IMX335_FORCAR_SENSOR_RESET_CTX(vi_pipe);
    return;
}
