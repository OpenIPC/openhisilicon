 ...
 * Description: Function of imx307_2l_cmos.c
 * Author: ISP SW
 * Create: 2012/06/28
 */
...
{
    return &g_aunImx307_2l_BusInfo[Vi_Pipe];
}

ISP_SNS_STATE_S *imx307_2l_get_ctx(VI_PIPE Vi_Pipe)
{
    return g_pastImx307_2l[Vi_Pipe];
}
...
#define IMX307_VMAX_1080P60_LINEAR             (1125 + IMX307_INCREASE_LINES)
...
const imx307_2l_video_mode_tbl g_imx307_2l_mode_tbl[3] = { /* max mode num 3 */
    {IMX307_VMAX_1080P30_LINEAR,        IMX307_FULL_LINES_MAX,          30, 0.119,
     1920, 1080, 0, WDR_MODE_NONE,       "IMX307_SENSOR_1080P_30FPS_LINEAR_MODE"},
    {IMX307_VMAX_1080P60TO30_WDR,       IMX307_FULL_LINES_MAX_2TO1_WDR, 30, 16.50,
     1920, 1080, 0, WDR_MODE_2To1_LINE,  "IMX307_SENSOR_1080P_30FPS_2t1_WDR_MODE"},
    {IMX307_VMAX_1080P60_LINEAR,        IMX307_FULL_LINES_MAX,          60, 0.259,
     1920, 1080, 1, WDR_MODE_NONE,       "IMX307_SENSOR_1080P_60FPS_LINEAR_MODE"},
};
    ...
    ISP_SNS_STATE_S *pstSnsState)
{
    ...
    memcpy_s(&pstAeSnsDft->stAERouteAttrEx, sizeof(ISP_AE_ROUTE_EX_S), &g_astInitAERouteEx[vi_pipe],
             sizeof(ISP_AE_ROUTE_EX_S));
    memcpy_s(&pstAeSnsDft->stAERouteSFAttr, sizeof(ISP_AE_ROUTE_S), &g_astInitAERouteSF[vi_pipe],
             sizeof(ISP_AE_ROUTE_S));
...
static HI_S32 cmos_get_ae_default(VI_PIPE vi_pipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    IMX307_2L_SENSOR_GET_CTX(vi_pipe, pstSnsState);
...
static HI_VOID cmos_fps_set(VI_PIPE vi_pipe, HI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ISP_SNS_STATE_S *pstSnsState = HI_NULL;
    HI_U8 image_mode;
    HI_U32 u32VMAX, std_vmax, vmax_limit;
    HI_FLOAT fps_max, fps_min;
    CMOS_CHECK_POINTER_VOID(pstAeSnsDft);
    IMX307_2L_SENSOR_GET_CTX(vi_pipe, pstSnsState);
    CMOS_CHECK_POINTER_VOID(pstSnsState);

    image_mode = pstSnsState->u8ImgMode;
    std_vmax = g_imx307_2l_mode_tbl[image_mode].verti_lines;
    fps_max = g_imx307_2l_mode_tbl[image_mode].max_fps;
    fps_min = g_imx307_2l_mode_tbl[image_mode].min_fps;
    vmax_limit = g_imx307_2l_mode_tbl[image_mode].max_verti_lines;

    if ((f32Fps > fps_max) || (f32Fps < fps_min)) {
        SNS_ERR_TRACE("Not support Fps: %f\n", f32Fps);
        return;
    }

    u32VMAX = std_vmax * fps_max / SNS_DIV_0_TO_1_FLOAT(f32Fps);
    u32VMAX = (u32VMAX > vmax_limit) ? vmax_limit : u32VMAX;
    pstAeSnsDft->u32LinesPer500ms = std_vmax * fps_max / 2; /* 1s div 2 is 500ms */
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ...
        g_astimx307_2l_State[vi_pipe].u32RHS1_MAX =
            (u32VMAX - g_astimx307_2l_State[vi_pipe].u32BRL) * 2 - 21; /* (VMAX - BRL) * 2 - 21 */
    } else {
...
static HI_VOID cmos_slow_framerate_set(VI_PIPE vi_pipe, HI_U32 u32FullLines, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ...
        g_astimx307_2l_State[vi_pipe].u32RHS1_MAX = pstSnsState->au32FL[0] -
            g_astimx307_2l_State[vi_pipe].u32BRL * 2 - 21; /* FL - BRL * 2 - 21 */
    } else {
...
{
    ISP_SNS_STATE_S *pstSnsState = HI_NULL;
    HI_U32 u32HCG = g_astimx307_2l_State[vi_pipe].u8Hcg;
    HI_U32 u32Tmp;
    ...
    HI_U32 *au32IntTimeMin, HI_U32 *pu32LFMaxIntTime)
{
    ...
    if (g_genFSWDRMode[vi_pipe] == ISP_FSWDR_LONG_FRAME_MODE) {
        ...
        u32ShortMax = (u32ShortMax0 < u32ShortMax) ? u32ShortMax0 : u32ShortMax;
        u32ShortMax = (u32ShortMax > (g_astimx307_2l_State[vi_pipe].u32RHS1_MAX - 3)) ? /* sub 3 */
            (g_astimx307_2l_State[vi_pipe].u32RHS1_MAX - 3) : u32ShortMax; /* sub 3 */
...
static HI_VOID cmos_set_pixel_detect(VI_PIPE vi_pipe, HI_BOOL bEnable)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ...
        } else if (pstSnsState->u8ImgMode == IMX307_SENSOR_1080P_60FPS_LINEAR_MODE) {
            u32FullLines_5Fps = (IMX307_VMAX_1080P60_LINEAR * 60) / 5; /* 60fps, 5fps */
        } else {
            return;
        }
    }
    ...
    if (bEnable) { /* setup for ISP pixel calibration mode */
        imx307_2l_write_register(vi_pipe, IMX307_GAIN_ADDR, 0x00);

        imx307_2l_write_register(vi_pipe, IMX307_VMAX_ADDR, LOW_8BITS(u32FullLines_5Fps));
        imx307_2l_write_register(vi_pipe, IMX307_VMAX_ADDR + 1, HIGH_8BITS(u32FullLines_5Fps));
        imx307_2l_write_register(vi_pipe, IMX307_VMAX_ADDR + 2, HIGHER_4BITS(u32FullLines_5Fps)); /* index 2 */

        imx307_2l_write_register(vi_pipe, IMX307_SHS1_ADDR, LOW_8BITS(u32MaxIntTime_5Fps));
        imx307_2l_write_register(vi_pipe, IMX307_SHS1_ADDR + 1, HIGH_8BITS(u32MaxIntTime_5Fps));
        imx307_2l_write_register(vi_pipe, IMX307_SHS1_ADDR + 2, HIGHER_4BITS(u32MaxIntTime_5Fps)); /* index 2 */
    } else { /* setup for ISP 'normal mode' */
        pstSnsState->u32FLStd = (pstSnsState->u32FLStd > 0x1FFFF) ? 0x1FFFF : pstSnsState->u32FLStd;
        imx307_2l_write_register(vi_pipe, IMX307_VMAX_ADDR, LOW_8BITS(pstSnsState->u32FLStd));
        imx307_2l_write_register(vi_pipe, IMX307_VMAX_ADDR + 1, HIGH_8BITS(pstSnsState->u32FLStd));
        imx307_2l_write_register(vi_pipe, IMX307_VMAX_ADDR + 2, HIGHER_4BITS(pstSnsState->u32FLStd)); /* index 2 */
...
static HI_VOID cmos_comm_sns_reg_info_init(VI_PIPE vi_pipe, ISP_SNS_STATE_S *pstSnsState)
{
    ...
    pstSnsState->astRegsInfo[0].unComBus.s8I2cDev = g_aunImx307_2l_BusInfo[vi_pipe].s8I2cDev;
    ...
    for (i = 0; i < pstSnsState->astRegsInfo[0].u32RegNum; i++) {
        ...
        pstSnsState->astRegsInfo[0].astI2cData[i].u8DevAddr = imx307_2l_get_serdes_i2c_addr(vi_pipe);
...
static HI_S32 cmos_set_image_mode(VI_PIPE vi_pipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
    ...
    if (pstSensorImageMode->f32Fps <= 60) { /* 60fps */
        switch (pstSnsState->enWDRMode) {
            case WDR_MODE_NONE:
                if (IMX307_RES_IS_1080P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)
                    && (pstSensorImageMode->u8SnsMode == 0)) {
                    u8SensorImageMode                  = IMX307_SENSOR_1080P_30FPS_LINEAR_MODE;
                    pstSnsState->u32FLStd              = IMX307_VMAX_1080P30_LINEAR;
                    g_astimx307_2l_State[vi_pipe].u8Hcg = 0x2;
                    break;
                } else if (IMX307_RES_IS_1080P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)
                    && (pstSensorImageMode->u8SnsMode == 1)) {
                    u8SensorImageMode                  = IMX307_SENSOR_1080P_60FPS_LINEAR_MODE;
                    pstSnsState->u32FLStd              = IMX307_VMAX_1080P60_LINEAR;
                    g_astimx307_2l_State[vi_pipe].u8Hcg = 0x1;
                    break;
                }
            ...
            case WDR_MODE_2To1_LINE:
                if (IMX307_RES_IS_1080P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
                    ...
                    g_astimx307_2l_State[vi_pipe].u32BRL = 1109; /* u32BRL 1109 */
                    g_astimx307_2l_State[vi_pipe].u8Hcg  = 0x1;
 ...
 * callback structure                                                       *
 ****************************************************************************/
static HI_S32 imx307_2l_set_bus_info(VI_PIPE vi_pipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
    g_aunImx307_2l_BusInfo[vi_pipe].s8I2cDev = unSNSBusInfo.s8I2cDev;
...
static HI_S32 sensor_ctx_init(VI_PIPE vi_pipe)
{
    ...
    IMX307_2L_SENSOR_GET_CTX(vi_pipe, pastSnsStateCtx);
    ...
    IMX307_2L_SENSOR_SET_CTX(vi_pipe, pastSnsStateCtx);
...
static HI_VOID sensor_ctx_exit(VI_PIPE vi_pipe)
{
    ...
    IMX307_2L_SENSOR_RESET_CTX(vi_pipe);
    return;
}
...
ISP_SNS_OBJ_S stSnsImx307_2l_Obj = {
    ...
    .pfnSetBusExInfo        = imx307_2l_set_bus_ex_info,
