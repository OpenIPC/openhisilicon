...
#define IMX385_FULL_LINES_MAX (0x3FFFF)
...
static HI_VOID cmos_fps_set(ISP_DEV IspDev, HI_FLOAT f32Fps,
  ...
  case IMX385_SENSOR_1080P_30FPS_3t1_WDR_MODE:
    if ((f32Fps <= 30) && (f32Fps >= 16.5)) {
      u32VMAX = IMX385_VMAX_1080P120TO30_WDR * 30 / f32Fps;
    } else {
      printf("Not support Fps: %f\n", f32Fps);
      return;
    }
    u32VMAX = (u32VMAX > IMX385_FULL_LINES_MAX_3TO1_WDR)
                  ? IMX385_FULL_LINES_MAX_3TO1_WDR
                  : u32VMAX;
    break;
  ...
  if (WDR_MODE_NONE == g_apstSnsState[IspDev]->enWDRMode) {
    ...
    g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[8].u32Data =
        (u32VMAX & 0xFF);
    g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[9].u32Data =
        ((u32VMAX & 0xFF00) >> 8);
    g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[10].u32Data =
        ((u32VMAX & 0xF0000) >> 16);
  }
...
static HI_VOID cmos_slow_framerate_set(ISP_DEV IspDev, HI_U32 u32FullLines,
  ...
  if (WDR_MODE_NONE == g_apstSnsState[IspDev]->enWDRMode) {
    ...
    g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[7].u32Data =
        ((g_apstSnsState[IspDev]->au32FL[0] & 0xF0000) >> 16);
  } else if (WDR_MODE_2To1_LINE == g_apstSnsState[IspDev]->enWDRMode) {
    g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[8].u32Data =
        ((g_apstSnsState[IspDev]->au32FL[0] >> 1) & 0xFF);
    g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[9].u32Data =
        (((g_apstSnsState[IspDev]->au32FL[0] >> 1) & 0xFF00) >> 8);
    g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[10].u32Data =
        (((g_apstSnsState[IspDev]->au32FL[0] >> 1) & 0xF0000) >> 16);
  } else if (WDR_MODE_3To1_LINE == g_apstSnsState[IspDev]->enWDRMode) {
    g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[8].u32Data =
        ((g_apstSnsState[IspDev]->au32FL[0] >> 2) & 0xFF);
    g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[9].u32Data =
        (((g_apstSnsState[IspDev]->au32FL[0] >> 2) & 0xFF00) >> 8);
    g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[10].u32Data =
        (((g_apstSnsState[IspDev]->au32FL[0] >> 2) & 0xF0000) >> 16);
  } else {
  }
...
static AWB_CCM_S g_stAwbCcm = {
    ...
    2640, {0x1B4, 0x80AA, 0x800A, 0x8091, 0x1A1, 0x8010, 0x0, 0x80DB, 0x1DB}};
...
static ISP_CMOS_GAMMA_S g_stIspGamma = {
    ...
    {0,    120,  220,  310,  390,  470,  540,  610,  670,  730,  786,  842,
     ...
     2791, 2802, 2814, 2825, 2837, 2848, 2859, 2871, 2882, 2893, 2904, 2915,
     ...
     3290, 3300, 3309, 3318, 3327, 3337, 3346, 3355, 3364, 3373, 3382, 3391,
...
static HI_VOID cmos_set_wdr_mode(ISP_DEV IspDev, HI_U8 u8Mode) {
  ...
  case WDR_MODE_2To1_LINE:
    ...
    g_apstSnsState[IspDev]->u32FLStd = (IMX385_VMAX_1080P60TO30_WDR * 60) / 25;
...
static HI_U32 cmos_get_sns_regs_info(ISP_DEV IspDev,
  ...
  if ((HI_FALSE == g_apstSnsState[IspDev]->bSyncInit) ||
    ...
    g_apstSnsState[IspDev]->astRegsInfo[0].u32RegNum = 0; // 8;
    ...
    else if ((WDR_MODE_2To1_FRAME_FULL_RATE ==
              g_apstSnsState[IspDev]->enWDRMode) ||
             (WDR_MODE_2To1_FRAME == g_apstSnsState[IspDev]->enWDRMode)) {
    ...
    g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[6].u32RegAddr =
        IMX385_VMAX_ADDR + 1;
    g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
    g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[7].u32RegAddr =
        IMX385_VMAX_ADDR + 2;
    ...
    if (WDR_MODE_2To1_LINE == g_apstSnsState[IspDev]->enWDRMode) {
      ...
      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[8].u32RegAddr =
          IMX385_VMAX_ADDR;
      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[9].u8DelayFrmNum = 1;
      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[9].u32RegAddr =
          IMX385_VMAX_ADDR + 1;
      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[10].u8DelayFrmNum = 1;
      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[10].u32RegAddr =
          IMX385_VMAX_ADDR + 2;

      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[11].u8DelayFrmNum = 1;
      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[11].u32RegAddr =
          IMX385_RHS1_ADDR;
      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[12].u8DelayFrmNum = 1;
      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[12].u32RegAddr =
          IMX385_RHS1_ADDR + 1;
      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[13].u8DelayFrmNum = 1;
      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[13].u32RegAddr =
          IMX385_RHS1_ADDR + 2;

      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[14].u8DelayFrmNum = 1;
      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[14].u32RegAddr =
          IMX385_Y_OUT_SIZE_ADDR;
      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[15].u8DelayFrmNum = 1;
      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[15].u32RegAddr =
          IMX385_Y_OUT_SIZE_ADDR + 1;
    ...
    // DOL 3t1 Mode Regs
      ...
      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[16].u32RegAddr =
          IMX385_RHS2_ADDR + 2;

      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[17].u8DelayFrmNum = 0;
      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[17].u32RegAddr =
          IMX385_SHS3_ADDR;
      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[18].u8DelayFrmNum = 0;
      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[18].u32RegAddr =
          IMX385_SHS3_ADDR + 1;
      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[19].u8DelayFrmNum = 0;
      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[19].u32RegAddr =
          IMX385_SHS3_ADDR + 2;

      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[20].u8DelayFrmNum = 1;
      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[20].u32RegAddr =
          IMX385_Y_OUT_SIZE_ADDR;
      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[21].u8DelayFrmNum = 1;
      g_apstSnsState[IspDev]->astRegsInfo[0].astI2cData[21].u32RegAddr =
          IMX385_Y_OUT_SIZE_ADDR + 1;
    }
...
static int sensor_register_callback(ISP_DEV IspDev, ALG_LIB_S *pstAeLib,
                                    ALG_LIB_S *pstAwbLib) {
  HI_S32 s32Ret;
...
static int sensor_unregister_callback(ISP_DEV IspDev, ALG_LIB_S *pstAeLib,
                                      ALG_LIB_S *pstAwbLib) {
  HI_S32 s32Ret;
...
ISP_SNS_OBJ_S stSnsImx385Obj = {.pfnRegisterCallback = sensor_register_callback,
