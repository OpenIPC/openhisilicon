				  ...
				  AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	case WDR_MODE_NONE: { /* linear mode */
		...
		pstAeSnsDft->u32MaxISPDgainTarget =
			2 << pstAeSnsDft->u32ISPDgainShift;
		break;
	}
				       ...
				       AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	AR0237_SENSOR_GET_CTX(ViPipe, pstSnsState);
	CMOS_CHECK_POINTER_VOID(pstSnsState);
	u32FullLines = (u32FullLines > FULL_LINES_MAX) ? FULL_LINES_MAX :
							 u32FullLines;
...
static GK_U32 again_table[51] = {
	...
	1024,  1075,  1132,  1195,  1265,  1344,  1434,	 1536,	1654,
	1792,  1814,  1873,  1935,  2002,  2074,  2150,	 2233,	2322,
	2419,  2524,  2639,  2765,  2903,  3056,  3226,	 3415,	3629,
	3871,  4147,  4466,  4838,  5278,  5806,  6451,	 7258,	7741,
	8294,  8932,  9677,  10557, 11612, 12902, 14515, 15483, 16589,
				 ...
				 GK_U32 u32Dgain)
{
	...
	if (u32Again < 10) {
		...
		if (u32Again < 5) {
			...
			// 0x10 + (again - 5) * 2
			g_stSnsRegsInfo.astI2cData[1].u32Data =
				u32Again * 2 + 0x6;
		}
	} else {
		if (bHCG == GK_FALSE) {
				    ...
				    GK_U32 *pu32LFMaxIntTime)
{
	...
	if ((pstSnsState->enWDRMode == WDR_MODE_2To1_LINE)) {
		...
		if (genFSWDRMode[ViPipe] == ISP_FSWDR_LONG_FRAME_MODE) {
			u32T2Max =
				pstSnsState->au32FL[1] - 16 - au32WDRIntTime[0];
			*au32IntTimeMax = pstSnsState->au32FL[0] - 16;
			*au32IntTimeMax = (u32T2Max < *au32IntTimeMax) ?
						  u32T2Max :
						  *au32IntTimeMax;
		} else if (genFSWDRMode[ViPipe] ==
			   ISP_FSWDR_AUTO_LONG_FRAME_MODE) {
			if (au32WDRIntTime[0] == au32IntTimeMin[0] &&
			    au32Ratio[0] == 0x40) {
				u32T2Max = pstSnsState->au32FL[1] - 16 -
					   au32WDRIntTime[0];
				*au32IntTimeMax = pstSnsState->au32FL[0] - 16;
				*au32IntTimeMax = (u32T2Max < *au32IntTimeMax) ?
							  u32T2Max :
				...
				u32IntTimeMaxTmp0 = ((pstSnsState->au32FL[1] -
						      30 - au32WDRIntTime[0]) *
						     0x40) /
						    DIV_0_TO_1(au32Ratio[0]);
				u32IntTimeMaxTmp =
					((pstSnsState->au32FL[0] - 30) * 0x40) /
					DIV_0_TO_1(au32Ratio[0] + 0x40);
				u32IntTimeMaxTmp =
					(u32IntTimeMaxTmp0 < u32IntTimeMaxTmp) ?
						u32IntTimeMaxTmp0 :
						u32IntTimeMaxTmp;
				u32IntTimeMaxTmp0 = pstSnsState->au32FL[0] -
						    30 -
						    (pstSnsState->au32FL[0] -
						     16 - Y_OUTPUT);
				u32IntTimeMaxTmp0 = (u32IntTimeMaxTmp0 * 0x40) /
						    DIV_0_TO_1(au32Ratio[0]);
				*au32IntTimeMax =
					(u32IntTimeMaxTmp0 > u32IntTimeMaxTmp) ?
						u32IntTimeMaxTmp0 :
						u32IntTimeMaxTmp;
				*pu32LFMaxIntTime =
					pstSnsState->au32FL[0] - 16 - Y_OUTPUT;
			}
		} else {
			u32T2Max = pstSnsState->au32FL[0] - 16 - Y_OUTPUT;
			*au32IntTimeMax = (pstSnsState->au32FL[0] - 30) * 0x40 /
					  DIV_0_TO_1(au32Ratio[0] + 0x40);
...
static GK_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, GK_BOOL bEnable)
{
	...
	if (bEnable) { /* setup for ISP pixel calibration mode */
		ar0237_write_register(ViPipe, VMAX_ADDR,
				      u32FullLines_5Fps); /* 5fps */
		ar0237_write_register(
			ViPipe, EXPOSURE_TIME_L,
			u32MaxExpTime_5Fps); /* max exposure time */
		ar0237_write_register(ViPipe, ANALOG_GAIN,
				      0x0); /* AG, Context A */
		ar0237_write_register(ViPipe, DIGITAL_GAIN,
				      0x0080); /* DG, Context A */
	} else { /* setup for ISP 'normal mode' */
		pstSnsState->u32FLStd = (pstSnsState->u32FLStd > 0xFFFF) ?
						0xFFFF :
						pstSnsState->u32FLStd;
		pstSnsState->au32FL[0] = pstSnsState->u32FLStd;
		ar0237_write_register(ViPipe, VMAX_ADDR,
				      pstSnsState->u32FLStd); /* 30fps */
...
static GK_S32 cmos_set_wdr_mode(VI_PIPE ViPipe, GK_U8 u8Mode)
{
	...
	case WDR_MODE_NONE: {
		...
		g_stSnsRegsInfo.astI2cData[4].u32Data =
			(pstSnsState->u32FLStd * 30) / DIV_0_TO_1(gf32Fps);
				     ...
				     ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
	...
	if (pstSnsState->bSyncInit == GK_FALSE) {
		...
		for (i = 0; i < g_stSnsRegsInfo.u32RegNum; i++) {
			...
			g_stSnsRegsInfo.astI2cData[i].u8DevAddr =
				ar0237_i2c_addr;
			g_stSnsRegsInfo.astI2cData[i].u32AddrByteNum =
				ar0237_addr_byte;
			g_stSnsRegsInfo.astI2cData[i].u32DataByteNum =
				ar0237_data_byte;
		}
		...
		for (i = 0; i < g_stSnsRegsInfo.u32RegNum; i++) {
			if (g_stSnsRegsInfo.astI2cData[i].u32Data ==
			    g_stPreSnsRegsInfo.astI2cData[i].u32Data) {
				g_stSnsRegsInfo.astI2cData[i].bUpdate =
					GK_FALSE;
			} else {
				g_stSnsRegsInfo.astI2cData[i].bUpdate = GK_TRUE;
			}
	...
	memcpy(&g_stPreSnsRegsInfo, &g_stSnsRegsInfo,
 ...
 * callback structure                                                       *
 ****************************************************************************/
static GK_S32 ar0237_set_bus_info(VI_PIPE ViPipe,
