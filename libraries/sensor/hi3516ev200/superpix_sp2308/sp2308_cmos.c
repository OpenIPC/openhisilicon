...
#define SP2308_ID 2308

/****************************************************************************
 * global variables                                                            *
 ****************************************************************************/
ISP_SNS_STATE_S  *g_pastSp2308[ISP_MAX_PIPE_NUM] = {GK_NULL};

#define SP2308_SENSOR_SET_CTX(dev, pstCtx)   (g_pastSp2308[dev] = pstCtx)
#define SP2308_SENSOR_RESET_CTX(dev)         (g_pastSp2308[dev] = GK_NULL)


//static ISP_SNS_STATE_S     *g_apstSnsState[ISP_MAX_PIPE_NUM] = {&g_pastSp2308[0], &g_pastSp2308[1]};


#define ISO_LUT_NUM     8

ISP_SNS_COMMBUS_U g_aunSp2308_BusInfo[ISP_MAX_PIPE_NUM] = {
    [0] = { .s8I2cDev = 0},
...
#define SP2308_FULL_LINES_MAX  (0xFFFF)  //sensor降帧后1帧所能达到的最大行数，一般设置为sensor所支持的最大行数

/*****Sp2308 Register Address*****/
#define SP2308_PAGE_ADDR (0xfd)
#define SP2308_EXP_GKGH_ADDR (0x03)
#define SP2308_EXP_LOW_ADDR (0x04)
#define SP2308_AGAIN_ADDR (0x24)
#define SP2308_VMAX_ADDR_H (0x05)
#define SP2308_VMAX_ADDR_L (0x06)


#define SP2308_INCREASE_LINES (1) /* make real fps less than stand fps because NVR require*/

#define SP2308_VMAX_1080P30_DEFAULT_VBLANK 49
#define SP2308_VMAX_1080P30_LINEAR  (1109+SP2308_INCREASE_LINES + SP2308_VMAX_1080P30_DEFAULT_VBLANK)  //帧长

//sensor fps mode
#define SP2308_SENSOR_1080P_30FPS_LINEAR_MODE      (0)


#define SP2308_ERR_MODE_PRINT(pstSensorImageMode,pstSnsState)\
    do{\
...
ISP_SNS_COMMBUS_U* sp2308_get_bus_Info(VI_PIPE vi_pipe_value)
{
    return &g_aunSp2308_BusInfo[vi_pipe_value];
}

ISP_SNS_STATE_S* sp2308_get_ctx(VI_PIPE vi_pipe_value)
{
    return g_pastSp2308[vi_pipe_value];
}
...
static GK_S32 cmos_get_ae_default(VI_PIPE ViPipe,AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    SP2308_SENSOR_GET_CTX(ViPipe, g_apstSnsState);
    ...
    pstAeSnsDft->u32FullLinesMax = SP2308_FULL_LINES_MAX;
    ...
    pstAeSnsDft->u32MaxISPDgainTarget = 8 << pstAeSnsDft->u32ISPDgainShift;  //ISP的最大增益
    ...
    switch(g_apstSnsState->enWDRMode)
    {
        ...
        case WDR_MODE_NONE:   /*linear mode*/
            ...
            pstAeSnsDft->u32MaxDgain = 1024;          //sensor的最大数字增益
...
static GK_VOID cmos_fps_set(VI_PIPE ViPipe, GK_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    GK_U32 u32VMAX = SP2308_VMAX_1080P30_LINEAR;
	//GK_U32 tp = 1117;
	GK_U32 VBLANK = 0;	      //VB
    ...
    {
      case SP2308_SENSOR_1080P_30FPS_LINEAR_MODE:
           if ((f32Fps <= 30) && (f32Fps >= 5))
           {
               u32VMAX = SP2308_VMAX_1080P30_LINEAR * 30 / f32Fps;
           }
           else
           {
           ...
           u32VMAX = (u32VMAX > SP2308_FULL_LINES_MAX) ? SP2308_FULL_LINES_MAX : u32VMAX;
		   VBLANK = u32VMAX - (SP2308_VMAX_1080P30_LINEAR - SP2308_VMAX_1080P30_DEFAULT_VBLANK);
           break;
      default:
          return;
    }
...
static GK_VOID cmos_slow_framerate_set(VI_PIPE ViPipe,GK_U32 u32FullLines,AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    u32FullLines = (u32FullLines > SP2308_FULL_LINES_MAX) ? SP2308_FULL_LINES_MAX : u32FullLines;
    g_apstSnsState->au32FL[0] = u32FullLines;
	VBLANK = u32FullLines - (SP2308_VMAX_1080P30_LINEAR - SP2308_VMAX_1080P30_DEFAULT_VBLANK);
    if(WDR_MODE_NONE == g_apstSnsState->enWDRMode)
    {
		g_apstSnsState->astRegsInfo[1].astI2cData[6].u32Data = 0x00;
        g_apstSnsState->astRegsInfo[0].astI2cData[0].u32Data = 0x01;
...
static GK_VOID cmos_gains_update(VI_PIPE ViPipe,GK_U32 u32Again, GK_U32 u32Dgain)
{
	...
	//Dgain = (again_table[u32Again] / 0x80);
...
static GK_S32 cmos_get_awb_default(VI_PIPE ViPipe,AWB_SENSOR_DEFAULT_S *pstAwbSnsDft)
{
    ...
    SP2308_SENSOR_GET_CTX(ViPipe, pstSnsState);
...
static GK_S32 cmos_get_isp_default(VI_PIPE ViPipe,ISP_CMOS_DEFAULT_S *pstDef)
{
    ISP_SNS_STATE_S *pstSnsState = GK_NULL;
     ...
     pstDef->stSensorMode.u32SensorID = SP2308_ID;
...
static GK_VOID cmos_set_pixel_detect(VI_PIPE ViPipe,GK_BOOL bEnable)
{
    ...
    if (SP2308_SENSOR_1080P_30FPS_LINEAR_MODE == g_apstSnsState->u8ImgMode)
    {
        u32FullLines_5Fps = (SP2308_VMAX_1080P30_LINEAR * 30) / 5;
    }
    ...
    u32FullLines_5Fps = (u32FullLines_5Fps > SP2308_FULL_LINES_MAX) ? SP2308_FULL_LINES_MAX : u32FullLines_5Fps;
    u32FullLines_5Fps = u32FullLines_5Fps - (SP2308_VMAX_1080P30_LINEAR - SP2308_VMAX_1080P30_DEFAULT_VBLANK);
    u32MaxIntTime_5Fps = 4;

    if (bEnable) /* setup for ISP pixel calibration mode */
    {
		sp2308_write_register (ViPipe,0xfd,0x01);
        sp2308_write_register (ViPipe,SP2308_AGAIN_ADDR,0x80);

        sp2308_write_register (ViPipe,SP2308_VMAX_ADDR_H, (u32FullLines_5Fps & 0xFF00) >> 8);
        sp2308_write_register (ViPipe,SP2308_VMAX_ADDR_L, u32FullLines_5Fps & 0xFF);

        sp2308_write_register (ViPipe,SP2308_EXP_GKGH_ADDR, (u32MaxIntTime_5Fps & 0xFF00) >> 8);
        sp2308_write_register (ViPipe,SP2308_EXP_LOW_ADDR, u32MaxIntTime_5Fps & 0xFF);
...
static GK_S32 cmos_set_wdr_mode(VI_PIPE ViPipe,GK_U8 u8Mode)
{
    ...
    switch(u8Mode)
    {
        case WDR_MODE_NONE:
            ...
            g_apstSnsState->u32FLStd = SP2308_VMAX_1080P30_LINEAR;
            g_apstSnsState->u8ImgMode = SP2308_SENSOR_1080P_30FPS_LINEAR_MODE;
            printf("linear mode\n");
        break;

        default:
            printf("NOT support tGKs mode!\n");
            return -1;
    }
...
static GK_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe,ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((GK_FALSE == g_apstSnsState->bSyncInit) || (GK_FALSE == pstSnsRegsInfo->bConfig))
    {
        ...
        g_apstSnsState->astRegsInfo[0].unComBus.s8I2cDev = g_aunSp2308_BusInfo[ViPipe].s8I2cDev;
        g_apstSnsState->astRegsInfo[0].u8Cfg2ValidDelayMax = 2;
        g_apstSnsState->astRegsInfo[0].u32RegNum = 7;

        for (i=0; i<g_apstSnsState->astRegsInfo[0].u32RegNum; i++)
        {
            g_apstSnsState->astRegsInfo[0].astI2cData[i].bUpdate = GK_TRUE;
            g_apstSnsState->astRegsInfo[0].astI2cData[i].u8DevAddr = SP2308_I2C_ADDR;
            g_apstSnsState->astRegsInfo[0].astI2cData[i].u32AddrByteNum = SP2308_ADDR_BYTE;
            g_apstSnsState->astRegsInfo[0].astI2cData[i].u32DataByteNum = SP2308_DATA_BYTE;
        }

        //Linear Mode Regs
        g_apstSnsState->astRegsInfo[0].astI2cData[0].u8DelayFrmNum = 0;
        g_apstSnsState->astRegsInfo[0].astI2cData[0].u32RegAddr = SP2308_PAGE_ADDR;
        g_apstSnsState->astRegsInfo[0].astI2cData[1].u8DelayFrmNum = 0;
        g_apstSnsState->astRegsInfo[0].astI2cData[1].u32RegAddr = SP2308_EXP_GKGH_ADDR;
        g_apstSnsState->astRegsInfo[0].astI2cData[2].u8DelayFrmNum = 0;
        g_apstSnsState->astRegsInfo[0].astI2cData[2].u32RegAddr = SP2308_EXP_LOW_ADDR;

        g_apstSnsState->astRegsInfo[0].astI2cData[3].u8DelayFrmNum = 0;       //make shutter and gain effective at the same time
        g_apstSnsState->astRegsInfo[0].astI2cData[3].u32RegAddr = SP2308_AGAIN_ADDR;  //again

        g_apstSnsState->astRegsInfo[0].astI2cData[4].u8DelayFrmNum = 0;
        g_apstSnsState->astRegsInfo[0].astI2cData[4].u32RegAddr = SP2308_VMAX_ADDR_H;
        g_apstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 0;
        g_apstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr = SP2308_VMAX_ADDR_L;   //vblank addr
...
static GK_S32 cmos_set_image_mode(VI_PIPE ViPipe,ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
    ...
    if ((pstSensorImageMode->u16Width <= 1920) && (pstSensorImageMode->u16Height <= 1080)) {
        if (pstSensorImageMode->f32Fps <= 30) {
            u8SensorImageMode     = SP2308_SENSOR_1080P_30FPS_LINEAR_MODE;
            pstSnsState->u32FLStd = SP2308_VMAX_1080P30_LINEAR;
        } else {
            SP2308_ERR_MODE_PRINT(pstSensorImageMode, pstSnsState);
            return GK_FAILURE;
        }
    } else {
        SP2308_ERR_MODE_PRINT(pstSensorImageMode, pstSnsState);
...
static GK_VOID sensor_global_init(VI_PIPE ViPipe)
{
    ...
    g_apstSnsState->enWDRMode = WDR_MODE_NONE;
    g_apstSnsState->u32FLStd = SP2308_VMAX_1080P30_LINEAR;
    g_apstSnsState->au32FL[0] = SP2308_VMAX_1080P30_LINEAR;
    g_apstSnsState->au32FL[1] = SP2308_VMAX_1080P30_LINEAR;
...
static GK_S32 cmos_init_sensor_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
    ...
    pstSensorExpFunc->pfn_cmos_sensor_init = sp2308_init;
    pstSensorExpFunc->pfn_cmos_sensor_exit = sp2308_exit;
...
static int sp2308_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
    g_aunSp2308_BusInfo[ViPipe].s8I2cDev = unSNSBusInfo.s8I2cDev;
...
static GK_S32 sensor_ctx_init(VI_PIPE ViPipe)
{
    ...
    SP2308_SENSOR_GET_CTX(ViPipe, pastSnsStateCtx);
    ...
    SP2308_SENSOR_SET_CTX(ViPipe, pastSnsStateCtx);
...
static GK_VOID sensor_ctx_exit(VI_PIPE ViPipe)
{
    ...
    SP2308_SENSOR_RESET_CTX(ViPipe);
    return;
}
...
static int sensor_register_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    stSnsAttrInfo.eSensorId = SP2308_ID;
...
static int sensor_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    s32Ret = GK_API_ISP_SensorUnRegCallBack(ViPipe, SP2308_ID);
    if (s32Ret != GK_SUCCESS) {
        ISP_TRACE(MODULE_DBG_ERR, "sensor unregister callback function failed!\n");
        return s32Ret;
    }

    s32Ret = GK_API_AE_SensorUnRegCallBack(ViPipe, pstAeLib, SP2308_ID);
    if (s32Ret != GK_SUCCESS) {
        ISP_TRACE(MODULE_DBG_ERR, "sensor unregister callback function to ae lib failed!\n");
        return s32Ret;
    }

    s32Ret = GK_API_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, SP2308_ID);
    if (s32Ret != GK_SUCCESS) {
...
ISP_SNS_OBJ_S stSnsSp2308Obj =
{
    .pfnRegisterCallback    = sensor_register_callback,
    .pfnUnRegisterCallback  = sensor_unregister_callback,
    .pfnStandby             = sp2308_standby,
    .pfnRestart             = sp2308_restart,
    .pfnWriteReg            = sp2308_write_register,
    .pfnReadReg             = sp2308_read_register,
    .pfnSetBusInfo          = sp2308_set_bus_info,
