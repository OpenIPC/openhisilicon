...
ISP_SNS_STATE_S *g_pastH63[ISP_MAX_PIPE_NUM] = { GK_NULL };

#define H63_SENSOR_GET_CTX(dev, pstCtx) (pstCtx = g_pastH63[dev])
#define H63_SENSOR_SET_CTX(dev, pstCtx) (g_pastH63[dev] = pstCtx)
#define H63_SENSOR_RESET_CTX(dev) (g_pastH63[dev] = GK_NULL)

ISP_SNS_COMMBUS_U g_aunH63BusInfo[ISP_MAX_PIPE_NUM] = {
	[0] = { .s8I2cDev = 0 },
 ...
 * extern                                                                   *
 ****************************************************************************/
extern const unsigned int h63_i2c_addr;
extern unsigned int h63_addr_byte;
extern unsigned int h63_data_byte;

extern void h63_init(VI_PIPE ViPipe);
extern void h63_exit(VI_PIPE ViPipe);
extern void h63_standby(VI_PIPE ViPipe);
extern void h63_restart(VI_PIPE ViPipe);
extern int h63_write_register(VI_PIPE ViPipe, int addr, int data);
extern int h63_read_register(VI_PIPE ViPipe, int addr);

/* sensor fps mode */
#define H63_SENSOR_FPS_MAX 30
#define H63_SENSOR_FPS_MIN 1
#define H63_720P_30FPS_LINEAR_MODE (1)
#define H63_RES_IS_720P(w, h) ((w) <= 1280 && (h) <= 720)
#define H63_INCREASE_LINES \
	(0) /* make real fps less than stand fps because NVR require */
#define H63_VMAX_720P30_LINEAR (750 + H63_INCREASE_LINES)
#define H63_FULL_LINES_MAX 0xFFFF
				  ...
				  AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	H63_SENSOR_GET_CTX(ViPipe, pstSnsState);
	CMOS_CHECK_POINTER(pstSnsState);

	memset(&pstAeSnsDft->stAERouteAttr, 0, sizeof(ISP_AE_ROUTE_S));

	if (pstSnsState->u8ImgMode == H63_720P_30FPS_LINEAR_MODE) {
		u32Fll = H63_VMAX_720P30_LINEAR;
	}

	pstSnsState->u32FLStd = u32Fll;
	pstAeSnsDft->u32FullLinesStd = pstSnsState->u32FLStd;
	pstAeSnsDft->u32FlickerFreq = 0;
	pstAeSnsDft->u32FullLinesMax = H63_FULL_LINES_MAX;
	...
	if (g_au32LinesPer500ms[ViPipe] == 0) {
		pstAeSnsDft->u32LinesPer500ms =
			((GK_U64)(u32Fll * H63_SENSOR_FPS_MAX)) >> 1;
	} else {
	...
	case WDR_MODE_NONE:
		...
		pstAeSnsDft->u32MaxIntTimeTarget = H63_FULL_LINES_MAX;
		pstAeSnsDft->u32MinIntTimeTarget = 2; /* min 2 */

		pstAeSnsDft->u32MaxAgain = 8192; /* 15872: 15.5x1024 */
...
static GK_VOID cmos_fps_set(VI_PIPE ViPipe, GK_FLOAT f32Fps,
			    AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	GK_U32 u32VMAX = H63_VMAX_720P30_LINEAR;
	ISP_SNS_STATE_S *pstSnsState = GK_NULL;
	...
	case H63_720P_30FPS_LINEAR_MODE:
		if ((f32Fps <= H63_SENSOR_FPS_MAX) &&
		    (f32Fps >= H63_SENSOR_FPS_MIN)) {
			u32VMAX = H63_VMAX_720P30_LINEAR * H63_SENSOR_FPS_MAX /
				  f32Fps;
		} else {
		...
		pstAeSnsDft->u32LinesPer500ms =
			(H63_VMAX_720P30_LINEAR * H63_SENSOR_FPS_MAX) >> 1;
		break;
	default:
	...
	pstSnsState->u32FLStd = (pstSnsState->u32FLStd > H63_FULL_LINES_MAX) ?
					H63_FULL_LINES_MAX :
				       ...
				       AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	u32FullLines = (u32FullLines > H63_FULL_LINES_MAX) ?
			       H63_FULL_LINES_MAX :
			       u32FullLines;
	pstSnsState->au32FL[0] = u32FullLines;

	pstAeSnsDft->u32FullLines = pstSnsState->au32FL[0];
	pstAeSnsDft->u32MaxIntTime = pstSnsState->au32FL[0] - 2; /* sub 4 */
...
static GK_S32 cmos_get_isp_default(VI_PIPE ViPipe, ISP_CMOS_DEFAULT_S *pstDef)
{
	...
	pstDef->stSensorMode.u32SensorID = H63_ID;
				       ...
				       ISP_CMOS_BLACK_LEVEL_S *pstBlackLevel)
{
	...
	for (i = 0; i < ISP_BAYER_CHN_NUM; i++) {
		pstBlackLevel->au16BlackLevel[i] = 0x42;
	}
...
static GK_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, GK_BOOL bEnable)
{
	...
	u32FullLines_5Fps =
		(H63_VMAX_720P30_LINEAR * H63_SENSOR_FPS_MAX) / 5; /* 5 fps */
	u32MaxIntTime_5Fps = u32FullLines_5Fps - 4; /* sub 4 */

	if (bEnable) { /* setup for ISP pixel calibration mode */
		h63_write_register(ViPipe, VMAX_ADDR_H,
				   HIGH_8BITS(u32FullLines_5Fps));
		h63_write_register(ViPipe, VMAX_ADDR_L,
				   LOW_8BITS(u32FullLines_5Fps));
		h63_write_register(ViPipe, EXP_ADDR_H,
				   HIGH_8BITS(u32MaxIntTime_5Fps));
		h63_write_register(ViPipe, EXP_ADDR_L,
				   LOW_8BITS(u32MaxIntTime_5Fps));
		h63_write_register(ViPipe, GAIN_ADDR, 0x00); /* min AG */
...
static GK_S32 cmos_set_wdr_mode(VI_PIPE ViPipe, GK_U8 u8Mode)
{
	...
	case WDR_MODE_NONE:
		...
		pstSnsState->u8ImgMode = H63_720P_30FPS_LINEAR_MODE;
		pstSnsState->u32FLStd = H63_VMAX_720P30_LINEAR;
				     ...
				     ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
	...
	if ((pstSnsState->bSyncInit == GK_FALSE) ||
		...
		pstSnsState->astRegsInfo[0].unComBus.s8I2cDev =
			g_aunH63BusInfo[ViPipe].s8I2cDev;
		...
		for (i = 0; i < pstSnsState->astRegsInfo[0].u32RegNum; i++) {
			...
			pstSnsState->astRegsInfo[0]
				.astI2cData[i]
				.u32AddrByteNum = h63_addr_byte;
			pstSnsState->astRegsInfo[0]
				.astI2cData[i]
				.u32DataByteNum = h63_data_byte;
		}
		    ...
		    ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
	...
	if (H63_RES_IS_720P(pstSensorImageMode->u16Width,
			     pstSensorImageMode->u16Height)) {
		if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
			if (pstSensorImageMode->f32Fps <= H63_SENSOR_FPS_MAX) {
				u8SensorImageMode = H63_720P_30FPS_LINEAR_MODE;
			}
		} else {
			ISP_TRACE(
				MODULE_DBG_ERR,
...
static GK_VOID sensor_global_init(VI_PIPE ViPipe)
{
	...
	pstSnsState->enWDRMode = WDR_MODE_NONE;
	pstSnsState->u32FLStd = H63_VMAX_720P30_LINEAR;
	pstSnsState->au32FL[0] = H63_VMAX_720P30_LINEAR;
	pstSnsState->au32FL[1] = H63_VMAX_720P30_LINEAR;
...
cmos_init_sensor_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
	...
	pstSensorExpFunc->pfn_cmos_sensor_init = h63_init;
	pstSensorExpFunc->pfn_cmos_sensor_exit = h63_exit;
...
static GK_S32 h63_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
	g_aunH63BusInfo[ViPipe].s8I2cDev = unSNSBusInfo.s8I2cDev;
...
static GK_S32 sensor_ctx_init(VI_PIPE ViPipe)
{
	...
	H63_SENSOR_GET_CTX(ViPipe, pastSnsStateCtx);
	...
	H63_SENSOR_SET_CTX(ViPipe, pastSnsStateCtx);
				       ...
				       ALG_LIB_S *pstAwbLib)
{
	...
	stSnsAttrInfo.eSensorId = H63_ID;
					 ...
					 ALG_LIB_S *pstAwbLib)
{
	...
	s32Ret = GK_API_ISP_SensorUnRegCallBack(ViPipe, H63_ID);
	if (s32Ret != GK_SUCCESS) {
		ISP_TRACE(MODULE_DBG_ERR,
	...
	s32Ret = GK_API_AE_SensorUnRegCallBack(ViPipe, pstAeLib, H63_ID);
	if (s32Ret != GK_SUCCESS) {
		ISP_TRACE(
			MODULE_DBG_ERR,
	...
	s32Ret = GK_API_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, H63_ID);
	if (s32Ret != GK_SUCCESS) {
		ISP_TRACE(
			MODULE_DBG_ERR,
...
ISP_SNS_OBJ_S stSnsSoiH63Obj = {
	.pfnRegisterCallback = sensor_register_callback,
	.pfnUnRegisterCallback = sensor_unregister_callback,
	.pfnStandby = h63_standby,
	.pfnRestart = h63_restart,
	.pfnMirrorFlip = GK_NULL,
	.pfnWriteReg = h63_write_register,
	.pfnReadReg = h63_read_register,
	.pfnSetBusInfo = h63_set_bus_info,
