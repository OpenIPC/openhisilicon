...
ISP_SNS_STATE_S *g_pastGc2053[ISP_MAX_PIPE_NUM] = {GK_NULL};
...
#define GC2053_SENSOR_RESET_CTX(dev)         (g_pastGc2053[dev] = GK_NULL)
...
static GK_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    switch (pstSnsState->enWDRMode) {
        ...
        case WDR_MODE_2To1_FRAME:
            ...
            pstAeSnsDft->stIntTimeAccu.f32Offset = -0.15417;
...
static GK_U32 regValTable[29][6] = {
...
static GK_VOID cmos_gains_update(VI_PIPE ViPipe, GK_U32 u32Again, GK_U32 u32Dgain)
{
    ...
    if(pstSnsState->enWDRMode == WDR_MODE_2To1_FRAME){
        pstSnsState->astRegsInfo[0].astI2cData[13].u32Data = regValTable[u32Again][4];
		pstSnsState->astRegsInfo[0].astI2cData[14].u32Data = regValTable[u32Again][5];
	}
    return;
}
...
static GK_S32 cmos_get_isp_black_level(VI_PIPE ViPipe, ISP_CMOS_BLACK_LEVEL_S *pstBlackLevel)
{
    ...
    if(pstSnsState->enWDRMode == WDR_MODE_2To1_FRAME) {
        pstBlackLevel->bUpdate = GK_FALSE;
        pstBlackLevel->au16BlackLevel[0] = 265;
        pstBlackLevel->au16BlackLevel[1] = 265;
        pstBlackLevel->au16BlackLevel[2] = 265;
        pstBlackLevel->au16BlackLevel[3] = 265;
    }
    return GK_SUCCESS;
...
static GK_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((pstSnsState->bSyncInit == GK_FALSE) || (pstSnsRegsInfo->bConfig == GK_FALSE)) {
        ...
        if ( pstSnsState->enWDRMode == WDR_MODE_2To1_FRAME) {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[13].u32RegAddr  = 0x38;/*For FSWDR OB Settings*/
            pstSnsState->astRegsInfo[0].astI2cData[14].u8DelayFrmNum  = 1;
            pstSnsState->astRegsInfo[0].astI2cData[15].u32RegAddr  = 0x7c;/*For FSWDR OB Settings*/
        }
        ...
        if ((pstSnsState->astRegsInfo[0].astI2cData[6].bUpdate == GK_TRUE) || (pstSnsState->astRegsInfo[0].astI2cData[7].bUpdate == GK_TRUE)) {
...
static GK_S32 gc2053_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
