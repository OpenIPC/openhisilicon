    ...
    switch (u8Mode) {
        case WDR_MODE_NONE:
            ...
            if (pstSnsState->u8ImgMode == SC2235_SENSOR_1080P_30FPS_LINEAR_MODE) {
                ...
                ISP_TRACE(MODULE_DBG_ERR, "SC2235_SENSOR_1080P_30FPS_LINEAR_MODE\n");
            } else {
                ISP_TRACE(MODULE_DBG_ERR, "cmos_set_wdr_mode_Mode's name is wrong\n");
                return GK_FAILURE;
            }
            break;
        default:
...
static GK_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((pstSnsState->bSyncInit == GK_FALSE) || (pstSnsRegsInfo->bConfig == GK_FALSE)) {
        ...
        if (GK_TRUE == (pstSnsState->astRegsInfo[0].astI2cData[0].bUpdate ||  pstSnsState->astRegsInfo[0].astI2cData[1].bUpdate || pstSnsState->astRegsInfo[0].astI2cData[2].bUpdate ||  pstSnsState->astRegsInfo[0].astI2cData[3].bUpdate)) {
            pstSnsState->astRegsInfo[0].astI2cData[6].bUpdate  = GK_TRUE;    // Grouphold_AEC/AGC_Logic
            ...
            pstSnsState->astRegsInfo[0].astI2cData[15].bUpdate = GK_TRUE;
        } else {
            pstSnsState->astRegsInfo[0].astI2cData[6].bUpdate  = GK_FALSE;    // Grouphold_AEC/AGC_Logic
            pstSnsState->astRegsInfo[0].astI2cData[7].bUpdate  = GK_FALSE;
            pstSnsState->astRegsInfo[0].astI2cData[8].bUpdate  = GK_FALSE;
            pstSnsState->astRegsInfo[0].astI2cData[9].bUpdate  = GK_FALSE;
            pstSnsState->astRegsInfo[0].astI2cData[10].bUpdate = GK_FALSE;
            pstSnsState->astRegsInfo[0].astI2cData[11].bUpdate = GK_FALSE;
            pstSnsState->astRegsInfo[0].astI2cData[12].bUpdate = GK_FALSE;
            pstSnsState->astRegsInfo[0].astI2cData[13].bUpdate = GK_FALSE;
            pstSnsState->astRegsInfo[0].astI2cData[14].bUpdate = GK_FALSE;
            pstSnsState->astRegsInfo[0].astI2cData[15].bUpdate = GK_FALSE;
...
static GK_S32 sc2235_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
