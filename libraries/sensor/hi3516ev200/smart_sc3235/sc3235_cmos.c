			    ...
			    AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	case SC3235_SENSOR_1296P_30FPS_LINEAR_MODE:
		if ((f32Fps <= 30) && (f32Fps >= 4)) {
			u32VMAX = SC3235_VMAX_1296P30_LINEAR * 30 /
	...
	u32VMAX = (u32VMAX > SC3235_FULL_LINES_MAX) ? SC3235_FULL_LINES_MAX :
						      u32VMAX;
	...
	pstSnsState->u32FLStd = (u32FullLines > SC3235_FULL_LINES_MAX) ?
					SC3235_FULL_LINES_MAX :
					u32FullLines;
				       ...
				       AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	u32FullLines = (u32FullLines > SC3235_FRAME_SLOW_MAX) ?
			       SC3235_FRAME_SLOW_MAX :
			       u32FullLines;
...
static GK_VOID cmos_inttime_update(VI_PIPE ViPipe, GK_U32 u32IntTime)
{
	...
	u32Val = (u32IntTime > SC3235_FULL_LINES_MAX) ? SC3235_FULL_LINES_MAX :
							u32IntTime;
...
static GK_U32 again_table[128] = {
	...
	0x329,	0x32a,	0x32b,	0x32c,	0x32d,	0x32e,	0x32f,	0x330,	0x331,
	0x332,	0x333,	0x334,	0x335,	0x336,	0x337,	0x338,	0x339,	0x33a,
	0x33b,	0x33c,	0x33d,	0x33e,	0x33f,	0x720,	0x721,	0x722,	0x723,
	0x724,	0x725,	0x726,	0x727,	0x728,	0x729,	0x72a,	0x72b,	0x72c,
	0x72d,	0x72e,	0x72f,	0x730,	0x731,	0x732,	0x733,	0x734,	0x735,
	0x736,	0x737,	0x738,	0x739,	0x73a,	0x73b,	0x73c,	0x73d,	0x73e,
	0x73f,	0xf20,	0xf21,	0xf22,	0xf23,	0xf24,	0xf25,	0xf26,	0xf27,
	0xf28,	0xf29,	0xf2a,	0xf2b,	0xf2c,	0xf2d,	0xf2e,	0xf2f,	0xf30,
	0xf31,	0xf32,	0xf33,	0xf34,	0xf35,	0xf36,	0xf37,	0xf38,	0xf39,
	0xf3a,	0xf3b,	0xf3c,	0xf3d,	0xf3e,	0xf3f,	0x1f20, 0x1f21, 0x1f22,
				 ...
				 GK_U32 u32Dgain)
{
	...
	if (u8Reg0x3e08 < 0x07) { // 0x0320~0x033f  gain<2x
		...
		pstSnsState->astRegsInfo[0].astI2cData[12].u32Data =
		   ...
		   (u8Reg0x3e08 < 0x0f)) { // 0x0720~0x073f 4x>gain >= 2x
		pstSnsState->astRegsInfo[0].astI2cData[10].u32Data =
			0x58; // R0x3632
		pstSnsState->astRegsInfo[0].astI2cData[11].u32Data =
			0x58; // R0x3306
		pstSnsState->astRegsInfo[0].astI2cData[12].u32Data =
		   ...
		   (0x1f > u8Reg0x3e08)) { // 0x0f20~0x0f3f 8x>gain >= 4x
		pstSnsState->astRegsInfo[0].astI2cData[10].u32Data =
			0x58; // R0x3632
		pstSnsState->astRegsInfo[0].astI2cData[11].u32Data =
			0x58; // R0x3306
...
static GK_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, GK_BOOL bEnable)
{
	...
	if (bEnable) { /* setup for ISP pixel calibration mode */
		...
		sc3235_write_register(ViPipe, SC3235_EXP_L_ADDR,
				      LOWER_4BITS(u32MaxIntTime_5Fps) << 4);
		sc3235_write_register(ViPipe, SC3235_EXP_M_ADDR,
				      HIGHER_8BITS(u32MaxIntTime_5Fps));
		sc3235_write_register(ViPipe, SC3235_EXP_H_ADDR,
				      HIGHER_4BITS(u32MaxIntTime_5Fps));
		sc3235_write_register(ViPipe, SC3235_VMAX_L_ADDR,
				      LOW_8BITS(u32FullLines_5Fps));
		sc3235_write_register(ViPipe, SC3235_VMAX_H_ADDR,
				      HIGH_8BITS(u32FullLines_5Fps));
	} else { /* setup for ISP 'normal mode' */
		pstSnsState->u32FLStd =
			(pstSnsState->u32FLStd > SC3235_FULL_LINES_MAX * 2) ?
				(SC3235_FULL_LINES_MAX * 2) :
				pstSnsState->u32FLStd;
		sc3235_write_register(ViPipe, SC3235_VMAX_L_ADDR,
				     ...
				     ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
	...
	if ((pstSnsState->bSyncInit == GK_FALSE) ||
		...
		pstSnsState->astRegsInfo[0].unComBus.s8I2cDev =
			g_aunSc3235BusInfo[ViPipe].s8I2cDev;
		...
		for (i = 0; i < pstSnsState->astRegsInfo[0].u32RegNum; i++) {
			...
			pstSnsState->astRegsInfo[0]
				.astI2cData[i]
				.u32AddrByteNum = sc3235_addr_byte;
			pstSnsState->astRegsInfo[0]
				.astI2cData[i]
				.u32DataByteNum = sc3235_data_byte;
		}
		    ...
		    ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
	...
	if (pstSensorImageMode->f32Fps <= 30) {
		if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
			if (SC3235_RES_IS_1296P(pstSensorImageMode->u16Width,
						pstSensorImageMode->u16Height)) {
				u8SensorImageMode =
					SC3235_SENSOR_1296P_30FPS_LINEAR_MODE;
				pstSnsState->u32FLStd =
					SC3235_VMAX_1296P30_LINEAR * 2;
			} else {
				ISP_TRACE(
					MODULE_DBG_ERR,
...
static GK_S32 sc3235_set_bus_info(VI_PIPE ViPipe,
