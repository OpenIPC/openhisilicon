...
ISP_SNS_STATE_S *g_pastImx327_2l[ISP_MAX_PIPE_NUM] = { GK_NULL };
...
#define IMX327_SENSOR_RESET_CTX(dev) (g_pastImx327_2l[dev] = GK_NULL)
			    ...
			    AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ...
        */
		g_astimx327_2l_State[ViPipe].u32RHS1_MAX =
			(u32VMAX - g_astimx327_2l_State[ViPipe].u32BRL) * 2 -
			21;
	} else {
				       ...
				       AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
		...
		g_astimx327_2l_State[ViPipe].u32RHS1_MAX =
			...
			g_astimx327_2l_State[ViPipe].u32BRL * 2 - 21;
...
{
	ISP_SNS_STATE_S *pstSnsState = GK_NULL;
	GK_U32 u32HCG = g_astimx327_2l_State[ViPipe].u8Hcg;
	GK_U32 u32Tmp;
...
static GK_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, GK_BOOL bEnable)
{
	...
	if (bEnable) { /* setup for ISP pixel calibration mode */
		...
		imx327_2l_write_register(ViPipe, IMX327_VMAX_ADDR,
					 u32FullLines_5Fps & 0xFF);
		imx327_2l_write_register(ViPipe, IMX327_VMAX_ADDR + 1,
					 (u32FullLines_5Fps & 0xFF00) >> 8);
		imx327_2l_write_register(ViPipe, IMX327_VMAX_ADDR + 2,
					 (u32FullLines_5Fps & 0xF0000) >> 16);

		imx327_2l_write_register(ViPipe, IMX327_SHS1_ADDR,
					 u32MaxIntTime_5Fps & 0xFF);
		imx327_2l_write_register(ViPipe, IMX327_SHS1_ADDR + 1,
					 (u32MaxIntTime_5Fps & 0xFF00) >> 8);
		imx327_2l_write_register(ViPipe, IMX327_SHS1_ADDR + 2,
				     ...
				     ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
	...
	if ((pstSnsState->bSyncInit == GK_FALSE) ||
		...
		pstSnsState->astRegsInfo[0].unComBus.s8I2cDev =
			g_aunImx327_2l_BusInfo[ViPipe].s8I2cDev;
		...
		for (i = 0; i < pstSnsState->astRegsInfo[0].u32RegNum; i++) {
			...
			pstSnsState->astRegsInfo[0]
				.astI2cData[i]
				.u32AddrByteNum = imx327_2l_addr_byte;
			pstSnsState->astRegsInfo[0]
				.astI2cData[i]
				.u32DataByteNum = imx327_2l_data_byte;
		}
	...
	if (pstSensorImageMode->f32Fps <= 30) {
		if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
			if (IMX327_RES_IS_1080P(pstSensorImageMode->u16Width,
				...
				IMX327_2L_ERR_MODE_PRINT(pstSensorImageMode,
							 pstSnsState);
				return GK_FAILURE;
			}
		} else if (pstSnsState->enWDRMode == WDR_MODE_2To1_FRAME) {
