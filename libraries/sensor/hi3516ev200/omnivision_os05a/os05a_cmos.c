...
#define INCREASE_LINES \
	(5) /* make real fps less than stand fps because NVR require */
				  ...
				  AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	pstAeSnsDft->u32MaxIntTime = (pstSnsState->u32FLStd >= 12) ?
					     (pstSnsState->u32FLStd - 12) :
					     (0);
			    ...
			    AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	case Os05a_5M_2688x1944_12bit_linear30:
		if ((f32Fps <= 30) && (f32Fps >= 1.87)) {
			f32maxFps = 30;
			u32Lines = OS05A_VMAX_5M30_12bit_LINEAR * f32maxFps /
				   f32Fps;
			pstAeSnsDft->u32LinesPer500ms =
				OS05A_VMAX_5M30_12bit_LINEAR * 15;
		} else {
	...
	case Os05a_5M_2688x1944_10bit_wdr30:
		if ((f32Fps <= 30) && (f32Fps >= 2.0)) {
			f32maxFps = 30;
			u32Lines =
				OS05A_VMAX_5M30_10bit_WDR * f32maxFps / f32Fps;
			pstAeSnsDft->u32LinesPer500ms =
				OS05A_VMAX_5M30_10bit_WDR * 15;
		} else {
			printf("Not support Fps: %f\n", f32Fps);
			return;
		}
				       ...
				       AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	pstSnsState->au32FL[0] = (u32FullLines > OS05A_FULL_LINES_MAX) ?
					 OS05A_FULL_LINES_MAX :
					 u32FullLines;
	pstAeSnsDft->u32FullLines = pstSnsState->au32FL[0];
...
static GK_VOID cmos_inttime_update(VI_PIPE ViPipe, GK_U32 u32IntTime)
{
	...
	if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
		if (bFirst) { /* short exposure */
			...
			if (gu32STimeFps == 30) {
				if ((u32LongIntTime >= 750) &&
				    (u32ShortIntTime < 750) &&
				    ((u32LongIntTime + 750) < 2450) &&
				    (gf32AGain >= 11)) {
					fCGain = gf32AGain * gf32DGain *
						 u32ShortIntTime / 750;
					if (fCGain < 16) {
						u32Again = fCGain * 128;
						u32Dgain = 1024;
			...
			pstSnsState->astRegsInfo[0].astI2cData[8].u32Data =
				LOW_8BITS(u32ShortIntTime);
			pstSnsState->astRegsInfo[0].astI2cData[9].u32Data =
				HIGH_8BITS(u32ShortIntTime);
				    ...
				    GK_U32 *pu32LFMaxIntTime)
{
	...
	if ((pstSnsState->enWDRMode == WDR_MODE_2To1_LINE)) {
		if (genFSWDRMode[ViPipe] ==
		    ISP_FSWDR_LONG_FRAME_MODE) { /* long frame mode enable */
			if (gu32MaxTimeGetCnt[ViPipe] > 15) {
				u32IntTimeMaxTmp =
					pstSnsState->au32FL[0] - 50 -
					pstSnsState->au32WDRIntTime[0];
				u32IntTimeMaxTmp1 = pstSnsState->au32FL[0] - 50;
			} else {
				u32IntTimeMaxTmp =
					((pstSnsState->au32FL[1] - 50 -
					  pstSnsState->au32WDRIntTime[0])
					 << 6) /
					DIV_0_TO_1(au32Ratio[0]);
				u32IntTimeMaxTmp1 =
					(pstSnsState->au32FL[0] - 50) * 0x40 /
					(au32Ratio[0] + 0x40);
			}
			...
			u32IntTimeMaxTmp = ((pstSnsState->au32FL[1] - 50 -
					     pstSnsState->au32WDRIntTime[0])
					    << 6) /
					   DIV_0_TO_1(au32Ratio[0]);
			u32IntTimeMaxTmp1 = (pstSnsState->au32FL[0] - 50) *
...
static GK_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, GK_BOOL bEnable)
{
	...
	if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
		...
		if (pstSnsState->u8ImgMode ==
		    Os05a_5M_2688x1944_12bit_linear30) {
			u32FullLines_5Fps =
				(OS05A_VMAX_5M30_12bit_LINEAR * 30) / 5;
		} else {
			return;
		}
	}
	...
	if (bEnable) { /* setup for ISP pixel calibration mode */
		os05a_write_register(ViPipe, VMAX_ADDR_L,
				     LOW_8BITS(u32FullLines_5Fps));
		os05a_write_register(ViPipe, VMAX_ADDR_H,
				     HIGH_8BITS(u32FullLines_5Fps));

		os05a_write_register(
			ViPipe, EXPOSURE_TIME_LONG_H,
			HIGH_8BITS(u32MaxIntTime_5Fps)); /* shutter */
		os05a_write_register(ViPipe, EXPOSURE_TIME_LONG_L,
				     ...
				     ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
	...
	if ((pstSnsState->bSyncInit == GK_FALSE) ||
		...
		pstSnsState->astRegsInfo[0].unComBus.s8I2cDev =
			g_aunOs05aBusInfo[ViPipe].s8I2cDev;
		...
		for (i = 0; i < pstSnsState->astRegsInfo[0].u32RegNum; i++) {
			...
			pstSnsState->astRegsInfo[0]
				.astI2cData[i]
				.u32AddrByteNum = os05a_addr_byte;
			pstSnsState->astRegsInfo[0]
				.astI2cData[i]
				.u32DataByteNum = os05a_data_byte;
		}
		    ...
		    ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
	...
	if ((pstSensorImageMode->u16Width <= 2688) &&
	    (pstSensorImageMode->u16Height <= 1944)) {
		if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
			if (pstSensorImageMode->f32Fps <= 30) {
				u8SensorImageMode =
					Os05a_5M_2688x1944_10bit_wdr30;
				pstSnsState->u32FLStd =
					OS05A_VMAX_5M30_10bit_WDR;
			} else {
				OS05A_ERR_MODE_PRINT(pstSensorImageMode,
						     pstSnsState);
				return GK_FAILURE;
			}
...
ISP_SNS_OBJ_S stSnsOs05aObj = { .pfnRegisterCallback = sensor_register_callback,
