...
extern void gc4653_2l_mirror_flip(VI_PIPE ViPipe, ISP_SNS_MIRRORFLIP_TYPE_E eSnsMirrorFlip);
...
#define GC4653_2L_INCREASE_LINES      (1)  /* make real fps less than stand fps because NVR require*/
...
#define GC4653_2L_AGAIN_MAX    (77660)     // the max again is 77660 mean 75.84 recommend
#define GC4653_2L_DGAIN_MIN    (1024)
#define GC4653_2L_DGAIN_MAX    (16368)     // the max dgain is 16368 mean 15.98 recommend
...
static GK_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    switch (pstSnsState->enWDRMode) {
        case WDR_MODE_NONE:
            ...
            pstAeSnsDft->u32MaxIntTimeTarget = GC4653_2L_SHUTTER_MAX; // 65535
...
static GK_VOID cmos_fps_set(VI_PIPE ViPipe, GK_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    switch (pstSnsState->u8ImgMode) {
        case GC4653_2L_SENSOR_4M_30FPS_LINEAR_MODE:
            if ((f32Fps <= 30.0) && (f32Fps >= 2.75)) {
                u32VMAX = GC4653_2L_VMAX_4M30_LINEAR * 30 / DIV_0_TO_1_FLOAT(f32Fps);
            } else {
    ...
    u32VMAX = (u32VMAX > GC4653_2L_FULL_LINES_MAX) ? GC4653_2L_FULL_LINES_MAX : u32VMAX;
    pstSnsState->u32FLStd = u32VMAX;
    pstSnsState->astRegsInfo[0].astI2cData[11].u32Data = ((u32VMAX & 0xFF00) >> 8);
                                           ...
                                           AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    GC4653_2L_SENSOR_GET_CTX(ViPipe, pstSnsState);
    CMOS_CHECK_POINTER_VOID(pstSnsState);
    u32FullLines = (u32FullLines > GC4653_2L_FULL_LINES_MAX) ? GC4653_2L_FULL_LINES_MAX : u32FullLines;
    pstSnsState->au32FL[0] = u32FullLines;

    pstSnsState->astRegsInfo[0].astI2cData[11].u32Data = ((u32FullLines & 0xFF00) >> 8);
    pstSnsState->astRegsInfo[0].astI2cData[12].u32Data = (u32FullLines & 0xFF);
...
static GK_VOID cmos_inttime_update(VI_PIPE ViPipe, GK_U32 u32IntTime)
{
    ...
    u32Val = (u32IntTime > GC4653_2L_FULL_LINES_MAX) ? GC4653_2L_SHUTTER_MAX : u32IntTime;

    pstSnsState->astRegsInfo[0].astI2cData[0].u32Data = ((u32Val & 0xFF00) >> 8);
...
static GK_U32 analog_gain_table[GAIN_TABLE_MAX] = {
    1024, 1213, 1434, 1699, 2048, 2427, 2867, 3398, 4096, 4854, 5734, 6795, 8192, 9708,
    11469, 13591, 16384, 19415, 22938, 27181, 32768, 38830, 45875, 54362, 65536, 77660
};
...
static GK_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((GK_FALSE == pstSnsState->bSyncInit) || (GK_FALSE == pstSnsRegsInfo->bConfig)) {
        ...
        pstSnsState->astRegsInfo[0].unComBus.s8I2cDev = g_aunGc4653_2l_BusInfo[ViPipe].s8I2cDev;
        ...
        for (i = 0; i < pstSnsState->astRegsInfo[0].u32RegNum; i++) {
            ...
            pstSnsState->astRegsInfo[0].astI2cData[i].u8DevAddr = gc4653_2l_i2c_addr;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32AddrByteNum = gc4653_2l_addr_byte;
            pstSnsState->astRegsInfo[0].astI2cData[i].u32DataByteNum = gc4653_2l_data_byte;
        }
        pstSnsState->astRegsInfo[0].astI2cData[0].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[0].u32RegAddr = GC4653_2L_EXPTIME_ADDR_H;
        pstSnsState->astRegsInfo[0].astI2cData[1].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[1].u32RegAddr = GC4653_2L_EXPTIME_ADDR_L;

        //gain
        pstSnsState->astRegsInfo[0].astI2cData[2].u8DelayFrmNum  = 0;
        pstSnsState->astRegsInfo[0].astI2cData[2].u32RegAddr  = GC4653_2L_AGAIN_ADDR_EXT0;

        pstSnsState->astRegsInfo[0].astI2cData[3].u8DelayFrmNum  = 0;
        pstSnsState->astRegsInfo[0].astI2cData[3].u32RegAddr  = GC4653_2L_AGAIN_ADDR_EXT1;
        pstSnsState->astRegsInfo[0].astI2cData[4].u8DelayFrmNum  = 0;
        pstSnsState->astRegsInfo[0].astI2cData[4].u32RegAddr  = GC4653_2L_AGAIN_ADDR_EXT2;
        pstSnsState->astRegsInfo[0].astI2cData[5].u8DelayFrmNum  = 0;
        pstSnsState->astRegsInfo[0].astI2cData[5].u32RegAddr  = GC4653_2L_AGAIN_ADDR_EXT3;
        pstSnsState->astRegsInfo[0].astI2cData[6].u8DelayFrmNum  = 0;
        pstSnsState->astRegsInfo[0].astI2cData[6].u32RegAddr  = GC4653_2L_AGAIN_ADDR_EXT4;
        pstSnsState->astRegsInfo[0].astI2cData[7].u8DelayFrmNum  = 0;
        pstSnsState->astRegsInfo[0].astI2cData[7].u32RegAddr  = GC4653_2L_AGAIN_ADDR_EXT5;
        pstSnsState->astRegsInfo[0].astI2cData[8].u8DelayFrmNum  = 0;
        pstSnsState->astRegsInfo[0].astI2cData[8].u32RegAddr  = GC4653_2L_AGAIN_ADDR_EXT6;

        pstSnsState->astRegsInfo[0].astI2cData[9].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[9].u32RegAddr = GC4653_2L_DGAIN_ADDR_H;
        pstSnsState->astRegsInfo[0].astI2cData[10].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[10].u32RegAddr = GC4653_2L_DGAIN_ADDR_L;

        //vmax
        pstSnsState->astRegsInfo[0].astI2cData[11].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[11].u32RegAddr = GC4653_2L_VMAX_ADDR_H;
        pstSnsState->astRegsInfo[0].astI2cData[12].u8DelayFrmNum = 0;
        pstSnsState->astRegsInfo[0].astI2cData[12].u32RegAddr = GC4653_2L_VMAX_ADDR_L;
        ...
        if ((pstSnsState->astRegsInfo[0].astI2cData[0].bUpdate == GK_TRUE) ||
            ( pstSnsState->astRegsInfo[1].astI2cData[6].bUpdate == GK_TRUE)) {
            pstSnsState->astRegsInfo[0].astI2cData[0].bUpdate = GK_TRUE;
            pstSnsState->astRegsInfo[0].astI2cData[1].bUpdate = GK_TRUE;
        }

        if ((pstSnsState->astRegsInfo[0].astI2cData[9].bUpdate == GK_TRUE) ||
            ( pstSnsState->astRegsInfo[0].astI2cData[10].bUpdate == GK_TRUE)) {
            pstSnsState->astRegsInfo[0].astI2cData[9].bUpdate = GK_TRUE;
            pstSnsState->astRegsInfo[0].astI2cData[10].bUpdate = GK_TRUE;
        }

        if ((pstSnsState->astRegsInfo[0].astI2cData[11].bUpdate == GK_TRUE) ||
...
static GK_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
    ...
    if ((pstSensorImageMode->u16Width <= 2560) && (pstSensorImageMode->u16Height <= 1440)) {
        if (pstSensorImageMode->f32Fps <= 30) {
            u8SensorImageMode     = GC4653_2L_SENSOR_4M_30FPS_LINEAR_MODE;
 ...
 * callback structure                                                       *
 ****************************************************************************/
static GK_S32 gc4653_2l_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
...
static GK_S32 sensor_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
    ...
    s32Ret = GK_API_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, GC4653_2L_ID);
    if (s32Ret != GK_SUCCESS) {
