...
static GK_S32 sc500ai_read_register_arecvbuf(VI_PIPE ViPipe, GK_U32 addr, GK_U8 *aRecvbuf)
{
    GK_S32 s32Ret;
    ...
    memset_s(aRecvbuf, 4 * sizeof(GK_U8), 0x0, 4 * sizeof(GK_U8));  /* 4,4 len of aRecvbuf */
...
int sc500ai_read_register(VI_PIPE ViPipe, GK_S32 addr)
{
    GK_S32 s32RegVal;
    if (g_fd[ViPipe] < 0) {
        ISP_TRACE(MODULE_DBG_ERR, "sc500ai_read_register fd not opened!\n");
        return GK_FAILURE;
    }
