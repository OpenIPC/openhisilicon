...
GK_S32 ISP_SensorCtxInit(VI_PIPE ViPipe)
{
    ...
    if (pstSensorCtx == GK_NULL) {
        ...
        if (pstSensorCtx == GK_NULL) {
            ISP_TRACE(MODULE_DBG_ERR, "Isp[%d] SensorCtx malloc memory failed!\n", ViPipe);
...
GK_S32 ISP_SensorRegCallBack(VI_PIPE ViPipe, ISP_SNS_ATTR_INFO_S *pstSnsAttrInfo, ISP_SENSOR_REGISTER_S *pstRegister)
{
    ...
    if (pstSensor->stRegister.stSnsExp.pfn_cmos_sensor_global_init != GK_NULL) {
...
GK_S32 ISP_SensorUpdateAll(VI_PIPE ViPipe)
{
    ...
    if (pstSensor->stRegister.stSnsExp.pfn_cmos_get_isp_default != GK_NULL) {
    ...
    if (pstSensor->stRegister.stSnsExp.pfn_cmos_get_isp_black_level != GK_NULL) {
...
GK_S32 ISP_SensorGetBlc(VI_PIPE ViPipe, ISP_CMOS_BLACK_LEVEL_S **ppstSnsBlackLevel)
{
    ISP_SENSOR_S *pstSensor = GK_NULL;
...
GK_S32 ISP_SensorGetMaxResolution(VI_PIPE ViPipe, ISP_CMOS_SENSOR_MAX_RESOLUTION_S **ppstSnsMaxResolution)
{
    ISP_SENSOR_S *pstSensor = GK_NULL;
...
GK_S32 ISP_SensorSetWDRMode(VI_PIPE ViPipe, GK_U8 u8Mode)
{
    ...
    if (pstSensor->stRegister.stSnsExp.pfn_cmos_set_wdr_mode != GK_NULL) {
        if (pstSensor->stRegister.stSnsExp.pfn_cmos_set_wdr_mode(ViPipe, u8Mode) != GK_SUCCESS) {
            return GK_FAILURE;
        }
    }
...
GK_S32 ISP_SensorSetImageMode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSnsImageMode)
{
...
GK_S32 ISP_SensorSetPixelDetect(VI_PIPE ViPipe, GK_BOOL bEnable)
{
    ...
    if (pstSensor->stRegister.stSnsExp.pfn_cmos_set_pixel_detect != GK_NULL) {
...
GK_S32 ISP_SensorUpdateSnsReg(VI_PIPE ViPipe)
{
    ...
    if (pstSensor->stRegister.stSnsExp.pfn_cmos_get_sns_reg_info != GK_NULL) {
