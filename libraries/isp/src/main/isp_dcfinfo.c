...
GK_S32 ISP_UpdateInfoInit(VI_PIPE ViPipe)
{
	...
	ext_system_update_infogh_phyaddr_write(ViPipe, ((u64PhyAddr >> 32) &
							0xFFFFFFFF));
	ext_system_update_info_low_phyaddr_write(ViPipe,
						 (u64PhyAddr & 0xFFFFFFFF));

	pstIspCtx->stUpdateInfoCtrl.pstIspUpdateInfo =
		MPI_SYS_Mmap(u64PhyAddr, (sizeof(ISP_DCF_UPDATE_INFO_S) *
	...
	if (pstIspCtx->stUpdateInfoCtrl.pstIspUpdateInfo == GK_NULL) {
		ISP_TRACE(MODULE_DBG_ERR,
			  "isp[%d] mmap update info buf failed!\n", ViPipe);
