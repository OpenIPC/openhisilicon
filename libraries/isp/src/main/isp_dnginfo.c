...
GK_S32 ISP_DngInfoInit(VI_PIPE ViPipe)
{
	...
	pstIspCtx->stDngInfoCtrl.pstIspDng =
		MPI_SYS_Mmap(u64PhyAddr, sizeof(DNG_IMAGE_STATIC_INFO_S));
...
GK_S32 ISP_DngInfoExit(VI_PIPE ViPipe)
{
	...
	if (pstIspCtx->stDngInfoCtrl.pstIspDng != GK_NULL) {
		MPI_SYS_Munmap(pstIspCtx->stDngInfoCtrl.pstIspDng,
...
GK_BOOL ISP_DngColorParamCheck(ISP_DNG_CCM_S *pstDngCcm,
			       ISP_DNG_CCM_S *pstPreDngCcm)
{
...
static const double SRGB_to_XYZD50[9] = { 0.4361, 0.3851, 0.1431,
					  0.2225, 0.7169, 0.0606,
					  0.0139, 0.0971, 0.7141 };
static const double SRGB_to_XYZA[9] = { 0.4969, 0.4388, 0.1630, 0.2225, 0.7169,
					0.0606, 0.0060, 0.0419, 0.3080 };

static const double XYZD50_to_sRGB[9] = { 3.1340,  -1.6169, -0.4907,
					  -0.9784, 1.9159,  0.0334,
					  0.0720,  -0.2290, 1.4049 };
...
void Invert3by3(const double *MatrixA, double *InvMatrix)
{
	...
	det = (a00 * temp[0] + a01 * temp[3] + a02 * temp[6]);
...
void MultiMatrix3x3(const double *MatrixA, const double *MatrixB,
		    double *MutMatrix)
{
	int i, j, k;
...
void ISP_Dng_LightSource_Checker(GK_U16 u16ColorTmep, GK_U8 *pu8LightSource)
{
	...
	if (u16ColorTmep >= (7500 - 500)) { /* D75 7500 */
		*pu8LightSource = 22;
	} else if ((u16ColorTmep < (6500 + 500)) &&
		   (u16ColorTmep >= (6500 - 500))) { /* D65 6500 */
		*pu8LightSource = 21;
	} else if ((u16ColorTmep < (5500 + 500)) &&
		   (u16ColorTmep >= (5500 - 250))) { /* D55 5500 */
		*pu8LightSource = 20;
	} else if ((u16ColorTmep < (5000 + 250)) &&
		   (u16ColorTmep >= (5000 - 100))) { /* daylight 5000 */
		*pu8LightSource = 23;
	} else if ((u16ColorTmep < (4800 + 100)) &&
		   (u16ColorTmep >= (4800 - 550))) { /* B 4800 */
		*pu8LightSource = 18;
	} else if ((u16ColorTmep < (4000 + 250)) &&
		   (u16ColorTmep >= (4000 - 800))) { /* fluorescent 4000 */
		*pu8LightSource = 2;
...
GK_S32 ISP_DngColorParamUpdate(VI_PIPE ViPipe)
{
	...
	case SNAP_TYPE_PRO:
		if ((pstIspCtx->stLinkage.enSnapPipeMode == ISP_SNAP_PICTURE) &&
		    (pstIspCtx->stLinkage.bLoadCCM == GK_TRUE)) {
			ISP_AlgsCtrl(pstIspCtx->astAlgs, AWB_DEV,
				     AWB_CCM_CONFIG_GET,
				     (GK_VOID *)&stAwbCcmConfig);
		} else {
	...
	memcpy(&stDngCcm.au16LowCcm, &stAwbCcmConfig.au16LowCCM,
	       sizeof(GK_U16) * 9);
	memcpy(&stDngCcm.au16HighCcm, &stAwbCcmConfig.au16HighCCM,
	       sizeof(GK_U16) * 9);
	...
	if (!bChanged) {
		if ((pstIspCtx->stPreDngColorParam.stWbGain1.u16Rgain !=
		     stDngColorParam.stWbGain1.u16Rgain) ||
		    (pstIspCtx->stPreDngColorParam.stWbGain1.u16Ggain !=
		     stDngColorParam.stWbGain1.u16Ggain) ||
		    (pstIspCtx->stPreDngColorParam.stWbGain1.u16Bgain !=
		     stDngColorParam.stWbGain1.u16Bgain) ||
		    (pstIspCtx->stPreDngColorParam.stWbGain2.u16Rgain !=
		     stDngColorParam.stWbGain2.u16Rgain) ||
		    (pstIspCtx->stPreDngColorParam.stWbGain2.u16Ggain !=
		     stDngColorParam.stWbGain2.u16Ggain) ||
		    (pstIspCtx->stPreDngColorParam.stWbGain2.u16Bgain !=
		     stDngColorParam.stWbGain2.u16Bgain)) {
			bChanged = GK_TRUE;
		}
	}
	/* save last CCM and WB gain */
	memcpy(&pstIspCtx->stPreDngCcm, &stDngCcm, sizeof(ISP_DNG_CCM_S));
	memcpy(&pstIspCtx->stPreDngColorParam, &stDngColorParam,
	       sizeof(ISP_DNG_COLORPARAM_S));
	...
	if (bChanged == GK_TRUE) {
		...
		MultiMatrix3x3(SRGB_to_XYZD50, adD50MultMatrix,
			       adInvD50ColorMatrix);
		...
		MultiMatrix3x3(adInvD50ColorMatrix, adD50Ccm,
			       adD50ForwardMatrix);
		for (i = 0; i < 9; i++) {
			pstIspCtx->stDngInfoCtrl.pstIspDng->astColorMatrix1[i]
				.s32Numerator =
				(GK_S32)(adAColorMatrix[i] * 1000000);
			pstIspCtx->stDngInfoCtrl.pstIspDng->astColorMatrix1[i]
				.s32Denominator = 1000000;

			pstIspCtx->stDngInfoCtrl.pstIspDng->astColorMatrix2[i]
				.s32Numerator =
				(GK_S32)(adD50ColorMatrix[i] * 1000000);
			pstIspCtx->stDngInfoCtrl.pstIspDng->astColorMatrix2[i]
				.s32Denominator = 1000000;

			pstIspCtx->stDngInfoCtrl.pstIspDng->astForwadMatrix1[i]
				.s32Numerator =
				(GK_S32)(adAForwardMatrix[i] * 1000000);
			pstIspCtx->stDngInfoCtrl.pstIspDng->astForwadMatrix1[i]
				.s32Denominator = 1000000;

			pstIspCtx->stDngInfoCtrl.pstIspDng->astForwadMatrix2[i]
				.s32Numerator =
				(GK_S32)(adD50ForwardMatrix[i] * 1000000);
			pstIspCtx->stDngInfoCtrl.pstIspDng->astForwadMatrix2[i]
				.s32Denominator = 1000000;

			pstIspCtx->stDngInfoCtrl.pstIspDng
				->astCameraCalibration1[i]
				.s32Numerator = 0;
			pstIspCtx->stDngInfoCtrl.pstIspDng
				->astCameraCalibration1[i]
				.s32Denominator = 1000000;
			...
			pstIspCtx->stDngInfoCtrl.pstIspDng
				->astCameraCalibration2[i]
				.s32Numerator = 0;
			pstIspCtx->stDngInfoCtrl.pstIspDng
				->astCameraCalibration2[i]
				.s32Denominator = 1000000;
		}
		pstIspCtx->stDngInfoCtrl.pstIspDng->astCameraCalibration1[0]
			.s32Numerator = 1000000;
		pstIspCtx->stDngInfoCtrl.pstIspDng->astCameraCalibration1[4]
			.s32Numerator = 1000000;
		pstIspCtx->stDngInfoCtrl.pstIspDng->astCameraCalibration1[8]
			.s32Numerator = 1000000;

		pstIspCtx->stDngInfoCtrl.pstIspDng->astCameraCalibration2[0]
			.s32Numerator = 1000000;
		pstIspCtx->stDngInfoCtrl.pstIspDng->astCameraCalibration2[4]
			.s32Numerator = 1000000;
		pstIspCtx->stDngInfoCtrl.pstIspDng->astCameraCalibration2[8]
			.s32Numerator = 1000000;

		ISP_Dng_LightSource_Checker(
			stDngCcm.u16LowColorTemp,
			&pstIspCtx->stDngInfoCtrl.pstIspDng
				 ->u8CalibrationIlluminant1);
		ISP_Dng_LightSource_Checker(
			stDngCcm.u16HighColorTemp,
			&pstIspCtx->stDngInfoCtrl.pstIspDng
				 ->u8CalibrationIlluminant2);
	}
...
GK_S32 ISP_UpdateDngImageDynamicInfo(VI_PIPE ViPipe)
{
	...
	if (pstSnsBlackLevel->bUpdate == GK_TRUE) {
		for (i = 0; i < ISP_BAYER_CHN_NUM; i++) {
			stDngImageDynamicInfo.au32BlackLevel[i] =
				pstSnsBlackLevel->au16BlackLevel[i];
		}
	} else {
		stDngImageDynamicInfo.au32BlackLevel[0] =
			ext_system_black_level_query_00_read(ViPipe);
		stDngImageDynamicInfo.au32BlackLevel[1] =
			ext_system_black_level_query_01_read(ViPipe);
		stDngImageDynamicInfo.au32BlackLevel[2] =
			ext_system_black_level_query_10_read(ViPipe);
		stDngImageDynamicInfo.au32BlackLevel[3] =
			ext_system_black_level_query_11_read(ViPipe);
	}

	stDngImageDynamicInfo.astAsShotNeutral[0].u32Denominator =
		MAX2(pstIspCtx->stLinkage.au32WhiteBalanceGain[0], 1);
	stDngImageDynamicInfo.astAsShotNeutral[0].u32Numerator =
		pstIspCtx->stLinkage.au32WhiteBalanceGain[1];
	stDngImageDynamicInfo.astAsShotNeutral[1].u32Denominator =
		MAX2(pstIspCtx->stLinkage.au32WhiteBalanceGain[1], 1);
	stDngImageDynamicInfo.astAsShotNeutral[1].u32Numerator =
		pstIspCtx->stLinkage.au32WhiteBalanceGain[1];
	stDngImageDynamicInfo.astAsShotNeutral[2].u32Denominator =
		MAX2(pstIspCtx->stLinkage.au32WhiteBalanceGain[3], 1);
	stDngImageDynamicInfo.astAsShotNeutral[2].u32Numerator =
		pstIspCtx->stLinkage.au32WhiteBalanceGain[1];
	...
	s32Ret = ioctl(g_as32IspFd[ViPipe], ISP_DNG_INFO_SET,
		       &stDngImageDynamicInfo);
