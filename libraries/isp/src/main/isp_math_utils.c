...
GK_U32 log2_int_to_fixed(const GK_U32 val, const GK_U8 out_precision, const GK_U8 shift_out)
{
    ...
    return (GK_U32)((((GK_U32)SignedLeftShift(pos, precision) + result) << shift_out) |
                    ((a & 0x7fff) >> (15 - shift_out)));
}

GK_U32 math_log2(const GK_U32 val,
                 const GK_U8 out_precision,
...
GK_U32 transition(GK_U32 *lut_in, GK_U32 *lut_out, GK_U32 lut_size, GK_U32 value, GK_U32 value_fraction_size)
{
    ...
    if (fast_search < lut_in[0]) {  //    fast search for minimum.
        ...
        if (fast_search >= lut_in[lut_size - 1]) {
            ...
            //    interpolation
            GK_S32 b10 = b1 - b0;
            GK_S32 ax0 = value - (GK_S32)SignedLeftShift(a0, value_fraction_size);
            GK_S32 a10 = a1 - a0;
            GK_S32 result;
            if (a10 == 0) {
                result = 0;
            ...
            ret = (GK_U32)(result + (GK_S32)SignedLeftShift(b0, value_fraction_size));
...
GK_S64 SignedLeftShift(GK_S64 s64Value, GK_U8 u8BitShift)
{
    ...
    if (s64Value < 0) {
        return (GK_S64)((0x1ULL << 63) | ((~(u64AbsValue << u8BitShift)) + 1));
    } else {
