...
GK_S32 ISP_DbgSet(VI_PIPE ViPipe, const ISP_DEBUG_INFO_S *pstDbgInfo)
{
    ...
    if (pstDbgInfo->bDebugEn) {
        if (pstDbgInfo->u64PhyAddr == 0) {
            ISP_TRACE(MODULE_DBG_ERR, "isp lib's debug phyaddr is 0!\n");
            return GK_FAILURE;
        }

        if (pstDbgInfo->u32Depth == 0) {
            ISP_TRACE(MODULE_DBG_ERR, "ae lib's debug depth is 0!\n");
            return GK_FAILURE;
        }
    ...
    if (pstDbgInfo->bDebugEn) {
        ...
        ext_system_sys_debuggh_addr_write(ViPipe, ((pstDbgInfo->u64PhyAddr >> 32) & 0xFFFFFFFF));
        ext_system_sys_debug_low_addr_write(ViPipe, (pstDbgInfo->u64PhyAddr & 0xFFFFFFFF));
...
GK_S32 ISP_DbgRunBgn(ISP_DBG_CTRL_S *pstDbg, GK_U32 u32FrmCnt)
{
    ...
    if ((pstDbg->bDebugEn) && (pstDbg->pstDbgAttr == GK_NULL)) {
        pstDbg->pstDbgAttr = (ISP_DBG_ATTR_S *)MPI_SYS_Mmap(pstDbg->u64PhyAddr, pstDbg->u32Size);
        if (pstDbg->pstDbgAttr == GK_NULL) {
            ISP_TRACE(MODULE_DBG_ERR, "isp map debug buf failed!\n");
            return GK_FAILURE;
        }
