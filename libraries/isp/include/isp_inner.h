...
#define MPI_ISP_PARAM_CHECK_RETURN(x, min, max, fmt, ...)                   \
	do {                                                                \
		if ((x) < (min) || (x) > (max)) {                           \
			printf("[Func]:%s [Line]:%d [Info]:", __FUNCTION__, \
			       __LINE__);                                   \
	...
	do {                                                                \
		...
		while (loops--) {                                           \
			if ((p[loops]) < (min) || (p[loops]) > (max)) {     \
				printf("[Func]:%s [Line]:%d [Info]:",       \
				       __FUNCTION__, __LINE__);             \
				printf("array[%d] = %#x " fmt, loops,       \
				       p[loops], ##__VA_ARGS__);            \
				return ERR_CODE_ISP_ILLEGAL_PARAM;          \
			}                                                   \
		}                                                           \
...
GK_S32 ISP_GetLightboxGain(VI_PIPE ViPipe,
			   ISP_AWB_Calibration_Gain_S *stAWBCalibrationGain);
GK_S32
...
GK_S32 ISP_SetLogLUTAttr(VI_PIPE ViPipe,
