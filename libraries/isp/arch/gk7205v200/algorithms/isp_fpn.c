...
GK_S32  ISP_SetCalibrateAttr(VI_PIPE ViPipe, ISP_FPN_CALIBRATE_ATTR_S *pstCalibrate)
{
    ...
    if (IS_WDR_MODE(u8WDRMode)) {
        ISP_TRACE(MODULE_DBG_ERR, "Do not support FPN Calibration in WDR mode!\n");
        return GK_FAILURE;
    }
    ...
    pstCalibrate->stFpnCaliFrame.u32Iso = ext_system_fpn_sensor_iso_read(ViPipe);
...
GK_S32 ISP_SetCorrectionAttr(VI_PIPE ViPipe, const ISP_FPN_ATTR_S *pstCorrection)
{
    ...
    if (IS_WDR_MODE(u8WDRMode)) {
        ISP_TRACE(MODULE_DBG_ERR, "Do not support FPN Correction in WDR mode!\n");
        return GK_FAILURE;
    }
    ...
    ext_system_manual_fpn_iso_write(ViPipe, pstCorrection->stFpnFrmInfo.u32Iso);
    ext_system_manual_fpn_Gain_write(ViPipe, pstCorrection->stManual.u32Strength);
...
{
    ext_system_fpn_sensor_iso_write(ViPipe, ISP_EXT_SYSTEM_FPN_SENSOR_ISO_DEFAULT);
    ext_system_manual_fpn_iso_write(ViPipe, ISP_EXT_SYSTEM_FPN_MANU_ISO_DEFAULT);

    ext_system_manual_fpn_CorrCfg_write(ViPipe, ISP_EXT_SYSTEM_FPN_MANU_CORRCFG_DEFAULT);
    ext_system_manual_fpn_Gain_write(ViPipe, ISP_EXT_SYSTEM_FPN_STRENGTH_DEFAULT);
    ext_system_manual_fpn_opmode_write(ViPipe, ISP_EXT_SYSTEM_FPN_OPMODE_DEFAULT);
    ext_system_manual_fpn_update_write(ViPipe, ISP_EXT_SYSTEM_FPN_MANU_UPDATE_DEFAULT);
...
GK_S32 FPNUpdateExtRegs(VI_PIPE ViPipe)
{
    ...
    ext_system_fpn_sensor_iso_write(ViPipe, pstIspCtx->stLinkage.u32SensorIso);
...
GK_S32 ISP_FPNRun(VI_PIPE ViPipe, const GK_VOID *pStatInfo,
...
{
    GK_U8 u8BlkDev = 0; /* BE Num, for SBS mode, BE0 & BE1 is the same FPN config, so just 0 is ok */
