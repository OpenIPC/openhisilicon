...
static GK_VOID BeDgStaticInit(VI_PIPE ViPipe, ISP_DG_STATIC_CFG_S *pstStaticRegCfg)
{
    pstStaticRegCfg->bResh = GK_TRUE;
}

static GK_VOID BeDgDynaInit(VI_PIPE ViPipe, ISP_DG_DYNA_CFG_S *pstDynaRegCfg, GK_U16 u16Gain)
{
...
static GK_VOID WDRDgStaticInit(VI_PIPE ViPipe, GK_U8 i, ISP_4DG_STATIC_CFG_S *pstStaticRegCfg)
{
...
static GK_VOID FeDgDynaInit(VI_PIPE ViPipe, ISP_FE_DG_DYNA_CFG_S *pstDynaRegCfg, GK_U16 u16Gain)
{
    GK_U8 i = 0;
...
static GK_VOID DgRegsInitialize(VI_PIPE ViPipe, ISP_REG_CFG_S *pstRegCfg)
{
    ...
    for (i = 0; i < pstRegCfg->u8CfgNum; i++) {
        ...
        BeDgDynaInit(ViPipe, &pstRegCfg->stAlgRegCfg[i].stDgRegCfg.stDynaRegCfg, u16Gain);
    ...
    FeDgDynaInit(ViPipe, &pstRegCfg->stAlgRegCfg[0].stFeDgRegCfg.stDynaRegCfg, u16Gain);
...
static GK_VOID ISP_DgWdrModeSet(VI_PIPE ViPipe, GK_VOID *pRegCfg)
{
    ...
    for (i = 0; i < pstRegCfg->u8CfgNum; i++) {
        WDRDgEnInit(ViPipe, i, pstRegCfg);
        pstRegCfg->stAlgRegCfg[i].stDgRegCfg.stStaticRegCfg.bResh   = GK_TRUE;
        pstRegCfg->stAlgRegCfg[i].st4DgRegCfg.stStaticRegCfg.bResh  = GK_TRUE;
...
GK_S32 ISP_DgRun(VI_PIPE ViPipe, const GK_VOID *pStatInfo,
                 GK_VOID *pRegCfg, GK_S32 s32Rsv)
{
    ...
    if (pstIspCtx->stLinkage.bDefectPixel && (ext_system_dpc_static_defect_type_read(ViPipe) == 0)) {
        for (i = 0; i < pstRegCfg->u8CfgNum; i++) {
    ...
    for (i = 0; i < 4; i++) {
        u32WDRGain = ((GK_U64)u32IspDgain * pstIspCtx->stLinkage.au32WDRGain[i]) >> 8;
