...
GK_VOID ISP_CalcGridInfo(GK_U16 u16Wdith, GK_U16 u16StartPox, GK_U16 u16BlockNum, GK_U16 *au16GridInfo)
{
    GK_U16 i;
    GK_U16 integer;
    GK_U16 remainder;
...
GK_U32 ISP_GetStripingActiveImgStart(GK_U8 u8BlockIndex, ISP_WORKING_MODE_S *pstIspWorkMode)
{
    GK_U32 u32OverLap;
...
GK_U32 ISP_GetStripingActiveImgWidth(GK_U8 u8BlockIndex, ISP_WORKING_MODE_S *pstIspWorkMode)
{
...
GK_U32 ISP_GetStripingGridXInfo(GK_U16 *au16GridPos, GK_U16 u16GridNum, ISP_WORKING_MODE_S *pstIspWorkMode)
{
    GK_U8  i;
    GK_U16 u16Start;
    GK_U16 u16Width;
...
GK_S32 ISP_GetFeFocusStatistics(VI_PIPE ViPipe, ISP_FE_FOCUS_STATISTICS_S *pstFEAFStat, ISP_STAT_S *pstIspActStat, GK_U8 u8WdrChn)
{
...
GK_S32 ISP_SetRcAttr(VI_PIPE ViPipe, const ISP_RC_ATTR_S *pstRcAttr)
{
    ...
    ext_system_rc_center_hor_coor_write(ViPipe, pstRcAttr->stCenterCoor.s32X);
    ext_system_rc_center_ver_coor_write(ViPipe, pstRcAttr->stCenterCoor.s32Y);
...
GK_S32 ISP_GetRcAttr(VI_PIPE ViPipe, ISP_RC_ATTR_S *pstRcAttr)
{
    ...
    pstRcAttr->stCenterCoor.s32X = ext_system_rc_center_hor_coor_read(ViPipe);
    pstRcAttr->stCenterCoor.s32Y = ext_system_rc_center_ver_coor_read(ViPipe);
...
GK_S32 ISP_SetRGBIRAttr(VI_PIPE ViPipe, const ISP_RGBIR_ATTR_S *pstRGBIRAttr)
{
    ...
    GK_S16 as16MonoCvtMatrix[12] = {1000, 0, 0, 0, 0, 1000, 0, 0, 0, 0, 1000, 0};
    ...
    ISP_CHECK_OPEN(ViPipe);
    ISP_CHECK_MEM_INIT(ViPipe);
    if ((pstRGBIRAttr->bEnable == GK_TRUE) && (IS_WDR_MODE(pstIspCtx->stWdrAttr.enWDRMode))) {
    ...
    if ((pstRGBIRAttr->au16ExpCtrl[0] >= 2048) || (pstRGBIRAttr->au16ExpCtrl[1] >= 2048)) {
        ISP_TRACE(MODULE_DBG_ERR, "Max Exposure control value is 2047, invalid IR expctrl input ExpCtrl1 = %d, ExpCtrp2 = %d!\n", pstRGBIRAttr->au16ExpCtrl[0], pstRGBIRAttr->au16ExpCtrl[1]);
        return ERR_CODE_ISP_ILLEGAL_PARAM;
    }

    if ((pstRGBIRAttr->au16Gain[0] >= 512) || (pstRGBIRAttr->au16Gain[1] >= 512)) {
        ISP_TRACE(MODULE_DBG_ERR, "Max Exposure control gain value is 511, invalid IR gain value input Gain1 = %d, Gain2 = %d!\n", pstRGBIRAttr->au16Gain[0], pstRGBIRAttr->au16Gain[1]);
        return ERR_CODE_ISP_ILLEGAL_PARAM;
    }
    ...
    for (i = 0; i < ISP_EXP_CTRL_NUM; i++) {
        ext_system_rgbir_expctrl_write(ViPipe, pstRGBIRAttr->au16ExpCtrl[i], i);
        ext_system_rgbir_gain_write(ViPipe, pstRGBIRAttr->au16Gain[i], i);
    }

    for (i = 0; i < ISP_RGBIR_CVTMATRIX_NUM; i++) {
        if (pstRGBIRAttr->enIRCvtMatMode == ISP_IR_CVTMAT_MODE_USER) {
            ext_system_rgbir_cvtmatrix_write(ViPipe, pstRGBIRAttr->as16CvtMatrix[i], i);
        } else if (pstRGBIRAttr->enIRCvtMatMode == ISP_IR_CVTMAT_MODE_MONO) {
            ext_system_rgbir_cvtmatrix_write(ViPipe, as16MonoCvtMatrix[i], i);
        } else {
            ext_system_rgbir_cvtmatrix_write(ViPipe, as16DftCvtMatrix[i], i);
        }
    }

    if (ioctl(g_as32IspFd[ViPipe], ISP_SET_RGBIR_FORMAT, &pstRGBIRAttr->enInPattern) != GK_SUCCESS) {
...
GK_S32 ISP_GetRGBIRAttr(VI_PIPE ViPipe, ISP_RGBIR_ATTR_S *pstRGBIRAttr)
{
    ...
    for (i = 0; i < ISP_EXP_CTRL_NUM; i++) {
        pstRGBIRAttr->au16ExpCtrl[i] = ext_system_rgbir_expctrl_read(ViPipe, i);
        pstRGBIRAttr->au16Gain[i] = ext_system_rgbir_gain_read(ViPipe, i);
    }

    for (i = 0; i < ISP_RGBIR_CVTMATRIX_NUM; i++) {
        pstRGBIRAttr->as16CvtMatrix[i] = ext_system_rgbir_cvtmatrix_read(ViPipe, i);
    }
