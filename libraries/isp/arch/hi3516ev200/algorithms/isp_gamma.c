...
static const GK_U8  g_au8NonUniStepLut[GAMMA_OUTSEG_NUMBER]   = {2, 2, 3, 4, 4, 4, 4, 5};
static const GK_U16 g_au16NonUniInSegLut[GAMMA_OUTSEG_NUMBER] = {16, 32, 48, 64, 96, 160, 224, 256};
...
GK_S32 GammaCtxInit(VI_PIPE ViPipe)
{
    ...
    if (pstGamma == GK_NULL) {
        ...
        if (pstGamma == GK_NULL) {
            ISP_TRACE(MODULE_DBG_ERR, "Isp[%d] GammaCtx malloc memory failed!\n", ViPipe);
...
GK_VOID GammaGetPos(ISP_GAMMA_S *pstGamma, GK_U16 *pau16ActualLut, GK_U16 *pau16InputLut)
{
    // This function is used to generate Pos value from InSeg and Step we entered
    GK_U16 i, j;
    GK_U16 InSegNum[GAMMA_OUTSEG_NUMBER]; // Number of Inner Segments in each Outer Segment, thus the first InSegNum should be the first number of au16InSeg;
...
static GK_VOID GammaUsrRegsInit(ISP_GAMMA_USR_CFG_S *pstUsrRegCfg, GK_U16 *pu16Gamma, ISP_GAMMA_S *pstGamma)
{
    ...
    memcpy(pstUsrRegCfg->au16GammaInSeg, pstGamma->au16InSeg, GAMMA_OUTSEG_NUMBER * sizeof(GK_U16));
    memcpy(pstUsrRegCfg->au16GammaPos,   pstGamma->au16Pos,   GAMMA_OUTSEG_NUMBER * sizeof(GK_U16));
    memcpy(pstUsrRegCfg->au8GammaStep,   pstGamma->au8Step,   GAMMA_OUTSEG_NUMBER * sizeof(GK_U8));
    memcpy(pstUsrRegCfg->au16GammaLUT,   pu16Gamma,           GAMMA_NODE_NUMBER   * sizeof(GK_U16));
}
...
static GK_VOID GammaExtRegsInitialize(VI_PIPE ViPipe)
{
    ...
    ext_system_gamma_curve_type_write(ViPipe, ISP_EXT_SYSTEM_GAMMA_CURVE_TYPE_DEFAULT);
}
...
static GK_S32 GammaReadExtRegs(VI_PIPE ViPipe)
{
    ...
    if (pstGammaCtx->bLutUpdate) {
        for (i = 0; i < GAMMA_EXT_NODE_NUMBER; i++) {
            pstGammaCtx->pu16ExtGAMMA[i] = ext_system_gamma_lut_read(ViPipe, i);
        }
    }
...
static GK_S32 GammaCheckCmosParam(VI_PIPE ViPipe, const ISP_CMOS_GAMMA_S *pstGamma)
{
    GK_U16 i;
...
static GK_S32 GammaInitialize(VI_PIPE ViPipe)
{
    ...
    if (pstGammaCtx->pu16GAMMA == GK_NULL) {
        pstGammaCtx->pu16GAMMA = (GK_U16 *)ISP_MALLOC(GAMMA_NODE_NUMBER * sizeof(GK_U16));

        if (pstGammaCtx->pu16GAMMA == GK_NULL) {
            ISP_TRACE(MODULE_DBG_ERR, "malloc pstGammaCtx->pau16GAMMA failed!\n");
            return GK_FAILURE;
        }
    }

    if (pstGammaCtx->pu16ExtGAMMA == GK_NULL) {
        pstGammaCtx->pu16ExtGAMMA = (GK_U16 *)ISP_MALLOC(GAMMA_EXT_NODE_NUMBER * sizeof(GK_U16));

        if (pstGammaCtx->pu16ExtGAMMA == GK_NULL) {
            ISP_TRACE(MODULE_DBG_ERR, "malloc pstGammaCtx->pau16ExtGAMMA failed!\n");
    ...
    if (pstSnsDft->unKey.bit1Gamma) {
        ...
        memcpy(pstGammaCtx->pu16ExtGAMMA, pstSnsDft->pstGamma->au16Gamma, GAMMA_EXT_NODE_NUMBER * sizeof(GK_U16));

        ext_system_gamma_curve_type_write(ViPipe, ISP_GAMMA_CURVE_USER_DEFINE);
    } else { /* Read from firmware */
        if (IS_LINEAR_MODE(u8WDRMode)) {
            memcpy(pstGammaCtx->pu16ExtGAMMA, g_au16LinModeGamma, GAMMA_EXT_NODE_NUMBER * sizeof(GK_U16));
        } else {
            memcpy(pstGammaCtx->pu16ExtGAMMA, g_au16WDRModeGamma, GAMMA_EXT_NODE_NUMBER * sizeof(GK_U16));
        }
...
#ifndef VREG_OLD
    for (i = 0; i < GAMMA_EXT_NODE_NUMBER; i++) {
        ext_system_gamma_lut_write(ViPipe, i, pstGammaCtx->pu16ExtGAMMA[i]);
    }
#else
    GK_U32 u32Addr;
    ...
    memcpy((GK_U16 *)pVirtAddr, pstGammaCtx->pu16ExtGAMMA, GAMMA_EXT_NODE_NUMBER * sizeof(GK_U16));
#endif
...
static GK_VOID ISP_GammaWdrModeSet(VI_PIPE ViPipe, GK_VOID *pRegCfg)
{
    ...
    for (i = 0; i < pstRegCfg->u8CfgNum; i++) {
        ...
        pstRegCfg->stAlgRegCfg[i].stGammaCfg.stUsrRegCfg.bSwitchMode    = GK_TRUE;
    }
}
...
GK_S32 ISP_GammaRun(VI_PIPE ViPipe, const GK_VOID *pStatInfo,
                    GK_VOID *pRegCfg, GK_S32 s32Rsv)
{
    ...
    if (GK_NULL == pstGammaCtx->pu16GAMMA || GK_NULL == pstGammaCtx->pu16ExtGAMMA) {
    ...
    if (pstGammaCtx->bLutUpdate) {
        ...
        for (i = 0; i < pstReg->u8CfgNum; i++) {
            ...
            memcpy(pstUsrRegCfg->au16GammaLUT, pstGammaCtx->pu16GAMMA, GAMMA_NODE_NUMBER * sizeof(GK_U16));
