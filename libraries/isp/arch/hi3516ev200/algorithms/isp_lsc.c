...
static GK_S32 LscCtxInit(VI_PIPE ViPipe)
{
    ...
    if (pastLscCtx == GK_NULL) {
        ...
        if (pastLscCtx == GK_NULL) {
            ISP_TRACE(MODULE_DBG_ERR, "Isp[%d] LscCtx malloc memory failed!\n", ViPipe);
...
static GK_VOID LscGetLutIndex(GK_U8 u8CurBlk, ISP_LSC_S *pstLsc, ISP_LSC_USR_CFG_S *pstUsrRegCfg)
{
...
static GK_S32 LscGetGainLutInitialize(VI_PIPE ViPipe, ISP_LSC_S *pstLsc, ISP_LSC_USR_CFG_S *pstUsrRegCfg)
{
    GK_U16 i, j;
    GK_U16 u16BlkColEnd;
    GK_U16 u16BlkColStart;
...
static GK_VOID LscGetGainLut(VI_PIPE ViPipe, ISP_LSC_S *pstLsc, ISP_LSC_USR_CFG_S *pstUsrRegCfg)
{
    GK_U16 i, j;
    GK_U16 u16BlkColEnd;
    GK_U16 u16BlkColStart;
    GK_U16 u16IndexOffset;
    GK_U16 u16SrcIndex;
    GK_U16 r_gain0, r_gain1, gr_gain0, gr_gain1, gb_gain0, gb_gain1, b_gain0, b_gain1;
    ...
    for (j = 0; j < ISP_LSC_GRID_ROW; j++) {
        for (i = 0; i <= (u16BlkColEnd - u16BlkColStart); i++) {
            ...
            r_gain0  = ext_system_isp_mesh_shading_r_gain0_read(ViPipe, u16SrcIndex);
            r_gain1  = ext_system_isp_mesh_shading_r_gain1_read(ViPipe, u16SrcIndex);

            gr_gain0 = ext_system_isp_mesh_shading_gr_gain0_read(ViPipe, u16SrcIndex);
            gr_gain1 = ext_system_isp_mesh_shading_gr_gain1_read(ViPipe, u16SrcIndex);

            gb_gain0 = ext_system_isp_mesh_shading_gb_gain0_read(ViPipe, u16SrcIndex);
            gb_gain1 = ext_system_isp_mesh_shading_gb_gain1_read(ViPipe, u16SrcIndex);

            b_gain0  = ext_system_isp_mesh_shading_b_gain0_read(ViPipe, u16SrcIndex);
            b_gain1  = ext_system_isp_mesh_shading_b_gain1_read(ViPipe, u16SrcIndex);
...
static GK_VOID LscStaticRegsInitialize(GK_U8 i, VI_PIPE ViPipe, ISP_LSC_STATIC_CFG_S *pstLscStaticRegCfg)
{
...
static GK_VOID LscUsrRegsMultiInitialize(GK_U8 u8CurBlk, VI_PIPE ViPipe, ISP_LSC_USR_CFG_S *pstUsrRegCfg)
{
...
static GK_VOID LscUsrRegsSingleInitialize(VI_PIPE ViPipe, ISP_LSC_USR_CFG_S *pstUsrRegCfg)
{
    GK_U16     i;
    GK_U32     u32DefGain;
    ISP_LSC_S  *pstLsc    = GK_NULL;
    ...
    memcpy(pstUsrRegCfg->au16DeltaX, pstLsc->au16DeltaX, sizeof(GK_U16) * (ISP_LSC_GRID_COL - 1));
    memcpy(pstUsrRegCfg->au16InvX,   pstLsc->au16InvX,   sizeof(GK_U16) * (ISP_LSC_GRID_COL - 1));

    memcpy(pstUsrRegCfg->au16DeltaY, pstLsc->au16DeltaY, sizeof(GK_U16) * (ISP_LSC_GRID_ROW - 1) / 2);
    memcpy(pstUsrRegCfg->au16InvY,   pstLsc->au16InvY,   sizeof(GK_U16) * (ISP_LSC_GRID_ROW - 1) / 2);
...
static GK_VOID LscRegsInitialize(VI_PIPE ViPipe, ISP_REG_CFG_S *pstRegCfg)
{
    ...
    if (pstRegCfg->u8CfgNum == 1) {
        ...
        pstRegCfg->stAlgRegCfg[0].stBeLutUpdateCfg.bLscLutUpdate = GK_TRUE;
    } else {
        for (i = 0; i < pstRegCfg->u8CfgNum; i++) {
            ...
            pstRegCfg->stAlgRegCfg[i].stLscRegCfg.bLut2SttEn         = GK_TRUE;
            pstRegCfg->stAlgRegCfg[i].stBeLutUpdateCfg.bLscLutUpdate = GK_TRUE;
        }
    }
...
static GK_S32 LscExtRegsInitialize(VI_PIPE ViPipe)
{
    ...
    ext_system_isp_mesh_shading_mesh_strength_write(ViPipe, pstLsc->u16MeshStrength);
    ext_system_isp_mesh_shading_blendratio_write(ViPipe, pstLsc->u16MeshWeight);
    ext_system_isp_mesh_shading_mesh_scale_write(ViPipe, pstLsc->u8MeshScale);

    for (i = 0; i < (ISP_LSC_GRID_COL - 1) / 2; i++) {
        ext_system_isp_mesh_shading_xgrid_write(ViPipe, i, pstLsc->au16DeltaX[i]);
    }

    for (i = 0; i < (ISP_LSC_GRID_ROW - 1) / 2; i++) {
        ext_system_isp_mesh_shading_ygrid_write(ViPipe, i, pstLsc->au16DeltaY[i]);
    }
    ...
    if (pstSnsDft->unKey.bit1Lsc) {
...
#ifndef VREG_OLD
        for (i = 0; i < ISP_LSC_GRID_POINTS; i++) {
            ext_system_isp_mesh_shading_r_gain0_write(ViPipe,  i, pstCmosLsc->astLscCalibTable[0].au16R_Gain[i]);
            ext_system_isp_mesh_shading_gr_gain0_write(ViPipe, i, pstCmosLsc->astLscCalibTable[0].au16Gr_Gain[i]);
            ext_system_isp_mesh_shading_gb_gain0_write(ViPipe, i, pstCmosLsc->astLscCalibTable[0].au16Gb_Gain[i]);
            ext_system_isp_mesh_shading_b_gain0_write(ViPipe,  i, pstCmosLsc->astLscCalibTable[0].au16B_Gain[i]);

            ext_system_isp_mesh_shading_r_gain1_write(ViPipe,  i, pstCmosLsc->astLscCalibTable[1].au16R_Gain[i]);
            ext_system_isp_mesh_shading_gr_gain1_write(ViPipe, i, pstCmosLsc->astLscCalibTable[1].au16Gr_Gain[i]);
            ext_system_isp_mesh_shading_gb_gain1_write(ViPipe, i, pstCmosLsc->astLscCalibTable[1].au16Gb_Gain[i]);
            ext_system_isp_mesh_shading_b_gain1_write(ViPipe,  i, pstCmosLsc->astLscCalibTable[1].au16B_Gain[i]);
        }
#else
        ...
        memcpy((GK_U16 *)pVirtAddr, pstCmosLsc->astLscCalibTable[0].au16R_Gain, ISP_LSC_GRID_POINTS * sizeof(GK_U16));

        pVirtAddr    = pVirtAddrTmp + 0x8D2;
        memcpy((GK_U16 *)pVirtAddr, pstCmosLsc->astLscCalibTable[0].au16Gr_Gain, ISP_LSC_GRID_POINTS * sizeof(GK_U16));

        pVirtAddr    = pVirtAddrTmp + 0x1154;
        memcpy((GK_U16 *)pVirtAddr, pstCmosLsc->astLscCalibTable[0].au16Gb_Gain, ISP_LSC_GRID_POINTS * sizeof(GK_U16));

        pVirtAddr    = pVirtAddrTmp + 0x19D6;
        memcpy((GK_U16 *)pVirtAddr, pstCmosLsc->astLscCalibTable[0].au16B_Gain, ISP_LSC_GRID_POINTS * sizeof(GK_U16));

        pVirtAddr    = pVirtAddrTmp + 0x2258;
        memcpy((GK_U16 *)pVirtAddr, pstCmosLsc->astLscCalibTable[1].au16R_Gain, ISP_LSC_GRID_POINTS * sizeof(GK_U16));

        pVirtAddr    = pVirtAddrTmp + 0x2ADA;
        memcpy((GK_U16 *)pVirtAddr, pstCmosLsc->astLscCalibTable[1].au16Gr_Gain, ISP_LSC_GRID_POINTS * sizeof(GK_U16));

        pVirtAddr    = pVirtAddrTmp + 0x335C;
        memcpy((GK_U16 *)pVirtAddr, pstCmosLsc->astLscCalibTable[1].au16Gb_Gain, ISP_LSC_GRID_POINTS * sizeof(GK_U16));

        pVirtAddr    = pVirtAddrTmp + 0x3BDE;
        memcpy((GK_U16 *)pVirtAddr, pstCmosLsc->astLscCalibTable[1].au16B_Gain, ISP_LSC_GRID_POINTS * sizeof(GK_U16));
#endif
    } else {
#ifndef VREG_OLD
        ...
        for (i = 0; i < ISP_LSC_GRID_POINTS; i++) {
            ...
            ext_system_isp_mesh_shading_r_gain0_write(ViPipe,  i, g_au16MeshGainDef[pstLsc->u8MeshScale]);
            ext_system_isp_mesh_shading_gr_gain0_write(ViPipe, i, g_au16MeshGainDef[pstLsc->u8MeshScale]);
            ext_system_isp_mesh_shading_gb_gain0_write(ViPipe, i, g_au16MeshGainDef[pstLsc->u8MeshScale]);
            ext_system_isp_mesh_shading_b_gain0_write(ViPipe,  i, g_au16MeshGainDef[pstLsc->u8MeshScale]);

            ext_system_isp_mesh_shading_r_gain1_write(ViPipe,  i, g_au16MeshGainDef[pstLsc->u8MeshScale]);
            ext_system_isp_mesh_shading_gr_gain1_write(ViPipe, i, g_au16MeshGainDef[pstLsc->u8MeshScale]);
            ext_system_isp_mesh_shading_gb_gain1_write(ViPipe, i, g_au16MeshGainDef[pstLsc->u8MeshScale]);
            ext_system_isp_mesh_shading_b_gain1_write(ViPipe,  i, g_au16MeshGainDef[pstLsc->u8MeshScale]);
        }
        ...
        pVirtAddrTmp = (GK_U8 *)VReg_GetVirtAddr(u32Addr);
        pVirtAddr    = pVirtAddrTmp + 0x50;
        MemsetU16((GK_U16 *)pVirtAddr, g_au16MeshGainDef[pstLsc->u8MeshScale], ISP_LSC_GRID_POINTS);
...
static GK_S32 LscReadExtRegs(VI_PIPE ViPipe)
{
    ...
    pstLsc->bLscCoefUpdata  = ext_system_isp_mesh_shading_attr_updata_read(ViPipe);
    ext_system_isp_mesh_shading_attr_updata_write(ViPipe, GK_FALSE);

    if (pstLsc->bLscCoefUpdata) {
        pstLsc->u16MeshStrength = ext_system_isp_mesh_shading_mesh_strength_read(ViPipe);
        pstLsc->u16MeshWeight   = ext_system_isp_mesh_shading_blendratio_read(ViPipe);
    }

    pstLsc->bLutUpdate      = ext_system_isp_mesh_shading_lut_attr_updata_read(ViPipe);
    ext_system_isp_mesh_shading_lut_attr_updata_write(ViPipe, GK_FALSE);

    if (pstLsc->bLutUpdate) {
        pstLsc->u8MeshScale = ext_system_isp_mesh_shading_mesh_scale_read(ViPipe);

        for (i = 0; i < (ISP_LSC_GRID_COL - 1) / 2; i++) {
            pstLsc->au16DeltaX[i] = ext_system_isp_mesh_shading_xgrid_read(ViPipe, i);
        }

        for (i = (ISP_LSC_GRID_COL - 1) / 2; i < ISP_LSC_GRID_COL - 1; i++) {
            pstLsc->au16DeltaX[i] = pstLsc->au16DeltaX[ISP_LSC_GRID_COL - 2 - i];
        }

        for (i = 0; i < (ISP_LSC_GRID_ROW - 1) / 2; i++) {
            pstLsc->au16DeltaY[i] = ext_system_isp_mesh_shading_ygrid_read(ViPipe, i);
        }
...
static GK_VOID geometricGridSizeLsc(GK_U16 *pu16Delta, GK_U16 *pu16Inv, GK_U16 u16Length, GK_U16 u16GridSize)
{
    GK_U16 i, sum;
...
static GK_VOID LscImageSize(VI_PIPE ViPipe, GK_U8 u8BlkNum, ISP_LSC_S  *pstLsc)
{
    ...
    geometricGridSizeLsc(pstLsc->au16DeltaX, pstLsc->au16InvX, pstIspCtx->stBlockAttr.stFrameRect.u32Width  / 2, ISP_LSC_GRID_COL);
    geometricGridSizeLsc(pstLsc->au16DeltaY, pstLsc->au16InvY, pstIspCtx->stBlockAttr.stFrameRect.u32Height / 2, ISP_LSC_GRID_ROW);

    for (i = 0; i < (ISP_LSC_GRID_COL - 1) / 2; i++) {
        pstLsc->au16DeltaX[ISP_LSC_GRID_COL - 2 - i] = pstLsc->au16DeltaX[i];
        pstLsc->au16InvX[ISP_LSC_GRID_COL - 2 - i]   = pstLsc->au16InvX[i];
    }
...
static GK_S32 LscCheckCmosParam(VI_PIPE ViPipe, const ISP_CMOS_LSC_S *pstCmosLsc)
{
    GK_U16 i;
...
static GK_VOID Lsc_Usr_Fw(VI_PIPE ViPipe, GK_U8 u8CurBlk, ISP_LSC_S *pstLsc, ISP_LSC_USR_CFG_S *pstUsrRegCfg)
{
    LscGetLutIndex(u8CurBlk, pstLsc, pstUsrRegCfg);
...
static GK_VOID  Lsc_Usr_Fw_Normal(VI_PIPE ViPipe, ISP_LSC_S *pstLsc, ISP_LSC_USR_CFG_S *pstUsrRegCfg)
{
    GK_U16 i;
    GK_U16 r_gain0, r_gain1, gr_gain0, gr_gain1, gb_gain0, gb_gain1, b_gain0, b_gain1;
...
GK_S32 ISP_LscRun(VI_PIPE ViPipe, const GK_VOID *pStatInfo,
                  GK_VOID *pRegCfg, GK_S32 s32Rsv)
{
    ...
    if (pstLsc->bLscCoefUpdata) {
        for (i = 0; i < pstRegCfg->u8CfgNum; i++) {
            pstRegCfg->stAlgRegCfg[i].stLscRegCfg.stUsrRegCfg.bCoefUpdate   = GK_TRUE;
