...
static GK_VOID CscDynaRegsConfig(ISP_CSC_ATTR *pstCscAttr, ISP_CSC_DYNA_CFG_S *pstDynaRegCfg)
{
...
static GK_S32 CscReadExtRegs(VI_PIPE ViPipe, ISP_CSC_S *pstCsc)
{
    ...
    if (pstCsc->bUpdate) {
        pstCsc->bLimitedRangeEn = ext_system_csc_limitrange_en_read(ViPipe);
        ...
        if (Gamut == COLOR_GAMUT_BT2020) {
            ISP_TRACE(MODULE_DBG_WARN, "Invalid Color Gamut type, Set back to previous gamut type!\n");
        } else {
        ...
        for (i = 0; i < 9; i++) {
            pstCsc->stIspCscAttr.as16CSCCoef[i] = ext_system_csc_coef_read(ViPipe, i);
        }

        // DC components read
        for (i = 0; i < 3; i++) {
            pstCsc->stIspCscAttr.as16CSCIdc[i] = ext_system_csc_dcin_read(ViPipe, i);
            pstCsc->stIspCscAttr.as16CSCOdc[i] = ext_system_csc_dcout_read(ViPipe, i);
        }
    }
...
GK_S32 CscCalMatrix(VI_PIPE ViPipe, ISP_CSC_S *pstCsc, ISP_CSC_ATTR *pstCscCoef)
{
    ...
    pstCscCoef->as16CSCCoef[0] = (GK_S16)((s32Contrast * ((GK_S32)pstCscCoefTmp->as16CSCCoef[0])) / 100);
    pstCscCoef->as16CSCCoef[1] = (GK_S16)((s32Contrast * ((GK_S32)pstCscCoefTmp->as16CSCCoef[1])) / 100);
    pstCscCoef->as16CSCCoef[2] = (GK_S16)((s32Contrast * ((GK_S32)pstCscCoefTmp->as16CSCCoef[2])) / 100);
    pstCscCoef->as16CSCCoef[3] = (GK_S16)((s32Contrast * s32Satu * ((GK_S32)(pstCscCoefTmp->as16CSCCoef[3] * ISP_CSC_COS_TABLE[s32Hue] + pstCscCoefTmp->as16CSCCoef[6] * ISP_CSC_SIN_TABLE[s32Hue]) / 1000)) / 10000);
    pstCscCoef->as16CSCCoef[4] = (GK_S16)((s32Contrast * s32Satu * ((GK_S32)(pstCscCoefTmp->as16CSCCoef[4] * ISP_CSC_COS_TABLE[s32Hue] + pstCscCoefTmp->as16CSCCoef[7] * ISP_CSC_SIN_TABLE[s32Hue]) / 1000)) / 10000);
    pstCscCoef->as16CSCCoef[5] = (GK_S16)((s32Contrast * s32Satu * ((GK_S32)(pstCscCoefTmp->as16CSCCoef[5] * ISP_CSC_COS_TABLE[s32Hue] + pstCscCoefTmp->as16CSCCoef[8] * ISP_CSC_SIN_TABLE[s32Hue]) / 1000)) / 10000);
    pstCscCoef->as16CSCCoef[6] = (GK_S16)((s32Contrast * s32Satu * ((GK_S32)(pstCscCoefTmp->as16CSCCoef[6] * ISP_CSC_COS_TABLE[s32Hue] - pstCscCoefTmp->as16CSCCoef[3] * ISP_CSC_SIN_TABLE[s32Hue]) / 1000)) / 10000);
    pstCscCoef->as16CSCCoef[7] = (GK_S16)((s32Contrast * s32Satu * ((GK_S32)(pstCscCoefTmp->as16CSCCoef[7] * ISP_CSC_COS_TABLE[s32Hue] - pstCscCoefTmp->as16CSCCoef[4] * ISP_CSC_SIN_TABLE[s32Hue]) / 1000)) / 10000);
    pstCscCoef->as16CSCCoef[8] = (GK_S16)((s32Contrast * s32Satu * ((GK_S32)(pstCscCoefTmp->as16CSCCoef[8] * ISP_CSC_COS_TABLE[s32Hue] - pstCscCoefTmp->as16CSCCoef[5] * ISP_CSC_SIN_TABLE[s32Hue]) / 1000)) / 10000);
    ...
    for (i = 0; i < 3; i++) {
        ...
        ext_system_csc_dcout_write(ViPipe, i, pstCscCoef->as16CSCOdc[i]);
    }

    if (pstCsc->enColorGamut != COLOR_GAMUT_USER) {
        for (i = 0; i < 9; i++) {
            ext_system_csc_coef_write(ViPipe, i, pstCscCoef->as16CSCCoef[i]);
        }
    }
...
GK_S32 CscColorGamutinfoGet(VI_PIPE ViPipe,  ISP_COLORGAMMUT_INFO_S  *pstColorGamutInfo)
{
...
GK_S32 ISP_CscRun(VI_PIPE ViPipe, const GK_VOID *pStatInfo,
