...
HI_S32 ISP_ProcInit(ISP_DEV IspDev)
{
    ...
    s32Ret = ioctl(g_as32IspFd[IspDev], ISP_PROC_PARAM_GET, &pstProc->u32ProcParam);
    if (s32Ret)
    {
    ...
    s32Ret = ioctl(g_as32IspFd[IspDev], ISP_PROC_INIT, &pstProc->stProcMem);
    if (s32Ret)
    {
...
freeproc:
    s32Ret = ioctl(g_as32IspFd[IspDev], ISP_PROC_EXIT);
...
HI_S32 ISP_ProcWrite(const ISP_ALG_NODE_S *astAlgs, ISP_DEV IspDev)
{
    ...
    s32Ret = ioctl(g_as32IspFd[IspDev], ISP_PROC_WRITE_ING);
    if (s32Ret)
    {
    ...
    s32Ret = ioctl(g_as32IspFd[IspDev], ISP_PROC_WRITE_OK);
    if (HI_SUCCESS != s32Ret)
    {
        printf("write proc ec %x!\n", s32Ret);
        return s32Ret;
    }
