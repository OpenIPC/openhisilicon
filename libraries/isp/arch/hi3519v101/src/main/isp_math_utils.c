...
{
    HI_S32 y = (1 <<fraction_size) + (((HI_U32)x <<fraction_size) / 4) - (((HI_U32)x <<fraction_size) / 8);
    return y;
}
...
HI_U32 transition(HI_U32 *lut_in, HI_U32 *lut_out, HI_U32 lut_size, HI_U32 value, HI_U32 value_fraction_size)
{
    ...
    if (fast_search < lut_in[0])    //    fast search for minimum.
        ...
        if (fast_search >= lut_in[lut_size - 1])
            ...
            //    interpolation
            HI_S32 b10 = b1 - b0;
            HI_S32 ax0 = value - ((HI_U32)a0 << value_fraction_size);
            HI_S32 a10 = a1 - a0;
            HI_S32 result;
            if (a10 == 0)
                result = 0;
                ...
                result = b10 * ax0 / a10;
            result = result + ((HI_U32)b0 << value_fraction_size);
            ret = (HI_U32)result >> value_fraction_size;
        }
    return ret;
}
