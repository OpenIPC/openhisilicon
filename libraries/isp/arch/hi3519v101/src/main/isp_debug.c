...
HI_S32 ISP_DbgSet(ISP_DEV IspDev, const ISP_DEBUG_INFO_S *pstDbgInfo)
{
    HI_U32  u32Size = 0;

    if (hi_ext_system_sys_debug_enable_read(IspDev) && pstDbgInfo->bDebugEn)
    {
    ...
    if (pstDbgInfo->bDebugEn)
    {
        ...
        u32Size = sizeof(ISP_DBG_ATTR_S) +
            sizeof(ISP_DBG_STATUS_S) * pstDbgInfo->u32Depth;
    }

    /* don't clear phyaddr and size when disable dbg info. */
    if (pstDbgInfo->bDebugEn)
    {
        hi_ext_system_sys_debug_enable_write(IspDev, pstDbgInfo->bDebugEn);
        hi_ext_system_sys_debug_addr_write(IspDev, pstDbgInfo->u32PhyAddr);
        hi_ext_system_sys_debug_depth_write(IspDev, pstDbgInfo->u32Depth);
        hi_ext_system_sys_debug_size_write(IspDev, u32Size);
    }
    else
    {
        hi_ext_system_sys_debug_enable_write(IspDev, pstDbgInfo->bDebugEn);
...
HI_S32 ISP_DbgGet(ISP_DEV IspDev, ISP_DEBUG_INFO_S *pstDbgInfo)
{
    pstDbgInfo->bDebugEn = hi_ext_system_sys_debug_enable_read(IspDev);
    pstDbgInfo->u32PhyAddr = hi_ext_system_sys_debug_addr_read(IspDev);
    pstDbgInfo->u32Depth = hi_ext_system_sys_debug_depth_read(IspDev);
