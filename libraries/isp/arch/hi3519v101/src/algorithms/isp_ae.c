...
static HI_S32 AeGetFrameInfo(ISP_DEV IspDev, ISP_AE_RESULT_S* pstAeResult)
{
    HI_U32 i = 0;
	ISP_CTX_S *pstIspCtx = HI_NULL;
	FRAME_SUPPLEMENT_INFO_S *pstIspFrame = HI_NULL;
    ...
    for(i=0; i<4; i++)
    {
        pstIspFrame->u32ExposureTime[i] = pstAeResult->u32IntTime[i];
        pstIspFrame->u32ISO[i] = pstAeResult->u32Iso;
        pstIspFrame->u32IspDgain[i] = pstAeResult->u32IspDgain << 2;
    }
    ...
    HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
	...
	stAeResult.u32IspDgain = MAX2(stAeResult.u32IspDgain, 0x100);
	...
	AeGetFrameInfo(IspDev, &stAeResult);

    hi_ext_system_sys_iso_write(IspDev, stAeResult.u32Iso);
