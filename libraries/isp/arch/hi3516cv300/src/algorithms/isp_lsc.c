...
{	
    ...
    if(hi_isp_wdr_enable_read(IspDev))
    {
		hi_isp_lsc_blc_r_write(IspDev,pstSnsBlackLevel->au16BlackLevel[0]  >> 4);
	    hi_isp_lsc_blc_gr_write(IspDev,pstSnsBlackLevel->au16BlackLevel[1] >> 4);
	    hi_isp_lsc_blc_gb_write(IspDev,pstSnsBlackLevel->au16BlackLevel[2] >> 4);
	    hi_isp_lsc_blc_b_write(IspDev,pstSnsBlackLevel->au16BlackLevel[3]  >> 4);
    }
    else
    {
		hi_isp_lsc_blc_r_write(IspDev,pstSnsBlackLevel->au16BlackLevel[0]  << 2);
		...
		if ((pstIspCtx->u32FrameCnt % 25) == 0)
		{
            ...
            if ((pstLsc->bLightChaged == HI_TRUE) || (pstLsc->bLscCoefUpdata == HI_TRUE))
			{
				for (i = 0; i < LSC_GRID_POINTS; i++)
				{
                    ...
                    hi_ext_system_isp_mesh_shading_gr_gain_write(i, (pstLsc->gbb_gain[i] >> 13) & 0x1fff);
				}
				for ( i = 0 ; i < 8 ; i++ )
                {
                    pstRegCfg->stLscRegCfg.xgrid[i]    = pstLsc->delta_x[i];
