...
HI_S32 ISP_AeInit(ISP_DEV IspDev)
{
	...
	hi_isp_mg_width_write(IspDev, u32Width-1);
	hi_isp_mg_height_write(IspDev, u32Height-1);
    for(i = 0; i < 4; i++)
    {
   ...
   if(IS_LINEAR_MODE(pstIspCtx->u8SnsWDRMode))
    ...
    for(i = 0; i < 4; i++)
    {
        hi_isp_ae_wdr_bitmove_write(IspDev, i, 0);
    }
    ...
    HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
    ...
    pstIspCtx->stUpdateInfoCtrl.pstIspUpdateInfo->u16ISOSpeedRatings = stAeResult.u32Iso;
    
    pstIspCtx->stLinkage.enFSWDRMode = stAeResult.enFSWDRMode;
    hi_ext_system_sys_iso_write(stAeResult.u32Iso);
    
    return s32Ret;
}

static __inline HI_S32 AEImageResWrite(ISP_DEV IspDev, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstRes)
{
    HI_U8 i;

    hi_isp_ae_width_write(IspDev, pstRes->u16Width - 1);
    hi_isp_ae_height_write(IspDev, pstRes->u16Height -1);
    for(i = 0; i < 4; i++)
    {
        hi_isp_ae_wdr_hsize_write(IspDev, i, pstRes->u16Width - 1);
        hi_isp_ae_wdr_vsize_write(IspDev, i, pstRes->u16Height -1);
    }
    hi_isp_mg_width_write(IspDev, pstRes->u16Width - 1);
    hi_isp_mg_height_write(IspDev, pstRes->u16Height -1);
...
{    
	...
	{
		AEImageResWrite(IspDev,(ISP_CMOS_SENSOR_IMAGE_MODE_S *)pValue);
	}
