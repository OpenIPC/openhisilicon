...
HI_VOID ISP_Lsc_Fw(ISP_LSC_S *pstLsc)
{
    ...
    {
        w1 = (HI_FLOAT)dist_min2 / DIV_0_TO_1(dist_min1 + dist_min2);
	    w2 = (HI_FLOAT)dist_min1 / DIV_0_TO_1(dist_min1 + dist_min2);
    }else{
        w1 = 1;
        w2 = 0;
    }
...
static HI_VOID LscExtRegsInitialize(ISP_DEV IspDev)
{
    ...
    memcpy(&pstLsc->delta_x[0], &pstSnsDft->stLsc.stLscUniParaTable.u32XGrid[0], sizeof(HI_U32)*((LSC_GRID_COLS-1)/2));
    memcpy(&pstLsc->delta_y[0], &pstSnsDft->stLsc.stLscUniParaTable.u32YGrid[0], sizeof(HI_U32)*((LSC_GRID_ROWS-1)/2));
